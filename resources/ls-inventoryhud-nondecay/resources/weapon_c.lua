        ...
        if QBCore.Shared.Weapons[weapon]["name"] ~= "weapon_unarmed" and QBCore.Shared.Weapons[weapon]["ammotype"] == type:upper() then
            ...
            if total < maxAmmo then
                QBCore.Functions.Progressbar("taking_bullets", Lang:t('info.loading_bullets'), Config.ReloadTime, false, true, {
                    ...
                    Config.Notify(Lang:t('error.canceled'), "error")
                end)
            else
                Config.Notify(Lang:t('error.max_ammo'), "error")
            end
        else
            Config.Notify(Lang:t('error.no_weapon'), "error")
        end
    else
        Config.Notify(Lang:t('error.no_weapon'), "error")
    end
                ...
                if IsPedShooting(ped) or IsControlJustPressed(0, 24) then
                    ...
                    if CanShoot then
                        ...
                        if weapon ~= -1569615261 then
                            ...
                            Config.Notify(Lang:t('error.weapon_broken'), "error")
                            MultiplierAmount = 0
                        end
                    end
                end
