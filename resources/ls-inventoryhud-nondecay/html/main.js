...
function ItemChanged(data) {
    ...
    if (data.toInventory != null) {
		console.log($(`#item-${data.itemD._id}`).length)
		console.log(data.itemD._id)
		console.log($(`#item-${data.itemD._id}`))
        if($(`#item-${data.itemD._id}`).length) {
			console.log("To Inventory " + data.toInventory)
			console.log( "First Inventory " + firstInventory._inventoryId + " | " + "Second Inventory " + secInventory._inventoryId)
			console.log( "First Inventory Equaltiy " + firstInventory._inventoryId == data.toInventory + " | " + "Second Inventory Equaltiy " + secInventory._inventoryId == data.toInventory)
            var item = data.itemD
            ...
            if (firstInventory._inventoryId != data.toInventory && secInventory._inventoryId != data.toInventory) {
                ...
                if (!$(`#grid-${item._parent}-${item.loc.x}-${item.loc.y}-${item.slotId}`).data("isTaken") && (firstInventory._inventoryId == data.toInventory || secInventory._inventoryId == data.toInventory) ) {
                    var itemC = ColorSettings[item._data.Backgroundcolor]
...
function InventorySec(inventoryData) {
    ...
    data.items = ReadyItems(data, inventoryData.rememberItems)
...
function CreateTopGrid(gridID, gridCLASS, gridDATA) {
	...
	console.log(gridDATA._data.Label)
    ...
    if (gridDATA._parent == secInventory._inventoryId) {
        $(`#inventory`).append(`<span id="grid-container-label-secinventory" class="grid-container-label grid-container-label-secinventory"><span class="grid-bottom-label2">  ${secInventory._inventoryId}    </span><br>  ${AddWeight(gridDATA)} kg ${AddEmpty(gridDATA)}</span>`);
    }
