    ...
    if type == "onPlay" then
        if globalOptionsCache[id] then
            if globalOptionsCache[id].onPlayStart then
                globalOptionsCache[id].onPlayStart(getInfo(id), data.time)
            end
        end
    end
    if type == "onEnd" then
        ...
        if soundInfo[id] then
            ...
            if soundInfo[id].destroyOnFinish and not soundInfo[id].loop and not (soundInfo[id].maxDuration ~= 0 or soundInfo[id].maxDuration ~= nil) then
                Destroy(id)
            end
        end
    end
