        ...
        for (var soundName in soundList)
        {
            ...
            if(sound.loaded() == false){
                ...
                if(sound.getAudioPlayer() != null){
                    ...
                    sound.getAudioPlayer().volume(0);
                }
                if(sound.isDynamic()) sound.setVolume(0);
                sound.setLoop(sound.isLoop())

                $.post('https://rcore_carnival/data_status', JSON.stringify(
                {
                    time: time,
                    type: "maxDuration",
                    id: sound.getName(),
                ...
                $.post('https://rcore_carnival/events', JSON.stringify(
                {
                    type: "onPlay",
                    id: sound.getName(),
                    time: time,
...
function isLooped(divId){
	for (var soundName in soundList)
	{
        ...
        if(sound.getDivId() === divId && sound.isLoop()){
            ...
            $.post('https://rcore_carnival/data_status', JSON.stringify({ type: "finished",id: soundName }));
            $.post('https://rcore_carnival/events', JSON.stringify(
            {
                type: "onEnd",
                id: sound.getName(),
...
function ended(divId){
    if(divId == null)
    {
    	for (var soundName in soundList)
    	{
            ...
            //console.log(soundName + " | " + sound.isLoop())
