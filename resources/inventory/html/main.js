...
function WeightReCalculate() {
    setTimeout(function () {
        ...
        $.post("https://inventory/WeightChanged", JSON.stringify({
...
function RemoveItem(RemoveData) {
    ...
    if (item.info.CurrentStack > amount) {
        ...
        $.post("https://inventory/RemoveItem", JSON.stringify({
            itemData: item,
            isFullyRemove: false,
...
function UseItemContext(itemD) {
    ...
    $.post("https://inventory/UseItemContext", JSON.stringify({
        inventoryItem: itemD,
    }));
}
...
function CloseInventory() {
    ...
    $.post("https://inventory/closeNUI", JSON.stringify({}));
...
function SetFastUse(status, toInventory, itemData, fastUseKey) {
    $.post("https://inventory/FastUseChanged", JSON.stringify({
        typeFastUse: status,
    ...
    $(itemOBJ).draggable({
        ...
        stop: function (e, ui) {
            ...
            if (objectPlacable) {
                                    ...
                                    $("body").keypress(function (event) {
                                        ...
                                        if (keycode == '13') {
                                            ...
                                            if (GetItemSlotInventory(dataList.parentId) == secInventory._inventoryId && secInventory._inventoryId.includes("SHOP-")) {
                                                $.post('https://inventory/checkPrice', JSON.stringify({
                                                    item: itemD,
                                                    amount: amount
                                                }), function (callback) {
                                                    if (callback) {
                ...
                $.post("https://inventory/checkCurrentWeapon", JSON.stringify({
...
function SetItem(itemDATA, fromInventory, toInventory, toLOC, rotate, slot, parent, doNotDelete) {
    ...
    $.post("https://inventory/ItemChanged", JSON.stringify({
...
function AttachmentMenu(contextMenuItem) {
    ...
    $.post("https://inventory/attachmentOpen", JSON.stringify({
    ...
    $("#attachment").mousemove(function (event) {
        ...
        if (AttachmentDragging && !itemDrag.isDragging && !isBlocked) {
            ...
            $.post("https://inventory/updateAttachRotate", JSON.stringify({
                x: dx,
                y: dy
            }));
        }
...
function SetupAttachment(data) {
    ...
    $.each(attachmentItem.info.attachment, function (key, value) {
        if (value != undefined) {
            ...
            $.post("https://inventory/addAttachment", JSON.stringify({
            ...
            $(`.attach-item-box`).click(function () {
                if ($(this).data("isDelete") == undefined) {
                    ...
                    if ($(`#attachment-box-${compatch}`).data("isAttached")) {
                        ...
                        $.post("https://inventory/AddItemFromAttachment", JSON.stringify({
                            itemData: foundItem,
                        }));
