...
Callback.Functions.CreateCallback("ls-inventory:s:getPlayerInventory", function(source, cb)
    ...
    if PlayerID ~= nil then
        if CurrentInventorys[PlayerID] ~= nil then
            ...
            local resultCheck = Config.Database(Config.DatabaseName, 'fetchAll', 'SELECT * FROM `user_inventory` WHERE `identifier` = @identifier', {['@identifier'] = PlayerID})
			print("TEST")
			print(resultCheck[1])
...
Callback.Functions.CreateCallback("ls-inventory:s:getSecondInventory", function(source, cb, inventory)
    if inventory.type == "vehicle" then
        if CurrentInventorys[inventory.identifier] ~= nil then
            ...
            local resultCheck = Config.Database(Config.DatabaseName, 'fetchAll', 'SELECT * FROM `user_inventory` WHERE `identifier` = @identifier', {['@identifier'] = inventory.identifier})
            if resultCheck[1] ~= nil then
...
function saveInventory()
    for _,v in pairs(CurrentInventorys) do
        if (v.temporary == nil or not v.temporary) then
            Config.Database(Config.DatabaseName, 'execute', 'UPDATE `user_inventory` SET `data` = @data WHERE `identifier` = @identifier', {
