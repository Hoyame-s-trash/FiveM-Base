RegisterNetEvent("bluestark_inventory:ITEM_MOVE_TO_SLOT", function(d)
...
RegisterNetEvent("bluestark_inventory:CLOSE_SECOND_INVENTORY", function(uniqueID)
    local source <const> = source
...
RegisterNetEvent("bluestark_inventory:ADD_NOTE_ITEM", function(data)
    local source <const> = source
...
RegisterNetEvent("bluestark_inventory:RENAME_ITEM", function(data)
    local source <const> = source
    local itemHash <const> = data.itemHash
    local uniqueID <const> = data.uniqueID
...
RegisterNetEvent("bluestark_inventory:OPEN_VEHICLE_TRUNK_INVENTORY", function(plate, modelHash)
    local source <const> = source
...
RegisterNetEvent("bluestark_inventory:OPEN_VEHICLE_GLOVEBOX_INVENTORY", function(plate, modelHash)
    local source <const> = source
    if not CONFIG.IS_VEHICLE_EXIST(plate) then return end
...
RegisterNetEvent("bluestark_inventory:OPEN_NEAR_SHOP", function(shopId)
    local source <const> = source
...
RegisterNetEvent("bluestark_inventory:OPEN_NEAR_FACTION_SAFE", function(faction)
    local source <const> = source
...
RegisterNetEvent("bluestark_inventory:BUY_FROM_SHOP", function(data)
    local source <const> = source
...
RegisterNetEvent("bluestark_inventory:GIVE_ITEM_TO_TARGET", function(data)
    local source <const> = source
    local itemHash <const> = data.itemHash
...
RegisterNetEvent("bluestark_inventory:DROP_ITEM_ON_GROUND", function(data)
    local source <const> = source
    local uniqueID <const> = data.uniqueID
    local itemHash <const> = data.itemHash
    local quantity = data.quantity
...
RegisterNetEvent("bluestark_inventory:OPEN_NEAR_DROPPED_GRID", function()
    local source <const> = source
    local pX <const>, pY <const>, pZ <const> = table.unpack(GetEntityCoords(GetPlayerPed(source)))
...
RegisterNetEvent("bluestark_inventory:REDUCE_WEAPON_AMMO", function()
    local source <const> = source
...
RegisterNetEvent("bluestark_inventory:USE_ITEM", function(data)
    local source <const> = source
    local itemHash <const> = data.itemHash
    local uniqueID <const> = data.uniqueID
...
RegisterNetEvent("bluestark_inventory:USE_SLOT", function(slot)
    local source <const> = source
    local inventory <const> = ScriptServer.Managers.Inventory:GetInventory({ source = source })
    if not inventory then return end
...
RegisterNetEvent("bluestark_inventory:OPEN_STASH", function(uniqueID)
    local source <const> = source
    exports[ScriptServer.resourceName]:OpenStash(source, uniqueID)
end)

RegisterNetEvent("bluestark_inventory:ITEM_ADD_ATTACHMENT_WEAPON", function(d)
    local source <const> = source
    local fromUniqueID <const> = d.fromUniqueID
    local toUniqueID <const> = d.toUniqueID
    ...
    if to_inventory.type == "player" then
        TriggerClientEvent("bluestark_inventory:UpdateWeaponAttachments", to_inventory.source, to_item)
    end
end)

RegisterNetEvent("bluestark_inventory:ITEM_REMOVE_ATTACHMENT_WEAPON", function(d)
    local source <const> = source
    local fromUniqueID <const> = d.fromUniqueID
    ...
    if response.success then
        ...
        if to_inventory.type == "player" then
            TriggerClientEvent("bluestark_inventory:UpdateWeaponAttachments", to_inventory.source, from_item)
        end
    end
end)
RegisterNetEvent("bluestark_inventory:REDUCE_WEAPON_DURABILITY", function()
    local source <const> = source
    local player_inventory <const> = ScriptServer.Managers.Inventory:GetInventory({ source = source }) --[[@as PlayerInventory]]
    if not player_inventory then return end
