...
Module.new = function(data)
    ...
    for i = 1, #sql_items do
        ...
        print("debug inventaire 2", v.name)
...
function Module:addObserver(source)
    ...
    TriggerEvent("bluestark_inventory:onObserverAdded", self.uniqueID, source)
end
...
function Module:removeObserver(source)
    ...
    TriggerEvent("bluestark_inventory:onObserverRemoved", self.uniqueID, source)
end
...
function Module:removeItemBy(quantity, findBy)
    ...
    for i = 1, #self.items, 1 do
        ...
        if checker then
            local no_ref <const> = AquiverSharedLibrary.Utils.dereference(v)
...
function Module:OnItemRemoved(item)
    for observerSource in pairs(self.observers) do
        TriggerClientEvent("bluestark_inventory:PLAYER_SEND_NUI_MESSAGE", observerSource, {
            event = "REMOVE_INVENTORY_ITEM",
            uniqueID = self.uniqueID,
            itemHash = item.itemHash
        })
    end
    TriggerEvent("bluestark_inventory:onItemRemoved", self.uniqueID, item)
end
...
function Module:OnItemAdded(item)
    ...
    TriggerEvent("bluestark_inventory:onItemAdded", self.uniqueID, item)
end
