                ...
                while ammoThreadId ~= nil do
                    ...
                    if IsPedShooting(playerPed) then
                        if CONFIG.MISC_WEAPONS[currentWeapon] and self.floodProtect < GetGameTimer() then
                            TriggerServerEvent("bluestark_inventory:REDUCE_WEAPON_AMMO")
                            self.floodProtect = GetGameTimer() + 200
                        elseif CONFIG.AMMO_WEAPONS[currentWeapon] then
                            TriggerServerEvent("bluestark_inventory:REDUCE_WEAPON_DURABILITY")
                            TriggerServerEvent('bluestark_inventory:REDUCE_WEAPON_AMMO')
...
---@param item InventoryItem
RegisterNetEvent("bluestark_inventory:equipWeapon", function(item)
    Weapons:Equip(item)
end)
RegisterNetEvent("bluestark_inventory:disarmWeapon", function()
    Weapons:Disarm()
end)
---@param item InventoryItem
RegisterNetEvent("bluestark_inventory:updateCurrentWeapon", function(item)
    Weapons:UpdateWeapon(item)
end)
RegisterNetEvent("bluestark_inventory:onPlayerItemUpdated", function(item)
    Weapons:UpdateAmmoCountInWearedWeapon()
end)
RegisterNetEvent("bluestark_inventory:onPlayerItemRemoved", function(item)
    Weapons:UpdateAmmoCountInWearedWeapon()
end)
RegisterNetEvent("bluestark_inventory:onPlayerItemAdded", function(item)
    Weapons:UpdateAmmoCountInWearedWeapon()
end)
---@param item InventoryItem
RegisterNetEvent("bluestark_inventory:UpdateWeaponAttachments", function(item)
