RegisterNUICallback("ITEM_MOVE_TO_SLOT", function(d, cb)
    TriggerServerEvent("bluestark_inventory:ITEM_MOVE_TO_SLOT", d)
    cb({})
end)
RegisterNUICallback("ADD_NOTE_ITEM", function(d, cb)
    TriggerServerEvent("bluestark_inventory:ADD_NOTE_ITEM", d)
    cb({})
end)
RegisterNUICallback("RENAME_ITEM", function(d, cb)
    TriggerServerEvent("bluestark_inventory:RENAME_ITEM", d)
    cb({})
end)
RegisterNUICallback("USE_ITEM", function(d, cb)
    TriggerServerEvent("bluestark_inventory:USE_ITEM", d)
    cb({})
end)
RegisterNUICallback("BUY_FROM_SHOP", function(d, cb)
    TriggerServerEvent("bluestark_inventory:BUY_FROM_SHOP", d)
    cb({})
end)
RegisterNUICallback("ITEM_ADD_ATTACHMENT_WEAPON", function(d, cb)
    TriggerServerEvent("bluestark_inventory:ITEM_ADD_ATTACHMENT_WEAPON", d)
    cb({})
end)
RegisterNUICallback("ITEM_REMOVE_ATTACHMENT_WEAPON", function(d, cb)
    TriggerServerEvent("bluestark_inventory:ITEM_REMOVE_ATTACHMENT_WEAPON", d)
    cb({})
end)
RegisterNUICallback("CLOSE_SECOND_INVENTORY", function(d, cb)
    local uniqueID = d.uniqueID
    TriggerServerEvent("bluestark_inventory:CLOSE_SECOND_INVENTORY", uniqueID)
    cb({})
end)
RegisterNUICallback("NEARBY_GET_PLAYERS", function(_, cb)
    ...
    if #nearPlayers < 1 then
        AquiverClientLibrary.LocalPlayer.notification("info", "No players are nearby!")
    end

    cb(nearPlayers)
end)
RegisterNUICallback("GIVE_ITEM_TO_TARGET", function(d, cb)
    TriggerServerEvent("bluestark_inventory:GIVE_ITEM_TO_TARGET", d)
    cb({})
end)
RegisterNUICallback("DROP_ITEM_ON_GROUND", function(d, cb)
    TriggerServerEvent("bluestark_inventory:DROP_ITEM_ON_GROUND", d)
    cb({})
end)
