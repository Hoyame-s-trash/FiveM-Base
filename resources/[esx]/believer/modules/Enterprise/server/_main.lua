...
RegisterServerEvent("Admin:enterpriseChangeRankId", function(enterpriseId, gradeId, newGradeId)
    local playerSrc = source
    if (not playerSrc) then return end
    ...
    if (not playerRank:getPermissionsValue("CHANGE_ENTERPRISE_RANK_ID", playerSelected.source)) then return end
    ...
    if (enterpriseSelected.ranks[gradeId].name == "owner") then
        TriggerClientEvent("esx:showNotification", playerSrc, "~r~Vous ne pouvez pas changer la hiéarchie du grade owner.")
        return
    end

    -- If the newRankId doen't exist then replace the old one with the newRankId and save it in the database elseif the newRankId exist then replace the old one with the newRankId and save it in the database

    if (not enterpriseSelected.ranks[newGradeId]) then
        print("NOUVEAU GRADE")
        enterpriseSelected.ranks[gradeId].id = newGradeId
        enterpriseSelected.ranks[newGradeId] = enterpriseSelected.ranks[gradeId]
        enterpriseSelected.ranks[gradeId] = nil
    else
        print("GRADE EXISTANT")
        local oldRank = enterpriseSelected.ranks[gradeId]
        enterpriseSelected.ranks[gradeId] = enterpriseSelected.ranks[newGradeId]
        enterpriseSelected.ranks[newGradeId] = oldRank
    end
