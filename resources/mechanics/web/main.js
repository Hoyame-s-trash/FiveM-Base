...
$(function () {
    window.addEventListener('message', function (event) {
        ...
        if (type === 'OpenMechanic') {
            ...
            $.post('http://mechanics/CloseBossMenu', JSON.stringify({}))
        } else if (type == "OdemeBasarili") {
            $('#mContainer').fadeOut(500);
            $('.ButtonKisim').html("");
            $('.FaturalandirmaPanel').fadeOut(500)
            $.post('http://mechanics/Klose', JSON.stringify({}));
...
$(document).on('click', '#SelectCar', function () {
    $.post('http://mechanics/SpawnTownTruck', JSON.stringify({
...
$(document).on('click', '#Alacam', function () {
    $.post('http://mechanics/Klose', JSON.stringify({}));
    $.post('http://mechanics/Verified', JSON.stringify({
...
$(document).on('click', '#KapatBirader', function () {
    $.post('http://mechanics/CloseMechanic', JSON.stringify({}));
    $.post('http://mechanics/AlmayacamBaba', JSON.stringify({
        calisanID: $(this).attr("felsefeci")
    }));
})

$(document).on('click', '#FixClean', function () {
    $.post('http://mechanics/FixCleanCar', JSON.stringify({
        ucret: repairfee
    }));
})
...
$(document).on('click', '#BireyselFaturaKes', function () {
    $.post('http://mechanics/BireyselOdeme', JSON.stringify({
...
$(document).on('click', '#OkudumKabulEdiyom', function () {
    $.post('http://mechanics/BuyModifiye', JSON.stringify({
        MusteriSource: $(this).attr("MusteriSource"),
        TotalUcret: toplamOdenecek,
        CalisaninAlacagi: CalisaninAlacagiYazik,
        MekanikID: mechanicId,
...
$(document).on('click', '#FreeMekanik', function () {
    ...
    HerseyiKapat5()
    $.post('http://mechanics/CloseMechanicBaba', JSON.stringify({}));
})

$(document).on('click', '#CinYurt', function () {
    $.post('http://mechanics/TablodanSil', JSON.stringify({
...
$(document).on('click', '#TekrarEskiyiAc', function () {
    ...
    $('.UrunListesi').html("");
    $('.ButtonKisim').html("");
    $.post('http://mechanics/FaturaKesilebilecekAdamlar', JSON.stringify({}));
})
...
$(document).on('click', '#GeriDon', function () {
    ...
    $('.UrunSepeti').html("")
    $("#mContainer").fadeIn(500);
    $.post('http://mechanics/TabloBosalt', JSON.stringify({}));
})
...
$(document).on('click', '#BasMoruk', function () {
    ...
    $('#YenileKardesim').html("");
    $.post('http://mechanics/FaturaKesMenusu', JSON.stringify({
...
$(document).on('click', '#TabloyuBaa', function () {
    $.post('http://mechanics/SilTabloyu', JSON.stringify({
        id: mechanicId
    }));
})
...
RenkTabloUplaod = function (table, icon, modtype, backMenu, menuType, menuStringID) {
    ...
    if (rgbColor == false) {
        ...
        renkSelect.addEventListener("input", function () {
            ...
            $.post('http://mechanics/RGBLoad', JSON.stringify({
                r: rgb.r,
                g: rgb.g,
                b: rgb.b,
                type: menuNumber,
...
$(document).on('click', '#RenkUpload', function () {
    $.post('http://mechanics/RenkUpload', JSON.stringify({
...
$(document).on('click', '#OdemeYap', function () {
    ...
    if (company) {
        ...
        $.post('http://mechanics/BireyselFatura', JSON.stringify({}));
    }
})
...
$(document).on('click', '#UpdateWage', function () {
    ...
    if (YuzdeUnput < 100) {
        $.post('http://mechanics/UpdateWage', JSON.stringify({
            yuzde: YuzdeUnput,
            mekanikID: mechanicId
        }));
    } else {
...
$(document).on('click', '#UpdateMechanicName', function () {
    $.post('http://mechanics/MechanicNameUpdate', JSON.stringify({
        mekanikID: mechanicId,
...
$(document).on('click', '#UpdateRate', function () {
    $.post('http://mechanics/MechanicDiscountRate', JSON.stringify({
        mekanikID: mechanicId,
...
$(document).on('click', '#UpdateRepair', function () {
    $.post('http://mechanics/MechanicRepairFee', JSON.stringify({
        mekanikID: mechanicId,
...
$(document).on('click', '#AlIse', function () {
    $.post('http://mechanics/UsagiAlIse', JSON.stringify({
...
$(document).on('click', '#Kov', function () {
    $.post('http://mechanics/UsagiKov', JSON.stringify({
...
$(document).on('click', '#IseAl', function () {
    $.post('http://mechanics/UsagiIseAl', JSON.stringify({
        mekanikID: mechanicId,
    }));
})
...
$(document).on('click', '#Cek', function () {
    ...
    $.post('http://mechanics/ParaIslemleri', JSON.stringify({
...
$(document).on('click', '#RenkSec', function () {
    $.post('http://mechanics/AltRenkMenusuSec', JSON.stringify({
        color: $(this).attr("secilenRenk"),
        modtype: $(this).attr("modtype"),
...
LoadOption = function (datax, icon, modtype, props, parcaid, tekerloc, back) {
    ...
    if (modtype == 17) {
    ...
    } else if ((modtype == 0) || (modtype == 1) || (modtype == 2) || (modtype == 3) || (modtype == 4) || (modtype == 6) || (modtype == 7) || (modtype == 10) || (modtype == 13) || (modtype == 36)) {
        $('.CarCustomize').html(`
        <div id="${back}" style="top: 0.2vh;" class="XPoart">
            <center><i class="fa-solid fa-rotate-left"></i></center>
            <h3>${lang["Back"]}</h3>
        ...
        for (let i = -1; i < datax; i++) {
            ...
            if (props[parcaid] == i) {
                ...
                xtext = `
                <div data-parca="${parcaid}"  id="modselect" data-type="${i}" data-fiyat="${Math.abs(totalUcret)}" data-modtype="${modtype}" class="XPoart">
                    <center><img class="mechaniclogo" src="${icon}"></center>
                    <h4>${lang["Level"]} ${i + 2}</h3>
                    <p style="top: -5.8vh;">$${Math.abs(totalUcret)}</p>
                </div>`;
            }
            $('.CarCustomize').append(xtext);
        }
        ...
        for (let index = 0; index < 5; index++) {
            ...
            if (props[parcaid] == index) {
                ...
                xtext = `
                <div data-parca="${parcaid}"  id="modselect" data-type="${index}" data-fiyat="${Math.abs(totalUcret)}" data-modtype="${modtype}" class="XPoart">
                    <center><img class="mechaniclogo" src="${icon}"></center>
...
$(document).on('click', '#modselect', function () {
    $.post('http://mechanics/ModLoad', JSON.stringify({
...
$(document).on('click', '#neonIptal', function () {
    $.post('http://mechanics/NeonRemove', JSON.stringify({
...
$(document).on('click', '#MekanikSat', function () {
    $.post('http://mechanics/MekanikSell', JSON.stringify({
        mekanikId: mechanicId
    }));
})
...
$(document).on('click', '#AllOpenDoorsButton', function () {
    $.post('http://mechanics/OpenAllDoors', JSON.stringify({}));
})

$(document).on('click', '#back', function () {
    $.post('http://mechanics/MenuBack', JSON.stringify({}));
})
...
$(document).on('click', '#back2', function () {
    $.post('http://mechanics/MenuBack2', JSON.stringify({}));
})


$(document).on('click', '#tekerlekmenusu', function () {
    $.post('http://mechanics/TekerlekMenusu', JSON.stringify({}));
})

$(document).on('click', '#back3', function () {
    $.post('http://mechanics/MenuBack3', JSON.stringify({}));
})

$(document).on('click', '#renkturleri', function () {
    $.post('http://mechanics/RenkTurleri', JSON.stringify({}));
})


$(document).on('click', '#back4', function () {
    $.post('http://mechanics/MenuBack4', JSON.stringify({}));
})

$(document).on('click', '#ustrenkler', function () {
    $.post('http://mechanics/UstRenkleriYukle', JSON.stringify({}));
})

$(document).on('click', '#menuButton', function () {
    $.post('http://mechanics/LoadMenu', JSON.stringify({
...
//     $.post('http://mechanics/CursorActive', JSON.stringify({}));
// })
...
$(document).on('click', '#buttonplate', function () {
    $.post('http://mechanics/OpenPlate', JSON.stringify({}));
})
