-- Variable declaration --
BlueStarkInventory                                  = {}
BlueStarkInventory.Functions                        = {}

-- This variable declaration, uses your database
BlueStarkInventory.DatabaseName = "mysql-async"

BlueStarkInventory.Notify = function(message, type) 
    BlueStarkInventory.ServerFramework.ShowHelpNotification(message)
end

BlueStarkInventory.Version = "1.0.3" -- DO NOT CHANGE IT WILL BE UPDATED!

BlueStarkInventory.Debug = false -- IF YOU WANT TO STABLE VERSION OF INVENTORY DO NOT MAKE TRUE!

BlueStarkInventory.CheckInventoryEveryStart = false

BlueStarkInventory.FivemAppeareance = true

-- Database function do not change, if your database doesnt in here you can add

BlueStarkInventory.Database                         = function(plugin,type,query,var)
    local wait = promise.new()
    if type == 'fetchAll' and plugin == 'mysql-async' then
        MySQL.Async.fetchAll(query, var, function(result)
            wait:resolve(result)
        end)
    end
    if type == 'execute' and plugin == 'mysql-async' then
        MySQL.Async.execute(query, var, function(result)
            wait:resolve(result)
        end)
    end
    if type == 'execute' and plugin == 'ghmattisql' then
        exports['ghmattimysql']:execute(query, var, function(result)
            wait:resolve(result)
        end)
    end
    if type == 'fetchAll' and plugin == 'ghmattisql' then
        exports.ghmattimysql:execute(query, var, function(result)
            wait:resolve(result)
        end)
    end
    if type == 'execute' and plugin == 'oxmysql' then
        exports.oxmysql:query(query, var, function(result)
            wait:resolve(result)
        end)
    end
    if type == 'fetchAll' and plugin == 'oxmysql' then
        exports.oxmysql:query(query, var, function(result)
            wait:resolve(result)
        end)
    end
    return Citizen.Await(wait)
end

-- When click "TAB" or custom key will be make blur screen
-- to close you need to change true to false

BlueStarkInventory.Blur                     = true

BlueStarkInventory.HAHud                    = true -- Enables armor and health hud top on character


-- Weight settings
-- "BlueStarkInventory.Weight.Divide", qb-core uses weights in grams if you use kilograms change 1000 to 1
-- "BlueStarkInventory.Weight.MaxPlayerWeight", player max weight limit this "limit can be EXCEEDED" but walk speed will slow down
-- "BlueStarkInventory.Weight.OverweightMultiplier", when if it exceeds the limit, multiplys with 2, calculation formula ( MaxWeight/(CurrentWeight*MultiplyNumber) )

BlueStarkInventory.Weight                       = {}
BlueStarkInventory.Weight.Divide                = 1
BlueStarkInventory.Weight.MaxPlayerWeight       = 92
BlueStarkInventory.Weight.OverweightMultiplier  = 2


-- Drop settings
-- "BlueStarkInventory.RefreshDrops", refreshes all drops in typed seconds, used in client-side
-- "BlueStarkInventory.RefreshServerDrops", refreshes and check all drops in typed seconds, used in server-side

BlueStarkInventory.RefreshDrops         = 1 -- In seconds
BlueStarkInventory.RefreshServerDrops   = 1 -- In seconds

BackEngineVehicles = {
    ["ninef"] = true,
    ["adder"] = true,
    ["vagner"] = true,
    ["t20"] = true,
    ["infernus"] = true,
    ["zentorno"] = true,
    ["reaper"] = true,
    ["comet2"] = true,
    ["comet3"] = true,
    ["jester"] = true,
    ["jester2"] = true,
    ["cheetah"] = true,
    ["cheetah2"] = true,
    ["prototipo"] = true,
    ["turismor"] = true,
    ["pfister811"] = true,
    ["ardent"] = true,
    ["nero"] = true,
    ["nero2"] = true,
    ["tempesta"] = true,
    ["vacca"] = true,
    ["bullet"] = true,
    ["osiris"] = true,
    ["entityxf"] = true,
    ["turismo2"] = true,
    ["fmj"] = true,
    ["re7b"] = true,
    ["tyrus"] = true,
    ["italigtb"] = true,
    ["penetrator"] = true,
    ["monroe"] = true,
    ["ninef2"] = true,
    ["stingergt"] = true,
    ["surfer"] = true,
    ["surfer2"] = true,
    ["gp1"] = true,
    ["autarch"] = true,
    ["tyrant"] = true
}

-- All clothing sizes, every clothes can have custom size


Variations = { Jackets = {Male = {}, Female = {}}, Hair = {Male = {}, Female = {}}, Bags = {Male = {}, Female = {}}, Visor = {Male = {}, Female = {}}, Gloves = { Male = { [16] = 4, [17] = 4, [18] = 4, [19] = 0, [20] = 1, [21] = 2, [22] = 4, [23] = 5, [24] = 6, [25] = 8, [26] = 11, [27] = 12, [28] = 14, [29] = 15, [30] = 0, [31] = 1, [32] = 2, [33] = 4, [34] = 5, [35] = 6, [36] = 8, [37] = 11, [38] = 12, [39] = 14, [40] = 15, [41] = 0, [42] = 1, [43] = 2, [44] = 4, [45] = 5, [46] = 6, [47] = 8, [48] = 11, [49] = 12, [50] = 14, [51] = 15, [52] = 0, [53] = 1, [54] = 2, [55] = 4, [56] = 5, [57] = 6, [58] = 8, [59] = 11, [60] = 12, [61] = 14, [62] = 15, [63] = 0, [64] = 1, [65] = 2, [66] = 4, [67] = 5, [68] = 6, [69] = 8, [70] = 11, [71] = 12, [72] = 14, [73] = 15, [74] = 0, [75] = 1, [76] = 2, [77] = 4, [78] = 5, [79] = 6, [80] = 8, [81] = 11, [82] = 12, [83] = 14, [84] = 15, [85] = 0, [86] = 1, [87] = 2, [88] = 4, [89] = 5, [90] = 6, [91] = 8, [92] = 11, [93] = 12, [94] = 14, [95] = 15, [96] = 4, [97] = 4, [98] = 4, [99] = 0, [100] = 1, [101] = 2, [102] = 4, [103] = 5, [104] = 6, [105] = 8, [106] = 11, [107] = 12, [108] = 14, [109] = 15, [110] = 4, [111] = 4, [115] = 112, [116] = 112, [117] = 112, [118] = 112, [119] = 112, [120] = 112, [121] = 112, [122] = 113, [123] = 113, [124] = 113, [125] = 113, [126] = 113, [127] = 113, [128] = 113, [129] = 114, [130] = 114, [131] = 114, [132] = 114, [133] = 114, [134] = 114, [135] = 114, [136] = 15, [137] = 15, [138] = 0, [139] = 1, [140] = 2, [141] = 4, [142] = 5, [143] = 6, [144] = 8, [145] = 11, [146] = 12, [147] = 14, [148] = 112, [149] = 113, [150] = 114, [151] = 0, [152] = 1, [153] = 2, [154] = 4, [155] = 5, [156] = 6, [157] = 8, [158] = 11, [159] = 12, [160] = 14, [161] = 112, [162] = 113, [163] = 114, [165] = 4, [166] = 4, [167] = 4, }, Female = { [16] = 11, [17] = 3, [18] = 3, [19] = 3, [20] = 0, [21] = 1, [22] = 2, [23] = 3, [24] = 4, [25] = 5, [26] = 6, [27] = 7, [28] = 9, [29] = 11, [30] = 12, [31] = 14, [32] = 15, [33] = 0, [34] = 1, [35] = 2, [36] = 3, [37] = 4, [38] = 5, [39] = 6, [40] = 7, [41] = 9, [42] = 11, [43] = 12, [44] = 14, [45] = 15, [46] = 0, [47] = 1, [48] = 2, [49] = 3, [50] = 4, [51] = 5, [52] = 6, [53] = 7, [54] = 9, [55] = 11, [56] = 12, [57] = 14, [58] = 15, [59] = 0, [60] = 1, [61] = 2, [62] = 3, [63] = 4, [64] = 5, [65] = 6, [66] = 7, [67] = 9, [68] = 11, [69] = 12, [70] = 14, [71] = 15, [72] = 0, [73] = 1, [74] = 4, [75] = 3, [76] = 4, [77] = 5, [78] = 6, [79] = 7, [80] = 9, [81] = 11, [82] = 12, [83] = 14, [84] = 15, [85] = 0, [86] = 1, [87] = 2, [88] = 3, [89] = 4, [90] = 5, [91] = 6, [92] = 7, [93] = 9, [94] = 11, [95] = 12, [96] = 14, [97] = 15, [98] = 0, [99] = 1, [100] = 2, [101] = 3, [102] = 4, [103] = 5, [104] = 6, [105] = 7, [106] = 9, [107] = 11, [108] = 12, [109] = 14, [110] = 15, [111] = 3, [112] = 3, [113] = 3, [114] = 0, [115] = 1, [116] = 2, [117] = 3, [118] = 4, [119] = 5, [120] = 6, [121] = 7, [122] = 9, [123] = 11, [124] = 12, [125] = 14, [126] = 15, [127] = 3, [128] = 3, [132] = 129, [133] = 129, [134] = 129, [135] = 129, [136] = 129, [137] = 129, [138] = 129, [139] = 130, [140] = 130, [141] = 130, [142] = 130, [143] = 130, [144] = 130, [145] = 130, [146] = 131, [147] = 131, [148] = 131, [149] = 131, [150] = 131, [151] = 131, [152] = 131, [154] = 153, [155] = 153, [156] = 153, [157] = 153, [158] = 153, [159] = 153, [160] = 153, [162] = 161, [163] = 161, [164] = 161, [165] = 161, [166] = 161, [167] = 161, [168] = 161, [169] = 15, [170] = 15, [171] = 0, [172] = 1, [173] = 2, [174] = 3, [175] = 4, [176] = 5, [177] = 6, [178] = 7, [179] = 9, [180] = 11, [181] = 12, [182] = 14, [183] = 129, [184] = 130, [185] = 131, [186] = 153, [187] = 0, [188] = 1, [189] = 2, [190] = 3, [191] = 4, [192] = 5, [193] = 6, [194] = 7, [195] = 9, [196] = 11, [197] = 12, [198] = 14, [199] = 129, [200] = 130, [201] = 131, [202] = 153, [203] = 161, [204] = 161, [206] = 3, [207] = 3, [208] = 3, } } }

BlueStarkInventory.ClothingWear = {
    ["top"] = {
		Drawable = 11,
        DatabaseName = "torso_1",
        TypeItem = "components",
		Table = { Male = 15, Female = 74},
		Emote = {Dict = "clothingtie", Anim = "try_tie_negative_a", Move = 51, Dur = 1200}
	},
    ["shirts"] = {
		Drawable = 8,
        DatabaseName = "tshirt_1",
        TypeItem = "components",
		Table = { Male = 15, Female = 15},
		Emote = {Dict = "clothingtie", Anim = "try_tie_negative_a", Move = 51, Dur = 1200}
	},
    ["pants"] = {
        Drawable = 4,
        DatabaseName = "pants_1",
        TypeItem = "components",
		Table = {Male = 61, Female = 14},
		Emote = {Dict = "re@construction", Anim = "out_of_breath", Move = 51, Dur = 1300}
    },
    ["backpack"] = {
        Drawable = 5,
        DatabaseName = "bags_1",
        TypeItem = "components",
		Table = {Male = 0, Female = 0},
		Emote = {Dict = "clothingtie", Anim = "try_tie_negative_a", Move = 51, Dur = 1200}
    },
    ["shoes"] = {
		Drawable = 6,
        DatabaseName = "shoes_1",
        TypeItem = "components",
		Table = {Male = 34, Female = 35},
		Emote = {Dict = "random@domestic", Anim = "pickup_low", Move = 0, Dur = 1200}
	},
    ["mask"] = {
		Drawable = 1,
        DatabaseName = "mask_1",
        TypeItem = "components",
		Table = {Male = 0, Female = 0 },
		Emote = {Dict = "mp_masks@standard_car@ds@", Anim = "put_on_mask", Move = 51, Dur = 800}
	},
    ["rig"] = {
		Drawable = 9,
        DatabaseName = "bproof_1",
        TypeItem = "components",
		Table = {Male = 0, Female = 0 },
		Emote = {Dict = "mp_masks@standard_car@ds@", Anim = "put_on_mask", Move = 51, Dur = 800}
	},
    ["gloves"] = {
		Drawable = 3,
        DatabaseName = "arms_1",
		Table = Variations.Gloves,
        DrawableCheck = 11,
        TypeItem = "components",
		Emote = {Dict = "nmt_3_rcm-10", Anim = "cs_nigel_dual-10", Move = 51, Dur = 1200}
	},
    ["hat"] = {
		Prop = 0,
        DatabaseName = "helmet_1",
        TypeItem = "props",
		Emote = {Dict = "mp_masks@standard_car@ds@", Anim = "put_on_mask", Move = 51, Dur = 600},
	},
    ["ear"] = {
		Prop = 2,
        DatabaseName = "ears_1",
        TypeItem = "props",
		Emote = {Dict = "mp_cp_stolen_tut", Anim = "b_think", Move = 51, Dur = 900},
	},
    ["watch"] = {
		Prop = 6,
        DatabaseName = "watches_1",
        TypeItem = "props",
		Emote = {Dict = "nmt_3_rcm-10", Anim = "cs_nigel_dual-10", Move = 51, Dur = 1200},
	},
    ["glasses"] = {
		Prop = 1,
        DatabaseName = "glasses_1",
        TypeItem = "props",
		Emote = {Dict = "clothingspecs", Anim = "take_off", Move = 51, Dur = 1400},
	},
    ["bracelet"] = {
		Prop = 7,
        DatabaseName = "bracelets_1",
        TypeItem = "props",
		Emote = {Dict = "nmt_3_rcm-10", Anim = "cs_nigel_dual-10", Move = 51, Dur = 1200},
	},
}

BlueStarkInventory.ReloadTime = math.random(4000, 6000)

BlueStarkInventory.DurabilityBlockedWeapons = {
    "weapon_stungun",
    "weapon_nightstick",
    "weapon_flashlight",
    "weapon_unarmed",
}

BlueStarkInventory.WeaponList = {
	-- // WEAPONS
	-- Melee
	[`weapon_unarmed`] 				 = {['name'] = 'weapon_unarmed', 		['label'] = 'Fists', 				['weapontype'] = 'Melee',	['ammotype'] = nil, ['damagereason'] = 'Melee killed / Whacked / Executed / Beat down / Murdered / Battered'},
	[`weapon_dagger`] 				 = {['name'] = 'weapon_dagger', 		['label'] = 'Dagger', 				['weapontype'] = 'Melee',	['ammotype'] = nil,	['damagereason'] = 'Knifed / Stabbed / Eviscerated'},
	[`weapon_bat`] 					 = {['name'] = 'weapon_bat', 			['label'] = 'Bat', 					['weapontype'] = 'Melee',	['ammotype'] = nil,	['damagereason'] = 'Melee killed / Whacked / Executed / Beat down / Murdered / Battered'},
	[`weapon_bottle`] 				 = {['name'] = 'weapon_bottle', 		['label'] = 'Broken Bottle', 		['weapontype'] = 'Melee',	['ammotype'] = nil,	['damagereason'] = 'Knifed / Stabbed / Eviscerated'},
	[`weapon_crowbar`] 				 = {['name'] = 'weapon_crowbar', 		['label'] = 'Crowbar', 				['weapontype'] = 'Melee',	['ammotype'] = nil,	['damagereason'] = 'Melee killed / Whacked / Executed / Beat down / Murdered / Battered'},
	[`weapon_flashlight`] 			 = {['name'] = 'weapon_flashlight', 	['label'] = 'Flashlight', 			['weapontype'] = 'Melee',	['ammotype'] = nil,	['damagereason'] = 'Melee killed / Whacked / Executed / Beat down / Murdered / Battered'},
	[`weapon_golfclub`] 			 = {['name'] = 'weapon_golfclub', 		['label'] = 'Golfclub', 			['weapontype'] = 'Melee',	['ammotype'] = nil,	['damagereason'] = 'Melee killed / Whacked / Executed / Beat down / Murdered / Battered'},
	[`weapon_hammer`] 				 = {['name'] = 'weapon_hammer', 		['label'] = 'Hammer', 				['weapontype'] = 'Melee',	['ammotype'] = nil,	['damagereason'] = 'Melee killed / Whacked / Executed / Beat down / Murdered / Battered'},
	[`weapon_hatchet`] 				 = {['name'] = 'weapon_hatchet', 		['label'] = 'Hatchet', 				['weapontype'] = 'Melee',	['ammotype'] = nil,	['damagereason'] = 'Knifed / Stabbed / Eviscerated'},
	[`weapon_knuckle`] 				 = {['name'] = 'weapon_knuckle', 		['label'] = 'Knuckle', 				['weapontype'] = 'Melee',	['ammotype'] = nil,	['damagereason'] = 'Melee killed / Whacked / Executed / Beat down / Murdered / Battered'},
	[`weapon_knife`] 				 = {['name'] = 'weapon_knife', 			['label'] = 'Knife', 				['weapontype'] = 'Melee',	['ammotype'] = nil,	['damagereason'] = 'Knifed / Stabbed / Eviscerated'},
	[`weapon_machete`] 				 = {['name'] = 'weapon_machete', 		['label'] = 'Machete', 				['weapontype'] = 'Melee',	['ammotype'] = nil,	['damagereason'] = 'Knifed / Stabbed / Eviscerated'},
	[`weapon_switchblade`] 			 = {['name'] = 'weapon_switchblade', 	['label'] = 'Switchblade', 			['weapontype'] = 'Melee',	['ammotype'] = nil,	['damagereason'] = 'Knifed / Stabbed / Eviscerated'},
	[`weapon_nightstick`] 			 = {['name'] = 'weapon_nightstick', 	['label'] = 'Nightstick', 			['weapontype'] = 'Melee',	['ammotype'] = nil,	['damagereason'] = 'Melee killed / Whacked / Executed / Beat down / Murdered / Battered'},
	[`weapon_wrench`] 				 = {['name'] = 'weapon_wrench', 		['label'] = 'Wrench', 				['weapontype'] = 'Melee',	['ammotype'] = nil,	['damagereason'] = 'Melee killed / Whacked / Executed / Beat down / Murdered / Battered'},
	[`weapon_battleaxe`] 			 = {['name'] = 'weapon_battleaxe', 		['label'] = 'Battle Axe', 			['weapontype'] = 'Melee',	['ammotype'] = nil,	['damagereason'] = 'Knifed / Stabbed / Eviscerated'},
	[`weapon_poolcue`] 				 = {['name'] = 'weapon_poolcue', 		['label'] = 'Poolcue', 				['weapontype'] = 'Melee',	['ammotype'] = nil,	['damagereason'] = 'Melee killed / Whacked / Executed / Beat down / Murdered / Battered'},
	[`weapon_briefcase`] 			 = {['name'] = 'weapon_briefcase', 		['label'] = 'Briefcase', 			['weapontype'] = 'Melee',	['ammotype'] = nil,	['damagereason'] = 'Melee killed / Whacked / Executed / Beat down / Murdered / Battered'},
	[`weapon_briefcase_02`] 		 = {['name'] = 'weapon_briefcase_02', 	['label'] = 'Briefcase', 			['weapontype'] = 'Melee',	['ammotype'] = nil,	['damagereason'] = 'Melee killed / Whacked / Executed / Beat down / Murdered / Battered'},
	[`weapon_garbagebag`] 			 = {['name'] = 'weapon_garbagebag', 	['label'] = 'Garbage Bag', 			['weapontype'] = 'Melee',	['ammotype'] = nil,	['damagereason'] = 'Melee killed / Whacked / Executed / Beat down / Murdered / Battered'},
	[`weapon_handcuffs`] 			 = {['name'] = 'weapon_handcuffs', 		['label'] = 'Handcuffs', 			['weapontype'] = 'Melee',	['ammotype'] = nil,	['damagereason'] = 'Melee killed / Whacked / Executed / Beat down / Murdered / Battered'},
	[`weapon_bread`] 				 = {['name'] = 'weapon_bread', 			['label'] = 'Baquette', 			['weapontype'] = 'Melee',	['ammotype'] = nil,	['damagereason'] = 'Melee killed / Whacked / Executed / Beat down / Murdered / Battered'},
	[`weapon_stone_hatchet`] 		 = {['name'] = 'weapon_stone_hatchet', 	['label'] = 'Stone Hatchet',        ['weapontype'] = 'Melee',	['ammotype'] = nil,	['damagereason'] = 'Knifed / Stabbed / Eviscerated'},

    -- Handguns
	[`weapon_pistol`] 				 = {['name'] = 'weapon_pistol', 		['label'] = 'Pistol', 				    ['weapontype'] = 'Pistol',	['ammotype'] = 'AMMO_PISTOL',	['damagereason'] = 'Pistoled / Blasted / Plugged / Bust a cap in'},
	[`weapon_pistol_mk2`] 			 = {['name'] = 'weapon_pistol_mk2', 	['label'] = 'Pistol Mk2', 			    ['weapontype'] = 'Pistol',	['ammotype'] = 'AMMO_PISTOL',	['damagereason'] = 'Pistoled / Blasted / Plugged / Bust a cap in'},
	[`weapon_combatpistol`] 		 = {['name'] = 'weapon_combatpistol', 	['label'] = 'Combat Pistol', 			['weapontype'] = 'Pistol',	['ammotype'] = 'AMMO_PISTOL',	['damagereason'] = 'Pistoled / Blasted / Plugged / Bust a cap in'},
	[`weapon_appistol`] 			 = {['name'] = 'weapon_appistol', 		['label'] = 'AP Pistol', 				['weapontype'] = 'Pistol',	['ammotype'] = 'AMMO_PISTOL',	['damagereason'] = 'Pistoled / Blasted / Plugged / Bust a cap in'},
	[`weapon_stungun`] 				 = {['name'] = 'weapon_stungun', 		['label'] = 'Taser', 					['weapontype'] = 'Pistol',	['ammotype'] = 'AMMO_STUNGUN',	['damagereason'] = 'Died'},
	[`weapon_pistol50`] 			 = {['name'] = 'weapon_pistol50', 		['label'] = 'Pistol .50 Cal', 			['weapontype'] = 'Pistol',	['ammotype'] = 'AMMO_PISTOL',	['damagereason'] = 'Pistoled / Blasted / Plugged / Bust a cap in'},
	[`weapon_snspistol`] 			 = {['name'] = 'weapon_snspistol', 		['label'] = 'SNS Pistol', 				['weapontype'] = 'Pistol',	['ammotype'] = 'AMMO_PISTOL',	['damagereason'] = 'Pistoled / Blasted / Plugged / Bust a cap in'},
	[`weapon_snspistol_mk2`] 	     = {['name'] = 'weapon_snspistol_mk2', 	['label'] = 'SNS Pistol MK2', 			['weapontype'] = 'Pistol',	['ammotype'] = 'AMMO_PISTOL',   ['damagereason'] = 'Pistoled / Blasted / Plugged / Bust a cap in'},
	[`weapon_heavypistol`] 			 = {['name'] = 'weapon_heavypistol', 	['label'] = 'Heavy Pistol', 			['weapontype'] = 'Pistol',	['ammotype'] = 'AMMO_PISTOL',	['damagereason'] = 'Pistoled / Blasted / Plugged / Bust a cap in'},
	[`weapon_vintagepistol`] 		 = {['name'] = 'weapon_vintagepistol', 	['label'] = 'Vintage Pistol', 			['weapontype'] = 'Pistol',	['ammotype'] = 'AMMO_PISTOL',	['damagereason'] = 'Pistoled / Blasted / Plugged / Bust a cap in'},
	[`weapon_flaregun`] 			 = {['name'] = 'weapon_flaregun', 		['label'] = 'Flare Gun', 				['weapontype'] = 'Pistol',	['ammotype'] = 'AMMO_FLARE',	['damagereason'] = 'Died'},
	[`weapon_marksmanpistol`] 		 = {['name'] = 'weapon_marksmanpistol', ['label'] = 'Marksman Pistol', 			['weapontype'] = 'Pistol',	['ammotype'] = 'AMMO_PISTOL',	['damagereason'] = 'Pistoled / Blasted / Plugged / Bust a cap in'},
	[`weapon_revolver`] 			 = {['name'] = 'weapon_revolver', 		['label'] = 'Revolver', 				['weapontype'] = 'Pistol',	['ammotype'] = 'AMMO_PISTOL',	['damagereason'] = 'Pistoled / Blasted / Plugged / Bust a cap in'},
	[`weapon_revolver_mk2`] 		 = {['name'] = 'weapon_revolver_mk2', 	['label'] = 'Revolver MK2', 		    ['weapontype'] = 'Pistol',	['ammotype'] = 'AMMO_PISTOL',	['damagereason'] = 'Pistoled / Blasted / Plugged / Bust a cap in'},
	[`weapon_doubleaction`] 	     = {['name'] = 'weapon_doubleaction', 	['label'] = 'Double Action Revolver',	['weapontype'] = 'Pistol',	['ammotype'] = 'AMMO_PISTOL',	['damagereason'] = 'Pistoled / Blasted / Plugged / Bust a cap in'},
	[`weapon_raypistol`]			 = {['name'] = 'weapon_raypistol',		['label'] = 'Ray Pistol',			    ['weapontype'] = 'Pistol',	['ammotype'] = 'AMMO_PISTOL',	['damagereason'] = 'Pistoled / Blasted / Plugged / Bust a cap in'},
	[`weapon_ceramicpistol`]		 = {['name'] = 'weapon_ceramicpistol', 	['label'] = 'Ceramic Pistol',		    ['weapontype'] = 'Pistol',	['ammotype'] = 'AMMO_PISTOL',	['damagereason'] = 'Pistoled / Blasted / Plugged / Bust a cap in'},
	[`weapon_navyrevolver`]        	 = {['name'] = 'weapon_navyrevolver', 	['label'] = 'Navy Revolver',		    ['weapontype'] = 'Pistol',	['ammotype'] = 'AMMO_PISTOL',	['damagereason'] = 'Pistoled / Blasted / Plugged / Bust a cap in'},
	[`weapon_gadgetpistol`] 		 = {['name'] = 'weapon_gadgetpistol', 	['label'] = 'Gadget Pistol',		    ['weapontype'] = 'Pistol',	['ammotype'] = 'AMMO_PISTOL',	['damagereason'] = 'Pistoled / Blasted / Plugged / Bust a cap in'},
	[`weapon_stungun_mp`] 			 = {['name'] = 'weapon_stungun_mp', 	['label'] = 'Taser', 					['weapontype'] = 'Pistol',	['ammotype'] = 'AMMO_STUNGUN',	['damagereason'] = 'Died'},

    -- Submachine Guns
	[`weapon_microsmg`] 			 = {['name'] = 'weapon_microsmg', 		['label'] = 'Micro SMG', 			['weapontype'] = 'Submachine Gun',	['ammotype'] = 'AMMO_SMG',		['damagereason'] = 'Riddled / Drilled / Finished / Submachine Gunned'},
	[`weapon_smg`] 				 	 = {['name'] = 'weapon_smg', 			['label'] = 'SMG', 					['weapontype'] = 'Submachine Gun',	['ammotype'] = 'AMMO_SMG',		['damagereason'] = 'Riddled / Drilled / Finished / Submachine Gunned'},
	[`weapon_smg_mk2`] 				 = {['name'] = 'weapon_smg_mk2', 		['label'] = 'SMG MK2', 			    ['weapontype'] = 'Submachine Gun',	['ammotype'] = 'AMMO_SMG',		['damagereason'] = 'Riddled / Drilled / Finished / Submachine Gunned'},
	[`weapon_assaultsmg`] 			 = {['name'] = 'weapon_assaultsmg', 	['label'] = 'Assault SMG', 			['weapontype'] = 'Submachine Gun',	['ammotype'] = 'AMMO_SMG',		['damagereason'] = 'Riddled / Drilled / Finished / Submachine Gunned'},
	[`weapon_combatpdw`] 			 = {['name'] = 'weapon_combatpdw', 		['label'] = 'Combat PDW', 			['weapontype'] = 'Submachine Gun',	['ammotype'] = 'AMMO_SMG',		['damagereason'] = 'Riddled / Drilled / Finished / Submachine Gunned'},
	[`weapon_machinepistol`] 		 = {['name'] = 'weapon_machinepistol', 	['label'] = 'Tec-9', 				['weapontype'] = 'Submachine Gun',	['ammotype'] = 'AMMO_PISTOL',	['damagereason'] = 'Riddled / Drilled / Finished / Submachine Gunned'},
	[`weapon_minismg`] 				 = {['name'] = 'weapon_minismg', 		['label'] = 'Mini SMG', 			['weapontype'] = 'Submachine Gun',	['ammotype'] = 'AMMO_SMG',		['damagereason'] = 'Riddled / Drilled / Finished / Submachine Gunned'},
	[`weapon_raycarbine`]	         = {['name'] = 'weapon_raycarbine', 	['label'] = 'Raycarbine',	        ['weapontype'] = 'Submachine Gun',	['ammotype'] = 'AMMO_SMG',		['damagereason'] = 'Riddled / Drilled / Finished / Submachine Gunned'},

    -- Shotguns
	[`weapon_pumpshotgun`] 			 = {['name'] = 'weapon_pumpshotgun', 	 	['label'] = 'Pump Shotgun', 			['weapontype'] = 'Shotgun',	['ammotype'] = 'AMMO_SHOTGUN',	['damagereason'] = 'Devastated / Pulverized / Shotgunned'},
	[`weapon_pumpshotgun_mk2`]		 = {['name'] = 'weapon_pumpshotgun_mk2',	['label'] = 'Pump Shotgun MK2', 		['weapontype'] = 'Shotgun',	['ammotype'] = 'AMMO_SHOTGUN',	['damagereason'] = 'Devastated / Pulverized / Shotgunned'},
	[`weapon_sawnoffshotgun`] 		 = {['name'] = 'weapon_sawnoffshotgun', 	['label'] = 'Sawn-off Shotgun', 		['weapontype'] = 'Shotgun',	['ammotype'] = 'AMMO_SHOTGUN',	['damagereason'] = 'Devastated / Pulverized / Shotgunned'},
	[`weapon_assaultshotgun`] 		 = {['name'] = 'weapon_assaultshotgun', 	['label'] = 'Assault Shotgun', 			['weapontype'] = 'Shotgun',	['ammotype'] = 'AMMO_SHOTGUN',	['damagereason'] = 'Devastated / Pulverized / Shotgunned'},
	[`weapon_bullpupshotgun`] 		 = {['name'] = 'weapon_bullpupshotgun', 	['label'] = 'Bullpup Shotgun', 			['weapontype'] = 'Shotgun',	['ammotype'] = 'AMMO_SHOTGUN',	['damagereason'] = 'Devastated / Pulverized / Shotgunned'},
	[`weapon_musket`] 			     = {['name'] = 'weapon_musket', 			['label'] = 'Musket', 					['weapontype'] = 'Shotgun',	['ammotype'] = 'AMMO_SHOTGUN',	['damagereason'] = 'Devastated / Pulverized / Shotgunned'},
	[`weapon_heavyshotgun`] 		 = {['name'] = 'weapon_heavyshotgun', 	 	['label'] = 'Heavy Shotgun', 			['weapontype'] = 'Shotgun',	['ammotype'] = 'AMMO_SHOTGUN',	['damagereason'] = 'Devastated / Pulverized / Shotgunned'},
	[`weapon_dbshotgun`] 			 = {['name'] = 'weapon_dbshotgun', 		 	['label'] = 'Double-barrel Shotgun', 	['weapontype'] = 'Shotgun',	['ammotype'] = 'AMMO_SHOTGUN',	['damagereason'] = 'Devastated / Pulverized / Shotgunned'},
	[`weapon_autoshotgun`] 			 = {['name'] = 'weapon_autoshotgun', 	 	['label'] = 'Auto Shotgun', 			['weapontype'] = 'Shotgun',	['ammotype'] = 'AMMO_SHOTGUN',	['damagereason'] = 'Devastated / Pulverized / Shotgunned'},
	[`weapon_combatshotgun`]		 = {['name'] = 'weapon_combatshotgun', 		['label'] = 'Combat Shotgun',		    ['weapontype'] = 'Shotgun',	['ammotype'] = 'AMMO_SHOTGUN',	['damagereason'] = 'Devastated / Pulverized / Shotgunned'},

    -- Assault Rifles
	[`weapon_assaultrifle`] 		 = {['name'] = 'weapon_assaultrifle', 	 	['label'] = 'Assault Rifle', 				['weapontype'] = 'Assault Rifle',	['ammotype'] = 'AMMO_RIFLE',	['damagereason'] = 'Ended / Rifled / Shot down / Floored'},
	[`weapon_assaultrifle_mk2`] 	 = {['name'] = 'weapon_assaultrifle_mk2', 	['label'] = 'Assault Rifle MK2', 			['weapontype'] = 'Assault Rifle',	['ammotype'] = 'AMMO_RIFLE',	['damagereason'] = 'Ended / Rifled / Shot down / Floored'},
	[`weapon_carbinerifle`] 		 = {['name'] = 'weapon_carbinerifle', 	 	['label'] = 'Carbine Rifle', 				['weapontype'] = 'Assault Rifle',	['ammotype'] = 'AMMO_RIFLE',	['damagereason'] = 'Ended / Rifled / Shot down / Floored'},
    [`weapon_carbinerifle_mk2`] 	 = {['name'] = 'weapon_carbinerifle_mk2', 	['label'] = 'Carbine Rifle MK2', 			['weapontype'] = 'Assault Rifle',	['ammotype'] = 'AMMO_RIFLE',	['damagereason'] = 'Ended / Rifled / Shot down / Floored'},
	[`weapon_advancedrifle`] 		 = {['name'] = 'weapon_advancedrifle', 	 	['label'] = 'Advanced Rifle', 				['weapontype'] = 'Assault Rifle',	['ammotype'] = 'AMMO_RIFLE',	['damagereason'] = 'Ended / Rifled / Shot down / Floored'},
	[`weapon_specialcarbine`] 		 = {['name'] = 'weapon_specialcarbine', 	['label'] = 'Special Carbine', 				['weapontype'] = 'Assault Rifle',	['ammotype'] = 'AMMO_RIFLE',	['damagereason'] = 'Ended / Rifled / Shot down / Floored'},
	[`weapon_specialcarbine_mk2`]	 = {['name'] = 'weapon_specialcarbine_mk2',	['label'] = 'Specialcarbine MK2',	        ['weapontype'] = 'Assault Rifle',	['ammotype'] = 'AMMO_RIFLE',	['damagereason'] = 'Ended / Rifled / Shot down / Floored'},
	[`weapon_bullpuprifle`] 		 = {['name'] = 'weapon_bullpuprifle', 	 	['label'] = 'Bullpup Rifle', 				['weapontype'] = 'Assault Rifle',	['ammotype'] = 'AMMO_RIFLE',	['damagereason'] = 'Ended / Rifled / Shot down / Floored'},
	[`weapon_bullpuprifle_mk2`]		 = {['name'] = 'weapon_bullpuprifle_mk2', 	['label'] = 'Bull Puprifle MK2',			['weapontype'] = 'Assault Rifle',	['ammotype'] = 'AMMO_RIFLE',	['damagereason'] = 'Ended / Rifled / Shot down / Floored'},
	[`weapon_compactrifle`] 		 = {['name'] = 'weapon_compactrifle', 	 	['label'] = 'Compact Rifle', 				['weapontype'] = 'Assault Rifle',	['ammotype'] = 'AMMO_RIFLE',	['damagereason'] = 'Ended / Rifled / Shot down / Floored'},
	[`weapon_militaryrifle`]		 = {['name'] = 'weapon_militaryrifle', 		['label'] = 'Military Rifle',   			['weapontype'] = 'Assault Rifle',	['ammotype'] = 'AMMO_RIFLE',	['damagereason'] = 'Ended / Rifled / Shot down / Floored'},
    [`weapon_heavyrifle`] 			 = {['name'] = 'weapon_heavyrifle', 	 	['label'] = 'Heavy Rifle', 					['weapontype'] = 'Assault Rifle',	['ammotype'] = 'AMMO_RIFLE',	['damagereason'] = 'Ended / Rifled / Shot down / Floored'},

    -- Light Machine Guns
	[`weapon_mg`] 					 = {['name'] = 'weapon_mg', 			['label'] = 'Machinegun', 			['weapontype'] = 'Light Machine Gun',	['ammotype'] = 'AMMO_MG',	['damagereason'] = 'Machine gunned / Sprayed / Ruined'},
	[`weapon_combatmg`] 			 = {['name'] = 'weapon_combatmg', 		['label'] = 'Combat MG', 			['weapontype'] = 'Light Machine Gun',	['ammotype'] = 'AMMO_MG',	['damagereason'] = 'Machine gunned / Sprayed / Ruined'},
	[`weapon_combatmg_mk2`]	 		 = {['name'] = 'weapon_combatmg_mk2', 	['label'] = 'Combat MG MK2',	    ['weapontype'] = 'Light Machine Gun',	['ammotype'] = 'AMMO_MG',	['damagereason'] = 'Machine gunned / Sprayed / Ruined'},
	[`weapon_gusenberg`] 			 = {['name'] = 'weapon_gusenberg', 		['label'] = 'Thompson SMG', 		['weapontype'] = 'Light Machine Gun',	['ammotype'] = 'AMMO_MG',	['damagereason'] = 'Machine gunned / Sprayed / Ruined'},

    -- Sniper Rifles
	[`weapon_sniperrifle`] 			 = {['name'] = 'weapon_sniperrifle', 	 	['label'] = 'Sniper Rifle', 			['weapontype'] = 'Sniper Rifle',	['ammotype'] = 'AMMO_SNIPER',			['damagereason'] = 'Sniped / Picked off / Scoped'},
	[`weapon_heavysniper`] 			 = {['name'] = 'weapon_heavysniper', 	 	['label'] = 'Heavy Sniper', 			['weapontype'] = 'Sniper Rifle',	['ammotype'] = 'AMMO_SNIPER',			['damagereason'] = 'Sniped / Picked off / Scoped'},
	[`weapon_heavysniper_mk2`]		 = {['name'] = 'weapon_heavysniper_mk2', 	['label'] = 'Heavysniper MK2',	        ['weapontype'] = 'Sniper Rifle',	['ammotype'] = 'AMMO_SNIPER',			['damagereason'] = 'Sniped / Picked off / Scoped'},
	[`weapon_marksmanrifle`] 		 = {['name'] = 'weapon_marksmanrifle', 	 	['label'] = 'Marksman Rifle', 			['weapontype'] = 'Sniper Rifle',	['ammotype'] = 'AMMO_SNIPER',			['damagereason'] = 'Sniped / Picked off / Scoped'},
	[`weapon_marksmanrifle_mk2`]	 = {['name'] = 'weapon_marksmanrifle_mk2',	['label'] = 'Marksman Rifle MK2',	    ['weapontype'] = 'Sniper Rifle',	['ammotype'] = 'AMMO_SNIPER',			['damagereason'] = 'Sniped / Picked off / Scoped'},
	[`weapon_remotesniper`] 		 = {['name'] = 'weapon_remotesniper', 	 	['label'] = 'Remote Sniper', 			['weapontype'] = 'Sniper Rifle',	['ammotype'] = 'AMMO_SNIPER_REMOTE',	['damagereason'] = 'Sniped / Picked off / Scoped'},

    -- Heavy Weapons
	[`weapon_rpg`] 					 = {['name'] = 'weapon_rpg', 			      	['label'] = 'RPG', 						['weapontype'] = 'Heavy Weapons',	['ammotype'] = 'AMMO_RPG',				['damagereason'] = 'Killed / Exploded / Obliterated / Destroyed / Erased / Annihilated'},
	[`weapon_grenadelauncher`] 		 = {['name'] = 'weapon_grenadelauncher', 	  	['label'] = 'Grenade Launcher', 		['weapontype'] = 'Heavy Weapons',	['ammotype'] = 'AMMO_GRENADELAUNCHER',	['damagereason'] = 'Killed / Exploded / Obliterated / Destroyed / Erased / Annihilated'},
	[`weapon_grenadelauncher_smoke`] = {['name'] = 'weapon_grenadelauncher_smoke',	['label'] = 'Smoke Grenade Launcher',	['weapontype'] = 'Heavy Weapons',	['ammotype'] = 'AMMO_GRENADELAUNCHER',	['damagereason'] = 'Killed / Exploded / Obliterated / Destroyed / Erased / Annihilated'},
	[`weapon_minigun`] 				 = {['name'] = 'weapon_minigun', 		      	['label'] = 'Minigun', 					['weapontype'] = 'Heavy Weapons',	['ammotype'] = 'AMMO_MINIGUN',			['damagereason'] = 'Killed / Exploded / Obliterated / Destroyed / Erased / Annihilated'},
	[`weapon_firework`] 			 = {['name'] = 'weapon_firework', 		 	  	['label'] = 'Firework Launcher', 		['weapontype'] = 'Heavy Weapons',	['ammotype'] = nil,						['damagereason'] = 'Killed / Exploded / Obliterated / Destroyed / Erased / Annihilated'},
	[`weapon_railgun`] 				 = {['name'] = 'weapon_railgun', 		 	  	['label'] = 'Railgun', 					['weapontype'] = 'Heavy Weapons',	['ammotype'] = nil,						['damagereason'] = 'Killed / Exploded / Obliterated / Destroyed / Erased / Annihilated'},
	[`weapon_hominglauncher`] 		 = {['name'] = 'weapon_hominglauncher', 	 	['label'] = 'Homing Launcher', 			['weapontype'] = 'Heavy Weapons',	['ammotype'] = 'AMMO_STINGER',			['damagereason'] = 'Killed / Exploded / Obliterated / Destroyed / Erased / Annihilated'},
	[`weapon_compactlauncher`] 		 = {['name'] = 'weapon_compactlauncher',  	  	['label'] = 'Compact Launcher', 		['weapontype'] = 'Heavy Weapons',	['ammotype'] = nil,						['damagereason'] = 'Killed / Exploded / Obliterated / Destroyed / Erased / Annihilated'},
	[`weapon_rayminigun`]			 = {['name'] = 'weapon_rayminigun', 		 	['label'] = 'Ray Minigun',		        ['weapontype'] = 'Heavy Weapons',	['ammotype'] = 'AMMO_MINIGUN',			['damagereason'] = 'Killed / Exploded / Obliterated / Destroyed / Erased / Annihilated'},
    [`weapon_emplauncher`] 			 = {['name'] = 'weapon_emplauncher', 			['label'] = 'EMP Launcher', 			['weapontype'] = 'Heavy Weapons',	['ammotype'] = 'AMMO_EMPLAUNCHER',		['damagereason'] = 'Died'},

    -- Throwables
	[`weapon_grenade`] 		        = {['name'] = 'weapon_grenade', 		['label'] = 'Grenade', 			['weapontype'] = 'Throwable',	['ammotype'] = nil,				['damagereason'] = 'Bombed / Exploded / Detonated / Blew up'},
	[`weapon_bzgas`] 		        = {['name'] = 'weapon_bzgas', 			['label'] = 'BZ Gas', 			['weapontype'] = 'Throwable',	['ammotype'] = nil,				['damagereason'] = 'Died'},
	[`weapon_molotov`] 		        = {['name'] = 'weapon_molotov', 		['label'] = 'Molotov', 			['weapontype'] = 'Throwable',	['ammotype'] = nil,				['damagereason'] = 'Torched / Flambeed / Barbecued'},
	[`weapon_stickybomb`] 	        = {['name'] = 'weapon_stickybomb', 	    ['label'] = 'C4', 				['weapontype'] = 'Throwable',	['ammotype'] = nil,				['damagereason'] = 'Bombed / Exploded / Detonated / Blew up'},
	[`weapon_proxmine`] 	        = {['name'] = 'weapon_proxmine', 		['label'] = 'Proxmine Grenade', ['weapontype'] = 'Throwable',	['ammotype'] = nil,				['damagereason'] = 'Bombed / Exploded / Detonated / Blew up'},
	[`weapon_snowball`] 	        = {['name'] = 'weapon_snowball', 		['label'] = 'Snowball', 		['weapontype'] = 'Throwable',	['ammotype'] = nil,				['damagereason'] = 'Died'},
	[`weapon_pipebomb`] 	        = {['name'] = 'weapon_pipebomb', 		['label'] = 'Pipe Bomb', 		['weapontype'] = 'Throwable',	['ammotype'] = nil,				['damagereason'] = 'Bombed / Exploded / Detonated / Blew up'},
	[`weapon_ball`] 		        = {['name'] = 'weapon_ball', 			['label'] = 'Ball', 			['weapontype'] = 'Throwable',	['ammotype'] = 'AMMO_BALL',		['damagereason'] = 'Died'},
	[`weapon_smokegrenade`]         = {['name'] = 'weapon_smokegrenade', 	['label'] = 'Smoke Grenade', 	['weapontype'] = 'Throwable',	['ammotype'] = nil,				['damagereason'] = 'Died'},
	[`weapon_flare`] 		        = {['name'] = 'weapon_flare', 			['label'] = 'Flare pistol', 	['weapontype'] = 'Throwable',	['ammotype'] = 'AMMO_FLARE',	['damagereason'] = 'Died'},

    -- Miscellaneous
	[`weapon_petrolcan`] 			= {['name'] = 'weapon_petrolcan', 		 	['label'] = 'Petrol Can', 				['weapontype'] = 'Miscellaneous',	['ammotype'] = 'AMMO_PETROLCAN',		['damagereason'] = 'Died'},
	[`gadget_parachute`] 			= {['name'] = 'gadget_parachute', 		 	['label'] = 'Parachute', 				['weapontype'] = 'Miscellaneous',	['ammotype'] = nil,						['damagereason'] = 'Died'},
	[`weapon_fireextinguisher`] 	= {['name'] = 'weapon_fireextinguisher',	['label'] = 'Fire Extinguisher',		['weapontype'] = 'Miscellaneous',	['ammotype'] = nil,						['damagereason'] = 'Died'},
	[`weapon_hazardcan`]			= {['name'] = 'weapon_hazardcan',			['label'] = 'Hazardcan',			    ['weapontype'] = 'Miscellaneous',	['ammotype'] = 'AMMO_PETROLCAN',		['damagereason'] = 'Died'},
    [`weapon_fertilizercan`]		= {['name'] = 'weapon_fertilizercan',		['label'] = 'Fertilizer Can',			['weapontype'] = 'Miscellaneous',	['ammotype'] = 'AMMO_FERTILIZERCAN',	['damagereason'] = 'Died'},
	[`weapon_barbed_wire`]			= {['name'] = 'weapon_barbed_wire',			['label'] = 'Barbed Wire',				['weapontype'] = 'Miscellaneous',	['ammotype'] = nil,						['damagereason'] = 'Prodded'},
	[`weapon_drowning`]				= {['name'] = 'weapon_drowning',			['label'] = 'Drowning',					['weapontype'] = 'Miscellaneous',	['ammotype'] = nil,						['damagereason'] = 'Died'},
	[`weapon_drowning_in_vehicle`]	= {['name'] = 'weapon_drowning_in_vehicle',	['label'] = 'Drowning in a Vehicle',	['weapontype'] = 'Miscellaneous',	['ammotype'] = nil,						['damagereason'] = 'Died'},
	[`weapon_bleeding`]				= {['name'] = 'weapon_bleeding',			['label'] = 'Bleeding',					['weapontype'] = 'Miscellaneous',	['ammotype'] = nil,						['damagereason'] = 'Bled out'},
	[`weapon_electric_fence`]		= {['name'] = 'weapon_electric_fence',		['label'] = 'Electric Fence',			['weapontype'] = 'Miscellaneous',	['ammotype'] = nil,						['damagereason'] = 'Fried'},
	[`weapon_explosion`]			= {['name'] = 'weapon_explosion',			['label'] = 'Explosion',				['weapontype'] = 'Miscellaneous',	['ammotype'] = nil,						['damagereason'] = 'Killed / Exploded / Obliterated / Destroyed / Erased / Annihilated'},
	[`weapon_fall`]					= {['name'] = 'weapon_fall',				['label'] = 'Fall',						['weapontype'] = 'Miscellaneous',	['ammotype'] = nil,						['damagereason'] = 'Committed suicide'},
	[`weapon_exhaustion`]			= {['name'] = 'weapon_exhaustion',			['label'] = 'Exhaustion',				['weapontype'] = 'Miscellaneous',	['ammotype'] = nil,						['damagereason'] = 'Died'},
	[`weapon_hit_by_water_cannon`]	= {['name'] = 'weapon_hit_by_water_cannon',	['label'] = 'Water Cannon',				['weapontype'] = 'Miscellaneous',	['ammotype'] = nil,						['damagereason'] = 'Died'},
	[`weapon_rammed_by_car`]		= {['name'] = 'weapon_rammed_by_car',		['label'] = 'Rammed - Vehicle',			['weapontype'] = 'Miscellaneous',	['ammotype'] = nil,						['damagereason'] = 'Flattened / Ran over / Ran down'},
	[`weapon_run_over_by_car`]		= {['name'] = 'weapon_run_over_by_car',		['label'] = 'Run Over - Vehicle',		['weapontype'] = 'Miscellaneous',	['ammotype'] = nil,						['damagereason'] = 'Flattened / Ran over / Ran down'},
	[`weapon_heli_crash`]			= {['name'] = 'weapon_heli_crash',			['label'] = 'Heli Crash',				['weapontype'] = 'Miscellaneous',	['ammotype'] = nil,						['damagereason'] = 'Helicopter Crash'},
	[`weapon_fire`]					= {['name'] = 'weapon_fire',				['label'] = 'Fire',						['weapontype'] = 'Miscellaneous',	['ammotype'] = nil,						['damagereason'] = 'Torched / Flambeed / Barbecued'},

	-- Animals
    [`weapon_animal`]               = {['name'] = 'weapon_animal',	['label'] = 'Animal',	['weapontype'] = 'Animals',	['ammotype'] = nil,	['damagereason'] = 'Mauled'},
    [`weapon_cougar`]               = {['name'] = 'weapon_cougar',	['label'] = 'Cougar',	['weapontype'] = 'Animals',	['ammotype'] = nil,	['damagereason'] = 'Mauled'},
}

BlueStarkInventory.DurabilityMultiplier = {
	-- Melee
	-- ['weapon_unarmed'] 				 = 0.15,
	['weapon_dagger'] 				 = 0.15,
	['weapon_bat'] 				 	 = 0.15,
	['weapon_bottle'] 				 = 0.15,
	['weapon_crowbar'] 				 = 0.15,
	-- ['weapon_flashlight'] 			 = 0.15,
	['weapon_golfclub'] 			 = 0.15,
	['weapon_hammer'] 				 = 0.15,
	['weapon_hatchet'] 				 = 0.15,
	['weapon_knuckle'] 				 = 0.15,
	['weapon_knife'] 				 = 0.15,
	['weapon_machete'] 				 = 0.15,
	['weapon_switchblade'] 			 = 0.15,
	-- ['weapon_nightstick'] 			 = 0.15,
	['weapon_wrench'] 				 = 0.15,
	['weapon_battleaxe'] 			 = 0.15,
	['weapon_poolcue'] 				 = 0.15,
	['weapon_briefcase'] 			 = 0.15,
	['weapon_briefcase_02'] 		 = 0.15,
	['weapon_garbagebag'] 			 = 0.15,
	['weapon_handcuffs'] 			 = 0.15,
	['weapon_bread'] 				 = 0.15,
	['weapon_stone_hatchet'] 		 = 0.15,

	-- Handguns
	['weapon_pistol'] 				 = 0.15,
	['weapon_pistol_mk2'] 			 = 0.15,
	['weapon_combatpistol'] 		 = 0.15,
	['weapon_appistol'] 			 = 0.15,
	-- ['weapon_stungun'] 				 = 0.15,
    -- ['weapon_stungun_mp'] 				 = 0.15,
	['weapon_pistol50'] 			 = 0.15,
	['weapon_snspistol'] 			 = 0.15,
	['weapon_heavypistol'] 			 = 0.15,
	['weapon_vintagepistol'] 		 = 0.15,
	['weapon_flaregun'] 			 = 0.15,
	['weapon_marksmanpistol'] 		 = 0.15,
	['weapon_revolver'] 			 = 0.15,
	['weapon_revolver_mk2'] 		 = 0.15,
	['weapon_doubleaction'] 		 = 0.15,
	['weapon_snspistol_mk2'] 		 = 0.15,
	['weapon_raypistol'] 			 = 0.15,
	['weapon_ceramicpistol'] 		 = 0.15,
	['weapon_navyrevolver'] 		 = 0.15,
	['weapon_gadgetpistol'] 		 = 0.15,

	-- Submachine Guns
	['weapon_microsmg'] 			 = 0.15,
	['weapon_smg'] 				     = 0.15,
	['weapon_smg_mk2'] 				 = 0.15,
	['weapon_assaultsmg'] 			 = 0.15,
	['weapon_combatpdw'] 			 = 0.15,
	['weapon_machinepistol'] 		 = 0.15,
	['weapon_minismg'] 				 = 0.15,
	['weapon_raycarbine'] 			 = 0.15,

	-- Shotguns
	['weapon_pumpshotgun'] 			= 0.15,
	['weapon_sawnoffshotgun'] 		= 0.15,
	['weapon_assaultshotgun'] 		= 0.15,
	['weapon_bullpupshotgun'] 		= 0.15,
	['weapon_musket'] 				= 0.15,
	['weapon_heavyshotgun'] 		= 0.15,
	['weapon_dbshotgun'] 			= 0.15,
	['weapon_autoshotgun'] 			= 0.15,
	['weapon_pumpshotgun_mk2'] 		= 0.15,
	['weapon_combatshotgun'] 		= 0.15,

	-- Assault Rifles
	['weapon_assaultrifle'] 		= 0.15,
	['weapon_assaultrifle_mk2'] 	= 0.15,
	['weapon_carbinerifle'] 		= 0.15,
	['weapon_carbinerifle_mk2'] 	= 0.15,
	['weapon_advancedrifle'] 		= 0.15,
	['weapon_specialcarbine'] 		= 0.15,
	['weapon_bullpuprifle'] 		= 0.15,
	['weapon_compactrifle'] 		= 0.15,
	['weapon_specialcarbine_mk2'] 	= 0.15,
	['weapon_bullpuprifle_mk2'] 	= 0.15,
	['weapon_militaryrifle'] 		= 0.15,
    ['weapon_heavyrifle']           = 0.15,

	-- Light Machine Guns
	['weapon_mg'] 				    = 0.15,
	['weapon_combatmg'] 			= 0.15,
	['weapon_gusenberg'] 			= 0.15,
	['weapon_combatmg_mk2'] 		= 0.15,

	-- Sniper Rifles
	['weapon_sniperrifle'] 			= 0.15,
	['weapon_heavysniper'] 			= 0.15,
	['weapon_marksmanrifle'] 		= 0.15,
	['weapon_remotesniper'] 		= 0.15,
	['weapon_heavysniper_mk2'] 		= 0.15,
	['weapon_marksmanrifle_mk2'] 	= 0.15,

	-- Heavy Weapons
	['weapon_rpg'] 				    = 0.15,
	['weapon_grenadelauncher'] 		= 0.15,
	['weapon_grenadelauncher_smoke']= 0.15,
    ['weapon_emplauncher']          = 0.15,
	['weapon_minigun'] 				= 0.15,
	['weapon_firework'] 			= 0.15,
	['weapon_railgun'] 				= 0.15,
	['weapon_hominglauncher'] 		= 0.15,
	['weapon_compactlauncher'] 		= 0.15,
	['weapon_rayminigun'] 			= 0.15,

	-- Throwables
	['weapon_grenade'] 				= 0.15,
	['weapon_bzgas'] 				= 0.15,
	['weapon_molotov'] 				= 0.15,
	['weapon_stickybomb'] 			= 0.15,
	['weapon_proxmine'] 			= 0.15,
	['weapon_snowball'] 			= 0.15,
	['weapon_pipebomb'] 			= 0.15,
	['weapon_ball'] 				= 0.15,
	['weapon_smokegrenade'] 		= 0.15,
	['weapon_flare'] 				= 0.15,

	-- Miscellaneous
	['weapon_petrolcan'] 			= 0.15,
	['weapon_fireextinguisher'] 	= 0.15,
	['weapon_hazardcan'] 			= 0.15,
    ['weapon_fertilizercan'] 		= 0.15,
}

BlueStarkInventory.WeaponRepairPoints = {
    [1] = {
        coords = vector3(964.02, -1267.41, 34.97),
        IsRepairing = false,
        RepairingData = {},
    }
}

BlueStarkInventory.WeaponRepairCosts = {
    ["pistol"] = 1000,
    ["smg"] = 3000,
    ["mg"] = 4000,
    ["rifle"] = 5000,
    ["sniper"] = 7000,
}

BlueStarkInventory.WeaponAttachment = {}
BlueStarkInventory.WeaponAttachment.Bones = {
    ["WAPClip"]         = {label = "Magazine", attach_component='clip', slot="0-0", add_left = 42, add_top = 42},
    ["Gun_GripR"]       = {label = "Finish", attach_component='luxary_finish', slot="1-0", add_left = 42, add_top = 42},
    ["WAPSupp"]      = {label = "Muzzle", attach_component='suppressor', slot="2-0", add_left = 42, add_top = -202},
    ["WAPFlshLasr"]         = {label = "Flashlight", attach_component='flashlight', slot="3-0", add_left = 45, add_top = 42},
    ["WAPScop"]         = {label = "Scope", attach_component='scope', slot="4-0", add_left = 42, add_top = 42},
    ["WAPGrip"]       = {label = "Grip", attach_component='grip', slot="5-0", add_left = 42, add_top = 42},
    ["gun_vfx_eject"]       = {label = "Tint", attach_component='tint', slot="6-0", add_left = 42, add_top = -202},
}

BlueStarkInventory.WeaponAttachment.Weapons = { 
    { 
        name = 'weapon_pistol', 
        components = { 
            {name = 'clip_default', hash = 'COMPONENT_PISTOL_CLIP_01'}, 
            {name = 'clip_extended', hash = 'COMPONENT_PISTOL_CLIP_02'}, 
            {name = 'flashlight', hash = 'COMPONENT_AT_PI_FLSH'}, 
            {name = 'suppressor', hash = 'COMPONENT_AT_PI_SUPP_02'}, 
            {name = 'luxary_finish', hash = 'COMPONENT_PISTOL_VARMOD_LUXE'} 
        } 
    }, 
    { 
        name = 'weapon_combatpistol', 
        components = { 
            {name = 'clip_default', hash = 'COMPONENT_COMBATPISTOL_CLIP_01'}, 
            {name = 'clip_extended', hash = 'COMPONENT_COMBATPISTOL_CLIP_02'}, 
            {name = 'flashlight', hash = 'COMPONENT_AT_PI_FLSH'}, 
            {name = 'suppressor', hash = 'COMPONENT_AT_PI_SUPP'}, 
            {name = 'luxary_finish', hash = 'COMPONENT_COMBATPISTOL_VARMOD_LOWRIDER'} 
        } 
    }, 
    { 
        name = 'weapon_appistol', 
        components = { 
            {name = 'clip_default', hash = 'COMPONENT_APPISTOL_CLIP_01'}, 
            {name = 'clip_extended', hash = 'COMPONENT_APPISTOL_CLIP_02'}, 
            {name = 'flashlight', hash = 'COMPONENT_AT_PI_FLSH'}, 
            {name = 'suppressor', hash = 'COMPONENT_AT_PI_SUPP'}, 
            {name = 'luxary_finish', hash = 'COMPONENT_APPISTOL_VARMOD_LUXE'} 
        } 
    }, 
    { 
        name = 'weapon_pistol50', 
        components = { 
            {name = 'clip_default', hash = 'COMPONENT_PISTOL50_CLIP_01'}, 
            {name = 'clip_extended', hash = 'COMPONENT_PISTOL50_CLIP_02'}, 
            {name = 'flashlight', hash = 'COMPONENT_AT_PI_FLSH'}, 
            {name = 'suppressor', hash = 'COMPONENT_AT_AR_SUPP_02'}, 
            {name = 'luxary_finish', hash = 'COMPONENT_PISTOL50_VARMOD_LUXE'} 
        } 
    }, 
    { 
        name = 'weapon_snspistol', 
        components = { 
            {name = 'clip_default', hash = 'COMPONENT_SNSPISTOL_CLIP_01'}, 
            {name = 'clip_extended', hash = 'COMPONENT_SNSPISTOL_CLIP_02'}, 
            {name = 'luxary_finish', hash = 'COMPONENT_SNSPISTOL_VARMOD_LOWRIDER'} 
        } 
    }, 
    { 
        name = 'weapon_heavypistol', 
        components = { 
            {name = 'clip_default', hash = 'COMPONENT_HEAVYPISTOL_CLIP_01'}, 
            {name = 'clip_extended', hash = 'COMPONENT_HEAVYPISTOL_CLIP_02'}, 
            {name = 'flashlight', hash = 'COMPONENT_AT_PI_FLSH'}, 
            {name = 'suppressor', hash = 'COMPONENT_AT_PI_SUPP'}, 
            {name = 'luxary_finish', hash = 'COMPONENT_HEAVYPISTOL_VARMOD_LUXE'} 
        } 
    }, 
    { 
        name = 'weapon_vintagepistol', 
        components = { 
            {name = 'clip_default', hash = 'COMPONENT_VINTAGEPISTOL_CLIP_01'}, 
            {name = 'clip_extended', hash = 'COMPONENT_VINTAGEPISTOL_CLIP_02'}, 
            {name = 'suppressor', hash = 'COMPONENT_AT_PI_SUPP'} 
        } 
    }, 
    { 
        name = 'weapon_machinepistol', 
        components = { 
            {name = 'clip_default', hash = 'COMPONENT_MACHINEPISTOL_CLIP_01'}, 
            {name = 'clip_extended', hash = 'COMPONENT_MACHINEPISTOL_CLIP_02'}, 
            {name = 'clip_drum', hash = 'COMPONENT_MACHINEPISTOL_CLIP_03'}, 
            {name = 'suppressor', hash = 'COMPONENT_AT_PI_SUPP'} 
        } 
    }, 
    { 
        name = 'weapon_smg', 
        components = { 
            {name = 'clip_default', hash = 'COMPONENT_SMG_CLIP_01'}, 
            {name = 'clip_extended', hash = 'COMPONENT_SMG_CLIP_02'}, 
            {name = 'clip_drum', hash = 'COMPONENT_SMG_CLIP_03'}, 
            {name = 'flashlight', hash = 'COMPONENT_AT_AR_FLSH'}, 
            {name = 'scope', hash = 'COMPONENT_AT_SCOPE_MACRO_02'}, 
            {name = 'suppressor', hash = 'COMPONENT_AT_PI_SUPP'}, 
            {name = 'luxary_finish', hash = 'COMPONENT_SMG_VARMOD_LUXE'} 
        } 
    }, 
    { 
        name = 'weapon_assaultsmg', 
        components = { 
            {name = 'clip_default', hash = 'COMPONENT_ASSAULTSMG_CLIP_01'}, 
            {name = 'clip_extended', hash = 'COMPONENT_ASSAULTSMG_CLIP_02'}, 
            {name = 'flashlight', hash = 'COMPONENT_AT_AR_FLSH'}, 
            {name = 'scope', hash = 'COMPONENT_AT_SCOPE_MACRO'}, 
            {name = 'suppressor', hash = 'COMPONENT_AT_AR_SUPP_02'}, 
            {name = 'luxary_finish', hash = 'COMPONENT_ASSAULTSMG_VARMOD_LOWRIDER'} 
        } 
    }, 
    { 
        name = 'weapon_microsmg', 
        components = { 
            {name = 'clip_default', hash = 'COMPONENT_MICROSMG_CLIP_01'}, 
            {name = 'clip_extended', hash = 'COMPONENT_MICROSMG_CLIP_02'}, 
            {name = 'flashlight', hash = 'COMPONENT_AT_PI_FLSH'}, 
            {name = 'scope', hash = 'COMPONENT_AT_SCOPE_MACRO'}, 
            {name = 'suppressor', hash = 'COMPONENT_AT_AR_SUPP_02'}, 
            {name = 'luxary_finish', hash = 'COMPONENT_MICROSMG_VARMOD_LUXE'} 
        } 
    }, 
    { 
        name = 'weapon_minismg', 
        components = { 
            {name = 'clip_default', hash = 'COMPONENT_MINISMG_CLIP_01'}, 
            {name = 'clip_extended', hash = 'COMPONENT_MINISMG_CLIP_02'} 
        } 
    }, 
    { 
        name = 'weapon_combatpdw', 
        components = { 
            {name = 'clip_default', hash = 'COMPONENT_COMBATPDW_CLIP_01'}, 
            {name = 'clip_extended', hash = 'COMPONENT_COMBATPDW_CLIP_02'}, 
            {name = 'clip_drum', hash = 'COMPONENT_COMBATPDW_CLIP_03'}, 
            {name = 'flashlight', hash = 'COMPONENT_AT_AR_FLSH'}, 
            {name = 'grip', hash = 'COMPONENT_AT_AR_AFGRIP'}, 
            {name = 'scope', hash = 'COMPONENT_AT_SCOPE_SMALL'} 
        } 
    }, 
    { 
        name = 'weapon_pumpshotgun', 
        components = { 
            {name = 'flashlight', hash = 'COMPONENT_AT_AR_FLSH'}, 
            {name = 'suppressor', hash = 'COMPONENT_AT_SR_SUPP'}, 
            {name = 'luxary_finish', hash = 'COMPONENT_PUMPSHOTGUN_VARMOD_LOWRIDER'} 
        } 
    }, 
    { 
        name = 'weapon_sawnoffshotgun', 
        components = { 
            {name = 'luxary_finish', hash = 'COMPONENT_SAWNOFFSHOTGUN_VARMOD_LUXE'} 
        } 
    }, 
    { 
        name = 'weapon_assaultshotgun', 
        components = { 
            {name = 'clip_default', hash = 'COMPONENT_ASSAULTSHOTGUN_CLIP_01'}, 
            {name = 'clip_extended', hash = 'COMPONENT_ASSAULTSHOTGUN_CLIP_02'}, 
            {name = 'flashlight', hash = 'COMPONENT_AT_AR_FLSH'}, 
            {name = 'suppressor', hash = 'COMPONENT_AT_AR_SUPP'}, 
            {name = 'grip', hash = 'COMPONENT_AT_AR_AFGRIP'} 
        } 
    }, 
    { 
        name = 'weapon_bullpupshotgun', 
        components = { 
            {name = 'flashlight', hash = 'COMPONENT_AT_AR_FLSH'}, 
            {name = 'suppressor', hash = 'COMPONENT_AT_AR_SUPP_02'}, 
            {name = 'grip', hash = 'COMPONENT_AT_AR_AFGRIP'} 
        } 
    }, 
    { 
        name = 'weapon_heavyshotgun', 
        components = { 
            {name = 'clip_default', hash = 'COMPONENT_HEAVYSHOTGUN_CLIP_01'}, 
            {name = 'clip_extended', hash = 'COMPONENT_HEAVYSHOTGUN_CLIP_02'}, 
            {name = 'clip_drum', hash = 'COMPONENT_HEAVYSHOTGUN_CLIP_03'}, 
            {name = 'flashlight', hash = 'COMPONENT_AT_AR_FLSH'}, 
            {name = 'suppressor', hash = 'COMPONENT_AT_AR_SUPP_02'}, 
            {name = 'grip', hash = 'COMPONENT_AT_AR_AFGRIP'} 
        } 
    }, 
    { 
        name = 'weapon_assaultrifle', 
        components = { 
            {name = 'clip_default', hash = 'COMPONENT_ASSAULTRIFLE_CLIP_01'}, 
            {name = 'clip_extended', hash = 'COMPONENT_ASSAULTRIFLE_CLIP_02'}, 
            {name = 'clip_drum', hash = 'COMPONENT_ASSAULTRIFLE_CLIP_03'}, 
            {name = 'flashlight', hash = 'COMPONENT_AT_AR_FLSH'}, 
            {name = 'scope', hash = 'COMPONENT_AT_SCOPE_MACRO'}, 
            {name = 'suppressor', hash = 'COMPONENT_AT_AR_SUPP_02'}, 
            {name = 'grip', hash = 'COMPONENT_AT_AR_AFGRIP'}, 
            {name = 'luxary_finish', hash = 'COMPONENT_ASSAULTRIFLE_VARMOD_LUXE'} 
        } 
    }, 
    { 
        name = 'weapon_carbinerifle', 
        components = { 
            {name = 'clip_default', hash = 'COMPONENT_CARBINERIFLE_CLIP_01'}, 
            {name = 'clip_extended', hash = 'COMPONENT_CARBINERIFLE_CLIP_02'}, 
            {name = 'clip_box', hash = 'COMPONENT_CARBINERIFLE_CLIP_03'}, 
            {name = 'flashlight', hash = 'COMPONENT_AT_AR_FLSH'}, 
            {name = 'scope', hash = 'COMPONENT_AT_SCOPE_MEDIUM'}, 
            {name = 'suppressor', hash = 'COMPONENT_AT_AR_SUPP'}, 
            {name = 'grip', hash = 'COMPONENT_AT_AR_AFGRIP'}, 
            {name = 'luxary_finish', hash = 'COMPONENT_CARBINERIFLE_VARMOD_LUXE'} 
        } 
    }, 
    { 
        name = 'weapon_advancedrifle', 
        components = { 
            {name = 'clip_default', hash = 'COMPONENT_ADVANCEDRIFLE_CLIP_01'}, 
            {name = 'clip_extended', hash = 'COMPONENT_ADVANCEDRIFLE_CLIP_02'}, 
            {name = 'flashlight', hash = 'COMPONENT_AT_AR_FLSH'}, 
            {name = 'scope', hash = 'COMPONENT_AT_SCOPE_SMALL'}, 
            {name = 'suppressor', hash = 'COMPONENT_AT_AR_SUPP'}, 
            {name = 'luxary_finish', hash = 'COMPONENT_ADVANCEDRIFLE_VARMOD_LUXE'} 
        }
    }, 
    { 
        name = 'weapon_specialcarbine', 
        components = { 
            {name = 'clip_default', hash = 'COMPONENT_SPECIALCARBINE_CLIP_01'}, 
            {name = 'clip_extended', hash = 'COMPONENT_SPECIALCARBINE_CLIP_02'}, 
            {name = 'clip_drum', hash = 'COMPONENT_SPECIALCARBINE_CLIP_03'}, 
            {name = 'flashlight', hash = 'COMPONENT_AT_AR_FLSH'}, 
            {name = 'scope', hash = 'COMPONENT_AT_SCOPE_MEDIUM'}, 
            {name = 'suppressor', hash = 'COMPONENT_AT_AR_SUPP_02'}, 
            {name = 'grip', hash = 'COMPONENT_AT_AR_AFGRIP'}, 
            {name = 'luxary_finish', hash = 'COMPONENT_SPECIALCARBINE_VARMOD_LOWRIDER'} 
        } 
    }, 
    { 
        name = 'weapon_bullpuprifle', 
        components = { 
            {name = 'clip_default', hash = 'COMPONENT_BULLPUPRIFLE_CLIP_01'}, 
            {name = 'clip_extended', hash = 'COMPONENT_BULLPUPRIFLE_CLIP_02'}, 
            {name = 'flashlight', hash = 'COMPONENT_AT_AR_FLSH'}, 
            {name = 'scope', hash = 'COMPONENT_AT_SCOPE_SMALL'}, 
            {name = 'suppressor', hash = 'COMPONENT_AT_AR_SUPP'}, 
            {name = 'grip', hash = 'COMPONENT_AT_AR_AFGRIP'}, 
            {name = 'luxary_finish', hash = 'COMPONENT_BULLPUPRIFLE_VARMOD_LOW'} 
        } 
    }, 
    { 
        name = 'weapon_compactrifle', 
        components = { 
            {name = 'clip_default', hash = 'COMPONENT_COMPACTRIFLE_CLIP_01'}, 
            {name = 'clip_extended', hash = 'COMPONENT_COMPACTRIFLE_CLIP_02'}, 
            {name = 'clip_drum', hash = 'COMPONENT_COMPACTRIFLE_CLIP_03'} 
        } 
    }, 
    { 
        name = 'weapon_mg', 
        components = { 
            {name = 'clip_default', hash = 'COMPONENT_MG_CLIP_01'}, 
            {name = 'clip_extended', hash = 'COMPONENT_MG_CLIP_02'}, 
            {name = 'scope', hash = 'COMPONENT_AT_SCOPE_SMALL_02'}, 
            {name = 'luxary_finish', hash = 'COMPONENT_MG_VARMOD_LOWRIDER'} 
        } 
    }, 
    { 
        name = 'weapon_combatmg', 
        components = { 
            {name = 'clip_default', hash = 'COMPONENT_COMBATMG_CLIP_01'}, 
            {name = 'clip_extended', hash = 'COMPONENT_COMBATMG_CLIP_02'}, 
            {name = 'scope', hash = 'COMPONENT_AT_SCOPE_MEDIUM'}, 
            {name = 'grip', hash = 'COMPONENT_AT_AR_AFGRIP'}, 
            {name = 'luxary_finish', hash = 'COMPONENT_COMBATMG_VARMOD_LOWRIDER'} 
        } 
    }, 
    { 
        name = 'weapon_gusenberg', 
        components = { 
            {name = 'clip_default', hash = 'COMPONENT_GUSENBERG_CLIP_01'}, 
            {name = 'clip_extended', hash = 'COMPONENT_GUSENBERG_CLIP_02'}, 
        } 
    }, 
    { 
        name = 'weapon_sniperrifle', 
        components = { 
            {name = 'scope', hash = 'COMPONENT_AT_SCOPE_LARGE'}, 
            {name = 'scope_advanced', hash = 'COMPONENT_AT_SCOPE_MAX'}, 
            {name = 'suppressor', hash = 'COMPONENT_AT_AR_SUPP_02'}, 
            {name = 'luxary_finish', hash = 'COMPONENT_SNIPERRIFLE_VARMOD_LUXE'} 
        } 
    }, 
    { 
        name = 'weapon_heavysniper', 
        components = { 
            {name = 'scope', hash = 'COMPONENT_AT_SCOPE_LARGE'}, 
            {name = 'scope_advanced', hash = 'COMPONENT_AT_SCOPE_MAX'} 
        } 
    }, 
    { 
        name = 'weapon_marksmanrifle', 
        components = { 
            {name = 'clip_default', hash = 'COMPONENT_MARKSMANRIFLE_CLIP_01'}, 
            {name = 'clip_extended', hash = 'COMPONENT_MARKSMANRIFLE_CLIP_02'}, 
            {name = 'flashlight', hash = 'COMPONENT_AT_AR_FLSH'}, 
            {name = 'scope', hash = 'COMPONENT_AT_SCOPE_LARGE_FIXED_ZOOM'}, 
            {name = 'suppressor', hash = 'COMPONENT_AT_AR_SUPP'}, 
            {name = 'grip', hash = 'COMPONENT_AT_AR_AFGRIP'}, 
            {name = 'luxary_finish', hash = 'COMPONENT_MARKSMANRIFLE_VARMOD_LUXE'} 
        } 
    } 
}

BlueStarkInventory.Weapons = BlueStarkInventory.WeaponAttachment.Weapons
BlueStarkInventory.WeaponAttachment.Ammo = BlueStarkInventory.WeaponList

BlueStarkInventory.Functions.Server = {
    GetFramework                    = function ()
        coreData = ESX
        return coreData
    end,


    GetPlayer                       = function ( source )
        local EasyFramework = BlueStarkInventory.ServerFramework
		if EasyFramework == nil then
            print("^1Empty framework detected!\n^7It's causes from framework, inventory doesn't affect framework!")
            return nil
        else
            return EasyFramework.GetPlayerFromId(source)
        end
    end,

    GetName                         = function ( source )
        return BlueStarkInventory.Functions.Server.GetPlayer( source ).identifier 
    end,

    GetIdentifier                   = function(source)
        if BlueStarkInventory.Functions.Server.GetPlayer(source) == nil then
            print("^1Empty Identifier detected!\n^7It's causes from framework or it wasn\'t important stuff.\nIf inventory doesn\'t works resetup the inventory!")
            return nil
        else
            local identifier = BlueStarkInventory.Functions.Server.GetPlayer( source ).identifier
            local new = identifier
            if string.match(identifier, ":") then
                new = identifier:gsub(":", "-")
            end
            return new
        end
    end,

    GetPlayerFromUniqueValue        = function(id)
        local EasyFramework = BlueStarkInventory.ServerFramework
        return EasyFramework.GetPlayerFromIdentifier(id)
    end,
}
BlueStarkInventory.VehiclesInventory = {}
BlueStarkInventory.VehiclesInventory.Models = {
    ["ASEA"] = {
        ["glovebox"] = "hhm3eomwf3cygww8e6kcgpt6",
    }
}
BlueStarkInventory.VehiclesInventory.Classes = {
    [0] = {
        ["trunk"] = "coks1u2elb452g0hsd25w6if",
    },
    [1] = {
        ["trunk"] = "a584yw9439bwluc1voo2x1rm"
    },
    [2] = {
        ["trunk"] = "zpop64fqra28l9tnle839emg"
    },
    [3] = {
        ["trunk"] = "0vh1lnz1a7ojflbvoul4yaz0"
    },
    [4] = {
        ["trunk"] = "ar9kya72yoirzu2cfwr8hz85"
    },
    [5] = {
        ["trunk"] = "yexq0qwrv2fsnjot0l0tzmom"
    },
    [6] = {
        ["trunk"] = "zqohzbplr45rooxm81e655vz"
    },
    [7] = {
        ["trunk"] = "r9vfk2316yp7uoz5z4d7k4pr"
    },
    [8] = {
        ["trunk"] = "dcapvociisv5u7drc611yyvq"
    },
    [9] = {
        ["trunk"] = "p70ufhnlm9ai0f42v7w85nj0"
    },
    [12] = {
        ["trunk"] = "p70ufhnlm9ai0f42v7w85nj0"
    },
    [13] = {
        ["trunk"] = "nfi5z2p26m6e4ewpuxkjh2la"
    },
    [14] = {
        ["trunk"] = "m3jpk09d64ij0r6px1g80pcy"
    },
    [15] = {
        ["trunk"] = "irn05edljdp6hvamcdgm6tgz"
    },
    [16] = {
        ["trunk"] = "e5z2xps7t2km3nhrz6lufaob"
    },
}

BlueStarkInventory.ServerFramework = BlueStarkInventory.Functions.Server.GetFramework()

BlueStarkInventory.Events = {
    PlayerLoaded = 'esx:playerLoaded',
    PlayerUnloaded = 'esx:playerDropped',

}

BlueStarkInventory.Items = {
    
    ["shop1"] = {
        ["item"] = {
            _id = "shop1",
            _name = "shop1",
            _parent = "",
            _type = "Node",
            _data = {
                Name = "Item Shop",
                Label = "Item Shop",
                Description = "",
                Width = 1,
                Height = 1,
                Weight = 1.5,
                ExamineTime = 3.0,
                Backgroundcolor = "black",
                ItemSound = "gear_generic",
                Type = "container",
                MaxStack = 1,
                Grids = {
                    [0] = {
                        _name ="1",
                        _id ="shop1",
                        _parent ="shop1",
                        cellsH = 10,
                        cellsV = 10,
                    },
                },
                Itemimage = "",
            }
        }
    },



    ["hhm3eomwf3cygww8e6kcgpt6"] = {
        ["item"] = {
            _id = "hhm3eomwf3cygww8e6kcgpt6",
            _name = "zort_port_rig",
            _parent = "",
            _type = "Node",
            _data = {
                Name = "Glove Box",
                Label = "Glove Box",
                Description = "",
                Width = 1,
                Height = 1,
                Weight = 1.5,
                ExamineTime = 3.0,
                Backgroundcolor = "black",
                ItemSound = "gear_generic",
                Type = "container",
                MaxStack = 1,
                Grids = {
                    [0] = {
                        _name ="1",
                        _id ="61bc85697113f767765c7fsd",
                        _parent ="hhm3eomwf3cygww8e6kcgpt6",
                        cellsH = 5,
                        cellsV = 10,
                    },
                },
                Itemimage = "",
            }
        }
    },

    ["zfrko4gdc7h0cg8cd9xfsjkk"] = {
        ["item"] = {
            _id = "zfrko4gdc7h0cg8cd9xfsjkk",
            _name = "Second Inventory",
            _parent = "",
            _type = "Node",
            _data = {
                Name = "Second Inventory",
                Label = "Second Inventory",
                Description = "",
                Width = 1,
                Height = 1,
                Weight =  0.0,
                Backgroundcolor =  "blue",
                ExamineTime =  0,
                ItemSound =  "",
                Type =  "container",
                MaxStack =  1,
                Grids =  {},
            }
        }
    },

    ["n6rko4gdc7h0cg8cd9xfsjkk"] = {
        ["item"] = {
            _id = "n6rko4gdc7h0cg8cd9xfsjkk",
            _name = "Second Inventory",
            _parent = "",
            _type = "Node",
            _data = {
                Name = "Second Inventory",
                Label = "Second Inventory",
                Description = "",
                Width = 1,
                Height = 1,
                Weight =  0.0,
                Backgroundcolor =  "blue",
                ExamineTime =  0,
                ItemSound =  "",
                Type =  "container",
                MaxStack =  1,
                Grids =  {
                    [0] = {
                        _name = "backpack",
                        _id = "61gd85697113f767765c7fsd",
                        _parent = "n6rko4gdc7h0cg8cd9xfsjkk",
                        cellsH =  1,
                        cellsV =  1,
                    },
                    [1] = {
                        _name = "rig",
                        _id = "61zy85697113f767765c7fgc",
                        _parent = "n6rko4gdc7h0cg8cd9xfsjkk",
                        cellsH =  1,
                        cellsV =  1,
                    },
                    [3] = {
                        _name = "firstweapon",
                        _id = "gclf85697113f767765c7fdd",
                        _parent = "c6jgbwjs9tc2vb8gtpwe7rywe",
                        cellsH =  1,
                        cellsV =  1,
                    },
                    [4] = {
                        _name = "secondweapon",
                        _id = "lclf85697113f767765c7fdd",
                        _parent = "c6jgbwjs9tc2vb8gtpwe7rywe",
                        cellsH =  1,
                        cellsV =  1,
                    },
                },
            }
        }
    },

    ["c6jgbwjs9tc2vb8gtpwe7rywe"] =  {
        ["item"] =  {
            _id =  "c6jgbwjs9tc2vb8gtpwe7rywe",
            _name =  "Player Inventory",
            _parent =  "",
            _type =  "Node",
            _data =  {
                Name =  "Player Inventory",
                Label =  "Player Inventory",
                Description =  "",
                Width =  1,
                Height =  1,
                Weight =  0.0,
                Backgroundcolor =  "blue",
                ExamineTime =  0,
                ItemSound =  "",
                Type =  "container",
                MaxStack =  1,
                Grids =  {
                    [0] = {
                        _name = "backpack",
                        _id = "61zd85697113f767765c7fsd",
                        _parent = "c6jgbwjs9tc2vb8gtpwe7rywe",
                        cellsH =  1,
                        cellsV =  1,
                    },
                    [1] = {
                        _name = "rig",
                        _id = "61jy85697113f767765c7fgc",
                        _parent = "c6jgbwjs9tc2vb8gtpwe7rywe",
                        cellsH =  1,
                        cellsV =  1,
                    },
                    [2] = {
                        _name = "shirts",
                        _id = "61lf85697113f767765c7fdd",
                        _parent = "c6jgbwjs9tc2vb8gtpwe7rywe",
                        cellsH =  1,
                        cellsV =  1,
                    },
                    [3] = {
                        _name = "firstweapon",
                        _id = "gclf85697113f767765c7fdd",
                        _parent = "c6jgbwjs9tc2vb8gtpwe7rywe",
                        cellsH =  1,
                        cellsV =  1,
                    },
                    [4] = {
                        _name = "secondweapon",
                        _id = "lclf85697113f767765c7fdd",
                        _parent = "c6jgbwjs9tc2vb8gtpwe7rywe",
                        cellsH =  1,
                        cellsV =  1,
                    },

                    [5] = {
                        _name = "pants",
                        _id = "61lf8569711yf767765c7fdd",
                        _parent = "c6jgbwjs9tc2vb8gtpwe7rywe",
                        cellsH =  1,
                        cellsV =  1,
                    },
                    [6] = {
                        _name = "top",
                        _id = "61lf85697113f767765cvfdd",
                        _parent = "c6jgbwjs9tc2vb8gtpwe7rywe",
                        cellsH =  1,
                        cellsV =  1,
                    },
                    [7] = {
                        _name = "shoes",
                        _id = "61lf85697113f767765cpfdd",
                        _parent = "c6jgbwjs9tc2vb8gtpwe7rywe",
                        cellsH =  1,
                        cellsV =  1,
                    },
                    [8] = {
                        _name = "mask",
                        _id = "61lf85697113f767765cufdd",
                        _parent = "c6jgbwjs9tc2vb8gtpwe7rywe",
                        cellsH =  1,
                        cellsV =  1,
                    },
                    [9] = {
                        _name = "gloves",
                        _id = "61lf85697113f767765cpfdd",
                        _parent = "c6jgbwjs9tc2vb8gtpwe7rywe",
                        cellsH =  1,
                        cellsV =  1,
                    },
                    [10] = {
                        _name = "hat",
                        _id = "61lf85697113f767765cufdd",
                        _parent = "c6jgbwjs9tc2vb8gtpwe7rywe",
                        cellsH =  1,
                        cellsV =  1,
                    },

                    [11] = {
                        _name = "glasses",
                        _id = "61lf85697113f767765cpfdd",
                        _parent = "c6jgbwjs9tc2vb8gtpwe7rywe",
                        cellsH =  1,
                        cellsV =  1,
                    },
                    [12] = {
                        _name = "ear",
                        _id = "61lf85697113f767765cufdd",
                        _parent = "c6jgbwjs9tc2vb8gtpwe7rywe",
                        cellsH =  1,
                        cellsV =  1,
                    },
                    [13] = {
                        _name = "bracelet",
                        _id = "61lf85697113f767765cpfdd",
                        _parent = "c6jgbwjs9tc2vb8gtpwe7rywe",
                        cellsH =  1,
                        cellsV =  1,
                    },
                    [14] = {
                        _name = "watch",
                        _id = "61lf85697113f767765cufdd",
                        _parent = "c6jgbwjs9tc2vb8gtpwe7rywe",
                        cellsH =  1,
                        cellsV =  1,
                    },
                },
            }
        }
    },

    ["pbep0kj5kinr4yuvrlq8x2p9"] =  {
        ["item"] =  {
            _id =  "pbep0kj5kinr4yuvrlq8x2p9",
            _name =  "Pocket Inventory",
            _parent =  "c6jgbwjs9tc2vb8gtpwe7rywe",
            _type =  "Item",
            _data =  {
                Name =  "Pockets",
                Label =  "Pockets",
                Description =  "",
                Width =  1,
                Height =  1,
                Weight =  0.0,
                Backgroundcolor =  "blue",
                ExamineTime =  2.5,
                ItemSound =  "gear_generic",
                Type =  "container",
                MaxStack =  1,
                Grids =  {
                    [0] = {
                        _name = "pckt1",
                        _id = "61bc85697113f767765c7fsd",
                        _parent = "pbep0kj5kinr4yuvrlq8x2p9",
                        cellsH =  1,
                        cellsV =  1,
                    },
                    [1] = {
                        _name = "pckt2",
                        _id = "61bc85697113f767765c7fgc",
                        _parent = "pbep0kj5kinr4yuvrlq8x2p9",
                        cellsH =  1,
                        cellsV =  1,
                    },
                    [2] = {
                        _name = "pckt3",
                        _id = "61bc85697113f767765c7fdd",
                        _parent = "pbep0kj5kinr4yuvrlq8x2p9",
                        cellsH =  1,
                        cellsV =  1,
                    },
                    [3] = {
                        _name = "pckt4",
                        _id = "61bc85697113f767765cgcdd",
                        _parent = "pbep0kj5kinr4yuvrlq8x2p9",
                        cellsH =  1,
                        cellsV =  1,
                    },
                },
            }
        }
    },

    ["a7y4c5de9coje2zvp0zhwlig"] = {
        ["item"] = {
            _id = "a7y4c5de9coje2zvp0zhwlig",
            _name = "ifak",
            _parent = "",
            _type =  "Item",
            _data =  {
                Name =  "IFAK",
                Label =  "IFAK",
                Description =  "",
                Width =  1,
                Height =  1,
                Weight =  0.8,
                ExamineTime =  0.7,
                Backgroundcolor =  "black",
                ItemSound =  "med_medkit",
                Itemimage =  "http://sc04.alicdn.com/kf/H08c6eda9e5c44ebca61987bbf4ecdc33G.png",
                Type =  "Medkit",
                MaxStack =  50,
            }
        }
    },

    ["14ehcp8zup6cqffe3nxwy8pz"] = {
        ["item"] = {
            _id = "14ehcp8zup6cqffe3nxwy8pz",
            _name = "weapon_pistol",
            _parent = "",
            _type = "Item",
            _data = {
                Name = "Weapon Pistol",
                Label = "Weapon Pistol",
                Description = "",
                Width = 3,
                Height = 2,
                Weight = 0.8,
                ExamineTime = 1.0,
                Backgroundcolor = "black",
                ItemSound = "weap_pistol",
                Itemimage = "icons/weapon_pistol.png",
                Type = "Weapon",
                MaxStack = 1,
            }
        }
    },

    ["suppressor"] = {
        ["item"] = {
            _id = "suppressor",
            _name = "suppressor",
            _parent = "",
            _type = "Item",
            _data = {
                Name = "Suppressor",
                Label = "Suppressor",
                Description = "",
                Width = 1,
                Height = 1,
                Weight = 0.8,
                ExamineTime = 1.0,
                Backgroundcolor = "black",
                ItemSound = "weap_pistol",
                Itemimage = "icons/pistol_suppressor.png",
                Type = "Attachment",
                MaxStack = 1,
            }
        }
    },
    ["flashlight"] = {
        ["item"] = {
            _id = "flashlight",
            _name = "flashlight",
            _parent = "",
            _type = "Item",
            _data = {
                Name = "Flashlight",
                Label = "Flashlight",
                Description = "",
                Width = 1,
                Height = 1,
                Weight = 0.8,
                ExamineTime = 1.0,
                Backgroundcolor = "black",
                ItemSound = "weap_pistol",
                Itemimage = "icons/smg_flashlight.png",
                Type = "Attachment",
                MaxStack = 1,
            }
        }
    },
    ["clip_extended"] = {
        ["item"] = {
            _id = "clip_extended",
            _name = "clip_extended",
            _parent = "",
            _type = "Item",
            _data = {
                Name = "Clip Extended",
                Label = "Clip Extended",
                Description = "",
                Width = 1,
                Height = 1,
                Weight = 0.8,
                ExamineTime = 1.0,
                Backgroundcolor = "black",
                ItemSound = "weap_pistol",
                Itemimage = "icons/pistol_extendedclip.png",
                Type = "Attachment",
                MaxStack = 1,
            }
        }
    },
    ["clip_drum"] = {
        ["item"] = {
            _id = "clip_drum",
            _name = "clip_drum",
            _parent = "",
            _type = "Item",
            _data = {
                Name = "Clip Drum",
                Label = "Clip Drum",
                Description = "",
                Width = 1,
                Height = 1,
                Weight = 0.8,
                ExamineTime = 1.0,
                Backgroundcolor = "black",
                ItemSound = "weap_pistol",
                Itemimage = "icons/rifle_drummag.png",
                Type = "Attachment",
                MaxStack = 1,
            }
        }
    },
    ["scope"] = {
        ["item"] = {
            _id = "scope",
            _name = "scope",
            _parent = "",
            _type = "Item",
            _data = {
                Name = "Scope",
                Label = "Scope",
                Description = "",
                Width = 1,
                Height = 1,
                Weight = 0.8,
                ExamineTime = 1.0,
                Backgroundcolor = "black",
                ItemSound = "weap_pistol",
                Itemimage = "icons/smg_scope.png",
                Type = "Attachment",
                MaxStack = 1,
            }
        }
    },

    ["zehd54y3no4yfvizu17qwe98"] = {
        ["item"] = {
            _id = "zehd54y3no4yfvizu17qwe98",
            _name = "sandwich",
            _parent = "",
            _type = "Item",
            _data = {
                Name = "Sandwich",
                Label = "Sandwich",
                Description = "",
                Width = 1,
                Height = 1,
                Weight = 0.8,
                ExamineTime = 1.0,
                Backgroundcolor = "black",
                ItemSound = "gear_generic",
                Itemimage = "icons/sandwich.png",
                Type = "Item",
                MaxStack = 250,
            }
        }
    },

    ["top"] = {
        ["item"] = {
            _id = "top",
            _name = "clothing_top",
            _parent = "c6jgbwjs9tc2vb8gtpwe7rywe",
            _type = "Item",
            _data = {
                Name = "Shirt",
                AttachableSlot = "top",
                Label = "Shirt",
                Description = "",
                Width = 1,
                Height = 2,
                Weight = 1.5,
                ExamineTime = 3.0,
                Backgroundcolor = "black",
                ItemSound = "gear_backpack",
                Type = "Clothing",
                MaxStack = 1,
                Grids = {},
                Itemimage = "icons/top.png",
            }
        }
    },

    ["shirts"] = {
        ["item"] = {
            _id = "shirts",
            _name = "clothing_shirts",
            _parent = "c6jgbwjs9tc2vb8gtpwe7rywe",
            _type = "Item",
            _data = {
                Name = "T-Shirt",
                AttachableSlot = "shirts",
                Label = "T-Shirt",
                Description = "",
                Width = 1,
                Height = 1,
                Weight = 1.5,
                ExamineTime = 3.0,
                Backgroundcolor = "black",
                ItemSound = "gear_backpack",
                Type = "Clothing",
                MaxStack = 1,
                Grids = {},
                Itemimage = "icons/shirt.png",
            }
        }
    },

    ["pants"] = {
        ["item"] = {
            _id = "pants",
            _name = "clothing_pants",
            _parent = "c6jgbwjs9tc2vb8gtpwe7rywe",
            _type = "Item",
            _data = {
                Name = "Pants",
                AttachableSlot = "pants",
                Label = "Pants",
                Description = "",
                Width = 1,
                Height = 2,
                Weight = 1.5,
                ExamineTime = 3.0,
                Backgroundcolor = "black",
                ItemSound = "gear_backpack",
                Type = "Clothing",
                MaxStack = 1,
                Grids = {},
                Itemimage = "icons/pants.png",
            }
        }
    },

    ["shoes"] = {
        ["item"] = {
            _id = "shoes",
            _name = "clothing_shoes",
            _parent = "c6jgbwjs9tc2vb8gtpwe7rywe",
            _type = "Item",
            _data = {
                Name = "Shoes",
                AttachableSlot = "shoes",
                Label = "Shoes",
                Description = "",
                Width = 1,
                Height = 1,
                Weight = 1.5,
                ExamineTime = 3.0,
                Backgroundcolor = "black",
                ItemSound = "gear_backpack",
                Type = "Clothing",
                MaxStack = 1,
                Grids = {},
                Itemimage = "icons/shoes.png",
            }
        }
    },

    ["mask"] = {
        ["item"] = {
            _id = "mask",
            _name = "clothing_mask",
            _parent = "c6jgbwjs9tc2vb8gtpwe7rywe",
            _type = "Item",
            _data = {
                Name = "Mask",
                AttachableSlot = "mask",
                Label = "Mask",
                Description = "",
                Width = 1,
                Height = 1,
                Weight = 1.5,
                ExamineTime = 3.0,
                Backgroundcolor = "black",
                ItemSound = "gear_backpack",
                Type = "Clothing",
                MaxStack = 1,
                Grids = {},
                Itemimage = "icons/mask.png",
            }
        }
    },

    ["gloves"] = {
        ["item"] = {
            _id = "gloves",
            _name = "clothing_gloves",
            _parent = "c6jgbwjs9tc2vb8gtpwe7rywe",
            _type = "Item",
            _data = {
                Name = "Gloves",
                AttachableSlot = "gloves",
                Label = "Gloves",
                Description = "",
                Width = 1,
                Height = 1,
                Weight = 1.5,
                ExamineTime = 3.0,
                Backgroundcolor = "black",
                ItemSound = "gear_backpack",
                Type = "Clothing",
                MaxStack = 1,
                Grids = {},
                Itemimage = "icons/gloves.png",
            }
        }
    },

    ["hat"] = {
        ["item"] = {
            _id = "hat",
            _name = "clothing_hat",
            _parent = "c6jgbwjs9tc2vb8gtpwe7rywe",
            _type = "Item",
            _data = {
                Name = "Hat",
                AttachableSlot = "hat",
                Label = "Hat",
                Description = "",
                Width = 1,
                Height = 1,
                Weight = 1.5,
                ExamineTime = 3.0,
                Backgroundcolor = "black",
                ItemSound = "gear_backpack",
                Type = "Clothing",
                MaxStack = 1,
                Grids = {},
                Itemimage = "icons/hat.png",
            }
        }
    },

    ["ear"] = {
        ["item"] = {
            _id = "ear",
            _name = "clothing_ear",
            _parent = "c6jgbwjs9tc2vb8gtpwe7rywe",
            _type = "Item",
            _data = {
                Name = "Ear",
                AttachableSlot = "ear",
                Label = "Earrings",
                Description = "",
                Width = 1,
                Height = 1,
                Weight = 1.5,
                ExamineTime = 3.0,
                Backgroundcolor = "black",
                ItemSound = "gear_backpack",
                Type = "Clothing",
                MaxStack = 1,
                Grids = {},
                Itemimage = "icons/ear.png",
            }
        }
    },

    ["watch"] = {
        ["item"] = {
            _id = "watch",
            _name = "clothing_watch",
            _parent = "c6jgbwjs9tc2vb8gtpwe7rywe",
            _type = "Item",
            _data = {
                Name = "Watches",
                AttachableSlot = "watch",
                Label = "Watches",
                Description = "",
                Width = 1,
                Height = 1,
                Weight = 1.5,
                ExamineTime = 3.0,
                Backgroundcolor = "black",
                ItemSound = "gear_backpack",
                Type = "Clothing",
                MaxStack = 1,
                Grids = {},
                Itemimage = "icons/watch.png",
            }
        }
    },

    ["glasses"] = {
        ["item"] = {
            _id = "glasses",
            _name = "clothing_glasses",
            _parent = "c6jgbwjs9tc2vb8gtpwe7rywe",
            _type = "Item",
            _data = {
                Name = "Glasses",
                AttachableSlot = "glasses",
                Label = "Glasses",
                Description = "",
                Width = 1,
                Height = 1,
                Weight = 1.5,
                ExamineTime = 3.0,
                Backgroundcolor = "black",
                ItemSound = "gear_backpack",
                Type = "Clothing",
                MaxStack = 1,
                Grids = {},
                Itemimage = "icons/glasses.png",
            }
        }
    },

    ["bracelet"] = {
        ["item"] = {
            _id = "bracelet",
            _name = "clothing_bracelet",
            _parent = "c6jgbwjs9tc2vb8gtpwe7rywe",
            _type = "Item",
            _data = {
                Name = "Bracelet",
                AttachableSlot = "bracelet",
                Label = "Bracelet",
                Description = "",
                Width = 1,
                Height = 1,
                Weight = 1.5,
                ExamineTime = 3.0,
                Backgroundcolor = "black",
                ItemSound = "gear_backpack",
                Type = "Clothing",
                MaxStack = 1,
                Grids = {},
                Itemimage = "icons/bracelet.png",
            }
        }
    },

    ["rig1"] = {
        ["item"] = {
            _id = "rig1",
            _name = "military_rig",
            _parent = "c6jgbwjs9tc2vb8gtpwe7rywe",
            _type = "Item",
            _data = {
                Name = "Tactical Rig",
                AttachableSlot = "rig",
                Label = "Military Rig",
                Description = "",
                Width = 4,
                Height = 3,
                Weight = 1.5,
                ExamineTime = 3.0,
                Backgroundcolor = "black",
                ItemSound = "gear_backpack",
                Type = "container",
                MaxStack = 1,
                Grids = {
                    [0] = {
                        _name ="tl_pouch",
                        _id ="tr_1",
                        _parent ="rig1",
                        cellsH = 2,
                        cellsV = 2,
                    },
                    [1] = {
                        _name ="tr_pouch1",
                        _id ="tr_2",
                        _parent ="rig1",
                        cellsH = 2,
                        cellsV = 1,
                    },
                    [2] = {
                        _name ="tr_pouch2",
                        _id ="tr_2",
                        _parent ="rig1",
                        cellsH = 2,
                        cellsV = 1,
                    },
                    [3] = {
                        _name ="b_pouch1",
                        _id ="tr_3",
                        _parent ="rig1",
                        cellsH = 2,
                        cellsV = 1,
                    },
                    [4] = {
                        _name ="b_pouch2",
                        _id ="tr_4",
                        _parent ="rig1",
                        cellsH = 2,
                        cellsV = 1,
                    },
                    [5] = {
                        _name ="b_pouch3",
                        _id ="tr_5",
                        _parent ="rig1",
                        cellsH = 2,
                        cellsV = 1,
                    },
                    [6] = {
                        _name ="b_pouch4",
                        _id ="tr_6",
                        _parent ="rig1",
                        cellsH = 2,
                        cellsV = 1,
                    },
                },
                Itemimage = "icons/armor.png",
            }
        }
    },

    ["bag1"] = {
        ["item"] = {
            _id = "bag1",
            _name = "sport_backpack",
            _parent = "c6jgbwjs9tc2vb8gtpwe7rywe",
            _type = "Item",
            _data = {
                Name = "Backpack",
                AttachableSlot = "backpack",
                Label = "Sport Bag",
                Description = "",
                Width = 4,
                Height = 3,
                Weight = 1.5,
                ExamineTime = 3.0,
                Backgroundcolor = "black",
                ItemSound = "gear_backpack",
                Type = "container",
                MaxStack = 1,
                Grids = {
                    [0] = {
                        _name ="1",
                        _id ="61bc85697113f767765c7fsd",
                        _parent ="bag1",
                        cellsH = 4,
                        cellsV = 4,
                    },
                    [1] = {
                        _name ="2",
                        _id ="61bc85697113f767765c7fgc",
                        _parent ="bag1",
                        cellsH = 1,
                        cellsV = 4,
                    },
                    [2] = {
                        _name ="3",
                        _id ="61bc85697113f767765c7fdd",
                        _parent ="bag1",
                        cellsH = 1,
                        cellsV = 4,
                    },
                },
                Itemimage = "icons/bag.png",
            }
        }
    },







































    ["coks1u2elb452g0hsd25w6if"] = {    -- TRUNK CLASS 0
        ["item"] = {
            _id = "coks1u2elb452g0hsd25w6if",
            _name = "zort_port_rig",
            _parent = "",
            _type = "Node",
            _data = {
                Name = "Trunk",
                Label = "Glove Box",
                Description = "",
                Width = 1,
                Height = 1,
                Weight = 1.5,
                ExamineTime = 3.0,
                Backgroundcolor = "black",
                ItemSound = "gear_generic",
                Type = "container",
                MaxStack = 1,
                Grids = {
                    [0] = {
                        _name ="1",
                        _id ="61bc85697113f767765c7fsd",
                        _parent ="coks1u2elb452g0hsd25w6if",
                        cellsH = 12,
                        cellsV = 5,
                    },
                },
                Itemimage = "",
            }
        }
    },
    ["a584yw9439bwluc1voo2x1rm"] = {    -- TRUNK CLASS 1
        ["item"] = {
            _id = "a584yw9439bwluc1voo2x1rm",
            _name = "zort_port_rig",
            _parent = "",
            _type = "Node",
            _data = {
                Name = "Trunk",
                Label = "Trunk",
                Description = "",
                Width = 1,
                Height = 1,
                Weight = 1.5,
                ExamineTime = 3.0,
                Backgroundcolor = "black",
                ItemSound = "gear_generic",
                Type = "container",
                MaxStack = 1,
                Grids = {
                    [0] = {
                        _name ="1",
                        _id ="61bc85697113f767765c7fsd",
                        _parent ="a584yw9439bwluc1voo2x1rm",
                        cellsH = 15,
                        cellsV = 6,
                    },
                },
                Itemimage = "",
            }
        }
    },
    ["zpop64fqra28l9tnle839emg"] = {    -- TRUNK CLASS 2
        ["item"] = {
            _id = "zpop64fqra28l9tnle839emg",
            _name = "zort_port_rig",
            _parent = "",
            _type = "Node",
            _data = {
                Name = "Trunk",
                Label = "Trunk",
                Description = "",
                Width = 1,
                Height = 1,
                Weight = 1.5,
                ExamineTime = 3.0,
                Backgroundcolor = "black",
                ItemSound = "gear_generic",
                Type = "container",
                MaxStack = 1,
                Grids = {
                    [0] = {
                        _name ="1",
                        _id ="61bc85697113f767765c7fsd",
                        _parent ="zpop64fqra28l9tnle839emg",
                        cellsH = 17,
                        cellsV = 6,
                    },
                },
                Itemimage = "",
            }
        }
    },
    ["0vh1lnz1a7ojflbvoul4yaz0"] = {    -- TRUNK CLASS 3
        ["item"] = {
            _id = "0vh1lnz1a7ojflbvoul4yaz0",
            _name = "zort_port_rig",
            _parent = "",
            _type = "Node",
            _data = {
                Name = "Trunk",
                Label = "Trunk",
                Description = "",
                Width = 1,
                Height = 1,
                Weight = 1.5,
                ExamineTime = 3.0,
                Backgroundcolor = "black",
                ItemSound = "gear_generic",
                Type = "container",
                MaxStack = 1,
                Grids = {
                    [0] = {
                        _name ="1",
                        _id ="61bc85697113f767765c7fsd",
                        _parent ="0vh1lnz1a7ojflbvoul4yaz0",
                        cellsH = 5,
                        cellsV = 10,
                    },
                },
                Itemimage = "",
            }
        }
    },
    ["ar9kya72yoirzu2cfwr8hz85"] = {    -- TRUNK CLASS 4
        ["item"] = {
            _id = "ar9kya72yoirzu2cfwr8hz85",
            _name = "zort_port_rig",
            _parent = "",
            _type = "Node",
            _data = {
                Name = "Trunk",
                Label = "Trunk",
                Description = "",
                Width = 1,
                Height = 1,
                Weight = 1.5,
                ExamineTime = 3.0,
                Backgroundcolor = "black",
                ItemSound = "gear_generic",
                Type = "container",
                MaxStack = 1,
                Grids = {
                    [0] = {
                        _name ="1",
                        _id ="61bc85697113f767765c7fsd",
                        _parent ="ar9kya72yoirzu2cfwr8hz85",
                        cellsH = 9,
                        cellsV = 3,
                    },
                },
                Itemimage = "",
            }
        }
    },
    ["yexq0qwrv2fsnjot0l0tzmom"] = {    -- TRUNK CLASS 5
        ["item"] = {
            _id = "yexq0qwrv2fsnjot0l0tzmom",
            _name = "zort_port_rig",
            _parent = "",
            _type = "Node",
            _data = {
                Name = "Trunk",
                Label = "Trunk",
                Description = "",
                Width = 1,
                Height = 1,
                Weight = 1.5,
                ExamineTime = 3.0,
                Backgroundcolor = "black",
                ItemSound = "gear_generic",
                Type = "container",
                MaxStack = 1,
                Grids = {
                    [0] = {
                        _name ="1",
                        _id ="61bc85697113f767765c7fsd",
                        _parent ="yexq0qwrv2fsnjot0l0tzmom",
                        cellsH = 6,
                        cellsV = 2,
                    },
                },
                Itemimage = "",
            }
        }
    },
    ["zqohzbplr45rooxm81e655vz"] = {    -- TRUNK CLASS 6
        ["item"] = {
            _id = "zqohzbplr45rooxm81e655vz",
            _name = "zort_port_rig",
            _parent = "",
            _type = "Node",
            _data = {
                Name = "Trunk",
                Label = "Trunk",
                Description = "",
                Width = 1,
                Height = 1,
                Weight = 1.5,
                ExamineTime = 3.0,
                Backgroundcolor = "black",
                ItemSound = "gear_generic",
                Type = "container",
                MaxStack = 1,
                Grids = {
                    [0] = {
                        _name ="1",
                        _id ="61bc85697113f767765c7fsd",
                        _parent ="zqohzbplr45rooxm81e655vz",
                        cellsH = 6,
                        cellsV = 2,
                    },
                },
                Itemimage = "",
            }
        }
    },
    ["r9vfk2316yp7uoz5z4d7k4pr"] = {    -- TRUNK CLASS 7
        ["item"] = {
            _id = "r9vfk2316yp7uoz5z4d7k4pr",
            _name = "zort_port_rig",
            _parent = "",
            _type = "Node",
            _data = {
                Name = "Trunk",
                Label = "Trunk",
                Description = "",
                Width = 1,
                Height = 1,
                Weight = 1.5,
                ExamineTime = 3.0,
                Backgroundcolor = "black",
                ItemSound = "gear_generic",
                Type = "container",
                MaxStack = 1,
                Grids = {
                    [0] = {
                        _name ="1",
                        _id ="61bc85697113f767765c7fsd",
                        _parent ="r9vfk2316yp7uoz5z4d7k4pr",
                        cellsH = 4,
                        cellsV = 2,
                    },
                },
                Itemimage = "",
            }
        }
    },
    ["dcapvociisv5u7drc611yyvq"] = {    -- TRUNK CLASS 8
        ["item"] = {
            _id = "dcapvociisv5u7drc611yyvq",
            _name = "zort_port_rig",
            _parent = "",
            _type = "Node",
            _data = {
                Name = "Trunk",
                Label = "Trunk",
                Description = "",
                Width = 1,
                Height = 1,
                Weight = 1.5,
                ExamineTime = 3.0,
                Backgroundcolor = "black",
                ItemSound = "gear_generic",
                Type = "container",
                MaxStack = 1,
                Grids = {
                    [0] = {
                        _name ="1",
                        _id ="61bc85697113f767765c7fsd",
                        _parent ="dcapvociisv5u7drc611yyvq",
                        cellsH = 2,
                        cellsV = 2,
                    },
                },
                Itemimage = "",
            }
        }
    },
    ["p70ufhnlm9ai0f42v7w85nj0"] = {    -- TRUNK CLASS 9/12
        ["item"] = {
            _id = "p70ufhnlm9ai0f42v7w85nj0",
            _name = "zort_port_rig",
            _parent = "",
            _type = "Node",
            _data = {
                Name = "Trunk",
                Label = "Trunk",
                Description = "",
                Width = 1,
                Height = 1,
                Weight = 1.5,
                ExamineTime = 3.0,
                Backgroundcolor = "black",
                ItemSound = "gear_generic",
                Type = "container",
                MaxStack = 1,
                Grids = {
                    [0] = {
                        _name ="1",
                        _id ="61bc85697113f767765c7fsd",
                        _parent ="p70ufhnlm9ai0f42v7w85nj0",
                        cellsH = 24,
                        cellsV = 12,
                    },
                },
                Itemimage = "",
            }
        }
    },
    ["nfi5z2p26m6e4ewpuxkjh2la"] = {    -- TRUNK CLASS 13
        ["item"] = {
            _id = "nfi5z2p26m6e4ewpuxkjh2la",
            _name = "zort_port_rig",
            _parent = "",
            _type = "Node",
            _data = {
                Name = "Trunk",
                Label = "Trunk",
                Description = "",
                Width = 1,
                Height = 1,
                Weight = 1.5,
                ExamineTime = 3.0,
                Backgroundcolor = "black",
                ItemSound = "gear_generic",
                Type = "container",
                MaxStack = 1,
                Grids = {
                    [0] = {
                        _name ="1",
                        _id ="61bc85697113f767765c7fsd",
                        _parent ="nfi5z2p26m6e4ewpuxkjh2la",
                        cellsH = 1,
                        cellsV = 1,
                    },
                },
                Itemimage = "",
            }
        }
    },
    ["m3jpk09d64ij0r6px1g80pcy"] = {    -- TRUNK CLASS 14
        ["item"] = {
            _id = "m3jpk09d64ij0r6px1g80pcy",
            _name = "zort_port_rig",
            _parent = "",
            _type = "Node",
            _data = {
                Name = "Trunk",
                Label = "Trunk",
                Description = "",
                Width = 1,
                Height = 1,
                Weight = 1.5,
                ExamineTime = 3.0,
                Backgroundcolor = "black",
                ItemSound = "gear_generic",
                Type = "container",
                MaxStack = 1,
                Grids = {
                    [0] = {
                        _name ="1",
                        _id ="61bc85697113f767765c7fsd",
                        _parent ="m3jpk09d64ij0r6px1g80pcy",
                        cellsH = 24,
                        cellsV = 12,
                    },
                },
                Itemimage = "",
            }
        }
    },
    ["irn05edljdp6hvamcdgm6tgz"] = {    -- TRUNK CLASS 15
        ["item"] = {
            _id = "irn05edljdp6hvamcdgm6tgz",
            _name = "zort_port_rig",
            _parent = "",
            _type = "Node",
            _data = {
                Name = "Trunk",
                Label = "Trunk",
                Description = "",
                Width = 1,
                Height = 1,
                Weight = 1.5,
                ExamineTime = 3.0,
                Backgroundcolor = "black",
                ItemSound = "gear_generic",
                Type = "container",
                MaxStack = 1,
                Grids = {
                    [0] = {
                        _name ="1",
                        _id ="61bc85697113f767765c7fsd",
                        _parent ="irn05edljdp6hvamcdgm6tgz",
                        cellsH = 26,
                        cellsV = 14,
                    },
                },
                Itemimage = "",
            }
        }
    },
    ["e5z2xps7t2km3nhrz6lufaob"] = {    -- TRUNK CLASS 16
        ["item"] = {
            _id = "e5z2xps7t2km3nhrz6lufaob",
            _name = "zort_port_rig",
            _parent = "",
            _type = "Node",
            _data = {
                Name = "Trunk",
                Label = "Trunk",
                Description = "",
                Width = 1,
                Height = 1,
                Weight = 1.5,
                ExamineTime = 3.0,
                Backgroundcolor = "black",
                ItemSound = "gear_generic",
                Type = "container",
                MaxStack = 1,
                Grids = {
                    [0] = {
                        _name ="1",
                        _id ="61bc85697113f767765c7fsd",
                        _parent ="e5z2xps7t2km3nhrz6lufaob",
                        cellsH = 64,
                        cellsV = 10,
                    },
                },
                Itemimage = "",
            }
        }
    },
    ["qfjj7h3sb456b2z320vlamvu"] = {    -- EMPTY
        ["item"] = {
            _id = "qfjj7h3sb456b2z320vlamvu",
            _name = "zort_port_rig",
            _parent = "",
            _type = "Node",
            _data = {
                Name = "Trunk",
                Label = "Trunk",
                Description = "",
                Width = 1,
                Height = 1,
                Weight = 1.5,
                ExamineTime = 3.0,
                Backgroundcolor = "black",
                ItemSound = "gear_generic",
                Type = "container",
                MaxStack = 1,
                Grids = {
                    [0] = {
                        _name ="1",
                        _id ="61bc85697113f767765c7fsd",
                        _parent ="qfjj7h3sb456b2z320vlamvu",
                        cellsH = 1,
                        cellsV = 1,
                    },
                },
                Itemimage = "",
            }
        }
    },
    ["b6riv7imc527u8r853clpy5j"] = {    -- DROP INVENTORY
        ["item"] = {
            _id = "b6riv7imc527u8r853clpy5j",
            _name = "zort_port_rig",
            _parent = "",
            _type = "Node",
            _data = {
                Name = "Drop Inventory",
                Label = "Drop Inventory",
                Description = "",
                Width = 1,
                Height = 1,
                Weight = 1.5,
                ExamineTime = 3.0,
                Backgroundcolor = "black",
                ItemSound = "gear_generic",
                Type = "container",
                MaxStack = 1,
                Grids = {
                    [0] = {
                        _name ="1",
                        _id ="61bc85697113f767765c7fsd",
                        _parent ="b6riv7imc527u8r853clpy5j",
                        cellsH = 30,
                        cellsV = 10,
                    },
                },
                Itemimage = "",
            }
        }
    },

    ["custom_inventory"] = {    -- DROP INVENTORY
        ["item"] = {
            _id = "custom_inventory",
            _name = "inv_custom",
            _parent = "",
            _type = "Node",
            _data = {
                Name = "Custom Inventory",
                Label = "Custom Inventory",
                Description = "",
                Width = 1,
                Height = 1,
                Weight = 1.5,
                ExamineTime = 3.0,
                Backgroundcolor = "black",
                ItemSound = "gear_generic",
                Type = "container",
                MaxStack = 1,
                Grids = {
                    [0] = {
                        _name ="1",
                        _id ="custom_inventory_grid",
                        _parent ="custom_inventory",
                        cellsH = 30,
                        cellsV = 10,
                    },
                },
                Itemimage = "",
            }
        }
    },



































    ---------------------------
    ------CONVERTED ITEMS------
    ---------------------------

























































    ['weapon_case'] = {
        ['item'] = {
            ['_id'] = 'weapon_case',
            ['_parent'] = '',
            ['_name'] = 'case_weapon_case',
            ['_data'] = {
                ['ExamineTime'] = 1.0,
                ['Rarity'] = 'common',
                ['Label'] = 'Weapon Case',
                ['MaxStack'] = 1,
                ['Itemimage'] = 'icons/weapon_case.png',
                ['Type'] = 'container',
                ['Backgroundcolor'] = 'black',
                ['Description'] = 'It is very fragile, watch out',
                ['Name'] = 'weapon_case',
                ['Weight'] = 2.5,
                ['ItemSound'] = 'gear_generic',
                ['Height'] = 2,
                ['Width'] = 4,
                restrictedItems = {},
                authorizedCategory = {
                    ["Weapon"] = true
                }, 
                authorizedItems = {},
                Grids = {
                    [0] = {
                        _name ="1",
                        _id ="61bc85697113f767765c7fsd",
                        _parent ="qfjj7h3sb456b2z320vlamvu",
                        cellsH = 7,
                        cellsV = 8,
                    },
                }
            },
            ['_type'] = 'Item'
        }
    },

        ['weapon_stickybomb'] = {
            ['item'] = {
                ['_id'] = 'weapon_stickybomb',
                ['_parent'] = '',
                ['_name'] = 'weapon_stickybomb',
                ['_data'] = {
                    ['ExamineTime'] = 1.0,
                    ['Rarity'] = 'rare',
                    ['Label'] = 'C4',
                    ['MaxStack'] = 1,
                    ['Itemimage'] = 'icons/weapon_stickybomb.png',
                    ['Type'] = 'Weapon',
                    ['Backgroundcolor'] = 'blue',
                    ['Description'] = 'An explosive charge covered with an adhesive that when thrown against an object sticks until it explodes',
                    ['Name'] = 'weapon_stickybomb',
                    ['Weight'] = 1.0,
                    ['ItemSound'] = 'gear_generic',
                    ['Height'] = 2,
                    ['Width'] = 2
                },
                ['_type'] = 'Item'
            }
        },
        ['thermite'] = {
            ['item'] = {
                ['_id'] = 'thermite',
                ['_parent'] = '',
                ['_name'] = 'thermite',
                ['_data'] = {
                    ['ExamineTime'] = 1.0,
                    ['Rarity'] = 'common',
                    ['Label'] = 'Thermite',
                    ['MaxStack'] = 100,
                    ['Itemimage'] = 'icons/thermite.png',
                    ['Type'] = 'Item',
                    ['Backgroundcolor'] = 'black',
                    ['Description'] = 'Sometimes you\'d wish for everything to burn',
                    ['Name'] = 'thermite',
                    ['Weight'] = 1.0,
                    ['ItemSound'] = 'gear_generic',
                    ['Height'] = 1,
                    ['Width'] = 1
                },
                ['_type'] = 'Item'
            }
        },
        ['weed_ak47'] = {
            ['item'] = {
                ['_id'] = 'weed_ak47',
                ['_parent'] = '',
                ['_name'] = 'weed_ak47',
                ['_data'] = {
                    ['ExamineTime'] = 1.0,
                    ['Rarity'] = 'common',
                    ['Label'] = 'AK47 2g',
                    ['MaxStack'] = 100,
                    ['Itemimage'] = 'icons/weed_baggy.png',
                    ['Type'] = 'Item',
                    ['Backgroundcolor'] = 'black',
                    ['Description'] = 'A weed bag with 2g AK47',
                    ['Name'] = 'weed_ak47',
                    ['Weight'] = 0.2,
                    ['ItemSound'] = 'gear_generic',
                    ['Height'] = 1,
                    ['Width'] = 1
                },
                ['_type'] = 'Item'
            }
        },
        ['weapon_hatchet'] = {
            ['item'] = {
                ['_id'] = 'weapon_hatchet',
                ['_parent'] = '',
                ['_name'] = 'weapon_hatchet',
                ['_data'] = {
                    ['ExamineTime'] = 1.0,
                    ['Rarity'] = 'rare',
                    ['Label'] = 'Hatchet',
                    ['MaxStack'] = 1,
                    ['Itemimage'] = 'icons/weapon_hatchet.png',
                    ['Type'] = 'Weapon',
                    ['Backgroundcolor'] = 'blue',
                    ['Description'] = 'A small axe with a short handle for use in one hand',
                    ['Name'] = 'weapon_hatchet',
                    ['Weight'] = 1.0,
                    ['ItemSound'] = 'gear_generic',
                    ['Height'] = 2,
                    ['Width'] = 2
                },
                ['_type'] = 'Item'
            }
        },
        ['antipatharia_coral'] = {
            ['item'] = {
                ['_id'] = 'antipatharia_coral',
                ['_parent'] = '',
                ['_name'] = 'antipatharia_coral',
                ['_data'] = {
                    ['ExamineTime'] = 1.0,
                    ['Rarity'] = 'common',
                    ['Label'] = 'Antipatharia',
                    ['MaxStack'] = 100,
                    ['Itemimage'] = 'icons/antipatharia_coral.png',
                    ['Type'] = 'Item',
                    ['Backgroundcolor'] = 'black',
                    ['Description'] = 'Its also known as black corals or thorn corals',
                    ['Name'] = 'antipatharia_coral',
                    ['Weight'] = 1.0,
                    ['ItemSound'] = 'gear_generic',
                    ['Height'] = 1,
                    ['Width'] = 1
                },
                ['_type'] = 'Item'
            }
        },
        ['electronickit'] = {
            ['item'] = {
                ['_id'] = 'electronickit',
                ['_parent'] = '',
                ['_name'] = 'electronickit',
                ['_data'] = {
                    ['ExamineTime'] = 1.0,
                    ['Rarity'] = 'common',
                    ['Label'] = 'Electronic Kit',
                    ['MaxStack'] = 100,
                    ['Itemimage'] = 'icons/electronickit.png',
                    ['Type'] = 'Item',
                    ['Backgroundcolor'] = 'black',
                    ['Description'] = 'If you\'ve always wanted to build a robot you can maybe start here. Maybe you\'ll be the new Elon Musk?',
                    ['Name'] = 'electronickit',
                    ['Weight'] = 0.1,
                    ['ItemSound'] = 'gear_generic',
                    ['Height'] = 1,
                    ['Width'] = 1
                },
                ['_type'] = 'Item'
            }
        },
        ['shotgun_ammo'] = {
            ['item'] = {
                ['_id'] = 'shotgun_ammo',
                ['_parent'] = '',
                ['_name'] = 'shotgun_ammo',
                ['_data'] = {
                    ['ExamineTime'] = 1.0,
                    ['Rarity'] = 'common',
                    ['Label'] = 'Shotgun ammo',
                    ['MaxStack'] = 100,
                    ['Itemimage'] = 'icons/shotgun_ammo.png',
                    ['Type'] = 'Item',
                    ['Backgroundcolor'] = 'black',
                    ['Description'] = 'Ammo for Shotguns',
                    ['Name'] = 'shotgun_ammo',
                    ['Weight'] = 0.5,
                    ['ItemSound'] = 'gear_generic',
                    ['Height'] = 1,
                    ['Width'] = 1
                },
                ['_type'] = 'Item'
            }
        },
        ['compactrifle_extendedclip'] = {
            ['item'] = {
                ['_id'] = 'compactrifle_extendedclip',
                ['_parent'] = '',
                ['_name'] = 'compactrifle_extendedclip',
                ['_data'] = {
                    ['ExamineTime'] = 1.0,
                    ['Rarity'] = 'common',
                    ['Label'] = 'Rifle EXT Clip',
                    ['MaxStack'] = 100,
                    ['Itemimage'] = 'icons/pistol_extendedclip.png',
                    ['Type'] = 'Item',
                    ['Backgroundcolor'] = 'black',
                    ['Description'] = 'Compact Rifle Extended Clip',
                    ['Name'] = 'compactrifle_extendedclip',
                    ['Weight'] = 1.0,
                    ['ItemSound'] = 'gear_generic',
                    ['Height'] = 1,
                    ['Width'] = 1
                },
                ['_type'] = 'Item'
            }
        },
        ['tunerlaptop'] = {
            ['item'] = {
                ['_id'] = 'tunerlaptop',
                ['_parent'] = '',
                ['_name'] = 'tunerlaptop',
                ['_data'] = {
                    ['ExamineTime'] = 1.0,
                    ['Rarity'] = 'common',
                    ['Label'] = 'Tunerchip',
                    ['MaxStack'] = 1,
                    ['Itemimage'] = 'icons/tunerchip.png',
                    ['Type'] = 'Item',
                    ['Backgroundcolor'] = 'black',
                    ['Description'] = 'With this tunerchip you can get your car on steroids... If you know what you\'re doing',
                    ['Name'] = 'tunerlaptop',
                    ['Weight'] = 2.0,
                    ['ItemSound'] = 'gear_generic',
                    ['Height'] = 1,
                    ['Width'] = 1
                },
                ['_type'] = 'Item'
            }
        },
        ['weapon_snspistol_mk2'] = {
            ['item'] = {
                ['_id'] = 'weapon_snspistol_mk2',
                ['_parent'] = '',
                ['_name'] = 'weapon_snspistol_mk2',
                ['_data'] = {
                    ['ExamineTime'] = 1.0,
                    ['Rarity'] = 'rare',
                    ['Label'] = 'SNS Pistol Mk II',
                    ['MaxStack'] = 1,
                    ['Itemimage'] = 'icons/weapon_snspistol_mk2.png',
                    ['Type'] = 'Weapon',
                    ['Backgroundcolor'] = 'blue',
                    ['Description'] = 'SNS Pistol MK2',
                    ['Name'] = 'weapon_snspistol_mk2',
                    ['Weight'] = 1.0,
                    ['ItemSound'] = 'gear_generic',
                    ['Height'] = 2,
                    ['Width'] = 2
                },
                ['_type'] = 'Item'
            }
        },
        ['weapon_bread'] = {
            ['item'] = {
                ['_id'] = 'weapon_bread',
                ['_parent'] = '',
                ['_name'] = 'weapon_bread',
                ['_data'] = {
                    ['ExamineTime'] = 1.0,
                    ['Rarity'] = 'rare',
                    ['Label'] = 'Baquette',
                    ['MaxStack'] = 1,
                    ['Itemimage'] = 'icons/baquette.png',
                    ['Type'] = 'Weapon',
                    ['Backgroundcolor'] = 'blue',
                    ['Description'] = 'Bread...?',
                    ['Name'] = 'weapon_bread',
                    ['Weight'] = 1.0,
                    ['ItemSound'] = 'gear_generic',
                    ['Height'] = 2,
                    ['Width'] = 2
                },
                ['_type'] = 'Item'
            }
        },
        ['beer'] = {
            ['item'] = {
                ['_id'] = 'beer',
                ['_parent'] = '',
                ['_name'] = 'beer',
                ['_data'] = {
                    ['ExamineTime'] = 1.0,
                    ['Rarity'] = 'common',
                    ['Label'] = 'Beer',
                    ['MaxStack'] = 100,
                    ['Itemimage'] = 'icons/beer.png',
                    ['Type'] = 'Item',
                    ['Backgroundcolor'] = 'black',
                    ['Description'] = 'Nothing like a good cold beer!',
                    ['Name'] = 'beer',
                    ['Weight'] = 0.5,
                    ['ItemSound'] = 'gear_generic',
                    ['Height'] = 1,
                    ['Width'] = 1
                },
                ['_type'] = 'Item'
            }
        },
        ['weapon_machinepistol'] = {
            ['item'] = {
                ['_id'] = 'weapon_machinepistol',
                ['_parent'] = '',
                ['_name'] = 'weapon_machinepistol',
                ['_data'] = {
                    ['ExamineTime'] = 1.0,
                    ['Rarity'] = 'rare',
                    ['Label'] = 'Tec-9',
                    ['MaxStack'] = 1,
                    ['Itemimage'] = 'icons/weapon_machinepistol.png',
                    ['Type'] = 'Weapon',
                    ['Backgroundcolor'] = 'blue',
                    ['Description'] = 'A self-loading pistol capable of burst or fully automatic fire',
                    ['Name'] = 'weapon_machinepistol',
                    ['Weight'] = 1.0,
                    ['ItemSound'] = 'gear_generic',
                    ['Height'] = 2,
                    ['Width'] = 2
                },
                ['_type'] = 'Item'
            }
        },
        ['weapon_nightstick'] = {
            ['item'] = {
                ['_id'] = 'weapon_nightstick',
                ['_parent'] = '',
                ['_name'] = 'weapon_nightstick',
                ['_data'] = {
                    ['ExamineTime'] = 1.0,
                    ['Rarity'] = 'rare',
                    ['Label'] = 'Nightstick',
                    ['MaxStack'] = 1,
                    ['Itemimage'] = 'icons/weapon_nightstick.png',
                    ['Type'] = 'Weapon',
                    ['Backgroundcolor'] = 'blue',
                    ['Description'] = 'A police officer\'s club or billy',
                    ['Name'] = 'weapon_nightstick',
                    ['Weight'] = 1.0,
                    ['ItemSound'] = 'gear_generic',
                    ['Height'] = 2,
                    ['Width'] = 2
                },
                ['_type'] = 'Item'
            }
        },
        ['driver_license'] = {
            ['item'] = {
                ['_id'] = 'driver_license',
                ['_parent'] = '',
                ['_name'] = 'driver_license',
                ['_data'] = {
                    ['ExamineTime'] = 1.0,
                    ['Rarity'] = 'common',
                    ['Label'] = 'Drivers License',
                    ['MaxStack'] = 1,
                    ['Itemimage'] = 'icons/driver_license.png',
                    ['Type'] = 'Item',
                    ['Backgroundcolor'] = 'black',
                    ['Description'] = 'Permit to show you can drive a vehicle',
                    ['Name'] = 'driver_license',
                    ['Weight'] = 0.0,
                    ['ItemSound'] = 'gear_generic',
                    ['Height'] = 1,
                    ['Width'] = 1
                },
                ['_type'] = 'Item'
            }
        },
        ['specialcarbine_defaultclip'] = {
            ['item'] = {
                ['_id'] = 'specialcarbine_defaultclip',
                ['_parent'] = '',
                ['_name'] = 'specialcarbine_defaultclip',
                ['_data'] = {
                    ['ExamineTime'] = 1.0,
                    ['Rarity'] = 'common',
                    ['Label'] = 'Rifle Clip',
                    ['MaxStack'] = 100,
                    ['Itemimage'] = 'icons/pistol_extendedclip.png',
                    ['Type'] = 'Item',
                    ['Backgroundcolor'] = 'black',
                    ['Description'] = 'Special Carbine Default Clip',
                    ['Name'] = 'specialcarbine_defaultclip',
                    ['Weight'] = 1.0,
                    ['ItemSound'] = 'gear_generic',
                    ['Height'] = 1,
                    ['Width'] = 1
                },
                ['_type'] = 'Item'
            }
        },
        ['pistol_defaultclip'] = {
            ['item'] = {
                ['_id'] = 'pistol_defaultclip',
                ['_parent'] = '',
                ['_name'] = 'pistol_defaultclip',
                ['_data'] = {
                    ['ExamineTime'] = 1.0,
                    ['Rarity'] = 'common',
                    ['Label'] = 'Pistol Clip',
                    ['MaxStack'] = 100,
                    ['Itemimage'] = 'icons/pistol_extendedclip.png',
                    ['Type'] = 'Item',
                    ['Backgroundcolor'] = 'black',
                    ['Description'] = 'Pistol Default Clip',
                    ['Name'] = 'pistol_defaultclip',
                    ['Weight'] = 1.0,
                    ['ItemSound'] = 'gear_generic',
                    ['Height'] = 1,
                    ['Width'] = 1
                },
                ['_type'] = 'Item'
            }
        },
        ['carbinerifle_luxuryfinish'] = {
            ['item'] = {
                ['_id'] = 'carbinerifle_luxuryfinish',
                ['_parent'] = '',
                ['_name'] = 'carbinerifle_luxuryfinish',
                ['_data'] = {
                    ['ExamineTime'] = 1.0,
                    ['Rarity'] = 'common',
                    ['Label'] = 'Rifle Finish',
                    ['MaxStack'] = 100,
                    ['Itemimage'] = 'icons/pistol_suppressor.png',
                    ['Type'] = 'Item',
                    ['Backgroundcolor'] = 'black',
                    ['Description'] = 'Carbine Rifle Luxury Finish',
                    ['Name'] = 'carbinerifle_luxuryfinish',
                    ['Weight'] = 1.0,
                    ['ItemSound'] = 'gear_generic',
                    ['Height'] = 1,
                    ['Width'] = 1
                },
                ['_type'] = 'Item'
            }
        },
        ['machinepistol_extendedclip'] = {
            ['item'] = {
                ['_id'] = 'machinepistol_extendedclip',
                ['_parent'] = '',
                ['_name'] = 'machinepistol_extendedclip',
                ['_data'] = {
                    ['ExamineTime'] = 1.0,
                    ['Rarity'] = 'common',
                    ['Label'] = 'SMG EXT Clip',
                    ['MaxStack'] = 100,
                    ['Itemimage'] = 'icons/pistol_extendedclip.png',
                    ['Type'] = 'Item',
                    ['Backgroundcolor'] = 'black',
                    ['Description'] = 'Machine Pistol Extended Clip',
                    ['Name'] = 'machinepistol_extendedclip',
                    ['Weight'] = 1.0,
                    ['ItemSound'] = 'gear_generic',
                    ['Height'] = 1,
                    ['Width'] = 1
                },
                ['_type'] = 'Item'
            }
        },
        ['plastic'] = {
            ['item'] = {
                ['_id'] = 'plastic',
                ['_parent'] = '',
                ['_name'] = 'plastic',
                ['_data'] = {
                    ['ExamineTime'] = 1.0,
                    ['Rarity'] = 'common',
                    ['Label'] = 'Plastic',
                    ['MaxStack'] = 100,
                    ['Itemimage'] = 'icons/plastic.png',
                    ['Type'] = 'Item',
                    ['Backgroundcolor'] = 'black',
                    ['Description'] = 'RECYCLE! - Greta Thunberg 2019',
                    ['Name'] = 'plastic',
                    ['Weight'] = 0.1,
                    ['ItemSound'] = 'gear_generic',
                    ['Height'] = 1,
                    ['Width'] = 1
                },
                ['_type'] = 'Item'
            }
        },
        ['weapon_combatmg_mk2'] = {
            ['item'] = {
                ['_id'] = 'weapon_combatmg_mk2',
                ['_parent'] = '',
                ['_name'] = 'weapon_combatmg_mk2',
                ['_data'] = {
                    ['ExamineTime'] = 1.0,
                    ['Rarity'] = 'rare',
                    ['Label'] = 'Combat MG Mk II',
                    ['MaxStack'] = 1,
                    ['Itemimage'] = 'icons/weapon_combatmg_mk2.png',
                    ['Type'] = 'Weapon',
                    ['Backgroundcolor'] = 'blue',
                    ['Description'] = 'Weapon Combatmg MK2',
                    ['Name'] = 'weapon_combatmg_mk2',
                    ['Weight'] = 1.0,
                    ['ItemSound'] = 'gear_generic',
                    ['Height'] = 2,
                    ['Width'] = 2
                },
                ['_type'] = 'Item'
            }
        },
        ['sawnoffshotgun_luxuryfinish'] = {
            ['item'] = {
                ['_id'] = 'sawnoffshotgun_luxuryfinish',
                ['_parent'] = '',
                ['_name'] = 'sawnoffshotgun_luxuryfinish',
                ['_data'] = {
                    ['ExamineTime'] = 1.0,
                    ['Rarity'] = 'common',
                    ['Label'] = 'Shotgun Finish',
                    ['MaxStack'] = 100,
                    ['Itemimage'] = 'icons/pistol_suppressor.png',
                    ['Type'] = 'Item',
                    ['Backgroundcolor'] = 'black',
                    ['Description'] = 'Sawn Off Shotgun Luxury Finish',
                    ['Name'] = 'sawnoffshotgun_luxuryfinish',
                    ['Weight'] = 1.0,
                    ['ItemSound'] = 'gear_generic',
                    ['Height'] = 1,
                    ['Width'] = 1
                },
                ['_type'] = 'Item'
            }
        },
        ['steel'] = {
            ['item'] = {
                ['_id'] = 'steel',
                ['_parent'] = '',
                ['_name'] = 'steel',
                ['_data'] = {
                    ['ExamineTime'] = 1.0,
                    ['Rarity'] = 'common',
                    ['Label'] = 'Steel',
                    ['MaxStack'] = 100,
                    ['Itemimage'] = 'icons/steel.png',
                    ['Type'] = 'Item',
                    ['Backgroundcolor'] = 'black',
                    ['Description'] = 'Nice piece of metal that you can probably use for something',
                    ['Name'] = 'steel',
                    ['Weight'] = 0.1,
                    ['ItemSound'] = 'gear_generic',
                    ['Height'] = 1,
                    ['Width'] = 1
                },
                ['_type'] = 'Item'
            }
        },
        ['weapon_handcuffs'] = {
            ['item'] = {
                ['_id'] = 'weapon_handcuffs',
                ['_parent'] = '',
                ['_name'] = 'weapon_handcuffs',
                ['_data'] = {
                    ['ExamineTime'] = 1.0,
                    ['Rarity'] = 'rare',
                    ['Label'] = 'Handcuffs',
                    ['MaxStack'] = 1,
                    ['Itemimage'] = 'icons/weapon_handcuffs.png',
                    ['Type'] = 'Weapon',
                    ['Backgroundcolor'] = 'blue',
                    ['Description'] = 'A pair of lockable linked metal rings for securing a prisoner\'s wrists',
                    ['Name'] = 'weapon_handcuffs',
                    ['Weight'] = 1.0,
                    ['ItemSound'] = 'gear_generic',
                    ['Height'] = 2,
                    ['Width'] = 2
                },
                ['_type'] = 'Item'
            }
        },
        ['moneybag'] = {
            ['item'] = {
                ['_id'] = 'moneybag',
                ['_parent'] = '',
                ['_name'] = 'moneybag',
                ['_data'] = {
                    ['ExamineTime'] = 1.0,
                    ['Rarity'] = 'common',
                    ['Label'] = 'Money Bag',
                    ['MaxStack'] = 1,
                    ['Itemimage'] = 'icons/moneybag.png',
                    ['Type'] = 'Item',
                    ['Backgroundcolor'] = 'black',
                    ['Description'] = 'A bag with cash',
                    ['Name'] = 'moneybag',
                    ['Weight'] = 0.0,
                    ['ItemSound'] = 'gear_generic',
                    ['Height'] = 1,
                    ['Width'] = 1
                },
                ['_type'] = 'Item'
            }
        },
        ['weapon_briefcase_02'] = {
            ['item'] = {
                ['_id'] = 'weapon_briefcase_02',
                ['_parent'] = '',
                ['_name'] = 'weapon_briefcase_02',
                ['_data'] = {
                    ['ExamineTime'] = 1.0,
                    ['Rarity'] = 'rare',
                    ['Label'] = 'Suitcase',
                    ['MaxStack'] = 1,
                    ['Itemimage'] = 'icons/weapon_briefcase2.png',
                    ['Type'] = 'Weapon',
                    ['Backgroundcolor'] = 'blue',
                    ['Description'] = 'Wonderfull for nice vacation to Liberty City',
                    ['Name'] = 'weapon_briefcase_02',
                    ['Weight'] = 1.0,
                    ['ItemSound'] = 'gear_generic',
                    ['Height'] = 2,
                    ['Width'] = 2
                },
                ['_type'] = 'Item'
            }
        },
        ['armor'] = {
            ['item'] = {
                ['_id'] = 'armor',
                ['_parent'] = '',
                ['_name'] = 'armor',
                ['_data'] = {
                    ['ExamineTime'] = 1.0,
                    ['Rarity'] = 'common',
                    ['Label'] = 'Armor',
                    ['MaxStack'] = 100,
                    ['Itemimage'] = 'icons/armor.png',
                    ['Type'] = 'Item',
                    ['Backgroundcolor'] = 'black',
                    ['Description'] = 'Some protection won\'t hurt... right?',
                    ['Name'] = 'armor',
                    ['Weight'] = 5.0,
                    ['ItemSound'] = 'gear_generic',
                    ['Height'] = 1,
                    ['Width'] = 1
                },
                ['_type'] = 'Item'
            }
        },
        ['weapon_combatshotgun'] = {
            ['item'] = {
                ['_id'] = 'weapon_combatshotgun',
                ['_parent'] = '',
                ['_name'] = 'weapon_combatshotgun',
                ['_data'] = {
                    ['ExamineTime'] = 1.0,
                    ['Rarity'] = 'rare',
                    ['Label'] = 'Combat Shotgun',
                    ['MaxStack'] = 1,
                    ['Itemimage'] = 'icons/weapon_combatshotgun.png',
                    ['Type'] = 'Weapon',
                    ['Backgroundcolor'] = 'blue',
                    ['Description'] = 'Weapon Combatshotgun',
                    ['Name'] = 'weapon_combatshotgun',
                    ['Weight'] = 1.0,
                    ['ItemSound'] = 'gear_generic',
                    ['Height'] = 2,
                    ['Width'] = 2
                },
                ['_type'] = 'Item'
            }
        },
        ['heavyshotgun_defaultclip'] = {
            ['item'] = {
                ['_id'] = 'heavyshotgun_defaultclip',
                ['_parent'] = '',
                ['_name'] = 'heavyshotgun_defaultclip',
                ['_data'] = {
                    ['ExamineTime'] = 1.0,
                    ['Rarity'] = 'common',
                    ['Label'] = 'Shotgun Clip',
                    ['MaxStack'] = 100,
                    ['Itemimage'] = 'icons/pistol_extendedclip.png',
                    ['Type'] = 'Item',
                    ['Backgroundcolor'] = 'black',
                    ['Description'] = 'Heavy Shotgun Default Clip',
                    ['Name'] = 'heavyshotgun_defaultclip',
                    ['Weight'] = 1.0,
                    ['ItemSound'] = 'gear_generic',
                    ['Height'] = 1,
                    ['Width'] = 1
                },
                ['_type'] = 'Item'
            }
        },
        ['revolver_vipvariant'] = {
            ['item'] = {
                ['_id'] = 'revolver_vipvariant',
                ['_parent'] = '',
                ['_name'] = 'revolver_vipvariant',
                ['_data'] = {
                    ['ExamineTime'] = 1.0,
                    ['Rarity'] = 'common',
                    ['Label'] = 'Pistol Variant',
                    ['MaxStack'] = 100,
                    ['Itemimage'] = 'icons/pistol_extendedclip.png',
                    ['Type'] = 'Item',
                    ['Backgroundcolor'] = 'black',
                    ['Description'] = 'Revovler Variant',
                    ['Name'] = 'revolver_vipvariant',
                    ['Weight'] = 1.0,
                    ['ItemSound'] = 'gear_generic',
                    ['Height'] = 1,
                    ['Width'] = 1
                },
                ['_type'] = 'Item'
            }
        },
        ['weapon_specialcarbine'] = {
            ['item'] = {
                ['_id'] = 'weapon_specialcarbine',
                ['_parent'] = '',
                ['_name'] = 'weapon_specialcarbine',
                ['_data'] = {
                    ['ExamineTime'] = 1.0,
                    ['Rarity'] = 'rare',
                    ['Label'] = 'Special Carbine',
                    ['MaxStack'] = 1,
                    ['Itemimage'] = 'icons/weapon_specialcarbine.png',
                    ['Type'] = 'Weapon',
                    ['Backgroundcolor'] = 'blue',
                    ['Description'] = 'An extremely versatile assault rifle for any combat situation',
                    ['Name'] = 'weapon_specialcarbine',
                    ['Weight'] = 1.0,
                    ['ItemSound'] = 'gear_generic',
                    ['Height'] = 2,
                    ['Width'] = 2
                },
                ['_type'] = 'Item'
            }
        },
        ['binoculars'] = {
            ['item'] = {
                ['_id'] = 'binoculars',
                ['_parent'] = '',
                ['_name'] = 'binoculars',
                ['_data'] = {
                    ['ExamineTime'] = 1.0,
                    ['Rarity'] = 'common',
                    ['Label'] = 'Binoculars',
                    ['MaxStack'] = 100,
                    ['Itemimage'] = 'icons/binoculars.png',
                    ['Type'] = 'Item',
                    ['Backgroundcolor'] = 'black',
                    ['Description'] = 'Sneaky Breaky...',
                    ['Name'] = 'binoculars',
                    ['Weight'] = 0.6,
                    ['ItemSound'] = 'gear_generic',
                    ['Height'] = 1,
                    ['Width'] = 1
                },
                ['_type'] = 'Item'
            }
        },
        ['weapontint_pink'] = {
            ['item'] = {
                ['_id'] = 'weapontint_pink',
                ['_parent'] = '',
                ['_name'] = 'weapontint_pink',
                ['_data'] = {
                    ['ExamineTime'] = 1.0,
                    ['Rarity'] = 'common',
                    ['Label'] = 'Pink Tint',
                    ['MaxStack'] = 100,
                    ['Itemimage'] = 'icons/weapontint_pink.png',
                    ['Type'] = 'Item',
                    ['Backgroundcolor'] = 'black',
                    ['Description'] = 'Pink Weapon Tint',
                    ['Name'] = 'weapontint_pink',
                    ['Weight'] = 1.0,
                    ['ItemSound'] = 'gear_generic',
                    ['Height'] = 1,
                    ['Width'] = 1
                },
                ['_type'] = 'Item'
            }
        },
        ['bandage'] = {
            ['item'] = {
                ['_id'] = 'bandage',
                ['_parent'] = '',
                ['_name'] = 'bandage',
                ['_data'] = {
                    ['ExamineTime'] = 1.0,
                    ['Rarity'] = 'common',
                    ['Label'] = 'Bandage',
                    ['MaxStack'] = 100,
                    ['Itemimage'] = 'icons/bandage.png',
                    ['Type'] = 'Item',
                    ['Backgroundcolor'] = 'black',
                    ['Description'] = 'A bandage works every time',
                    ['Name'] = 'bandage',
                    ['Weight'] = 0.0,
                    ['ItemSound'] = 'gear_generic',
                    ['Height'] = 1,
                    ['Width'] = 1
                },
                ['_type'] = 'Item'
            }
        },
        ['heavypistol_extendedclip'] = {
            ['item'] = {
                ['_id'] = 'heavypistol_extendedclip',
                ['_parent'] = '',
                ['_name'] = 'heavypistol_extendedclip',
                ['_data'] = {
                    ['ExamineTime'] = 1.0,
                    ['Rarity'] = 'common',
                    ['Label'] = 'Pistol EXT Clip',
                    ['MaxStack'] = 100,
                    ['Itemimage'] = 'icons/pistol_extendedclip.png',
                    ['Type'] = 'Item',
                    ['Backgroundcolor'] = 'black',
                    ['Description'] = 'Heavy Pistol Extended Clip',
                    ['Name'] = 'heavypistol_extendedclip',
                    ['Weight'] = 1.0,
                    ['ItemSound'] = 'gear_generic',
                    ['Height'] = 1,
                    ['Width'] = 1
                },
                ['_type'] = 'Item'
            }
        },
        ['pistol_ammo'] = {
            ['item'] = {
                ['_id'] = 'pistol_ammo',
                ['_parent'] = '',
                ['_name'] = 'pistol_ammo',
                ['_data'] = {
                    ['ExamineTime'] = 1.0,
                    ['Rarity'] = 'common',
                    ['Label'] = 'Pistol ammo',
                    ['MaxStack'] = 100,
                    ['Itemimage'] = 'icons/pistol_ammo.png',
                    ['Type'] = 'Item',
                    ['Backgroundcolor'] = 'black',
                    ['Description'] = 'Ammo for Pistols',
                    ['Name'] = 'pistol_ammo',
                    ['Weight'] = 0.2,
                    ['ItemSound'] = 'gear_generic',
                    ['Height'] = 1,
                    ['Width'] = 1
                },
                ['_type'] = 'Item'
            }
        },
        ['marksmanrifle_scope'] = {
            ['item'] = {
                ['_id'] = 'marksmanrifle_scope',
                ['_parent'] = '',
                ['_name'] = 'marksmanrifle_scope',
                ['_data'] = {
                    ['ExamineTime'] = 1.0,
                    ['Rarity'] = 'common',
                    ['Label'] = 'Sniper Scope',
                    ['MaxStack'] = 100,
                    ['Itemimage'] = 'icons/smg_scope.png',
                    ['Type'] = 'Item',
                    ['Backgroundcolor'] = 'black',
                    ['Description'] = 'Marksman Rifle Scope Attachment',
                    ['Name'] = 'marksmanrifle_scope',
                    ['Weight'] = 1.0,
                    ['ItemSound'] = 'gear_generic',
                    ['Height'] = 1,
                    ['Width'] = 1
                },
                ['_type'] = 'Item'
            }
        },
        ['weapon_raypistol'] = {
            ['item'] = {
                ['_id'] = 'weapon_raypistol',
                ['_parent'] = '',
                ['_name'] = 'weapon_raypistol',
                ['_data'] = {
                    ['ExamineTime'] = 1.0,
                    ['Rarity'] = 'rare',
                    ['Label'] = 'Up-n-Atomizer',
                    ['MaxStack'] = 1,
                    ['Itemimage'] = 'icons/weapon_raypistol.png',
                    ['Type'] = 'Weapon',
                    ['Backgroundcolor'] = 'blue',
                    ['Description'] = 'Weapon Raypistol',
                    ['Name'] = 'weapon_raypistol',
                    ['Weight'] = 1.0,
                    ['ItemSound'] = 'gear_generic',
                    ['Height'] = 2,
                    ['Width'] = 2
                },
                ['_type'] = 'Item'
            }
        },
        ['weed_white-widow'] = {
            ['item'] = {
                ['_id'] = 'weed_white-widow',
                ['_parent'] = '',
                ['_name'] = 'weed_white-widow',
                ['_data'] = {
                    ['ExamineTime'] = 1.0,
                    ['Rarity'] = 'common',
                    ['Label'] = 'White Widow 2g',
                    ['MaxStack'] = 100,
                    ['Itemimage'] = 'icons/weed_baggy.png',
                    ['Type'] = 'Item',
                    ['Backgroundcolor'] = 'black',
                    ['Description'] = 'A weed bag with 2g White Widow',
                    ['Name'] = 'weed_white-widow',
                    ['Weight'] = 0.2,
                    ['ItemSound'] = 'gear_generic',
                    ['Height'] = 1,
                    ['Width'] = 1
                },
                ['_type'] = 'Item'
            }
        },
        ['assaultsmg_luxuryfinish'] = {
            ['item'] = {
                ['_id'] = 'assaultsmg_luxuryfinish',
                ['_parent'] = '',
                ['_name'] = 'assaultsmg_luxuryfinish',
                ['_data'] = {
                    ['ExamineTime'] = 1.0,
                    ['Rarity'] = 'common',
                    ['Label'] = 'SMG Finish',
                    ['MaxStack'] = 100,
                    ['Itemimage'] = 'icons/pistol_suppressor.png',
                    ['Type'] = 'Item',
                    ['Backgroundcolor'] = 'black',
                    ['Description'] = 'Assault SMG Luxury Finish',
                    ['Name'] = 'assaultsmg_luxuryfinish',
                    ['Weight'] = 1.0,
                    ['ItemSound'] = 'gear_generic',
                    ['Height'] = 1,
                    ['Width'] = 1
                },
                ['_type'] = 'Item'
            }
        },
        ['combatpistol_luxuryfinish'] = {
            ['item'] = {
                ['_id'] = 'combatpistol_luxuryfinish',
                ['_parent'] = '',
                ['_name'] = 'combatpistol_luxuryfinish',
                ['_data'] = {
                    ['ExamineTime'] = 1.0,
                    ['Rarity'] = 'common',
                    ['Label'] = 'Pistol Finish',
                    ['MaxStack'] = 100,
                    ['Itemimage'] = 'icons/pistol_suppressor.png',
                    ['Type'] = 'Item',
                    ['Backgroundcolor'] = 'black',
                    ['Description'] = 'Combat Pistol Luxury Finish',
                    ['Name'] = 'combatpistol_luxuryfinish',
                    ['Weight'] = 1.0,
                    ['ItemSound'] = 'gear_generic',
                    ['Height'] = 1,
                    ['Width'] = 1
                },
                ['_type'] = 'Item'
            }
        },
        ['weed_brick'] = {
            ['item'] = {
                ['_id'] = 'weed_brick',
                ['_parent'] = '',
                ['_name'] = 'weed_brick',
                ['_data'] = {
                    ['ExamineTime'] = 1.0,
                    ['Rarity'] = 'common',
                    ['Label'] = 'Weed Brick',
                    ['MaxStack'] = 100,
                    ['Itemimage'] = 'icons/weed_brick.png',
                    ['Type'] = 'Item',
                    ['Backgroundcolor'] = 'black',
                    ['Description'] = '1KG Weed Brick to sell to large customers.',
                    ['Name'] = 'weed_brick',
                    ['Weight'] = 1.0,
                    ['ItemSound'] = 'gear_generic',
                    ['Height'] = 1,
                    ['Width'] = 1
                },
                ['_type'] = 'Item'
            }
        },
        ['microsmg_scope'] = {
            ['item'] = {
                ['_id'] = 'microsmg_scope',
                ['_parent'] = '',
                ['_name'] = 'microsmg_scope',
                ['_data'] = {
                    ['ExamineTime'] = 1.0,
                    ['Rarity'] = 'common',
                    ['Label'] = 'SMG Scope',
                    ['MaxStack'] = 100,
                    ['Itemimage'] = 'icons/smg_scope.png',
                    ['Type'] = 'Item',
                    ['Backgroundcolor'] = 'black',
                    ['Description'] = 'Micro SMG Scope Attachment',
                    ['Name'] = 'microsmg_scope',
                    ['Weight'] = 1.0,
                    ['ItemSound'] = 'gear_generic',
                    ['Height'] = 1,
                    ['Width'] = 1
                },
                ['_type'] = 'Item'
            }
        },
        ['pouch'] = {
            ['item'] = {
                ['_id'] = 'pouch',
                ['_parent'] = '',
                ['_name'] = 'pouch',
                ['_data'] = {
                    ['ExamineTime'] = 1.0,
                    ['Rarity'] = 'common',
                    ['Label'] = 'Pouch',
                    ['MaxStack'] = 100,
                    ['Itemimage'] = 'icons/ifaks.png',
                    ['Type'] = 'Item',
                    ['Backgroundcolor'] = 'black',
                    ['Description'] = 'ifaks for healing and a complete stress remover.',
                    ['Name'] = 'pouch',
                    ['Weight'] = 0.2,
                    ['ItemSound'] = 'gear_generic',
                    ['Height'] = 1,
                    ['Width'] = 1
                },
                ['_type'] = 'Item'
            }
        },
        ['mastercard'] = {
            ['item'] = {
                ['_id'] = 'mastercard',
                ['_parent'] = '',
                ['_name'] = 'mastercard',
                ['_data'] = {
                    ['ExamineTime'] = 1.0,
                    ['Rarity'] = 'common',
                    ['Label'] = 'Master Card',
                    ['MaxStack'] = 1,
                    ['Itemimage'] = 'icons/mastercard.png',
                    ['Type'] = 'Item',
                    ['Backgroundcolor'] = 'black',
                    ['Description'] = 'MasterCard can be used via ATM',
                    ['Name'] = 'mastercard',
                    ['Weight'] = 0.0,
                    ['ItemSound'] = 'gear_generic',
                    ['Height'] = 1,
                    ['Width'] = 1
                },
                ['_type'] = 'Item'
            }
        },
        ['microsmg_defaultclip'] = {
            ['item'] = {
                ['_id'] = 'microsmg_defaultclip',
                ['_parent'] = '',
                ['_name'] = 'microsmg_defaultclip',
                ['_data'] = {
                    ['ExamineTime'] = 1.0,
                    ['Rarity'] = 'common',
                    ['Label'] = 'SMG Clip',
                    ['MaxStack'] = 100,
                    ['Itemimage'] = 'icons/pistol_extendedclip.png',
                    ['Type'] = 'Item',
                    ['Backgroundcolor'] = 'black',
                    ['Description'] = 'Micro SMG Default Clip',
                    ['Name'] = 'microsmg_defaultclip',
                    ['Weight'] = 1.0,
                    ['ItemSound'] = 'gear_generic',
                    ['Height'] = 1,
                    ['Width'] = 1
                },
                ['_type'] = 'Item'
            }
        },
        ['gusenberg_defaultclip'] = {
            ['item'] = {
                ['_id'] = 'gusenberg_defaultclip',
                ['_parent'] = '',
                ['_name'] = 'gusenberg_defaultclip',
                ['_data'] = {
                    ['ExamineTime'] = 1.0,
                    ['Rarity'] = 'common',
                    ['Label'] = 'Rifle Clip',
                    ['MaxStack'] = 100,
                    ['Itemimage'] = 'icons/pistol_extendedclip.png',
                    ['Type'] = 'Item',
                    ['Backgroundcolor'] = 'black',
                    ['Description'] = 'Gusenberg Default Clip',
                    ['Name'] = 'gusenberg_defaultclip',
                    ['Weight'] = 1.0,
                    ['ItemSound'] = 'gear_generic',
                    ['Height'] = 1,
                    ['Width'] = 1
                },
                ['_type'] = 'Item'
            }
        },
        ['snikkel_candy'] = {
            ['item'] = {
                ['_id'] = 'snikkel_candy',
                ['_parent'] = '',
                ['_name'] = 'snikkel_candy',
                ['_data'] = {
                    ['ExamineTime'] = 1.0,
                    ['Rarity'] = 'common',
                    ['Label'] = 'Snikkel',
                    ['MaxStack'] = 100,
                    ['Itemimage'] = 'icons/snikkel_candy.png',
                    ['Type'] = 'Item',
                    ['Backgroundcolor'] = 'black',
                    ['Description'] = 'Some delicious candy :O',
                    ['Name'] = 'snikkel_candy',
                    ['Weight'] = 0.1,
                    ['ItemSound'] = 'gear_generic',
                    ['Height'] = 1,
                    ['Width'] = 1
                },
                ['_type'] = 'Item'
            }
        },
        ['weapon_pipebomb'] = {
            ['item'] = {
                ['_id'] = 'weapon_pipebomb',
                ['_parent'] = '',
                ['_name'] = 'weapon_pipebomb',
                ['_data'] = {
                    ['ExamineTime'] = 1.0,
                    ['Rarity'] = 'rare',
                    ['Label'] = 'Pipe Bomb',
                    ['MaxStack'] = 1,
                    ['Itemimage'] = 'icons/weapon_pipebomb.png',
                    ['Type'] = 'Weapon',
                    ['Backgroundcolor'] = 'blue',
                    ['Description'] = 'A homemade bomb, the components of which are contained in a pipe',
                    ['Name'] = 'weapon_pipebomb',
                    ['Weight'] = 1.0,
                    ['ItemSound'] = 'gear_generic',
                    ['Height'] = 2,
                    ['Width'] = 2
                },
                ['_type'] = 'Item'
            }
        },
        ['police_stormram'] = {
            ['item'] = {
                ['_id'] = 'police_stormram',
                ['_parent'] = '',
                ['_name'] = 'police_stormram',
                ['_data'] = {
                    ['ExamineTime'] = 1.0,
                    ['Rarity'] = 'common',
                    ['Label'] = 'Stormram',
                    ['MaxStack'] = 100,
                    ['Itemimage'] = 'icons/police_stormram.png',
                    ['Type'] = 'Item',
                    ['Backgroundcolor'] = 'black',
                    ['Description'] = 'A nice tool to break into doors',
                    ['Name'] = 'police_stormram',
                    ['Weight'] = 18.0,
                    ['ItemSound'] = 'gear_generic',
                    ['Height'] = 1,
                    ['Width'] = 1
                },
                ['_type'] = 'Item'
            }
        },
        ['iron'] = {
            ['item'] = {
                ['_id'] = 'iron',
                ['_parent'] = '',
                ['_name'] = 'iron',
                ['_data'] = {
                    ['ExamineTime'] = 1.0,
                    ['Rarity'] = 'common',
                    ['Label'] = 'Iron',
                    ['MaxStack'] = 100,
                    ['Itemimage'] = 'icons/iron.png',
                    ['Type'] = 'Item',
                    ['Backgroundcolor'] = 'black',
                    ['Description'] = 'Handy piece of metal that you can probably use for something',
                    ['Name'] = 'iron',
                    ['Weight'] = 0.1,
                    ['ItemSound'] = 'gear_generic',
                    ['Height'] = 1,
                    ['Width'] = 1
                },
                ['_type'] = 'Item'
            }
        },
        ['weapon_pumpshotgun_mk2'] = {
            ['item'] = {
                ['_id'] = 'weapon_pumpshotgun_mk2',
                ['_parent'] = '',
                ['_name'] = 'weapon_pumpshotgun_mk2',
                ['_data'] = {
                    ['ExamineTime'] = 1.0,
                    ['Rarity'] = 'rare',
                    ['Label'] = 'Pumpshotgun Mk II',
                    ['MaxStack'] = 1,
                    ['Itemimage'] = 'icons/weapon_pumpshotgun_mk2.png',
                    ['Type'] = 'Weapon',
                    ['Backgroundcolor'] = 'blue',
                    ['Description'] = 'Pumpshotgun MK2',
                    ['Name'] = 'weapon_pumpshotgun_mk2',
                    ['Weight'] = 1.0,
                    ['ItemSound'] = 'gear_generic',
                    ['Height'] = 2,
                    ['Width'] = 2
                },
                ['_type'] = 'Item'
            }
        },
        ['kurkakola'] = {
            ['item'] = {
                ['_id'] = 'kurkakola',
                ['_parent'] = '',
                ['_name'] = 'kurkakola',
                ['_data'] = {
                    ['ExamineTime'] = 1.0,
                    ['Rarity'] = 'common',
                    ['Label'] = 'Cola',
                    ['MaxStack'] = 100,
                    ['Itemimage'] = 'icons/cola.png',
                    ['Type'] = 'Item',
                    ['Backgroundcolor'] = 'black',
                    ['Description'] = 'For all the thirsty out there',
                    ['Name'] = 'kurkakola',
                    ['Weight'] = 0.5,
                    ['ItemSound'] = 'gear_generic',
                    ['Height'] = 1,
                    ['Width'] = 1
                },
                ['_type'] = 'Item'
            }
        },
        ['emp_ammo'] = {
            ['item'] = {
                ['_id'] = 'emp_ammo',
                ['_parent'] = '',
                ['_name'] = 'emp_ammo',
                ['_data'] = {
                    ['ExamineTime'] = 1.0,
                    ['Rarity'] = 'common',
                    ['Label'] = 'EMP Ammo',
                    ['MaxStack'] = 100,
                    ['Itemimage'] = 'icons/emp_ammo.png',
                    ['Type'] = 'Item',
                    ['Backgroundcolor'] = 'black',
                    ['Description'] = 'Ammo for EMP Launcher',
                    ['Name'] = 'emp_ammo',
                    ['Weight'] = 0.2,
                    ['ItemSound'] = 'gear_generic',
                    ['Height'] = 1,
                    ['Width'] = 1
                },
                ['_type'] = 'Item'
            }
        },
        ['carbinerifle_scope'] = {
            ['item'] = {
                ['_id'] = 'carbinerifle_scope',
                ['_parent'] = '',
                ['_name'] = 'carbinerifle_scope',
                ['_data'] = {
                    ['ExamineTime'] = 1.0,
                    ['Rarity'] = 'common',
                    ['Label'] = 'Rifle Scope',
                    ['MaxStack'] = 100,
                    ['Itemimage'] = 'icons/smg_scope.png',
                    ['Type'] = 'Item',
                    ['Backgroundcolor'] = 'black',
                    ['Description'] = 'Carbine Rifle Scope',
                    ['Name'] = 'carbinerifle_scope',
                    ['Weight'] = 1.0,
                    ['ItemSound'] = 'gear_generic',
                    ['Height'] = 1,
                    ['Width'] = 1
                },
                ['_type'] = 'Item'
            }
        },
        ['weapon_doubleaction'] = {
            ['item'] = {
                ['_id'] = 'weapon_doubleaction',
                ['_parent'] = '',
                ['_name'] = 'weapon_doubleaction',
                ['_data'] = {
                    ['ExamineTime'] = 1.0,
                    ['Rarity'] = 'rare',
                    ['Label'] = 'Double Action Revolver',
                    ['MaxStack'] = 1,
                    ['Itemimage'] = 'icons/weapon_doubleaction.png',
                    ['Type'] = 'Weapon',
                    ['Backgroundcolor'] = 'blue',
                    ['Description'] = 'Double Action Revolver',
                    ['Name'] = 'weapon_doubleaction',
                    ['Weight'] = 1.0,
                    ['ItemSound'] = 'gear_generic',
                    ['Height'] = 2,
                    ['Width'] = 2
                },
                ['_type'] = 'Item'
            }
        },
        ['combatpistol_defaultclip'] = {
            ['item'] = {
                ['_id'] = 'combatpistol_defaultclip',
                ['_parent'] = '',
                ['_name'] = 'combatpistol_defaultclip',
                ['_data'] = {
                    ['ExamineTime'] = 1.0,
                    ['Rarity'] = 'common',
                    ['Label'] = 'Pistol Clip',
                    ['MaxStack'] = 100,
                    ['Itemimage'] = 'icons/pistol_extendedclip.png',
                    ['Type'] = 'Item',
                    ['Backgroundcolor'] = 'black',
                    ['Description'] = 'Combat Pistol Default Clip',
                    ['Name'] = 'combatpistol_defaultclip',
                    ['Weight'] = 1.0,
                    ['ItemSound'] = 'gear_generic',
                    ['Height'] = 1,
                    ['Width'] = 1
                },
                ['_type'] = 'Item'
            }
        },
        ['water_bottle'] = {
            ['item'] = {
                ['_id'] = 'water_bottle',
                ['_parent'] = '',
                ['_name'] = 'water_bottle',
                ['_data'] = {
                    ['ExamineTime'] = 1.0,
                    ['Rarity'] = 'common',
                    ['Label'] = 'Bottle of Water',
                    ['MaxStack'] = 100,
                    ['Itemimage'] = 'icons/water_bottle.png',
                    ['Type'] = 'Item',
                    ['Backgroundcolor'] = 'black',
                    ['Description'] = 'For all the thirsty out there',
                    ['Name'] = 'water_bottle',
                    ['Weight'] = 0.5,
                    ['ItemSound'] = 'gear_generic',
                    ['Height'] = 1,
                    ['Width'] = 1
                },
                ['_type'] = 'Item'
            }
        },
        ['weapon_minismg'] = {
            ['item'] = {
                ['_id'] = 'weapon_minismg',
                ['_parent'] = '',
                ['_name'] = 'weapon_minismg',
                ['_data'] = {
                    ['ExamineTime'] = 1.0,
                    ['Rarity'] = 'rare',
                    ['Label'] = 'Mini SMG',
                    ['MaxStack'] = 1,
                    ['Itemimage'] = 'icons/weapon_minismg.png',
                    ['Type'] = 'Weapon',
                    ['Backgroundcolor'] = 'blue',
                    ['Description'] = 'A mini handheld lightweight machine gun',
                    ['Name'] = 'weapon_minismg',
                    ['Weight'] = 1.0,
                    ['ItemSound'] = 'gear_generic',
                    ['Height'] = 2,
                    ['Width'] = 2
                },
                ['_type'] = 'Item'
            }
        },
        ['gatecrack'] = {
            ['item'] = {
                ['_id'] = 'gatecrack',
                ['_parent'] = '',
                ['_name'] = 'gatecrack',
                ['_data'] = {
                    ['ExamineTime'] = 1.0,
                    ['Rarity'] = 'common',
                    ['Label'] = 'Gatecrack',
                    ['MaxStack'] = 100,
                    ['Itemimage'] = 'icons/usb_device.png',
                    ['Type'] = 'Item',
                    ['Backgroundcolor'] = 'black',
                    ['Description'] = 'Handy software to tear down some fences',
                    ['Name'] = 'gatecrack',
                    ['Weight'] = 0.0,
                    ['ItemSound'] = 'gear_generic',
                    ['Height'] = 1,
                    ['Width'] = 1
                },
                ['_type'] = 'Item'
            }
        },
        ['parachute'] = {
            ['item'] = {
                ['_id'] = 'parachute',
                ['_parent'] = '',
                ['_name'] = 'parachute',
                ['_data'] = {
                    ['ExamineTime'] = 1.0,
                    ['Rarity'] = 'common',
                    ['Label'] = 'Parachute',
                    ['MaxStack'] = 1,
                    ['Itemimage'] = 'icons/parachute.png',
                    ['Type'] = 'Item',
                    ['Backgroundcolor'] = 'black',
                    ['Description'] = 'The sky is the limit! Woohoo!',
                    ['Name'] = 'parachute',
                    ['Weight'] = 30.0,
                    ['ItemSound'] = 'gear_generic',
                    ['Height'] = 1,
                    ['Width'] = 1
                },
                ['_type'] = 'Item'
            }
        },
        ['pumpshotgun_luxuryfinish'] = {
            ['item'] = {
                ['_id'] = 'pumpshotgun_luxuryfinish',
                ['_parent'] = '',
                ['_name'] = 'pumpshotgun_luxuryfinish',
                ['_data'] = {
                    ['ExamineTime'] = 1.0,
                    ['Rarity'] = 'common',
                    ['Label'] = 'Shotgun Finish',
                    ['MaxStack'] = 100,
                    ['Itemimage'] = 'icons/pistol_extendedclip.png',
                    ['Type'] = 'Item',
                    ['Backgroundcolor'] = 'black',
                    ['Description'] = 'Pump Shotgun Luxury Finish',
                    ['Name'] = 'pumpshotgun_luxuryfinish',
                    ['Weight'] = 1.0,
                    ['ItemSound'] = 'gear_generic',
                    ['Height'] = 1,
                    ['Width'] = 1
                },
                ['_type'] = 'Item'
            }
        },
        ['weapon_advancedrifle'] = {
            ['item'] = {
                ['_id'] = 'weapon_advancedrifle',
                ['_parent'] = '',
                ['_name'] = 'weapon_advancedrifle',
                ['_data'] = {
                    ['ExamineTime'] = 1.0,
                    ['Rarity'] = 'rare',
                    ['Label'] = 'Advanced Rifle',
                    ['MaxStack'] = 1,
                    ['Itemimage'] = 'icons/weapon_advancedrifle.png',
                    ['Type'] = 'Weapon',
                    ['Backgroundcolor'] = 'blue',
                    ['Description'] = 'An assault version of a rapid-fire, magazine-fed automatic rifle designed for infantry use',
                    ['Name'] = 'weapon_advancedrifle',
                    ['Weight'] = 1.0,
                    ['ItemSound'] = 'gear_generic',
                    ['Height'] = 2,
                    ['Width'] = 2
                },
                ['_type'] = 'Item'
            }
        },
        ['drill'] = {
            ['item'] = {
                ['_id'] = 'drill',
                ['_parent'] = '',
                ['_name'] = 'drill',
                ['_data'] = {
                    ['ExamineTime'] = 1.0,
                    ['Rarity'] = 'common',
                    ['Label'] = 'Drill',
                    ['MaxStack'] = 100,
                    ['Itemimage'] = 'icons/drill.png',
                    ['Type'] = 'Item',
                    ['Backgroundcolor'] = 'black',
                    ['Description'] = 'The real deal...',
                    ['Name'] = 'drill',
                    ['Weight'] = 20.0,
                    ['ItemSound'] = 'gear_generic',
                    ['Height'] = 1,
                    ['Width'] = 1
                },
                ['_type'] = 'Item'
            }
        },
        ['tourniquet'] = {
            ['item'] = {
                ['_id'] = 'tourniquet',
                ['_parent'] = '',
                ['_name'] = 'tourniquet',
                ['_data'] = {
                    ['ExamineTime'] = 1.0,
                    ['Rarity'] = 'common',
                    ['Label'] = 'Tourniquet',
                    ['MaxStack'] = 100,
                    ['Itemimage'] = 'icons/ifaks.png',
                    ['Type'] = 'Item',
                    ['Backgroundcolor'] = 'black',
                    ['Description'] = 'ifaks for healing and a complete stress remover.',
                    ['Name'] = 'tourniquet',
                    ['Weight'] = 0.0,
                    ['ItemSound'] = 'gear_generic',
                    ['Height'] = 1,
                    ['Width'] = 1
                },
                ['_type'] = 'Item'
            }
        },
        ['weapon_knife'] = {
            ['item'] = {
                ['_id'] = 'weapon_knife',
                ['_parent'] = '',
                ['_name'] = 'weapon_knife',
                ['_data'] = {
                    ['ExamineTime'] = 1.0,
                    ['Rarity'] = 'rare',
                    ['Label'] = 'Knife',
                    ['MaxStack'] = 1,
                    ['Itemimage'] = 'icons/weapon_knife.png',
                    ['Type'] = 'Weapon',
                    ['Backgroundcolor'] = 'blue',
                    ['Description'] = 'An instrument composed of a blade fixed into a handle, used for cutting or as a weapon',
                    ['Name'] = 'weapon_knife',
                    ['Weight'] = 1.0,
                    ['ItemSound'] = 'gear_generic',
                    ['Height'] = 2,
                    ['Width'] = 2
                },
                ['_type'] = 'Item'
            }
        },
        ['pistol_suppressor'] = {
            ['item'] = {
                ['_id'] = 'pistol_suppressor',
                ['_parent'] = '',
                ['_name'] = 'pistol_suppressor',
                ['_data'] = {
                    ['ExamineTime'] = 1.0,
                    ['Rarity'] = 'common',
                    ['Label'] = 'Pistol Suppressor',
                    ['MaxStack'] = 100,
                    ['Itemimage'] = 'icons/pistol_suppressor.png',
                    ['Type'] = 'Item',
                    ['Backgroundcolor'] = 'black',
                    ['Description'] = 'Pistol Suppressor Attachment',
                    ['Name'] = 'pistol_suppressor',
                    ['Weight'] = 1.0,
                    ['ItemSound'] = 'gear_generic',
                    ['Height'] = 1,
                    ['Width'] = 1
                },
                ['_type'] = 'Item'
            }
        },
        ['painkillers'] = {
            ['item'] = {
                ['_id'] = 'painkillers',
                ['_parent'] = '',
                ['_name'] = 'painkillers',
                ['_data'] = {
                    ['ExamineTime'] = 1.0,
                    ['Rarity'] = 'common',
                    ['Label'] = 'Painkillers',
                    ['MaxStack'] = 100,
                    ['Itemimage'] = 'icons/painkillers.png',
                    ['Type'] = 'Item',
                    ['Backgroundcolor'] = 'black',
                    ['Description'] = 'For pain you can\'t stand anymore, take this pill that\'d make you feel great again',
                    ['Name'] = 'painkillers',
                    ['Weight'] = 0.0,
                    ['ItemSound'] = 'gear_generic',
                    ['Height'] = 1,
                    ['Width'] = 1
                },
                ['_type'] = 'Item'
            }
        },
        ['heavyarmor'] = {
            ['item'] = {
                ['_id'] = 'heavyarmor',
                ['_parent'] = '',
                ['_name'] = 'heavyarmor',
                ['_data'] = {
                    ['ExamineTime'] = 1.0,
                    ['Rarity'] = 'common',
                    ['Label'] = 'Heavy Armor',
                    ['MaxStack'] = 100,
                    ['Itemimage'] = 'icons/armor.png',
                    ['Type'] = 'Item',
                    ['Backgroundcolor'] = 'black',
                    ['Description'] = 'Some protection won\'t hurt... right?',
                    ['Name'] = 'heavyarmor',
                    ['Weight'] = 5.0,
                    ['ItemSound'] = 'gear_generic',
                    ['Height'] = 1,
                    ['Width'] = 1
                },
                ['_type'] = 'Item'
            }
        },
        ['firework3'] = {
            ['item'] = {
                ['_id'] = 'firework3',
                ['_parent'] = '',
                ['_name'] = 'firework3',
                ['_data'] = {
                    ['ExamineTime'] = 1.0,
                    ['Rarity'] = 'common',
                    ['Label'] = 'WipeOut',
                    ['MaxStack'] = 100,
                    ['Itemimage'] = 'icons/firework3.png',
                    ['Type'] = 'Item',
                    ['Backgroundcolor'] = 'black',
                    ['Description'] = 'Fireworks',
                    ['Name'] = 'firework3',
                    ['Weight'] = 1.0,
                    ['ItemSound'] = 'gear_generic',
                    ['Height'] = 1,
                    ['Width'] = 1
                },
                ['_type'] = 'Item'
            }
        },
        ['assaultrifle_drum'] = {
            ['item'] = {
                ['_id'] = 'assaultrifle_drum',
                ['_parent'] = '',
                ['_name'] = 'assaultrifle_drum',
                ['_data'] = {
                    ['ExamineTime'] = 1.0,
                    ['Rarity'] = 'common',
                    ['Label'] = 'Rifle Drum',
                    ['MaxStack'] = 100,
                    ['Itemimage'] = 'icons/rifle_drummag.png',
                    ['Type'] = 'Item',
                    ['Backgroundcolor'] = 'black',
                    ['Description'] = 'Assault Rifle Drum',
                    ['Name'] = 'assaultrifle_drum',
                    ['Weight'] = 1.0,
                    ['ItemSound'] = 'gear_generic',
                    ['Height'] = 1,
                    ['Width'] = 1
                },
                ['_type'] = 'Item'
            }
        },
        ['rifle_suppressor'] = {
            ['item'] = {
                ['_id'] = 'rifle_suppressor',
                ['_parent'] = '',
                ['_name'] = 'rifle_suppressor',
                ['_data'] = {
                    ['ExamineTime'] = 1.0,
                    ['Rarity'] = 'common',
                    ['Label'] = 'Rifle Suppressor',
                    ['MaxStack'] = 100,
                    ['Itemimage'] = 'icons/pistol_suppressor.png',
                    ['Type'] = 'Item',
                    ['Backgroundcolor'] = 'black',
                    ['Description'] = 'Rifle Suppressor Attachment',
                    ['Name'] = 'rifle_suppressor',
                    ['Weight'] = 1.0,
                    ['ItemSound'] = 'gear_generic',
                    ['Height'] = 1,
                    ['Width'] = 1
                },
                ['_type'] = 'Item'
            }
        },
        ['dendrogyra_coral'] = {
            ['item'] = {
                ['_id'] = 'dendrogyra_coral',
                ['_parent'] = '',
                ['_name'] = 'dendrogyra_coral',
                ['_data'] = {
                    ['ExamineTime'] = 1.0,
                    ['Rarity'] = 'common',
                    ['Label'] = 'Dendrogyra',
                    ['MaxStack'] = 100,
                    ['Itemimage'] = 'icons/dendrogyra_coral.png',
                    ['Type'] = 'Item',
                    ['Backgroundcolor'] = 'black',
                    ['Description'] = 'Its also known as pillar coral',
                    ['Name'] = 'dendrogyra_coral',
                    ['Weight'] = 1.0,
                    ['ItemSound'] = 'gear_generic',
                    ['Height'] = 1,
                    ['Width'] = 1
                },
                ['_type'] = 'Item'
            }
        },
        ['weapontint_orange'] = {
            ['item'] = {
                ['_id'] = 'weapontint_orange',
                ['_parent'] = '',
                ['_name'] = 'weapontint_orange',
                ['_data'] = {
                    ['ExamineTime'] = 1.0,
                    ['Rarity'] = 'common',
                    ['Label'] = 'Orange Tint',
                    ['MaxStack'] = 100,
                    ['Itemimage'] = 'icons/weapontint_orange.png',
                    ['Type'] = 'Item',
                    ['Backgroundcolor'] = 'black',
                    ['Description'] = 'Orange Weapon Tint',
                    ['Name'] = 'weapontint_orange',
                    ['Weight'] = 1.0,
                    ['ItemSound'] = 'gear_generic',
                    ['Height'] = 1,
                    ['Width'] = 1
                },
                ['_type'] = 'Item'
            }
        },
        ['ironoxide'] = {
            ['item'] = {
                ['_id'] = 'ironoxide',
                ['_parent'] = '',
                ['_name'] = 'ironoxide',
                ['_data'] = {
                    ['ExamineTime'] = 1.0,
                    ['Rarity'] = 'common',
                    ['Label'] = 'Iron Powder',
                    ['MaxStack'] = 100,
                    ['Itemimage'] = 'icons/ironoxide.png',
                    ['Type'] = 'Item',
                    ['Backgroundcolor'] = 'black',
                    ['Description'] = 'Some powder to mix with.',
                    ['Name'] = 'ironoxide',
                    ['Weight'] = 0.1,
                    ['ItemSound'] = 'gear_generic',
                    ['Height'] = 1,
                    ['Width'] = 1
                },
                ['_type'] = 'Item'
            }
        },
        ['markedbills'] = {
            ['item'] = {
                ['_id'] = 'markedbills',
                ['_parent'] = '',
                ['_name'] = 'markedbills',
                ['_data'] = {
                    ['ExamineTime'] = 1.0,
                    ['Rarity'] = 'common',
                    ['Label'] = 'Marked Money',
                    ['MaxStack'] = 1,
                    ['Itemimage'] = 'icons/markedbills.png',
                    ['Type'] = 'Item',
                    ['Backgroundcolor'] = 'black',
                    ['Description'] = 'Money?',
                    ['Name'] = 'markedbills',
                    ['Weight'] = 1.0,
                    ['ItemSound'] = 'gear_generic',
                    ['Height'] = 1,
                    ['Width'] = 1
                },
                ['_type'] = 'Item'
            }
        },
        ['rubber'] = {
            ['item'] = {
                ['_id'] = 'rubber',
                ['_parent'] = '',
                ['_name'] = 'rubber',
                ['_data'] = {
                    ['ExamineTime'] = 1.0,
                    ['Rarity'] = 'common',
                    ['Label'] = 'Rubber',
                    ['MaxStack'] = 100,
                    ['Itemimage'] = 'icons/rubber.png',
                    ['Type'] = 'Item',
                    ['Backgroundcolor'] = 'black',
                    ['Description'] = 'Rubber, I believe you can make your own rubber ducky with it :D',
                    ['Name'] = 'rubber',
                    ['Weight'] = 0.1,
                    ['ItemSound'] = 'gear_generic',
                    ['Height'] = 1,
                    ['Width'] = 1
                },
                ['_type'] = 'Item'
            }
        },
        ['marksmanrifle_defaultclip'] = {
            ['item'] = {
                ['_id'] = 'marksmanrifle_defaultclip',
                ['_parent'] = '',
                ['_name'] = 'marksmanrifle_defaultclip',
                ['_data'] = {
                    ['ExamineTime'] = 1.0,
                    ['Rarity'] = 'common',
                    ['Label'] = 'Sniper Clip',
                    ['MaxStack'] = 100,
                    ['Itemimage'] = 'icons/pistol_extendedclip.png',
                    ['Type'] = 'Item',
                    ['Backgroundcolor'] = 'black',
                    ['Description'] = 'Marksman Rifle Default Clip',
                    ['Name'] = 'marksmanrifle_defaultclip',
                    ['Weight'] = 1.0,
                    ['ItemSound'] = 'gear_generic',
                    ['Height'] = 1,
                    ['Width'] = 1
                },
                ['_type'] = 'Item'
            }
        },
        ['assaultshotgun_extendedclip'] = {
            ['item'] = {
                ['_id'] = 'assaultshotgun_extendedclip',
                ['_parent'] = '',
                ['_name'] = 'assaultshotgun_extendedclip',
                ['_data'] = {
                    ['ExamineTime'] = 1.0,
                    ['Rarity'] = 'common',
                    ['Label'] = 'Shotgun EXT Clip',
                    ['MaxStack'] = 100,
                    ['Itemimage'] = 'icons/pistol_extendedclip.png',
                    ['Type'] = 'Item',
                    ['Backgroundcolor'] = 'black',
                    ['Description'] = 'Assault Shotgun Extended Clip',
                    ['Name'] = 'assaultshotgun_extendedclip',
                    ['Weight'] = 1.0,
                    ['ItemSound'] = 'gear_generic',
                    ['Height'] = 1,
                    ['Width'] = 1
                },
                ['_type'] = 'Item'
            }
        },
        ['walkstick'] = {
            ['item'] = {
                ['_id'] = 'walkstick',
                ['_parent'] = '',
                ['_name'] = 'walkstick',
                ['_data'] = {
                    ['ExamineTime'] = 1.0,
                    ['Rarity'] = 'common',
                    ['Label'] = 'Walking Stick',
                    ['MaxStack'] = 100,
                    ['Itemimage'] = 'icons/walkstick.png',
                    ['Type'] = 'Item',
                    ['Backgroundcolor'] = 'black',
                    ['Description'] = 'Walking stick for ya\'ll grannies out there.. HAHA',
                    ['Name'] = 'walkstick',
                    ['Weight'] = 1.0,
                    ['ItemSound'] = 'gear_generic',
                    ['Height'] = 1,
                    ['Width'] = 1
                },
                ['_type'] = 'Item'
            }
        },
        ['weapon_bat'] = {
            ['item'] = {
                ['_id'] = 'weapon_bat',
                ['_parent'] = '',
                ['_name'] = 'weapon_bat',
                ['_data'] = {
                    ['ExamineTime'] = 1.0,
                    ['Rarity'] = 'rare',
                    ['Label'] = 'Bat',
                    ['MaxStack'] = 1,
                    ['Itemimage'] = 'icons/weapon_bat.png',
                    ['Type'] = 'Weapon',
                    ['Backgroundcolor'] = 'blue',
                    ['Description'] = 'Used for hitting a ball in sports (or other things)',
                    ['Name'] = 'weapon_bat',
                    ['Weight'] = 1.0,
                    ['ItemSound'] = 'gear_generic',
                    ['Height'] = 2,
                    ['Width'] = 2
                },
                ['_type'] = 'Item'
            }
        },
        ['weapon_marksmanrifle_mk2'] = {
            ['item'] = {
                ['_id'] = 'weapon_marksmanrifle_mk2',
                ['_parent'] = '',
                ['_name'] = 'weapon_marksmanrifle_mk2',
                ['_data'] = {
                    ['ExamineTime'] = 1.0,
                    ['Rarity'] = 'rare',
                    ['Label'] = 'Marksman Rifle Mk II',
                    ['MaxStack'] = 1,
                    ['Itemimage'] = 'icons/weapon_marksmanrifle_mk2.png',
                    ['Type'] = 'Weapon',
                    ['Backgroundcolor'] = 'blue',
                    ['Description'] = 'Weapon Marksmanrifle MK2',
                    ['Name'] = 'weapon_marksmanrifle_mk2',
                    ['Weight'] = 1.0,
                    ['ItemSound'] = 'gear_generic',
                    ['Height'] = 2,
                    ['Width'] = 2
                },
                ['_type'] = 'Item'
            }
        },
        ['weapon_microsmg'] = {
            ['item'] = {
                ['_id'] = 'weapon_microsmg',
                ['_parent'] = '',
                ['_name'] = 'weapon_microsmg',
                ['_data'] = {
                    ['ExamineTime'] = 1.0,
                    ['Rarity'] = 'rare',
                    ['Label'] = 'Micro SMG',
                    ['MaxStack'] = 1,
                    ['Itemimage'] = 'icons/weapon_microsmg.png',
                    ['Type'] = 'Weapon',
                    ['Backgroundcolor'] = 'blue',
                    ['Description'] = 'A handheld lightweight machine gun',
                    ['Name'] = 'weapon_microsmg',
                    ['Weight'] = 1.0,
                    ['ItemSound'] = 'gear_generic',
                    ['Height'] = 2,
                    ['Width'] = 2
                },
                ['_type'] = 'Item'
            }
        },
        ['screwdriverset'] = {
            ['item'] = {
                ['_id'] = 'screwdriverset',
                ['_parent'] = '',
                ['_name'] = 'screwdriverset',
                ['_data'] = {
                    ['ExamineTime'] = 1.0,
                    ['Rarity'] = 'common',
                    ['Label'] = 'Toolkit',
                    ['MaxStack'] = 100,
                    ['Itemimage'] = 'icons/screwdriverset.png',
                    ['Type'] = 'Item',
                    ['Backgroundcolor'] = 'black',
                    ['Description'] = 'Very useful to screw... screws...',
                    ['Name'] = 'screwdriverset',
                    ['Weight'] = 1.0,
                    ['ItemSound'] = 'gear_generic',
                    ['Height'] = 1,
                    ['Width'] = 1
                },
                ['_type'] = 'Item'
            }
        },
        ['snipermax_scope'] = {
            ['item'] = {
                ['_id'] = 'snipermax_scope',
                ['_parent'] = '',
                ['_name'] = 'snipermax_scope',
                ['_data'] = {
                    ['ExamineTime'] = 1.0,
                    ['Rarity'] = 'common',
                    ['Label'] = 'Sniper Max Scope',
                    ['MaxStack'] = 100,
                    ['Itemimage'] = 'icons/smg_scope.png',
                    ['Type'] = 'Item',
                    ['Backgroundcolor'] = 'black',
                    ['Description'] = 'Sniper Rifle Max Scope Attachment',
                    ['Name'] = 'snipermax_scope',
                    ['Weight'] = 1.0,
                    ['ItemSound'] = 'gear_generic',
                    ['Height'] = 1,
                    ['Width'] = 1
                },
                ['_type'] = 'Item'
            }
        },
        ['harness'] = {
            ['item'] = {
                ['_id'] = 'harness',
                ['_parent'] = '',
                ['_name'] = 'harness',
                ['_data'] = {
                    ['ExamineTime'] = 1.0,
                    ['Rarity'] = 'common',
                    ['Label'] = 'Race Harness',
                    ['MaxStack'] = 1,
                    ['Itemimage'] = 'icons/harness.png',
                    ['Type'] = 'Item',
                    ['Backgroundcolor'] = 'black',
                    ['Description'] = 'Racing Harness so no matter what you stay in the car',
                    ['Name'] = 'harness',
                    ['Weight'] = 1.0,
                    ['ItemSound'] = 'gear_generic',
                    ['Height'] = 1,
                    ['Width'] = 1
                },
                ['_type'] = 'Item'
            }
        },
        ['weapon_mg'] = {
            ['item'] = {
                ['_id'] = 'weapon_mg',
                ['_parent'] = '',
                ['_name'] = 'weapon_mg',
                ['_data'] = {
                    ['ExamineTime'] = 1.0,
                    ['Rarity'] = 'rare',
                    ['Label'] = 'Machinegun',
                    ['MaxStack'] = 1,
                    ['Itemimage'] = 'icons/weapon_mg.png',
                    ['Type'] = 'Weapon',
                    ['Backgroundcolor'] = 'blue',
                    ['Description'] = 'An automatic gun that fires bullets in rapid succession for as long as the trigger is pressed',
                    ['Name'] = 'weapon_mg',
                    ['Weight'] = 1.0,
                    ['ItemSound'] = 'gear_generic',
                    ['Height'] = 2,
                    ['Width'] = 2
                },
                ['_type'] = 'Item'
            }
        },
        ['weapon_appistol'] = {
            ['item'] = {
                ['_id'] = 'weapon_appistol',
                ['_parent'] = '',
                ['_name'] = 'weapon_appistol',
                ['_data'] = {
                    ['ExamineTime'] = 1.0,
                    ['Rarity'] = 'rare',
                    ['Label'] = 'AP Pistol',
                    ['MaxStack'] = 1,
                    ['Itemimage'] = 'icons/weapon_appistol.png',
                    ['Type'] = 'Weapon',
                    ['Backgroundcolor'] = 'blue',
                    ['Description'] = 'A small firearm designed to be held in one hand that is automatic',
                    ['Name'] = 'weapon_appistol',
                    ['Weight'] = 1.0,
                    ['ItemSound'] = 'gear_generic',
                    ['Height'] = 1,
                    ['Width'] = 3,
                },
                ['_type'] = 'Item'
            }
        },
        ['rolex'] = {
            ['item'] = {
                ['_id'] = 'rolex',
                ['_parent'] = '',
                ['_name'] = 'rolex',
                ['_data'] = {
                    ['ExamineTime'] = 1.0,
                    ['Rarity'] = 'common',
                    ['Label'] = 'Golden Watch',
                    ['MaxStack'] = 100,
                    ['Itemimage'] = 'icons/rolex.png',
                    ['Type'] = 'Item',
                    ['Backgroundcolor'] = 'black',
                    ['Description'] = 'A golden watch seems like the jackpot to me!',
                    ['Name'] = 'rolex',
                    ['Weight'] = 1.5,
                    ['ItemSound'] = 'gear_generic',
                    ['Height'] = 1,
                    ['Width'] = 1
                },
                ['_type'] = 'Item'
            }
        },
        ['xtcbaggy'] = {
            ['item'] = {
                ['_id'] = 'xtcbaggy',
                ['_parent'] = '',
                ['_name'] = 'xtcbaggy',
                ['_data'] = {
                    ['ExamineTime'] = 1.0,
                    ['Rarity'] = 'common',
                    ['Label'] = 'Bag of XTC',
                    ['MaxStack'] = 100,
                    ['Itemimage'] = 'icons/xtc_baggy.png',
                    ['Type'] = 'Item',
                    ['Backgroundcolor'] = 'black',
                    ['Description'] = 'Pop those pills baby',
                    ['Name'] = 'xtcbaggy',
                    ['Weight'] = 0.0,
                    ['ItemSound'] = 'gear_generic',
                    ['Height'] = 1,
                    ['Width'] = 1
                },
                ['_type'] = 'Item'
            }
        },
        ['weed_purple-haze'] = {
            ['item'] = {
                ['_id'] = 'weed_purple-haze',
                ['_parent'] = '',
                ['_name'] = 'weed_purple-haze',
                ['_data'] = {
                    ['ExamineTime'] = 1.0,
                    ['Rarity'] = 'common',
                    ['Label'] = 'Purple Haze 2g',
                    ['MaxStack'] = 100,
                    ['Itemimage'] = 'icons/weed_baggy.png',
                    ['Type'] = 'Item',
                    ['Backgroundcolor'] = 'black',
                    ['Description'] = 'A weed bag with 2g Purple Haze',
                    ['Name'] = 'weed_purple-haze',
                    ['Weight'] = 0.2,
                    ['ItemSound'] = 'gear_generic',
                    ['Height'] = 1,
                    ['Width'] = 1
                },
                ['_type'] = 'Item'
            }
        },
        ['snowball'] = {
            ['item'] = {
                ['_id'] = 'snowball',
                ['_parent'] = '',
                ['_name'] = 'snowball',
                ['_data'] = {
                    ['ExamineTime'] = 1.0,
                    ['Rarity'] = 'common',
                    ['Label'] = 'Snowball',
                    ['MaxStack'] = 100,
                    ['Itemimage'] = 'icons/snowball.png',
                    ['Type'] = 'Item',
                    ['Backgroundcolor'] = 'black',
                    ['Description'] = 'Should have catched it :D',
                    ['Name'] = 'snowball',
                    ['Weight'] = 0.0,
                    ['ItemSound'] = 'gear_generic',
                    ['Height'] = 1,
                    ['Width'] = 1
                },
                ['_type'] = 'Item'
            }
        },
        ['weapon_militaryrifle'] = {
            ['item'] = {
                ['_id'] = 'weapon_militaryrifle',
                ['_parent'] = '',
                ['_name'] = 'weapon_militaryrifle',
                ['_data'] = {
                    ['ExamineTime'] = 1.0,
                    ['Rarity'] = 'rare',
                    ['Label'] = 'Military Rifle',
                    ['MaxStack'] = 1,
                    ['Itemimage'] = 'icons/weapon_militaryrifle.png',
                    ['Type'] = 'Weapon',
                    ['Backgroundcolor'] = 'blue',
                    ['Description'] = 'Weapon Militaryrifle',
                    ['Name'] = 'weapon_militaryrifle',
                    ['Weight'] = 1.0,
                    ['ItemSound'] = 'gear_generic',
                    ['Height'] = 2,
                    ['Width'] = 2
                },
                ['_type'] = 'Item'
            }
        },
        ['weapon_smg'] = {
            ['item'] = {
                ['_id'] = 'weapon_smg',
                ['_parent'] = '',
                ['_name'] = 'weapon_smg',
                ['_data'] = {
                    ['ExamineTime'] = 1.0,
                    ['Rarity'] = 'rare',
                    ['Label'] = 'SMG',
                    ['MaxStack'] = 1,
                    ['Itemimage'] = 'icons/weapon_smg.png',
                    ['Type'] = 'Weapon',
                    ['Backgroundcolor'] = 'blue',
                    ['Description'] = 'A handheld lightweight machine gun',
                    ['Name'] = 'weapon_smg',
                    ['Weight'] = 1.0,
                    ['ItemSound'] = 'gear_generic',
                    ['Height'] = 2,
                    ['Width'] = 2
                },
                ['_type'] = 'Item'
            }
        },
        ['cleaningkit'] = {
            ['item'] = {
                ['_id'] = 'cleaningkit',
                ['_parent'] = '',
                ['_name'] = 'cleaningkit',
                ['_data'] = {
                    ['ExamineTime'] = 1.0,
                    ['Rarity'] = 'common',
                    ['Label'] = 'Cleaning Kit',
                    ['MaxStack'] = 100,
                    ['Itemimage'] = 'icons/cleaningkit.png',
                    ['Type'] = 'Item',
                    ['Backgroundcolor'] = 'black',
                    ['Description'] = 'A microfiber cloth with some soap will let your car sparkle again!',
                    ['Name'] = 'cleaningkit',
                    ['Weight'] = 0.25,
                    ['ItemSound'] = 'gear_generic',
                    ['Height'] = 1,
                    ['Width'] = 1
                },
                ['_type'] = 'Item'
            }
        },
        ['weapon_assaultshotgun'] = {
            ['item'] = {
                ['_id'] = 'weapon_assaultshotgun',
                ['_parent'] = '',
                ['_name'] = 'weapon_assaultshotgun',
                ['_data'] = {
                    ['ExamineTime'] = 1.0,
                    ['Rarity'] = 'rare',
                    ['Label'] = 'Assault Shotgun',
                    ['MaxStack'] = 1,
                    ['Itemimage'] = 'icons/weapon_assaultshotgun.png',
                    ['Type'] = 'Weapon',
                    ['Backgroundcolor'] = 'blue',
                    ['Description'] = 'An assault version of asmoothbore gun for firing small shot at short range',
                    ['Name'] = 'weapon_assaultshotgun',
                    ['Weight'] = 1.0,
                    ['ItemSound'] = 'gear_generic',
                    ['Height'] = 2,
                    ['Width'] = 2
                },
                ['_type'] = 'Item'
            }
        },
        ['firework2'] = {
            ['item'] = {
                ['_id'] = 'firework2',
                ['_parent'] = '',
                ['_name'] = 'firework2',
                ['_data'] = {
                    ['ExamineTime'] = 1.0,
                    ['Rarity'] = 'common',
                    ['Label'] = 'Poppelers',
                    ['MaxStack'] = 100,
                    ['Itemimage'] = 'icons/firework2.png',
                    ['Type'] = 'Item',
                    ['Backgroundcolor'] = 'black',
                    ['Description'] = 'Fireworks',
                    ['Name'] = 'firework2',
                    ['Weight'] = 1.0,
                    ['ItemSound'] = 'gear_generic',
                    ['Height'] = 1,
                    ['Width'] = 1
                },
                ['_type'] = 'Item'
            }
        },
        ['revolver_defaultclip'] = {
            ['item'] = {
                ['_id'] = 'revolver_defaultclip',
                ['_parent'] = '',
                ['_name'] = 'revolver_defaultclip',
                ['_data'] = {
                    ['ExamineTime'] = 1.0,
                    ['Rarity'] = 'common',
                    ['Label'] = 'Pistol Clip',
                    ['MaxStack'] = 100,
                    ['Itemimage'] = 'icons/pistol_extendedclip.png',
                    ['Type'] = 'Item',
                    ['Backgroundcolor'] = 'black',
                    ['Description'] = 'Revovler Default Clip',
                    ['Name'] = 'revolver_defaultclip',
                    ['Weight'] = 1.0,
                    ['ItemSound'] = 'gear_generic',
                    ['Height'] = 1,
                    ['Width'] = 1
                },
                ['_type'] = 'Item'
            }
        },
        ['heavyshotgun_extendedclip'] = {
            ['item'] = {
                ['_id'] = 'heavyshotgun_extendedclip',
                ['_parent'] = '',
                ['_name'] = 'heavyshotgun_extendedclip',
                ['_data'] = {
                    ['ExamineTime'] = 1.0,
                    ['Rarity'] = 'common',
                    ['Label'] = 'Shotgun EXT Clip',
                    ['MaxStack'] = 100,
                    ['Itemimage'] = 'icons/pistol_extendedclip.png',
                    ['Type'] = 'Item',
                    ['Backgroundcolor'] = 'black',
                    ['Description'] = 'Heavy Shotgun Extended Clip',
                    ['Name'] = 'heavyshotgun_extendedclip',
                    ['Weight'] = 1.0,
                    ['ItemSound'] = 'gear_generic',
                    ['Height'] = 1,
                    ['Width'] = 1
                },
                ['_type'] = 'Item'
            }
        },
        ['pistol_flashlight'] = {
            ['item'] = {
                ['_id'] = 'pistol_flashlight',
                ['_parent'] = '',
                ['_name'] = 'pistol_flashlight',
                ['_data'] = {
                    ['ExamineTime'] = 1.0,
                    ['Rarity'] = 'common',
                    ['Label'] = 'Pistol Flashlight',
                    ['MaxStack'] = 100,
                    ['Itemimage'] = 'icons/smg_flashlight.png',
                    ['Type'] = 'Item',
                    ['Backgroundcolor'] = 'black',
                    ['Description'] = 'Pistol Flashlight Attachment',
                    ['Name'] = 'pistol_flashlight',
                    ['Weight'] = 1.0,
                    ['ItemSound'] = 'gear_generic',
                    ['Height'] = 1,
                    ['Width'] = 1
                },
                ['_type'] = 'Item'
            }
        },
        ['cotton'] = {
            ['item'] = {
                ['_id'] = 'cotton',
                ['_parent'] = '',
                ['_name'] = 'cotton',
                ['_data'] = {
                    ['ExamineTime'] = 1.0,
                    ['Rarity'] = 'common',
                    ['Label'] = 'Cotton',
                    ['MaxStack'] = 100,
                    ['Itemimage'] = 'icons/ifaks.png',
                    ['Type'] = 'Item',
                    ['Backgroundcolor'] = 'black',
                    ['Description'] = 'ifaks for healing and a complete stress remover.',
                    ['Name'] = 'cotton',
                    ['Weight'] = 0.0,
                    ['ItemSound'] = 'gear_generic',
                    ['Height'] = 1,
                    ['Width'] = 1
                },
                ['_type'] = 'Item'
            }
        },
        ['twerks_candy'] = {
            ['item'] = {
                ['_id'] = 'twerks_candy',
                ['_parent'] = '',
                ['_name'] = 'twerks_candy',
                ['_data'] = {
                    ['ExamineTime'] = 1.0,
                    ['Rarity'] = 'common',
                    ['Label'] = 'Twerks',
                    ['MaxStack'] = 100,
                    ['Itemimage'] = 'icons/twerks_candy.png',
                    ['Type'] = 'Item',
                    ['Backgroundcolor'] = 'black',
                    ['Description'] = 'Some delicious candy :O',
                    ['Name'] = 'twerks_candy',
                    ['Weight'] = 0.1,
                    ['ItemSound'] = 'gear_generic',
                    ['Height'] = 1,
                    ['Width'] = 1
                },
                ['_type'] = 'Item'
            }
        },
        ['weapon_snowball'] = {
            ['item'] = {
                ['_id'] = 'weapon_snowball',
                ['_parent'] = '',
                ['_name'] = 'weapon_snowball',
                ['_data'] = {
                    ['ExamineTime'] = 1.0,
                    ['Rarity'] = 'rare',
                    ['Label'] = 'Snowball',
                    ['MaxStack'] = 1,
                    ['Itemimage'] = 'icons/weapon_snowball.png',
                    ['Type'] = 'Weapon',
                    ['Backgroundcolor'] = 'blue',
                    ['Description'] = 'A ball of packed snow, especially one made for throwing at other people for fun',
                    ['Name'] = 'weapon_snowball',
                    ['Weight'] = 1.0,
                    ['ItemSound'] = 'gear_generic',
                    ['Height'] = 2,
                    ['Width'] = 2
                },
                ['_type'] = 'Item'
            }
        },
        ['sniper_scope'] = {
            ['item'] = {
                ['_id'] = 'sniper_scope',
                ['_parent'] = '',
                ['_name'] = 'sniper_scope',
                ['_data'] = {
                    ['ExamineTime'] = 1.0,
                    ['Rarity'] = 'common',
                    ['Label'] = 'Sniper Scope',
                    ['MaxStack'] = 100,
                    ['Itemimage'] = 'icons/smg_scope.png',
                    ['Type'] = 'Item',
                    ['Backgroundcolor'] = 'black',
                    ['Description'] = 'Sniper Rifle Scope Attachment',
                    ['Name'] = 'sniper_scope',
                    ['Weight'] = 1.0,
                    ['ItemSound'] = 'gear_generic',
                    ['Height'] = 1,
                    ['Width'] = 1
                },
                ['_type'] = 'Item'
            }
        },
        ['weapontint_black'] = {
            ['item'] = {
                ['_id'] = 'weapontint_black',
                ['_parent'] = '',
                ['_name'] = 'weapontint_black',
                ['_data'] = {
                    ['ExamineTime'] = 1.0,
                    ['Rarity'] = 'common',
                    ['Label'] = 'Default Tint',
                    ['MaxStack'] = 100,
                    ['Itemimage'] = 'icons/weapontint_black.png',
                    ['Type'] = 'Item',
                    ['Backgroundcolor'] = 'black',
                    ['Description'] = 'Default/Black Weapon Tint',
                    ['Name'] = 'weapontint_black',
                    ['Weight'] = 1.0,
                    ['ItemSound'] = 'gear_generic',
                    ['Height'] = 1,
                    ['Width'] = 1
                },
                ['_type'] = 'Item'
            }
        },
        ['weapontint_blue'] = {
            ['item'] = {
                ['_id'] = 'weapontint_blue',
                ['_parent'] = '',
                ['_name'] = 'weapontint_blue',
                ['_data'] = {
                    ['ExamineTime'] = 1.0,
                    ['Rarity'] = 'common',
                    ['Label'] = 'blue Tint',
                    ['MaxStack'] = 100,
                    ['Itemimage'] = 'icons/weapontint_blue.png',
                    ['Type'] = 'Item',
                    ['Backgroundcolor'] = 'black',
                    ['Description'] = 'blue Weapon Tint',
                    ['Name'] = 'weapontint_blue',
                    ['Weight'] = 1.0,
                    ['ItemSound'] = 'gear_generic',
                    ['Height'] = 1,
                    ['Width'] = 1
                },
                ['_type'] = 'Item'
            }
        },
        ['smg_drum'] = {
            ['item'] = {
                ['_id'] = 'smg_drum',
                ['_parent'] = '',
                ['_name'] = 'smg_drum',
                ['_data'] = {
                    ['ExamineTime'] = 1.0,
                    ['Rarity'] = 'common',
                    ['Label'] = 'SMG Drum',
                    ['MaxStack'] = 100,
                    ['Itemimage'] = 'icons/rifle_drummag.png',
                    ['Type'] = 'Item',
                    ['Backgroundcolor'] = 'black',
                    ['Description'] = 'SMG Drum',
                    ['Name'] = 'smg_drum',
                    ['Weight'] = 1.0,
                    ['ItemSound'] = 'gear_generic',
                    ['Height'] = 1,
                    ['Width'] = 1
                },
                ['_type'] = 'Item'
            }
        },
        ['compactrifle_defaultclip'] = {
            ['item'] = {
                ['_id'] = 'compactrifle_defaultclip',
                ['_parent'] = '',
                ['_name'] = 'compactrifle_defaultclip',
                ['_data'] = {
                    ['ExamineTime'] = 1.0,
                    ['Rarity'] = 'common',
                    ['Label'] = 'Rifle Clip',
                    ['MaxStack'] = 100,
                    ['Itemimage'] = 'icons/pistol_extendedclip.png',
                    ['Type'] = 'Item',
                    ['Backgroundcolor'] = 'black',
                    ['Description'] = 'Compact Rifle Default Clip',
                    ['Name'] = 'compactrifle_defaultclip',
                    ['Weight'] = 1.0,
                    ['ItemSound'] = 'gear_generic',
                    ['Height'] = 1,
                    ['Width'] = 1
                },
                ['_type'] = 'Item'
            }
        },
        ['weapon_rpg'] = {
            ['item'] = {
                ['_id'] = 'weapon_rpg',
                ['_parent'] = '',
                ['_name'] = 'weapon_rpg',
                ['_data'] = {
                    ['ExamineTime'] = 1.0,
                    ['Rarity'] = 'rare',
                    ['Label'] = 'RPG',
                    ['MaxStack'] = 1,
                    ['Itemimage'] = 'icons/weapon_rpg.png',
                    ['Type'] = 'Weapon',
                    ['Backgroundcolor'] = 'blue',
                    ['Description'] = 'A rocket-propelled grenade launcher',
                    ['Name'] = 'weapon_rpg',
                    ['Weight'] = 1.0,
                    ['ItemSound'] = 'gear_generic',
                    ['Height'] = 2,
                    ['Width'] = 2
                },
                ['_type'] = 'Item'
            }
        },
        ['weed_og-kush'] = {
            ['item'] = {
                ['_id'] = 'weed_og-kush',
                ['_parent'] = '',
                ['_name'] = 'weed_og-kush',
                ['_data'] = {
                    ['ExamineTime'] = 1.0,
                    ['Rarity'] = 'common',
                    ['Label'] = 'OGKush 2g',
                    ['MaxStack'] = 100,
                    ['Itemimage'] = 'icons/weed_baggy.png',
                    ['Type'] = 'Item',
                    ['Backgroundcolor'] = 'black',
                    ['Description'] = 'A weed bag with 2g OG Kush',
                    ['Name'] = 'weed_og-kush',
                    ['Weight'] = 0.2,
                    ['ItemSound'] = 'gear_generic',
                    ['Height'] = 1,
                    ['Width'] = 1
                },
                ['_type'] = 'Item'
            }
        },
        ['pistol_luxuryfinish'] = {
            ['item'] = {
                ['_id'] = 'pistol_luxuryfinish',
                ['_parent'] = '',
                ['_name'] = 'pistol_luxuryfinish',
                ['_data'] = {
                    ['ExamineTime'] = 1.0,
                    ['Rarity'] = 'common',
                    ['Label'] = 'Pistol Finish',
                    ['MaxStack'] = 100,
                    ['Itemimage'] = 'icons/pistol_suppressor.png',
                    ['Type'] = 'Item',
                    ['Backgroundcolor'] = 'black',
                    ['Description'] = 'Pistol Luxury Finish',
                    ['Name'] = 'pistol_luxuryfinish',
                    ['Weight'] = 1.0,
                    ['ItemSound'] = 'gear_generic',
                    ['Height'] = 1,
                    ['Width'] = 1
                },
                ['_type'] = 'Item'
            }
        },
        ['weapon_gadgetpistol'] = {
            ['item'] = {
                ['_id'] = 'weapon_gadgetpistol',
                ['_parent'] = '',
                ['_name'] = 'weapon_gadgetpistol',
                ['_data'] = {
                    ['ExamineTime'] = 1.0,
                    ['Rarity'] = 'rare',
                    ['Label'] = 'Perico Pistol',
                    ['MaxStack'] = 1,
                    ['Itemimage'] = 'icons/weapon_gadgetpistol.png',
                    ['Type'] = 'Weapon',
                    ['Backgroundcolor'] = 'blue',
                    ['Description'] = 'Weapon Gadgetpistol',
                    ['Name'] = 'weapon_gadgetpistol',
                    ['Weight'] = 1.0,
                    ['ItemSound'] = 'gear_generic',
                    ['Height'] = 2,
                    ['Width'] = 2
                },
                ['_type'] = 'Item'
            }
        },
        ['cokebaggy'] = {
            ['item'] = {
                ['_id'] = 'cokebaggy',
                ['_parent'] = '',
                ['_name'] = 'cokebaggy',
                ['_data'] = {
                    ['ExamineTime'] = 1.0,
                    ['Rarity'] = 'common',
                    ['Label'] = 'Bag of Coke',
                    ['MaxStack'] = 100,
                    ['Itemimage'] = 'icons/cocaine_baggy.png',
                    ['Type'] = 'Item',
                    ['Backgroundcolor'] = 'black',
                    ['Description'] = 'To get happy real quick',
                    ['Name'] = 'cokebaggy',
                    ['Weight'] = 0.0,
                    ['ItemSound'] = 'gear_generic',
                    ['Height'] = 1,
                    ['Width'] = 1
                },
                ['_type'] = 'Item'
            }
        },
        ['weapon_knuckle'] = {
            ['item'] = {
                ['_id'] = 'weapon_knuckle',
                ['_parent'] = '',
                ['_name'] = 'weapon_knuckle',
                ['_data'] = {
                    ['ExamineTime'] = 1.0,
                    ['Rarity'] = 'rare',
                    ['Label'] = 'Knuckle',
                    ['MaxStack'] = 1,
                    ['Itemimage'] = 'icons/weapon_knuckle.png',
                    ['Type'] = 'Weapon',
                    ['Backgroundcolor'] = 'blue',
                    ['Description'] = 'A metal guard worn over the knuckles in fighting, especially to increase the effect of the blows',
                    ['Name'] = 'weapon_knuckle',
                    ['Weight'] = 1.0,
                    ['ItemSound'] = 'gear_generic',
                    ['Height'] = 2,
                    ['Width'] = 2
                },
                ['_type'] = 'Item'
            }
        },
        ['fitbit'] = {
            ['item'] = {
                ['_id'] = 'fitbit',
                ['_parent'] = '',
                ['_name'] = 'fitbit',
                ['_data'] = {
                    ['ExamineTime'] = 1.0,
                    ['Rarity'] = 'common',
                    ['Label'] = 'Fitbit',
                    ['MaxStack'] = 1,
                    ['Itemimage'] = 'icons/fitbit.png',
                    ['Type'] = 'Item',
                    ['Backgroundcolor'] = 'black',
                    ['Description'] = 'I like fitbit',
                    ['Name'] = 'fitbit',
                    ['Weight'] = 0.5,
                    ['ItemSound'] = 'gear_generic',
                    ['Height'] = 1,
                    ['Width'] = 1
                },
                ['_type'] = 'Item'
            }
        },
        ['id_card'] = {
            ['item'] = {
                ['_id'] = 'id_card',
                ['_parent'] = '',
                ['_name'] = 'id_card',
                ['_data'] = {
                    ['ExamineTime'] = 1.0,
                    ['Rarity'] = 'common',
                    ['Label'] = 'ID Card',
                    ['MaxStack'] = 1,
                    ['Itemimage'] = 'icons/id_card.png',
                    ['Type'] = 'Item',
                    ['Backgroundcolor'] = 'black',
                    ['Description'] = 'A card containing all your information to identify yourself',
                    ['Name'] = 'id_card',
                    ['Weight'] = 0.0,
                    ['ItemSound'] = 'gear_generic',
                    ['Height'] = 1,
                    ['Width'] = 1
                },
                ['_type'] = 'Item'
            }
        },
        ['smg_ammo'] = {
            ['item'] = {
                ['_id'] = 'smg_ammo',
                ['_parent'] = '',
                ['_name'] = 'smg_ammo',
                ['_data'] = {
                    ['ExamineTime'] = 1.0,
                    ['Rarity'] = 'common',
                    ['Label'] = 'SMG ammo',
                    ['MaxStack'] = 100,
                    ['Itemimage'] = 'icons/smg_ammo.png',
                    ['Type'] = 'Item',
                    ['Backgroundcolor'] = 'black',
                    ['Description'] = 'Ammo for Sub Machine Guns',
                    ['Name'] = 'smg_ammo',
                    ['Weight'] = 0.5,
                    ['ItemSound'] = 'gear_generic',
                    ['Height'] = 1,
                    ['Width'] = 1
                },
                ['_type'] = 'Item'
            }
        },
        ['rifle_grip'] = {
            ['item'] = {
                ['_id'] = 'rifle_grip',
                ['_parent'] = '',
                ['_name'] = 'rifle_grip',
                ['_data'] = {
                    ['ExamineTime'] = 1.0,
                    ['Rarity'] = 'common',
                    ['Label'] = 'Rifle Grip',
                    ['MaxStack'] = 100,
                    ['Itemimage'] = 'icons/pistol_suppressor.png',
                    ['Type'] = 'Item',
                    ['Backgroundcolor'] = 'black',
                    ['Description'] = 'Rifle Grip Attachment',
                    ['Name'] = 'rifle_grip',
                    ['Weight'] = 1.0,
                    ['ItemSound'] = 'gear_generic',
                    ['Height'] = 1,
                    ['Width'] = 1
                },
                ['_type'] = 'Item'
            }
        },
        ['specialcarbine_drum'] = {
            ['item'] = {
                ['_id'] = 'specialcarbine_drum',
                ['_parent'] = '',
                ['_name'] = 'specialcarbine_drum',
                ['_data'] = {
                    ['ExamineTime'] = 1.0,
                    ['Rarity'] = 'common',
                    ['Label'] = 'Rifle Drum',
                    ['MaxStack'] = 100,
                    ['Itemimage'] = 'icons/rifle_drummag.png',
                    ['Type'] = 'Item',
                    ['Backgroundcolor'] = 'black',
                    ['Description'] = 'Special Carbine Drum',
                    ['Name'] = 'specialcarbine_drum',
                    ['Weight'] = 1.0,
                    ['ItemSound'] = 'gear_generic',
                    ['Height'] = 1,
                    ['Width'] = 1
                },
                ['_type'] = 'Item'
            }
        },
        ['combatpdw_scope'] = {
            ['item'] = {
                ['_id'] = 'combatpdw_scope',
                ['_parent'] = '',
                ['_name'] = 'combatpdw_scope',
                ['_data'] = {
                    ['ExamineTime'] = 1.0,
                    ['Rarity'] = 'common',
                    ['Label'] = 'SMG Scope',
                    ['MaxStack'] = 100,
                    ['Itemimage'] = 'icons/smg_scope.png',
                    ['Type'] = 'Item',
                    ['Backgroundcolor'] = 'black',
                    ['Description'] = 'Combat PDW Scope Attachment',
                    ['Name'] = 'combatpdw_scope',
                    ['Weight'] = 1.0,
                    ['ItemSound'] = 'gear_generic',
                    ['Height'] = 1,
                    ['Width'] = 1
                },
                ['_type'] = 'Item'
            }
        },
        ['sniperrifle_defaultclip'] = {
            ['item'] = {
                ['_id'] = 'sniperrifle_defaultclip',
                ['_parent'] = '',
                ['_name'] = 'sniperrifle_defaultclip',
                ['_data'] = {
                    ['ExamineTime'] = 1.0,
                    ['Rarity'] = 'common',
                    ['Label'] = 'Sniper Suppressor',
                    ['MaxStack'] = 100,
                    ['Itemimage'] = 'icons/pistol_suppressor.png',
                    ['Type'] = 'Item',
                    ['Backgroundcolor'] = 'black',
                    ['Description'] = 'Sniper Rifle Default Clip',
                    ['Name'] = 'sniperrifle_defaultclip',
                    ['Weight'] = 1.0,
                    ['ItemSound'] = 'gear_generic',
                    ['Height'] = 1,
                    ['Width'] = 1
                },
                ['_type'] = 'Item'
            }
        },
        ['weapon_combatpdw'] = {
            ['item'] = {
                ['_id'] = 'weapon_combatpdw',
                ['_parent'] = '',
                ['_name'] = 'weapon_combatpdw',
                ['_data'] = {
                    ['ExamineTime'] = 1.0,
                    ['Rarity'] = 'rare',
                    ['Label'] = 'Combat PDW',
                    ['MaxStack'] = 1,
                    ['Itemimage'] = 'icons/weapon_combatpdw.png',
                    ['Type'] = 'Weapon',
                    ['Backgroundcolor'] = 'blue',
                    ['Description'] = 'A combat version of a handheld lightweight machine gun',
                    ['Name'] = 'weapon_combatpdw',
                    ['Weight'] = 1.0,
                    ['ItemSound'] = 'gear_generic',
                    ['Height'] = 2,
                    ['Width'] = 2
                },
                ['_type'] = 'Item'
            }
        },
        ['weapon_raycarbine'] = {
            ['item'] = {
                ['_id'] = 'weapon_raycarbine',
                ['_parent'] = '',
                ['_name'] = 'weapon_raycarbine',
                ['_data'] = {
                    ['ExamineTime'] = 1.0,
                    ['Rarity'] = 'rare',
                    ['Label'] = 'Unholy Hellbringer',
                    ['MaxStack'] = 1,
                    ['Itemimage'] = 'icons/weapon_raycarbine.png',
                    ['Type'] = 'Weapon',
                    ['Backgroundcolor'] = 'blue',
                    ['Description'] = 'Weapon Raycarbine',
                    ['Name'] = 'weapon_raycarbine',
                    ['Weight'] = 1.0,
                    ['ItemSound'] = 'gear_generic',
                    ['Height'] = 2,
                    ['Width'] = 2
                },
                ['_type'] = 'Item'
            }
        },
        ['weapon_stungun'] = {
            ['item'] = {
                ['_id'] = 'weapon_stungun',
                ['_parent'] = '',
                ['_name'] = 'weapon_stungun',
                ['_data'] = {
                    ['ExamineTime'] = 1.0,
                    ['Rarity'] = 'rare',
                    ['Label'] = 'Taser',
                    ['MaxStack'] = 1,
                    ['Itemimage'] = 'icons/weapon_stungun.png',
                    ['Type'] = 'Weapon',
                    ['Backgroundcolor'] = 'blue',
                    ['Description'] = 'A weapon firing barbs attached by wires to batteries, causing temporary paralysis',
                    ['Name'] = 'weapon_stungun',
                    ['Weight'] = 1.0,
                    ['ItemSound'] = 'gear_generic',
                    ['Height'] = 2,
                    ['Width'] = 2
                },
                ['_type'] = 'Item'
            }
        },
        ['carbinerifle_defaultclip'] = {
            ['item'] = {
                ['_id'] = 'carbinerifle_defaultclip',
                ['_parent'] = '',
                ['_name'] = 'carbinerifle_defaultclip',
                ['_data'] = {
                    ['ExamineTime'] = 1.0,
                    ['Rarity'] = 'common',
                    ['Label'] = 'Rifle Clip',
                    ['MaxStack'] = 100,
                    ['Itemimage'] = 'icons/pistol_extendedclip.png',
                    ['Type'] = 'Item',
                    ['Backgroundcolor'] = 'black',
                    ['Description'] = 'Carbine Rifle Default Clip',
                    ['Name'] = 'carbinerifle_defaultclip',
                    ['Weight'] = 1.0,
                    ['ItemSound'] = 'gear_generic',
                    ['Height'] = 1,
                    ['Width'] = 1
                },
                ['_type'] = 'Item'
            }
        },
        ['heavyshotgun_drum'] = {
            ['item'] = {
                ['_id'] = 'heavyshotgun_drum',
                ['_parent'] = '',
                ['_name'] = 'heavyshotgun_drum',
                ['_data'] = {
                    ['ExamineTime'] = 1.0,
                    ['Rarity'] = 'common',
                    ['Label'] = 'Shotgun Drum',
                    ['MaxStack'] = 100,
                    ['Itemimage'] = 'icons/rifle_drummag.png',
                    ['Type'] = 'Item',
                    ['Backgroundcolor'] = 'black',
                    ['Description'] = 'Heavy Shotgun Drum',
                    ['Name'] = 'heavyshotgun_drum',
                    ['Weight'] = 1.0,
                    ['ItemSound'] = 'gear_generic',
                    ['Height'] = 1,
                    ['Width'] = 1
                },
                ['_type'] = 'Item'
            }
        },
        ['weapon_molotov'] = {
            ['item'] = {
                ['_id'] = 'weapon_molotov',
                ['_parent'] = '',
                ['_name'] = 'weapon_molotov',
                ['_data'] = {
                    ['ExamineTime'] = 1.0,
                    ['Rarity'] = 'rare',
                    ['Label'] = 'Molotov',
                    ['MaxStack'] = 1,
                    ['Itemimage'] = 'icons/weapon_molotov.png',
                    ['Type'] = 'Weapon',
                    ['Backgroundcolor'] = 'blue',
                    ['Description'] = 'A crude bomb made of a bottle filled with a flammable liquid and fitted with a wick for lighting',
                    ['Name'] = 'weapon_molotov',
                    ['Weight'] = 1.0,
                    ['ItemSound'] = 'gear_generic',
                    ['Height'] = 2,
                    ['Width'] = 2
                },
                ['_type'] = 'Item'
            }
        },
        ['casinochips'] = {
            ['item'] = {
                ['_id'] = 'casinochips',
                ['_parent'] = '',
                ['_name'] = 'casinochips',
                ['_data'] = {
                    ['ExamineTime'] = 1.0,
                    ['Rarity'] = 'common',
                    ['Label'] = 'Casino Chips',
                    ['MaxStack'] = 100,
                    ['Itemimage'] = 'icons/casinochips.png',
                    ['Type'] = 'Item',
                    ['Backgroundcolor'] = 'black',
                    ['Description'] = 'Chips For Casino Gambling',
                    ['Name'] = 'casinochips',
                    ['Weight'] = 0.0,
                    ['ItemSound'] = 'gear_generic',
                    ['Height'] = 1,
                    ['Width'] = 1
                },
                ['_type'] = 'Item'
            }
        },
        ['minismg_defaultclip'] = {
            ['item'] = {
                ['_id'] = 'minismg_defaultclip',
                ['_parent'] = '',
                ['_name'] = 'minismg_defaultclip',
                ['_data'] = {
                    ['ExamineTime'] = 1.0,
                    ['Rarity'] = 'common',
                    ['Label'] = 'SMG Clip',
                    ['MaxStack'] = 100,
                    ['Itemimage'] = 'icons/pistol_extendedclip.png',
                    ['Type'] = 'Item',
                    ['Backgroundcolor'] = 'black',
                    ['Description'] = 'Mini SMG Default Clip',
                    ['Name'] = 'minismg_defaultclip',
                    ['Weight'] = 1.0,
                    ['ItemSound'] = 'gear_generic',
                    ['Height'] = 1,
                    ['Width'] = 1
                },
                ['_type'] = 'Item'
            }
        },
        ['weapon_revolver_mk2'] = {
            ['item'] = {
                ['_id'] = 'weapon_revolver_mk2',
                ['_parent'] = '',
                ['_name'] = 'weapon_revolver_mk2',
                ['_data'] = {
                    ['ExamineTime'] = 1.0,
                    ['Rarity'] = 'rare',
                    ['Label'] = 'Violence',
                    ['MaxStack'] = 1,
                    ['Itemimage'] = 'icons/weapon_revolver_mk2.png',
                    ['Type'] = 'Weapon',
                    ['Backgroundcolor'] = 'blue',
                    ['Description'] = 'da Violence',
                    ['Name'] = 'weapon_revolver_mk2',
                    ['Weight'] = 1.0,
                    ['ItemSound'] = 'gear_generic',
                    ['Height'] = 2,
                    ['Width'] = 2
                },
                ['_type'] = 'Item'
            }
        },
        ['snspistol_defaultclip'] = {
            ['item'] = {
                ['_id'] = 'snspistol_defaultclip',
                ['_parent'] = '',
                ['_name'] = 'snspistol_defaultclip',
                ['_data'] = {
                    ['ExamineTime'] = 1.0,
                    ['Rarity'] = 'common',
                    ['Label'] = 'Pistol Clip',
                    ['MaxStack'] = 100,
                    ['Itemimage'] = 'icons/pistol_extendedclip.png',
                    ['Type'] = 'Item',
                    ['Backgroundcolor'] = 'black',
                    ['Description'] = 'SNS Pistol Default Clip',
                    ['Name'] = 'snspistol_defaultclip',
                    ['Weight'] = 1.0,
                    ['ItemSound'] = 'gear_generic',
                    ['Height'] = 1,
                    ['Width'] = 1
                },
                ['_type'] = 'Item'
            }
        },
        ['smg_luxuryfinish'] = {
            ['item'] = {
                ['_id'] = 'smg_luxuryfinish',
                ['_parent'] = '',
                ['_name'] = 'smg_luxuryfinish',
                ['_data'] = {
                    ['ExamineTime'] = 1.0,
                    ['Rarity'] = 'common',
                    ['Label'] = 'SMG Finish',
                    ['MaxStack'] = 100,
                    ['Itemimage'] = 'icons/pistol_suppressor.png',
                    ['Type'] = 'Item',
                    ['Backgroundcolor'] = 'black',
                    ['Description'] = 'SMG Luxury Finish',
                    ['Name'] = 'smg_luxuryfinish',
                    ['Weight'] = 1.0,
                    ['ItemSound'] = 'gear_generic',
                    ['Height'] = 1,
                    ['Width'] = 1
                },
                ['_type'] = 'Item'
            }
        },
        ['marksmanrifle_extendedclip'] = {
            ['item'] = {
                ['_id'] = 'marksmanrifle_extendedclip',
                ['_parent'] = '',
                ['_name'] = 'marksmanrifle_extendedclip',
                ['_data'] = {
                    ['ExamineTime'] = 1.0,
                    ['Rarity'] = 'common',
                    ['Label'] = 'Sniper EXT Clip',
                    ['MaxStack'] = 100,
                    ['Itemimage'] = 'icons/pistol_extendedclip.png',
                    ['Type'] = 'Item',
                    ['Backgroundcolor'] = 'black',
                    ['Description'] = 'Marksman Rifle Extended Clip',
                    ['Name'] = 'marksmanrifle_extendedclip',
                    ['Weight'] = 1.0,
                    ['ItemSound'] = 'gear_generic',
                    ['Height'] = 1,
                    ['Width'] = 1
                },
                ['_type'] = 'Item'
            }
        },
        ['weapon_flare'] = {
            ['item'] = {
                ['_id'] = 'weapon_flare',
                ['_parent'] = '',
                ['_name'] = 'weapon_flare',
                ['_data'] = {
                    ['ExamineTime'] = 1.0,
                    ['Rarity'] = 'rare',
                    ['Label'] = 'Flare pistol',
                    ['MaxStack'] = 1,
                    ['Itemimage'] = 'icons/weapon_flare.png',
                    ['Type'] = 'Weapon',
                    ['Backgroundcolor'] = 'blue',
                    ['Description'] = 'A small pyrotechnic devices used for illumination and signalling',
                    ['Name'] = 'weapon_flare',
                    ['Weight'] = 1.0,
                    ['ItemSound'] = 'gear_generic',
                    ['Height'] = 2,
                    ['Width'] = 2
                },
                ['_type'] = 'Item'
            }
        },
        ['meth'] = {
            ['item'] = {
                ['_id'] = 'meth',
                ['_parent'] = '',
                ['_name'] = 'meth',
                ['_data'] = {
                    ['ExamineTime'] = 1.0,
                    ['Rarity'] = 'common',
                    ['Label'] = 'Meth',
                    ['MaxStack'] = 100,
                    ['Itemimage'] = 'icons/meth_baggy.png',
                    ['Type'] = 'Item',
                    ['Backgroundcolor'] = 'black',
                    ['Description'] = 'A baggie of Meth',
                    ['Name'] = 'meth',
                    ['Weight'] = 0.1,
                    ['ItemSound'] = 'gear_generic',
                    ['Height'] = 1,
                    ['Width'] = 1
                },
                ['_type'] = 'Item'
            }
        },
        ['joint'] = {
            ['item'] = {
                ['_id'] = 'joint',
                ['_parent'] = '',
                ['_name'] = 'joint',
                ['_data'] = {
                    ['ExamineTime'] = 1.0,
                    ['Rarity'] = 'common',
                    ['Label'] = 'Joint',
                    ['MaxStack'] = 100,
                    ['Itemimage'] = 'icons/joint.png',
                    ['Type'] = 'Item',
                    ['Backgroundcolor'] = 'black',
                    ['Description'] = 'Sidney would be very proud at you',
                    ['Name'] = 'joint',
                    ['Weight'] = 0.0,
                    ['ItemSound'] = 'gear_generic',
                    ['Height'] = 1,
                    ['Width'] = 1
                },
                ['_type'] = 'Item'
            }
        },
        ['heavypistol_grip'] = {
            ['item'] = {
                ['_id'] = 'heavypistol_grip',
                ['_parent'] = '',
                ['_name'] = 'heavypistol_grip',
                ['_data'] = {
                    ['ExamineTime'] = 1.0,
                    ['Rarity'] = 'common',
                    ['Label'] = 'Pistol Grip',
                    ['MaxStack'] = 100,
                    ['Itemimage'] = 'icons/pistol_suppressor.png',
                    ['Type'] = 'Item',
                    ['Backgroundcolor'] = 'black',
                    ['Description'] = 'Heavy Pistol Grip Attachment',
                    ['Name'] = 'heavypistol_grip',
                    ['Weight'] = 1.0,
                    ['ItemSound'] = 'gear_generic',
                    ['Height'] = 1,
                    ['Width'] = 1
                },
                ['_type'] = 'Item'
            }
        },
        ['weapon_rayminigun'] = {
            ['item'] = {
                ['_id'] = 'weapon_rayminigun',
                ['_parent'] = '',
                ['_name'] = 'weapon_rayminigun',
                ['_data'] = {
                    ['ExamineTime'] = 1.0,
                    ['Rarity'] = 'rare',
                    ['Label'] = 'Widowmaker',
                    ['MaxStack'] = 1,
                    ['Itemimage'] = 'icons/weapon_rayminigun.png',
                    ['Type'] = 'Weapon',
                    ['Backgroundcolor'] = 'blue',
                    ['Description'] = 'Weapon Rayminigun',
                    ['Name'] = 'weapon_rayminigun',
                    ['Weight'] = 1.0,
                    ['ItemSound'] = 'gear_generic',
                    ['Height'] = 2,
                    ['Width'] = 2
                },
                ['_type'] = 'Item'
            }
        },
        ['vodka'] = {
            ['item'] = {
                ['_id'] = 'vodka',
                ['_parent'] = '',
                ['_name'] = 'vodka',
                ['_data'] = {
                    ['ExamineTime'] = 1.0,
                    ['Rarity'] = 'common',
                    ['Label'] = 'Vodka',
                    ['MaxStack'] = 100,
                    ['Itemimage'] = 'icons/vodka.png',
                    ['Type'] = 'Item',
                    ['Backgroundcolor'] = 'black',
                    ['Description'] = 'For all the thirsty out there',
                    ['Name'] = 'vodka',
                    ['Weight'] = 0.5,
                    ['ItemSound'] = 'gear_generic',
                    ['Height'] = 1,
                    ['Width'] = 1
                },
                ['_type'] = 'Item'
            }
        },
        ['weapon_vintagepistol'] = {
            ['item'] = {
                ['_id'] = 'weapon_vintagepistol',
                ['_parent'] = '',
                ['_name'] = 'weapon_vintagepistol',
                ['_data'] = {
                    ['ExamineTime'] = 1.0,
                    ['Rarity'] = 'rare',
                    ['Label'] = 'Vintage Pistol',
                    ['MaxStack'] = 1,
                    ['Itemimage'] = 'icons/weapon_vintagepistol.png',
                    ['Type'] = 'Weapon',
                    ['Backgroundcolor'] = 'blue',
                    ['Description'] = 'An antique firearm designed to be held in one hand',
                    ['Name'] = 'weapon_vintagepistol',
                    ['Weight'] = 1.0,
                    ['ItemSound'] = 'gear_generic',
                    ['Height'] = 1,
                    ['Width'] = 2
                },
                ['_type'] = 'Item'
            }
        },
        ['weapon_sawnoffshotgun'] = {
            ['item'] = {
                ['_id'] = 'weapon_sawnoffshotgun',
                ['_parent'] = '',
                ['_name'] = 'weapon_sawnoffshotgun',
                ['_data'] = {
                    ['ExamineTime'] = 1.0,
                    ['Rarity'] = 'rare',
                    ['Label'] = 'Sawn-off Shotgun',
                    ['MaxStack'] = 1,
                    ['Itemimage'] = 'icons/weapon_sawnoffshotgun.png',
                    ['Type'] = 'Weapon',
                    ['Backgroundcolor'] = 'blue',
                    ['Description'] = 'A sawn-off smoothbore gun for firing small shot at short range',
                    ['Name'] = 'weapon_sawnoffshotgun',
                    ['Weight'] = 1.0,
                    ['ItemSound'] = 'gear_generic',
                    ['Height'] = 2,
                    ['Width'] = 2
                },
                ['_type'] = 'Item'
            }
        },
        ['ifaks'] = {
            ['item'] = {
                ['_id'] = 'ifaks',
                ['_parent'] = '',
                ['_name'] = 'ifaks',
                ['_data'] = {
                    ['ExamineTime'] = 1.0,
                    ['Rarity'] = 'common',
                    ['Label'] = 'ifaks',
                    ['MaxStack'] = 100,
                    ['Itemimage'] = 'icons/ifaks.png',
                    ['Type'] = 'Item',
                    ['Backgroundcolor'] = 'black',
                    ['Description'] = 'ifaks for healing and a complete stress remover.',
                    ['Name'] = 'ifaks',
                    ['Weight'] = 0.2,
                    ['ItemSound'] = 'gear_generic',
                    ['Height'] = 1,
                    ['Width'] = 1
                },
                ['_type'] = 'Item'
            }
        },
        ['cryptostick'] = {
            ['item'] = {
                ['_id'] = 'cryptostick',
                ['_parent'] = '',
                ['_name'] = 'cryptostick',
                ['_data'] = {
                    ['ExamineTime'] = 1.0,
                    ['Rarity'] = 'common',
                    ['Label'] = 'Crypto Stick',
                    ['MaxStack'] = 1,
                    ['Itemimage'] = 'icons/cryptostick.png',
                    ['Type'] = 'Item',
                    ['Backgroundcolor'] = 'black',
                    ['Description'] = 'Why would someone ever buy money that doesn\'t exist.. How many would it contain..?',
                    ['Name'] = 'cryptostick',
                    ['Weight'] = 0.2,
                    ['ItemSound'] = 'gear_generic',
                    ['Height'] = 1,
                    ['Width'] = 1
                },
                ['_type'] = 'Item'
            }
        },
        ['specialcarbine_extendedclip'] = {
            ['item'] = {
                ['_id'] = 'specialcarbine_extendedclip',
                ['_parent'] = '',
                ['_name'] = 'specialcarbine_extendedclip',
                ['_data'] = {
                    ['ExamineTime'] = 1.0,
                    ['Rarity'] = 'common',
                    ['Label'] = 'Rifle EXT Clip',
                    ['MaxStack'] = 100,
                    ['Itemimage'] = 'icons/pistol_extendedclip.png',
                    ['Type'] = 'Item',
                    ['Backgroundcolor'] = 'black',
                    ['Description'] = 'Special Carbine Extended Clip',
                    ['Name'] = 'specialcarbine_extendedclip',
                    ['Weight'] = 1.0,
                    ['ItemSound'] = 'gear_generic',
                    ['Height'] = 1,
                    ['Width'] = 1
                },
                ['_type'] = 'Item'
            }
        },
        ['weapontint_gold'] = {
            ['item'] = {
                ['_id'] = 'weapontint_gold',
                ['_parent'] = '',
                ['_name'] = 'weapontint_gold',
                ['_data'] = {
                    ['ExamineTime'] = 1.0,
                    ['Rarity'] = 'common',
                    ['Label'] = 'Gold Tint',
                    ['MaxStack'] = 100,
                    ['Itemimage'] = 'icons/weapontint_gold.png',
                    ['Type'] = 'Item',
                    ['Backgroundcolor'] = 'black',
                    ['Description'] = 'Gold Weapon Tint',
                    ['Name'] = 'weapontint_gold',
                    ['Weight'] = 1.0,
                    ['ItemSound'] = 'gear_generic',
                    ['Height'] = 1,
                    ['Width'] = 1
                },
                ['_type'] = 'Item'
            }
        },
        ['weapon_bzgas'] = {
            ['item'] = {
                ['_id'] = 'weapon_bzgas',
                ['_parent'] = '',
                ['_name'] = 'weapon_bzgas',
                ['_data'] = {
                    ['ExamineTime'] = 1.0,
                    ['Rarity'] = 'rare',
                    ['Label'] = 'BZ Gas',
                    ['MaxStack'] = 1,
                    ['Itemimage'] = 'icons/weapon_bzgas.png',
                    ['Type'] = 'Weapon',
                    ['Backgroundcolor'] = 'blue',
                    ['Description'] = 'A cannister of gas that causes extreme pain',
                    ['Name'] = 'weapon_bzgas',
                    ['Weight'] = 1.0,
                    ['ItemSound'] = 'gear_generic',
                    ['Height'] = 2,
                    ['Width'] = 2
                },
                ['_type'] = 'Item'
            }
        },
        ['weapon_flaregun'] = {
            ['item'] = {
                ['_id'] = 'weapon_flaregun',
                ['_parent'] = '',
                ['_name'] = 'weapon_flaregun',
                ['_data'] = {
                    ['ExamineTime'] = 1.0,
                    ['Rarity'] = 'rare',
                    ['Label'] = 'Flare Gun',
                    ['MaxStack'] = 1,
                    ['Itemimage'] = 'icons/weapon_flaregun.png',
                    ['Type'] = 'Weapon',
                    ['Backgroundcolor'] = 'blue',
                    ['Description'] = 'A handgun for firing signal rockets',
                    ['Name'] = 'weapon_flaregun',
                    ['Weight'] = 1.0,
                    ['ItemSound'] = 'gear_generic',
                    ['Height'] = 2,
                    ['Width'] = 2
                },
                ['_type'] = 'Item'
            }
        },
        ['weapon_sniperrifle'] = {
            ['item'] = {
                ['_id'] = 'weapon_sniperrifle',
                ['_parent'] = '',
                ['_name'] = 'weapon_sniperrifle',
                ['_data'] = {
                    ['ExamineTime'] = 1.0,
                    ['Rarity'] = 'rare',
                    ['Label'] = 'Sniper Rifle',
                    ['MaxStack'] = 1,
                    ['Itemimage'] = 'icons/weapon_sniperrifle.png',
                    ['Type'] = 'Weapon',
                    ['Backgroundcolor'] = 'blue',
                    ['Description'] = 'A high-precision, long-range rifle',
                    ['Name'] = 'weapon_sniperrifle',
                    ['Weight'] = 1.0,
                    ['ItemSound'] = 'gear_generic',
                    ['Height'] = 2,
                    ['Width'] = 2
                },
                ['_type'] = 'Item'
            }
        },
        ['weapon_bullpuprifle_mk2'] = {
            ['item'] = {
                ['_id'] = 'weapon_bullpuprifle_mk2',
                ['_parent'] = '',
                ['_name'] = 'weapon_bullpuprifle_mk2',
                ['_data'] = {
                    ['ExamineTime'] = 1.0,
                    ['Rarity'] = 'rare',
                    ['Label'] = 'Bullpup Rifle Mk II',
                    ['MaxStack'] = 1,
                    ['Itemimage'] = 'icons/weapon_bullpuprifle_mk2.png',
                    ['Type'] = 'Weapon',
                    ['Backgroundcolor'] = 'blue',
                    ['Description'] = 'Bull Puprifle MK2',
                    ['Name'] = 'weapon_bullpuprifle_mk2',
                    ['Weight'] = 1.0,
                    ['ItemSound'] = 'gear_generic',
                    ['Height'] = 2,
                    ['Width'] = 2
                },
                ['_type'] = 'Item'
            }
        },
        ['weapon_heavysniper_mk2'] = {
            ['item'] = {
                ['_id'] = 'weapon_heavysniper_mk2',
                ['_parent'] = '',
                ['_name'] = 'weapon_heavysniper_mk2',
                ['_data'] = {
                    ['ExamineTime'] = 1.0,
                    ['Rarity'] = 'rare',
                    ['Label'] = 'Heavy Sniper Mk II',
                    ['MaxStack'] = 1,
                    ['Itemimage'] = 'icons/weapon_heavysniper_mk2.png',
                    ['Type'] = 'Weapon',
                    ['Backgroundcolor'] = 'blue',
                    ['Description'] = 'Weapon Heavysniper MK2',
                    ['Name'] = 'weapon_heavysniper_mk2',
                    ['Weight'] = 1.0,
                    ['ItemSound'] = 'gear_generic',
                    ['Height'] = 2,
                    ['Width'] = 2
                },
                ['_type'] = 'Item'
            }
        },
        ['firework1'] = {
            ['item'] = {
                ['_id'] = 'firework1',
                ['_parent'] = '',
                ['_name'] = 'firework1',
                ['_data'] = {
                    ['ExamineTime'] = 1.0,
                    ['Rarity'] = 'common',
                    ['Label'] = '2Brothers',
                    ['MaxStack'] = 100,
                    ['Itemimage'] = 'icons/firework1.png',
                    ['Type'] = 'Item',
                    ['Backgroundcolor'] = 'black',
                    ['Description'] = 'Fireworks',
                    ['Name'] = 'firework1',
                    ['Weight'] = 1.0,
                    ['ItemSound'] = 'gear_generic',
                    ['Height'] = 1,
                    ['Width'] = 1
                },
                ['_type'] = 'Item'
            }
        },
        ['weapontint_plat'] = {
            ['item'] = {
                ['_id'] = 'weapontint_plat',
                ['_parent'] = '',
                ['_name'] = 'weapontint_plat',
                ['_data'] = {
                    ['ExamineTime'] = 1.0,
                    ['Rarity'] = 'common',
                    ['Label'] = 'Platinum Tint',
                    ['MaxStack'] = 100,
                    ['Itemimage'] = 'icons/weapontint_plat.png',
                    ['Type'] = 'Item',
                    ['Backgroundcolor'] = 'black',
                    ['Description'] = 'Platinum Weapon Tint',
                    ['Name'] = 'weapontint_plat',
                    ['Weight'] = 1.0,
                    ['ItemSound'] = 'gear_generic',
                    ['Height'] = 1,
                    ['Width'] = 1
                },
                ['_type'] = 'Item'
            }
        },
        ['weapon_navyrevolver'] = {
            ['item'] = {
                ['_id'] = 'weapon_navyrevolver',
                ['_parent'] = '',
                ['_name'] = 'weapon_navyrevolver',
                ['_data'] = {
                    ['ExamineTime'] = 1.0,
                    ['Rarity'] = 'rare',
                    ['Label'] = 'Navy Revolver',
                    ['MaxStack'] = 1,
                    ['Itemimage'] = 'icons/weapon_navyrevolver.png',
                    ['Type'] = 'Weapon',
                    ['Backgroundcolor'] = 'blue',
                    ['Description'] = 'Weapon Navyrevolver',
                    ['Name'] = 'weapon_navyrevolver',
                    ['Weight'] = 1.0,
                    ['ItemSound'] = 'gear_generic',
                    ['Height'] = 2,
                    ['Width'] = 2
                },
                ['_type'] = 'Item'
            }
        },
        ['mg_ammo'] = {
            ['item'] = {
                ['_id'] = 'mg_ammo',
                ['_parent'] = '',
                ['_name'] = 'mg_ammo',
                ['_data'] = {
                    ['ExamineTime'] = 1.0,
                    ['Rarity'] = 'common',
                    ['Label'] = 'MG ammo',
                    ['MaxStack'] = 100,
                    ['Itemimage'] = 'icons/mg_ammo.png',
                    ['Type'] = 'Item',
                    ['Backgroundcolor'] = 'black',
                    ['Description'] = 'Ammo for Machine Guns',
                    ['Name'] = 'mg_ammo',
                    ['Weight'] = 1.0,
                    ['ItemSound'] = 'gear_generic',
                    ['Height'] = 1,
                    ['Width'] = 1
                },
                ['_type'] = 'Item'
            }
        },
        ['weed_skunk_seed'] = {
            ['item'] = {
                ['_id'] = 'weed_skunk_seed',
                ['_parent'] = '',
                ['_name'] = 'weed_skunk_seed',
                ['_data'] = {
                    ['ExamineTime'] = 1.0,
                    ['Rarity'] = 'common',
                    ['Label'] = 'Skunk Seed',
                    ['MaxStack'] = 100,
                    ['Itemimage'] = 'icons/weed_seed.png',
                    ['Type'] = 'Item',
                    ['Backgroundcolor'] = 'black',
                    ['Description'] = 'A weed seed of Skunk',
                    ['Name'] = 'weed_skunk_seed',
                    ['Weight'] = 0.0,
                    ['ItemSound'] = 'gear_generic',
                    ['Height'] = 1,
                    ['Width'] = 1
                },
                ['_type'] = 'Item'
            }
        },
        ['printerdocument'] = {
            ['item'] = {
                ['_id'] = 'printerdocument',
                ['_parent'] = '',
                ['_name'] = 'printerdocument',
                ['_data'] = {
                    ['ExamineTime'] = 1.0,
                    ['Rarity'] = 'common',
                    ['Label'] = 'Document',
                    ['MaxStack'] = 1,
                    ['Itemimage'] = 'icons/printerdocument.png',
                    ['Type'] = 'Item',
                    ['Backgroundcolor'] = 'black',
                    ['Description'] = 'A nice document',
                    ['Name'] = 'printerdocument',
                    ['Weight'] = 0.5,
                    ['ItemSound'] = 'gear_generic',
                    ['Height'] = 1,
                    ['Width'] = 1
                },
                ['_type'] = 'Item'
            }
        },
        ['weapon_briefcase'] = {
            ['item'] = {
                ['_id'] = 'weapon_briefcase',
                ['_parent'] = '',
                ['_name'] = 'weapon_briefcase',
                ['_data'] = {
                    ['ExamineTime'] = 1.0,
                    ['Rarity'] = 'rare',
                    ['Label'] = 'Briefcase',
                    ['MaxStack'] = 1,
                    ['Itemimage'] = 'icons/weapon_briefcase.png',
                    ['Type'] = 'Weapon',
                    ['Backgroundcolor'] = 'blue',
                    ['Description'] = 'A briefcase for storing important documents',
                    ['Name'] = 'weapon_briefcase',
                    ['Weight'] = 1.0,
                    ['ItemSound'] = 'gear_generic',
                    ['Height'] = 2,
                    ['Width'] = 2
                },
                ['_type'] = 'Item'
            }
        },
        ['bullpuprifle_luxuryfinish'] = {
            ['item'] = {
                ['_id'] = 'bullpuprifle_luxuryfinish',
                ['_parent'] = '',
                ['_name'] = 'bullpuprifle_luxuryfinish',
                ['_data'] = {
                    ['ExamineTime'] = 1.0,
                    ['Rarity'] = 'common',
                    ['Label'] = 'Rifle Finish',
                    ['MaxStack'] = 100,
                    ['Itemimage'] = 'icons/pistol_suppressor.png',
                    ['Type'] = 'Item',
                    ['Backgroundcolor'] = 'black',
                    ['Description'] = 'Bullpup Rifle Luxury Finish',
                    ['Name'] = 'bullpuprifle_luxuryfinish',
                    ['Weight'] = 1.0,
                    ['ItemSound'] = 'gear_generic',
                    ['Height'] = 1,
                    ['Width'] = 1
                },
                ['_type'] = 'Item'
            }
        },
        ['weapon_marksmanrifle'] = {
            ['item'] = {
                ['_id'] = 'weapon_marksmanrifle',
                ['_parent'] = '',
                ['_name'] = 'weapon_marksmanrifle',
                ['_data'] = {
                    ['ExamineTime'] = 1.0,
                    ['Rarity'] = 'rare',
                    ['Label'] = 'Marksman Rifle',
                    ['MaxStack'] = 1,
                    ['Itemimage'] = 'icons/weapon_marksmanrifle.png',
                    ['Type'] = 'Weapon',
                    ['Backgroundcolor'] = 'blue',
                    ['Description'] = 'A very accurate single-fire rifle',
                    ['Name'] = 'weapon_marksmanrifle',
                    ['Weight'] = 1.0,
                    ['ItemSound'] = 'gear_generic',
                    ['Height'] = 2,
                    ['Width'] = 2
                },
                ['_type'] = 'Item'
            }
        },
        ['bullpuprifle_defaultclip'] = {
            ['item'] = {
                ['_id'] = 'bullpuprifle_defaultclip',
                ['_parent'] = '',
                ['_name'] = 'bullpuprifle_defaultclip',
                ['_data'] = {
                    ['ExamineTime'] = 1.0,
                    ['Rarity'] = 'common',
                    ['Label'] = 'Rifle Clip',
                    ['MaxStack'] = 100,
                    ['Itemimage'] = 'icons/pistol_extendedclip.png',
                    ['Type'] = 'Item',
                    ['Backgroundcolor'] = 'black',
                    ['Description'] = 'Bullpup Rifle Default Clip',
                    ['Name'] = 'bullpuprifle_defaultclip',
                    ['Weight'] = 1.0,
                    ['ItemSound'] = 'gear_generic',
                    ['Height'] = 1,
                    ['Width'] = 1
                },
                ['_type'] = 'Item'
            }
        },
        ['smg_extendedclip'] = {
            ['item'] = {
                ['_id'] = 'smg_extendedclip',
                ['_parent'] = '',
                ['_name'] = 'smg_extendedclip',
                ['_data'] = {
                    ['ExamineTime'] = 1.0,
                    ['Rarity'] = 'common',
                    ['Label'] = 'SMG EXT Clip',
                    ['MaxStack'] = 100,
                    ['Itemimage'] = 'icons/pistol_extendedclip.png',
                    ['Type'] = 'Item',
                    ['Backgroundcolor'] = 'black',
                    ['Description'] = 'SMG Extended Clip',
                    ['Name'] = 'smg_extendedclip',
                    ['Weight'] = 1.0,
                    ['ItemSound'] = 'gear_generic',
                    ['Height'] = 1,
                    ['Width'] = 1
                },
                ['_type'] = 'Item'
            }
        },
        ['filled_evidence_bag'] = {
            ['item'] = {
                ['_id'] = 'filled_evidence_bag',
                ['_parent'] = '',
                ['_name'] = 'filled_evidence_bag',
                ['_data'] = {
                    ['ExamineTime'] = 1.0,
                    ['Rarity'] = 'common',
                    ['Label'] = 'Evidence Bag',
                    ['MaxStack'] = 1,
                    ['Itemimage'] = 'icons/evidence.png',
                    ['Type'] = 'Item',
                    ['Backgroundcolor'] = 'black',
                    ['Description'] = 'A filled evidence bag to see who committed the crime >:(',
                    ['Name'] = 'filled_evidence_bag',
                    ['Weight'] = 0.2,
                    ['ItemSound'] = 'gear_generic',
                    ['Height'] = 1,
                    ['Width'] = 1
                },
                ['_type'] = 'Item'
            }
        },
        ['iphone'] = {
            ['item'] = {
                ['_id'] = 'iphone',
                ['_parent'] = '',
                ['_name'] = 'iphone',
                ['_data'] = {
                    ['ExamineTime'] = 1.0,
                    ['Rarity'] = 'common',
                    ['Label'] = 'iPhone',
                    ['MaxStack'] = 100,
                    ['Itemimage'] = 'icons/iphone.png',
                    ['Type'] = 'Item',
                    ['Backgroundcolor'] = 'black',
                    ['Description'] = 'Very expensive phone',
                    ['Name'] = 'iphone',
                    ['Weight'] = 1.0,
                    ['ItemSound'] = 'gear_generic',
                    ['Height'] = 1,
                    ['Width'] = 1
                },
                ['_type'] = 'Item'
            }
        },
        ['coke_small_brick'] = {
            ['item'] = {
                ['_id'] = 'coke_small_brick',
                ['_parent'] = '',
                ['_name'] = 'coke_small_brick',
                ['_data'] = {
                    ['ExamineTime'] = 1.0,
                    ['Rarity'] = 'common',
                    ['Label'] = 'Coke Package',
                    ['MaxStack'] = 1,
                    ['Itemimage'] = 'icons/coke_small_brick.png',
                    ['Type'] = 'Item',
                    ['Backgroundcolor'] = 'black',
                    ['Description'] = 'Small package of cocaine, mostly used for deals and takes a lot of space',
                    ['Name'] = 'coke_small_brick',
                    ['Weight'] = 0.35,
                    ['ItemSound'] = 'gear_generic',
                    ['Height'] = 1,
                    ['Width'] = 1
                },
                ['_type'] = 'Item'
            }
        },
        ['revolver_bodyguardvariant'] = {
            ['item'] = {
                ['_id'] = 'revolver_bodyguardvariant',
                ['_parent'] = '',
                ['_name'] = 'revolver_bodyguardvariant',
                ['_data'] = {
                    ['ExamineTime'] = 1.0,
                    ['Rarity'] = 'common',
                    ['Label'] = 'Pistol Variant',
                    ['MaxStack'] = 100,
                    ['Itemimage'] = 'icons/pistol_suppressor.png',
                    ['Type'] = 'Item',
                    ['Backgroundcolor'] = 'black',
                    ['Description'] = 'Revovler Variant',
                    ['Name'] = 'revolver_bodyguardvariant',
                    ['Weight'] = 1.0,
                    ['ItemSound'] = 'gear_generic',
                    ['Height'] = 1,
                    ['Width'] = 1
                },
                ['_type'] = 'Item'
            }
        },
        ['weapon_garbagebag'] = {
            ['item'] = {
                ['_id'] = 'weapon_garbagebag',
                ['_parent'] = '',
                ['_name'] = 'weapon_garbagebag',
                ['_data'] = {
                    ['ExamineTime'] = 1.0,
                    ['Rarity'] = 'rare',
                    ['Label'] = 'Garbage Bag',
                    ['MaxStack'] = 1,
                    ['Itemimage'] = 'icons/weapon_garbagebag.png',
                    ['Type'] = 'Weapon',
                    ['Backgroundcolor'] = 'blue',
                    ['Description'] = 'A garbage bag',
                    ['Name'] = 'weapon_garbagebag',
                    ['Weight'] = 1.0,
                    ['ItemSound'] = 'gear_generic',
                    ['Height'] = 2,
                    ['Width'] = 2
                },
                ['_type'] = 'Item'
            }
        },
        ['machinepistol_drum'] = {
            ['item'] = {
                ['_id'] = 'machinepistol_drum',
                ['_parent'] = '',
                ['_name'] = 'machinepistol_drum',
                ['_data'] = {
                    ['ExamineTime'] = 1.0,
                    ['Rarity'] = 'common',
                    ['Label'] = 'SMG Drum',
                    ['MaxStack'] = 100,
                    ['Itemimage'] = 'icons/rifle_drummag.png',
                    ['Type'] = 'Item',
                    ['Backgroundcolor'] = 'black',
                    ['Description'] = 'Machine Pistol Drum',
                    ['Name'] = 'machinepistol_drum',
                    ['Weight'] = 1.0,
                    ['ItemSound'] = 'gear_generic',
                    ['Height'] = 1,
                    ['Width'] = 1
                },
                ['_type'] = 'Item'
            }
        },
        ['pistol50_extendedclip'] = {
            ['item'] = {
                ['_id'] = 'pistol50_extendedclip',
                ['_parent'] = '',
                ['_name'] = 'pistol50_extendedclip',
                ['_data'] = {
                    ['ExamineTime'] = 1.0,
                    ['Rarity'] = 'common',
                    ['Label'] = 'Pistol EXT Clip',
                    ['MaxStack'] = 100,
                    ['Itemimage'] = 'icons/pistol_extendedclip.png',
                    ['Type'] = 'Item',
                    ['Backgroundcolor'] = 'black',
                    ['Description'] = '.50 Pistol Extended Clip',
                    ['Name'] = 'pistol50_extendedclip',
                    ['Weight'] = 1.0,
                    ['ItemSound'] = 'gear_generic',
                    ['Height'] = 1,
                    ['Width'] = 1
                },
                ['_type'] = 'Item'
            }
        },
        ['advancedlockpick'] = {
            ['item'] = {
                ['_id'] = 'advancedlockpick',
                ['_parent'] = '',
                ['_name'] = 'advancedlockpick',
                ['_data'] = {
                    ['ExamineTime'] = 1.0,
                    ['Rarity'] = 'common',
                    ['Label'] = 'Advanced Lockpick',
                    ['MaxStack'] = 100,
                    ['Itemimage'] = 'icons/advancedlockpick.png',
                    ['Type'] = 'Item',
                    ['Backgroundcolor'] = 'black',
                    ['Description'] = 'If you lose your keys a lot this is very useful... Also useful to open your beers',
                    ['Name'] = 'advancedlockpick',
                    ['Weight'] = 0.5,
                    ['ItemSound'] = 'gear_generic',
                    ['Height'] = 1,
                    ['Width'] = 1
                },
                ['_type'] = 'Item'
            }
        },
        ['lockpick'] = {
            ['item'] = {
                ['_id'] = 'lockpick',
                ['_parent'] = '',
                ['_name'] = 'lockpick',
                ['_data'] = {
                    ['ExamineTime'] = 1.0,
                    ['Rarity'] = 'common',
                    ['Label'] = 'Lockpick',
                    ['MaxStack'] = 100,
                    ['Itemimage'] = 'icons/lockpick.png',
                    ['Type'] = 'Item',
                    ['Backgroundcolor'] = 'black',
                    ['Description'] = 'Very useful if you lose your keys a lot.. or if you want to use it for something else...',
                    ['Name'] = 'lockpick',
                    ['Weight'] = 0.3,
                    ['ItemSound'] = 'gear_generic',
                    ['Height'] = 1,
                    ['Width'] = 1
                },
                ['_type'] = 'Item'
            }
        },
        ['weed_ak47_seed'] = {
            ['item'] = {
                ['_id'] = 'weed_ak47_seed',
                ['_parent'] = '',
                ['_name'] = 'weed_ak47_seed',
                ['_data'] = {
                    ['ExamineTime'] = 1.0,
                    ['Rarity'] = 'common',
                    ['Label'] = 'AK47 Seed',
                    ['MaxStack'] = 100,
                    ['Itemimage'] = 'icons/weed_seed.png',
                    ['Type'] = 'Item',
                    ['Backgroundcolor'] = 'black',
                    ['Description'] = 'A weed seed of AK47',
                    ['Name'] = 'weed_ak47_seed',
                    ['Weight'] = 0.0,
                    ['ItemSound'] = 'gear_generic',
                    ['Height'] = 1,
                    ['Width'] = 1
                },
                ['_type'] = 'Item'
            }
        },
        ['weapon_golfclub'] = {
            ['item'] = {
                ['_id'] = 'weapon_golfclub',
                ['_parent'] = '',
                ['_name'] = 'weapon_golfclub',
                ['_data'] = {
                    ['ExamineTime'] = 1.0,
                    ['Rarity'] = 'rare',
                    ['Label'] = 'Golfclub',
                    ['MaxStack'] = 1,
                    ['Itemimage'] = 'icons/weapon_golfclub.png',
                    ['Type'] = 'Weapon',
                    ['Backgroundcolor'] = 'blue',
                    ['Description'] = 'A club used to hit the ball in golf',
                    ['Name'] = 'weapon_golfclub',
                    ['Weight'] = 1.0,
                    ['ItemSound'] = 'gear_generic',
                    ['Height'] = 2,
                    ['Width'] = 2
                },
                ['_type'] = 'Item'
            }
        },
        ['minismg_extendedclip'] = {
            ['item'] = {
                ['_id'] = 'minismg_extendedclip',
                ['_parent'] = '',
                ['_name'] = 'minismg_extendedclip',
                ['_data'] = {
                    ['ExamineTime'] = 1.0,
                    ['Rarity'] = 'common',
                    ['Label'] = 'SMG EXT Clip',
                    ['MaxStack'] = 100,
                    ['Itemimage'] = 'icons/pistol_extendedclip.png',
                    ['Type'] = 'Item',
                    ['Backgroundcolor'] = 'black',
                    ['Description'] = 'Mini SMG Extended Clip',
                    ['Name'] = 'minismg_extendedclip',
                    ['Weight'] = 1.0,
                    ['ItemSound'] = 'gear_generic',
                    ['Height'] = 1,
                    ['Width'] = 1
                },
                ['_type'] = 'Item'
            }
        },
        ['weapon_heavysniper'] = {
            ['item'] = {
                ['_id'] = 'weapon_heavysniper',
                ['_parent'] = '',
                ['_name'] = 'weapon_heavysniper',
                ['_data'] = {
                    ['ExamineTime'] = 1.0,
                    ['Rarity'] = 'rare',
                    ['Label'] = 'Heavy Sniper',
                    ['MaxStack'] = 1,
                    ['Itemimage'] = 'icons/weapon_heavysniper.png',
                    ['Type'] = 'Weapon',
                    ['Backgroundcolor'] = 'blue',
                    ['Description'] = 'An upgraded high-precision, long-range rifle',
                    ['Name'] = 'weapon_heavysniper',
                    ['Weight'] = 1.0,
                    ['ItemSound'] = 'gear_generic',
                    ['Height'] = 2,
                    ['Width'] = 2
                },
                ['_type'] = 'Item'
            }
        },
        ['machinepistol_defaultclip'] = {
            ['item'] = {
                ['_id'] = 'machinepistol_defaultclip',
                ['_parent'] = '',
                ['_name'] = 'machinepistol_defaultclip',
                ['_data'] = {
                    ['ExamineTime'] = 1.0,
                    ['Rarity'] = 'common',
                    ['Label'] = 'SMG Clip',
                    ['MaxStack'] = 100,
                    ['Itemimage'] = 'icons/pistol_extendedclip.png',
                    ['Type'] = 'Item',
                    ['Backgroundcolor'] = 'black',
                    ['Description'] = 'Machine Pistol Default Clip',
                    ['Name'] = 'machinepistol_defaultclip',
                    ['Weight'] = 1.0,
                    ['ItemSound'] = 'gear_generic',
                    ['Height'] = 1,
                    ['Width'] = 1
                },
                ['_type'] = 'Item'
            }
        },
        ['weapontint_army'] = {
            ['item'] = {
                ['_id'] = 'weapontint_army',
                ['_parent'] = '',
                ['_name'] = 'weapontint_army',
                ['_data'] = {
                    ['ExamineTime'] = 1.0,
                    ['Rarity'] = 'common',
                    ['Label'] = 'Army Tint',
                    ['MaxStack'] = 100,
                    ['Itemimage'] = 'icons/weapontint_army.png',
                    ['Type'] = 'Item',
                    ['Backgroundcolor'] = 'black',
                    ['Description'] = 'Army Weapon Tint',
                    ['Name'] = 'weapontint_army',
                    ['Weight'] = 1.0,
                    ['ItemSound'] = 'gear_generic',
                    ['Height'] = 1,
                    ['Width'] = 1
                },
                ['_type'] = 'Item'
            }
        },
        ['weapon_grenadelauncher'] = {
            ['item'] = {
                ['_id'] = 'weapon_grenadelauncher',
                ['_parent'] = '',
                ['_name'] = 'weapon_grenadelauncher',
                ['_data'] = {
                    ['ExamineTime'] = 1.0,
                    ['Rarity'] = 'rare',
                    ['Label'] = 'Grenade Launcher',
                    ['MaxStack'] = 1,
                    ['Itemimage'] = 'icons/weapon_grenadelauncher.png',
                    ['Type'] = 'Weapon',
                    ['Backgroundcolor'] = 'blue',
                    ['Description'] = 'A weapon that fires a specially-designed large-caliber projectile, often with an explosive, smoke or gas warhead',
                    ['Name'] = 'weapon_grenadelauncher',
                    ['Weight'] = 1.0,
                    ['ItemSound'] = 'gear_generic',
                    ['Height'] = 2,
                    ['Width'] = 2
                },
                ['_type'] = 'Item'
            }
        },
        ['copper'] = {
            ['item'] = {
                ['_id'] = 'copper',
                ['_parent'] = '',
                ['_name'] = 'copper',
                ['_data'] = {
                    ['ExamineTime'] = 1.0,
                    ['Rarity'] = 'common',
                    ['Label'] = 'Copper',
                    ['MaxStack'] = 100,
                    ['Itemimage'] = 'icons/copper.png',
                    ['Type'] = 'Item',
                    ['Backgroundcolor'] = 'black',
                    ['Description'] = 'Nice piece of metal that you can probably use for something',
                    ['Name'] = 'copper',
                    ['Weight'] = 0.1,
                    ['ItemSound'] = 'gear_generic',
                    ['Height'] = 1,
                    ['Width'] = 1
                },
                ['_type'] = 'Item'
            }
        },
        ['needle'] = {
            ['item'] = {
                ['_id'] = 'needle',
                ['_parent'] = '',
                ['_name'] = 'needle',
                ['_data'] = {
                    ['ExamineTime'] = 1.0,
                    ['Rarity'] = 'common',
                    ['Label'] = 'Needle',
                    ['MaxStack'] = 100,
                    ['Itemimage'] = 'icons/ifaks.png',
                    ['Type'] = 'Item',
                    ['Backgroundcolor'] = 'black',
                    ['Description'] = 'ifaks for healing and a complete stress remover.',
                    ['Name'] = 'needle',
                    ['Weight'] = 0.0,
                    ['ItemSound'] = 'gear_generic',
                    ['Height'] = 1,
                    ['Width'] = 1
                },
                ['_type'] = 'Item'
            }
        },
        ['trojan_usb'] = {
            ['item'] = {
                ['_id'] = 'trojan_usb',
                ['_parent'] = '',
                ['_name'] = 'trojan_usb',
                ['_data'] = {
                    ['ExamineTime'] = 1.0,
                    ['Rarity'] = 'common',
                    ['Label'] = 'Trojan USB',
                    ['MaxStack'] = 100,
                    ['Itemimage'] = 'icons/usb_device.png',
                    ['Type'] = 'Item',
                    ['Backgroundcolor'] = 'black',
                    ['Description'] = 'Handy software to shut down some systems',
                    ['Name'] = 'trojan_usb',
                    ['Weight'] = 0.0,
                    ['ItemSound'] = 'gear_generic',
                    ['Height'] = 1,
                    ['Width'] = 1
                },
                ['_type'] = 'Item'
            }
        },
        ['advancedrifle_luxuryfinish'] = {
            ['item'] = {
                ['_id'] = 'advancedrifle_luxuryfinish',
                ['_parent'] = '',
                ['_name'] = 'advancedrifle_luxuryfinish',
                ['_data'] = {
                    ['ExamineTime'] = 1.0,
                    ['Rarity'] = 'common',
                    ['Label'] = 'Rifle Finish',
                    ['MaxStack'] = 100,
                    ['Itemimage'] = 'icons/pistol_suppressor.png',
                    ['Type'] = 'Item',
                    ['Backgroundcolor'] = 'black',
                    ['Description'] = 'Advanced Rifle Luxury Finish',
                    ['Name'] = 'advancedrifle_luxuryfinish',
                    ['Weight'] = 1.0,
                    ['ItemSound'] = 'gear_generic',
                    ['Height'] = 1,
                    ['Width'] = 1
                },
                ['_type'] = 'Item'
            }
        },
        ['smg_defaultclip'] = {
            ['item'] = {
                ['_id'] = 'smg_defaultclip',
                ['_parent'] = '',
                ['_name'] = 'smg_defaultclip',
                ['_data'] = {
                    ['ExamineTime'] = 1.0,
                    ['Rarity'] = 'common',
                    ['Label'] = 'SMG Clip',
                    ['MaxStack'] = 100,
                    ['Itemimage'] = 'icons/pistol_extendedclip.png',
                    ['Type'] = 'Item',
                    ['Backgroundcolor'] = 'black',
                    ['Description'] = 'SMG Default Clip',
                    ['Name'] = 'smg_defaultclip',
                    ['Weight'] = 1.0,
                    ['ItemSound'] = 'gear_generic',
                    ['Height'] = 1,
                    ['Width'] = 1
                },
                ['_type'] = 'Item'
            }
        },
        ['lighter'] = {
            ['item'] = {
                ['_id'] = 'lighter',
                ['_parent'] = '',
                ['_name'] = 'lighter',
                ['_data'] = {
                    ['ExamineTime'] = 1.0,
                    ['Rarity'] = 'common',
                    ['Label'] = 'Lighter',
                    ['MaxStack'] = 100,
                    ['Itemimage'] = 'icons/lighter.png',
                    ['Type'] = 'Item',
                    ['Backgroundcolor'] = 'black',
                    ['Description'] = 'On new years eve a nice fire to stand next to',
                    ['Name'] = 'lighter',
                    ['Weight'] = 0.0,
                    ['ItemSound'] = 'gear_generic',
                    ['Height'] = 1,
                    ['Width'] = 1
                },
                ['_type'] = 'Item'
            }
        },
        ['aluminum'] = {
            ['item'] = {
                ['_id'] = 'aluminum',
                ['_parent'] = '',
                ['_name'] = 'aluminum',
                ['_data'] = {
                    ['ExamineTime'] = 1.0,
                    ['Rarity'] = 'common',
                    ['Label'] = 'Aluminium',
                    ['MaxStack'] = 100,
                    ['Itemimage'] = 'icons/aluminum.png',
                    ['Type'] = 'Item',
                    ['Backgroundcolor'] = 'black',
                    ['Description'] = 'Nice piece of metal that you can probably use for something',
                    ['Name'] = 'aluminum',
                    ['Weight'] = 0.1,
                    ['ItemSound'] = 'gear_generic',
                    ['Height'] = 1,
                    ['Width'] = 1
                },
                ['_type'] = 'Item'
            }
        },
        ['weapon_proxmine'] = {
            ['item'] = {
                ['_id'] = 'weapon_proxmine',
                ['_parent'] = '',
                ['_name'] = 'weapon_proxmine',
                ['_data'] = {
                    ['ExamineTime'] = 1.0,
                    ['Rarity'] = 'rare',
                    ['Label'] = 'Proxmine Grenade',
                    ['MaxStack'] = 1,
                    ['Itemimage'] = 'icons/weapon_proximitymine.png',
                    ['Type'] = 'Weapon',
                    ['Backgroundcolor'] = 'blue',
                    ['Description'] = 'A bomb placed on the ground that detonates when going within its proximity',
                    ['Name'] = 'weapon_proxmine',
                    ['Weight'] = 1.0,
                    ['ItemSound'] = 'gear_generic',
                    ['Height'] = 2,
                    ['Width'] = 2
                },
                ['_type'] = 'Item'
            }
        },
        ['weapon_dbshotgun'] = {
            ['item'] = {
                ['_id'] = 'weapon_dbshotgun',
                ['_parent'] = '',
                ['_name'] = 'weapon_dbshotgun',
                ['_data'] = {
                    ['ExamineTime'] = 1.0,
                    ['Rarity'] = 'rare',
                    ['Label'] = 'Double-barrel Shotgun',
                    ['MaxStack'] = 1,
                    ['Itemimage'] = 'icons/weapon_dbshotgun.png',
                    ['Type'] = 'Weapon',
                    ['Backgroundcolor'] = 'blue',
                    ['Description'] = 'A shotgun with two parallel barrels, allowing two single shots to be fired in quick succession',
                    ['Name'] = 'weapon_dbshotgun',
                    ['Weight'] = 1.0,
                    ['ItemSound'] = 'gear_generic',
                    ['Height'] = 2,
                    ['Width'] = 2
                },
                ['_type'] = 'Item'
            }
        },
        ['weed_purple-haze_seed'] = {
            ['item'] = {
                ['_id'] = 'weed_purple-haze_seed',
                ['_parent'] = '',
                ['_name'] = 'weed_purple-haze_seed',
                ['_data'] = {
                    ['ExamineTime'] = 1.0,
                    ['Rarity'] = 'common',
                    ['Label'] = 'Purple Haze Seed',
                    ['MaxStack'] = 100,
                    ['Itemimage'] = 'icons/weed_seed.png',
                    ['Type'] = 'Item',
                    ['Backgroundcolor'] = 'black',
                    ['Description'] = 'A weed seed of Purple Haze',
                    ['Name'] = 'weed_purple-haze_seed',
                    ['Weight'] = 0.0,
                    ['ItemSound'] = 'gear_generic',
                    ['Height'] = 1,
                    ['Width'] = 1
                },
                ['_type'] = 'Item'
            }
        },
        ['rolling_paper'] = {
            ['item'] = {
                ['_id'] = 'rolling_paper',
                ['_parent'] = '',
                ['_name'] = 'rolling_paper',
                ['_data'] = {
                    ['ExamineTime'] = 1.0,
                    ['Rarity'] = 'common',
                    ['Label'] = 'Rolling Paper',
                    ['MaxStack'] = 100,
                    ['Itemimage'] = 'icons/rolling_paper.png',
                    ['Type'] = 'Item',
                    ['Backgroundcolor'] = 'black',
                    ['Description'] = 'Paper made specifically for encasing and smoking tobacco or cannabis.',
                    ['Name'] = 'rolling_paper',
                    ['Weight'] = 0.0,
                    ['ItemSound'] = 'gear_generic',
                    ['Height'] = 1,
                    ['Width'] = 1
                },
                ['_type'] = 'Item'
            }
        },
        ['weapon_bottle'] = {
            ['item'] = {
                ['_id'] = 'weapon_bottle',
                ['_parent'] = '',
                ['_name'] = 'weapon_bottle',
                ['_data'] = {
                    ['ExamineTime'] = 1.0,
                    ['Rarity'] = 'rare',
                    ['Label'] = 'Broken Bottle',
                    ['MaxStack'] = 1,
                    ['Itemimage'] = 'icons/weapon_bottle.png',
                    ['Type'] = 'Weapon',
                    ['Backgroundcolor'] = 'blue',
                    ['Description'] = 'A broken bottle',
                    ['Name'] = 'weapon_bottle',
                    ['Weight'] = 1.0,
                    ['ItemSound'] = 'gear_generic',
                    ['Height'] = 2,
                    ['Width'] = 2
                },
                ['_type'] = 'Item'
            }
        },
        ['pistol50_luxuryfinish'] = {
            ['item'] = {
                ['_id'] = 'pistol50_luxuryfinish',
                ['_parent'] = '',
                ['_name'] = 'pistol50_luxuryfinish',
                ['_data'] = {
                    ['ExamineTime'] = 1.0,
                    ['Rarity'] = 'common',
                    ['Label'] = 'Pistol Finish',
                    ['MaxStack'] = 100,
                    ['Itemimage'] = 'icons/pistol_suppressor.png',
                    ['Type'] = 'Item',
                    ['Backgroundcolor'] = 'black',
                    ['Description'] = '.50 Pistol Luxury Finish',
                    ['Name'] = 'pistol50_luxuryfinish',
                    ['Weight'] = 1.0,
                    ['ItemSound'] = 'gear_generic',
                    ['Height'] = 1,
                    ['Width'] = 1
                },
                ['_type'] = 'Item'
            }
        },
        ['weed_amnesia_seed'] = {
            ['item'] = {
                ['_id'] = 'weed_amnesia_seed',
                ['_parent'] = '',
                ['_name'] = 'weed_amnesia_seed',
                ['_data'] = {
                    ['ExamineTime'] = 1.0,
                    ['Rarity'] = 'common',
                    ['Label'] = 'Amnesia Seed',
                    ['MaxStack'] = 100,
                    ['Itemimage'] = 'icons/weed_seed.png',
                    ['Type'] = 'Item',
                    ['Backgroundcolor'] = 'black',
                    ['Description'] = 'A weed seed of Amnesia',
                    ['Name'] = 'weed_amnesia_seed',
                    ['Weight'] = 0.0,
                    ['ItemSound'] = 'gear_generic',
                    ['Height'] = 1,
                    ['Width'] = 1
                },
                ['_type'] = 'Item'
            }
        },
        ['weapon_revolver'] = {
            ['item'] = {
                ['_id'] = 'weapon_revolver',
                ['_parent'] = '',
                ['_name'] = 'weapon_revolver',
                ['_data'] = {
                    ['ExamineTime'] = 1.0,
                    ['Rarity'] = 'rare',
                    ['Label'] = 'Revolver',
                    ['MaxStack'] = 1,
                    ['Itemimage'] = 'icons/weapon_revolver.png',
                    ['Type'] = 'Weapon',
                    ['Backgroundcolor'] = 'blue',
                    ['Description'] = 'A pistol with revolving chambers enabling several shots to be fired without reloading',
                    ['Name'] = 'weapon_revolver',
                    ['Weight'] = 1.0,
                    ['ItemSound'] = 'gear_generic',
                    ['Height'] = 2,
                    ['Width'] = 2
                },
                ['_type'] = 'Item'
            }
        },
        ['assaultshotgun_defaultclip'] = {
            ['item'] = {
                ['_id'] = 'assaultshotgun_defaultclip',
                ['_parent'] = '',
                ['_name'] = 'assaultshotgun_defaultclip',
                ['_data'] = {
                    ['ExamineTime'] = 1.0,
                    ['Rarity'] = 'common',
                    ['Label'] = 'Shotgun Clip',
                    ['MaxStack'] = 100,
                    ['Itemimage'] = 'icons/pistol_extendedclip.png',
                    ['Type'] = 'Item',
                    ['Backgroundcolor'] = 'black',
                    ['Description'] = 'Assault Shotgun Default Clip',
                    ['Name'] = 'assaultshotgun_defaultclip',
                    ['Weight'] = 1.0,
                    ['ItemSound'] = 'gear_generic',
                    ['Height'] = 1,
                    ['Width'] = 1
                },
                ['_type'] = 'Item'
            }
        },
        ['weapon_switchblade'] = {
            ['item'] = {
                ['_id'] = 'weapon_switchblade',
                ['_parent'] = '',
                ['_name'] = 'weapon_switchblade',
                ['_data'] = {
                    ['ExamineTime'] = 1.0,
                    ['Rarity'] = 'rare',
                    ['Label'] = 'Switchblade',
                    ['MaxStack'] = 1,
                    ['Itemimage'] = 'icons/weapon_switchblade.png',
                    ['Type'] = 'Weapon',
                    ['Backgroundcolor'] = 'blue',
                    ['Description'] = 'A knife with a blade that springs out from the handle when a button is pressed',
                    ['Name'] = 'weapon_switchblade',
                    ['Weight'] = 1.0,
                    ['ItemSound'] = 'gear_generic',
                    ['Height'] = 2,
                    ['Width'] = 2
                },
                ['_type'] = 'Item'
            }
        },
        ['nitrous'] = {
            ['item'] = {
                ['_id'] = 'nitrous',
                ['_parent'] = '',
                ['_name'] = 'nitrous',
                ['_data'] = {
                    ['ExamineTime'] = 1.0,
                    ['Rarity'] = 'common',
                    ['Label'] = 'Nitrous',
                    ['MaxStack'] = 100,
                    ['Itemimage'] = 'icons/nitrous.png',
                    ['Type'] = 'Item',
                    ['Backgroundcolor'] = 'black',
                    ['Description'] = 'Speed up, gas pedal! :D',
                    ['Name'] = 'nitrous',
                    ['Weight'] = 1.0,
                    ['ItemSound'] = 'gear_generic',
                    ['Height'] = 1,
                    ['Width'] = 1
                },
                ['_type'] = 'Item'
            }
        },
        ['weapon_pistol'] = {
            ['item'] = {
                ['_id'] = 'weapon_pistol',
                ['_parent'] = '',
                ['_name'] = 'weapon_pistol',
                ['_data'] = {
                    ['ExamineTime'] = 1.0,
                    ['Rarity'] = 'rare',
                    ['Label'] = 'Walther P99',
                    ['MaxStack'] = 1,
                    ['Itemimage'] = 'icons/weapon_pistol.png',
                    ['Type'] = 'Weapon',
                    ['Backgroundcolor'] = 'blue',
                    ['Description'] = 'A small firearm designed to be held in one hand',
                    ['Name'] = 'weapon_pistol',
                    ['Weight'] = 1.0,
                    ['ItemSound'] = 'gear_generic',
                    ['Height'] = 1,
                    ['Width'] = 2,
                },
                ['_type'] = 'Item'
            }
        },
        ['microsmg_luxuryfinish'] = {
            ['item'] = {
                ['_id'] = 'microsmg_luxuryfinish',
                ['_parent'] = '',
                ['_name'] = 'microsmg_luxuryfinish',
                ['_data'] = {
                    ['ExamineTime'] = 1.0,
                    ['Rarity'] = 'common',
                    ['Label'] = 'SMG Finish',
                    ['MaxStack'] = 100,
                    ['Itemimage'] = 'icons/pistol_suppressor.png',
                    ['Type'] = 'Item',
                    ['Backgroundcolor'] = 'black',
                    ['Description'] = 'Micro SMG Luxury Finish',
                    ['Name'] = 'microsmg_luxuryfinish',
                    ['Weight'] = 1.0,
                    ['ItemSound'] = 'gear_generic',
                    ['Height'] = 1,
                    ['Width'] = 1
                },
                ['_type'] = 'Item'
            }
        },
        ['weapon_poolcue'] = {
            ['item'] = {
                ['_id'] = 'weapon_poolcue',
                ['_parent'] = '',
                ['_name'] = 'weapon_poolcue',
                ['_data'] = {
                    ['ExamineTime'] = 1.0,
                    ['Rarity'] = 'rare',
                    ['Label'] = 'Poolcue',
                    ['MaxStack'] = 1,
                    ['Itemimage'] = 'icons/weapon_poolcue.png',
                    ['Type'] = 'Weapon',
                    ['Backgroundcolor'] = 'blue',
                    ['Description'] = 'A stick used to strike a ball, usually the cue ball (or other things)',
                    ['Name'] = 'weapon_poolcue',
                    ['Weight'] = 1.0,
                    ['ItemSound'] = 'gear_generic',
                    ['Height'] = 2,
                    ['Width'] = 2
                },
                ['_type'] = 'Item'
            }
        },
        ['printerdocument2'] = {
            ['item'] = {
                ['_id'] = 'printerdocument2',
                ['_parent'] = '',
                ['_name'] = 'printerdocument2',
                ['_data'] = {
                    ['ExamineTime'] = 1.0,
                    ['Rarity'] = 'common',
                    ['Label'] = 'Document2',
                    ['MaxStack'] = 1,
                    ['Itemimage'] = 'icons/printerdocument.png',
                    ['Type'] = 'Item',
                    ['Backgroundcolor'] = 'black',
                    ['Description'] = 'A nice document',
                    ['Name'] = 'printerdocument2',
                    ['Weight'] = 0.5,
                    ['ItemSound'] = 'gear_generic',
                    ['Height'] = 1,
                    ['Width'] = 1
                },
                ['_type'] = 'Item'
            }
        },
        ['weapon_pumpshotgun'] = {
            ['item'] = {
                ['_id'] = 'weapon_pumpshotgun',
                ['_parent'] = '',
                ['_name'] = 'weapon_pumpshotgun',
                ['_data'] = {
                    ['ExamineTime'] = 1.0,
                    ['Rarity'] = 'rare',
                    ['Label'] = 'Pump Shotgun',
                    ['MaxStack'] = 1,
                    ['Itemimage'] = 'icons/weapon_pumpshotgun.png',
                    ['Type'] = 'Weapon',
                    ['Backgroundcolor'] = 'blue',
                    ['Description'] = 'A pump-action smoothbore gun for firing small shot at short range',
                    ['Name'] = 'weapon_pumpshotgun',
                    ['Weight'] = 1.0,
                    ['ItemSound'] = 'gear_generic',
                    ['Height'] = 2,
                    ['Width'] = 2
                },
                ['_type'] = 'Item'
            }
        },
        ['weapon_carbinerifle_mk2'] = {
            ['item'] = {
                ['_id'] = 'weapon_carbinerifle_mk2',
                ['_parent'] = '',
                ['_name'] = 'weapon_carbinerifle_mk2',
                ['_data'] = {
                    ['ExamineTime'] = 1.0,
                    ['Rarity'] = 'rare',
                    ['Label'] = 'Carbine Rifle Mk II',
                    ['MaxStack'] = 1,
                    ['Itemimage'] = 'icons/weapon_carbinerifle_mk2.png',
                    ['Type'] = 'Weapon',
                    ['Backgroundcolor'] = 'blue',
                    ['Description'] = 'Carbine Rifle MK2',
                    ['Name'] = 'weapon_carbinerifle_mk2',
                    ['Weight'] = 1.0,
                    ['ItemSound'] = 'gear_generic',
                    ['Height'] = 2,
                    ['Width'] = 3
                },
                ['_type'] = 'Item'
            }
        },
        ['bag'] = {
            ['item'] = {
                ['_id'] = 'bag',
                ['_parent'] = '',
                ['_name'] = 'clothing_bag',
                ['_data'] = {
                    ['ExamineTime'] = 1.0,
                    ['Rarity'] = 'common',
                    ['Label'] = 'Bag',
                    ['MaxStack'] = 1,
                    ['Itemimage'] = 'icons/bag.png',
                    ['Type'] = 'Clothing',
                    ['Backgroundcolor'] = 'black',
                    ['Description'] = 'A cloth',
                    ['Name'] = 'bag',
                    ['Weight'] = 0.5,
                    ['ItemSound'] = 'gear_generic',
                    ['Height'] = 1,
                    ['Width'] = 1
                },
                ['_type'] = 'Item'
            }
        },
        ['microsmg_extendedclip'] = {
            ['item'] = {
                ['_id'] = 'microsmg_extendedclip',
                ['_parent'] = '',
                ['_name'] = 'microsmg_extendedclip',
                ['_data'] = {
                    ['ExamineTime'] = 1.0,
                    ['Rarity'] = 'common',
                    ['Label'] = 'SMG EXT Clip',
                    ['MaxStack'] = 100,
                    ['Itemimage'] = 'icons/pistol_extendedclip.png',
                    ['Type'] = 'Item',
                    ['Backgroundcolor'] = 'black',
                    ['Description'] = 'Micro SMG Extended Clip',
                    ['Name'] = 'microsmg_extendedclip',
                    ['Weight'] = 1.0,
                    ['ItemSound'] = 'gear_generic',
                    ['Height'] = 1,
                    ['Width'] = 1
                },
                ['_type'] = 'Item'
            }
        },
        ['rifle_ammo'] = {
            ['item'] = {
                ['_id'] = 'rifle_ammo',
                ['_parent'] = '',
                ['_name'] = 'rifle_ammo',
                ['_data'] = {
                    ['ExamineTime'] = 1.0,
                    ['Rarity'] = 'common',
                    ['Label'] = 'Rifle ammo',
                    ['MaxStack'] = 100,
                    ['Itemimage'] = 'icons/rifle_ammo.png',
                    ['Type'] = 'Item',
                    ['Backgroundcolor'] = 'black',
                    ['Description'] = 'Ammo for Rifles',
                    ['Name'] = 'rifle_ammo',
                    ['Weight'] = 1.0,
                    ['ItemSound'] = 'gear_generic',
                    ['Height'] = 1,
                    ['Width'] = 1
                },
                ['_type'] = 'Item'
            }
        },
        ['shirt'] = {
            ['item'] = {
                ['_id'] = 'shirt',
                ['_parent'] = '',
                ['_name'] = 'clothing_shirt',
                ['_data'] = {
                    ['ExamineTime'] = 1.0,
                    ['Rarity'] = 'common',
                    ['Label'] = 'Shirt',
                    ['MaxStack'] = 1,
                    ['Itemimage'] = 'icons/shirt.png',
                    ['Type'] = 'Clothing',
                    ['Backgroundcolor'] = 'black',
                    ['Description'] = 'A cloth',
                    ['Name'] = 'shirt',
                    ['Weight'] = 0.5,
                    ['ItemSound'] = 'gear_generic',
                    ['Height'] = 1,
                    ['Width'] = 1
                },
                ['_type'] = 'Item'
            }
        },
        ['combatpdw_defaultclip'] = {
            ['item'] = {
                ['_id'] = 'combatpdw_defaultclip',
                ['_parent'] = '',
                ['_name'] = 'combatpdw_defaultclip',
                ['_data'] = {
                    ['ExamineTime'] = 1.0,
                    ['Rarity'] = 'common',
                    ['Label'] = 'SMG Clip',
                    ['MaxStack'] = 100,
                    ['Itemimage'] = 'icons/pistol_extendedclip.png',
                    ['Type'] = 'Item',
                    ['Backgroundcolor'] = 'black',
                    ['Description'] = 'Combat PDW Default Clip',
                    ['Name'] = 'combatpdw_defaultclip',
                    ['Weight'] = 1.0,
                    ['ItemSound'] = 'gear_generic',
                    ['Height'] = 1,
                    ['Width'] = 1
                },
                ['_type'] = 'Item'
            }
        },
        ['weapon_marksmanpistol'] = {
            ['item'] = {
                ['_id'] = 'weapon_marksmanpistol',
                ['_parent'] = '',
                ['_name'] = 'weapon_marksmanpistol',
                ['_data'] = {
                    ['ExamineTime'] = 1.0,
                    ['Rarity'] = 'rare',
                    ['Label'] = 'Marksman Pistol',
                    ['MaxStack'] = 1,
                    ['Itemimage'] = 'icons/weapon_marksmanpistol.png',
                    ['Type'] = 'Weapon',
                    ['Backgroundcolor'] = 'blue',
                    ['Description'] = 'A very accurate small firearm designed to be held in one hand',
                    ['Name'] = 'weapon_marksmanpistol',
                    ['Weight'] = 1.0,
                    ['ItemSound'] = 'gear_generic',
                    ['Height'] = 2,
                    ['Width'] = 2
                },
                ['_type'] = 'Item'
            }
        },
        ['alchol'] = {
            ['item'] = {
                ['_id'] = 'alchol',
                ['_parent'] = '',
                ['_name'] = 'alchol',
                ['_data'] = {
                    ['ExamineTime'] = 1.0,
                    ['Rarity'] = 'common',
                    ['Label'] = 'Alchol',
                    ['MaxStack'] = 100,
                    ['Itemimage'] = 'icons/bandage.png',
                    ['Type'] = 'Item',
                    ['Backgroundcolor'] = 'black',
                    ['Description'] = 'Mmh alchol, tastes good!',
                    ['Name'] = 'alchol',
                    ['Weight'] = 0.0,
                    ['ItemSound'] = 'gear_generic',
                    ['Height'] = 1,
                    ['Width'] = 1
                },
                ['_type'] = 'Item'
            }
        },
        ['goldchain'] = {
            ['item'] = {
                ['_id'] = 'goldchain',
                ['_parent'] = '',
                ['_name'] = 'goldchain',
                ['_data'] = {
                    ['ExamineTime'] = 1.0,
                    ['Rarity'] = 'common',
                    ['Label'] = 'Golden Chain',
                    ['MaxStack'] = 100,
                    ['Itemimage'] = 'icons/goldchain.png',
                    ['Type'] = 'Item',
                    ['Backgroundcolor'] = 'black',
                    ['Description'] = 'A golden chain seems like the jackpot to me!',
                    ['Name'] = 'goldchain',
                    ['Weight'] = 1.5,
                    ['ItemSound'] = 'gear_generic',
                    ['Height'] = 1,
                    ['Width'] = 1
                },
                ['_type'] = 'Item'
            }
        },
        ['weapon_combatpistol'] = {
            ['item'] = {
                ['_id'] = 'weapon_combatpistol',
                ['_parent'] = '',
                ['_name'] = 'weapon_combatpistol',
                ['_data'] = {
                    ['ExamineTime'] = 1.0,
                    ['Rarity'] = 'rare',
                    ['Label'] = 'Combat Pistol',
                    ['MaxStack'] = 1,
                    ['Itemimage'] = 'icons/weapon_combatpistol.png',
                    ['Type'] = 'Weapon',
                    ['Backgroundcolor'] = 'blue',
                    ['Description'] = 'A combat version small firearm designed to be held in one hand',
                    ['Name'] = 'weapon_combatpistol',
                    ['Weight'] = 1.0,
                    ['ItemSound'] = 'gear_generic',
                    ['Height'] = 1,
                    ['Width'] = 2
                },
                ['_type'] = 'Item'
            }
        },
        ['radioscanner'] = {
            ['item'] = {
                ['_id'] = 'radioscanner',
                ['_parent'] = '',
                ['_name'] = 'radioscanner',
                ['_data'] = {
                    ['ExamineTime'] = 1.0,
                    ['Rarity'] = 'common',
                    ['Label'] = 'Radio Scanner',
                    ['MaxStack'] = 100,
                    ['Itemimage'] = 'icons/radioscanner.png',
                    ['Type'] = 'Item',
                    ['Backgroundcolor'] = 'black',
                    ['Description'] = 'With this you can get some police alerts. Not 100% effective however',
                    ['Name'] = 'radioscanner',
                    ['Weight'] = 1.0,
                    ['ItemSound'] = 'gear_generic',
                    ['Height'] = 1,
                    ['Width'] = 1
                },
                ['_type'] = 'Item'
            }
        },
        ['marksmanrifle_luxuryfinish'] = {
            ['item'] = {
                ['_id'] = 'marksmanrifle_luxuryfinish',
                ['_parent'] = '',
                ['_name'] = 'marksmanrifle_luxuryfinish',
                ['_data'] = {
                    ['ExamineTime'] = 1.0,
                    ['Rarity'] = 'common',
                    ['Label'] = 'Sniper Finish',
                    ['MaxStack'] = 100,
                    ['Itemimage'] = 'icons/pistol_suppressor.png',
                    ['Type'] = 'Item',
                    ['Backgroundcolor'] = 'black',
                    ['Description'] = 'Marksman Rifle Luxury Finish',
                    ['Name'] = 'marksmanrifle_luxuryfinish',
                    ['Weight'] = 1.0,
                    ['ItemSound'] = 'gear_generic',
                    ['Height'] = 1,
                    ['Width'] = 1
                },
                ['_type'] = 'Item'
            }
        },
        ['weapon_machete'] = {
            ['item'] = {
                ['_id'] = 'weapon_machete',
                ['_parent'] = '',
                ['_name'] = 'weapon_machete',
                ['_data'] = {
                    ['ExamineTime'] = 1.0,
                    ['Rarity'] = 'rare',
                    ['Label'] = 'Machete',
                    ['MaxStack'] = 1,
                    ['Itemimage'] = 'icons/weapon_machete.png',
                    ['Type'] = 'Weapon',
                    ['Backgroundcolor'] = 'blue',
                    ['Description'] = 'A broad, heavy knife used as a weapon',
                    ['Name'] = 'weapon_machete',
                    ['Weight'] = 1.0,
                    ['ItemSound'] = 'gear_generic',
                    ['Height'] = 2,
                    ['Width'] = 2
                },
                ['_type'] = 'Item'
            }
        },
        ['weapon_stone_hatchet'] = {
            ['item'] = {
                ['_id'] = 'weapon_stone_hatchet',
                ['_parent'] = '',
                ['_name'] = 'weapon_stone_hatchet',
                ['_data'] = {
                    ['ExamineTime'] = 1.0,
                    ['Rarity'] = 'rare',
                    ['Label'] = 'Stone Hatchet',
                    ['MaxStack'] = 1,
                    ['Itemimage'] = 'icons/weapon_stone_hatchet.png',
                    ['Type'] = 'Weapon',
                    ['Backgroundcolor'] = 'blue',
                    ['Description'] = 'Stone ax',
                    ['Name'] = 'weapon_stone_hatchet',
                    ['Weight'] = 1.0,
                    ['ItemSound'] = 'gear_generic',
                    ['Height'] = 2,
                    ['Width'] = 2
                },
                ['_type'] = 'Item'
            }
        },
        ['weapon_carbinerifle'] = {
            ['item'] = {
                ['_id'] = 'weapon_carbinerifle',
                ['_parent'] = '',
                ['_name'] = 'weapon_carbinerifle',
                ['_data'] = {
                    ['ExamineTime'] = 1.0,
                    ['Rarity'] = 'rare',
                    ['Label'] = 'Carbine Rifle',
                    ['MaxStack'] = 1,
                    ['Itemimage'] = 'icons/weapon_carbinerifle.png',
                    ['Type'] = 'Weapon',
                    ['Backgroundcolor'] = 'blue',
                    ['Description'] = 'A lightweight automatic rifle',
                    ['Name'] = 'weapon_carbinerifle',
                    ['Weight'] = 1.0,
                    ['ItemSound'] = 'gear_generic',
                    ['Height'] = 2,
                    ['Width'] = 3
                },
                ['_type'] = 'Item'
            }
        },
        ['weapon_grenade'] = {
            ['item'] = {
                ['_id'] = 'weapon_grenade',
                ['_parent'] = '',
                ['_name'] = 'weapon_grenade',
                ['_data'] = {
                    ['ExamineTime'] = 1.0,
                    ['Rarity'] = 'rare',
                    ['Label'] = 'Grenade',
                    ['MaxStack'] = 1,
                    ['Itemimage'] = 'icons/weapon_grenade.png',
                    ['Type'] = 'Weapon',
                    ['Backgroundcolor'] = 'blue',
                    ['Description'] = 'A handheld throwable bomb',
                    ['Name'] = 'weapon_grenade',
                    ['Weight'] = 1.0,
                    ['ItemSound'] = 'gear_generic',
                    ['Height'] = 2,
                    ['Width'] = 2
                },
                ['_type'] = 'Item'
            }
        },
        ['assaultrifle_luxuryfinish'] = {
            ['item'] = {
                ['_id'] = 'assaultrifle_luxuryfinish',
                ['_parent'] = '',
                ['_name'] = 'assaultrifle_luxuryfinish',
                ['_data'] = {
                    ['ExamineTime'] = 1.0,
                    ['Rarity'] = 'common',
                    ['Label'] = 'Rifle Finish',
                    ['MaxStack'] = 100,
                    ['Itemimage'] = 'icons/pistol_suppressor.png',
                    ['Type'] = 'Item',
                    ['Backgroundcolor'] = 'black',
                    ['Description'] = 'Assault Rifle Luxury Finish',
                    ['Name'] = 'assaultrifle_luxuryfinish',
                    ['Weight'] = 1.0,
                    ['ItemSound'] = 'gear_generic',
                    ['Height'] = 1,
                    ['Width'] = 1
                },
                ['_type'] = 'Item'
            }
        },
        ['weed_skunk'] = {
            ['item'] = {
                ['_id'] = 'weed_skunk',
                ['_parent'] = '',
                ['_name'] = 'weed_skunk',
                ['_data'] = {
                    ['ExamineTime'] = 1.0,
                    ['Rarity'] = 'common',
                    ['Label'] = 'Skunk 2g',
                    ['MaxStack'] = 100,
                    ['Itemimage'] = 'icons/weed_baggy.png',
                    ['Type'] = 'Item',
                    ['Backgroundcolor'] = 'black',
                    ['Description'] = 'A weed bag with 2g Skunk',
                    ['Name'] = 'weed_skunk',
                    ['Weight'] = 0.2,
                    ['ItemSound'] = 'gear_generic',
                    ['Height'] = 1,
                    ['Width'] = 1
                },
                ['_type'] = 'Item'
            }
        },
        ['goldbar'] = {
            ['item'] = {
                ['_id'] = 'goldbar',
                ['_parent'] = '',
                ['_name'] = 'goldbar',
                ['_data'] = {
                    ['ExamineTime'] = 1.0,
                    ['Rarity'] = 'common',
                    ['Label'] = 'Gold Bar',
                    ['MaxStack'] = 100,
                    ['Itemimage'] = 'icons/goldbar.png',
                    ['Type'] = 'Item',
                    ['Backgroundcolor'] = 'black',
                    ['Description'] = 'Looks pretty expensive to me',
                    ['Name'] = 'goldbar',
                    ['Weight'] = 7.0,
                    ['ItemSound'] = 'gear_generic',
                    ['Height'] = 1,
                    ['Width'] = 1
                },
                ['_type'] = 'Item'
            }
        },
        ['weed_amnesia'] = {
            ['item'] = {
                ['_id'] = 'weed_amnesia',
                ['_parent'] = '',
                ['_name'] = 'weed_amnesia',
                ['_data'] = {
                    ['ExamineTime'] = 1.0,
                    ['Rarity'] = 'common',
                    ['Label'] = 'Amnesia 2g',
                    ['MaxStack'] = 100,
                    ['Itemimage'] = 'icons/weed_baggy.png',
                    ['Type'] = 'Item',
                    ['Backgroundcolor'] = 'black',
                    ['Description'] = 'A weed bag with 2g Amnesia',
                    ['Name'] = 'weed_amnesia',
                    ['Weight'] = 0.2,
                    ['ItemSound'] = 'gear_generic',
                    ['Height'] = 1,
                    ['Width'] = 1
                },
                ['_type'] = 'Item'
            }
        },
        ['weapon_pistol_mk2'] = {
            ['item'] = {
                ['_id'] = 'weapon_pistol_mk2',
                ['_parent'] = '',
                ['_name'] = 'weapon_pistol_mk2',
                ['_data'] = {
                    ['ExamineTime'] = 1.0,
                    ['Rarity'] = 'rare',
                    ['Label'] = 'Pistol Mk II',
                    ['MaxStack'] = 1,
                    ['Itemimage'] = 'icons/weapon_pistol_mk2.png',
                    ['Type'] = 'Weapon',
                    ['Backgroundcolor'] = 'blue',
                    ['Description'] = 'An upgraded small firearm designed to be held in one hand',
                    ['Name'] = 'weapon_pistol_mk2',
                    ['Weight'] = 1.0,
                    ['ItemSound'] = 'gear_generic',
                    ['Height'] = 1,
                    ['Width'] = 2
                },
                ['_type'] = 'Item'
            }
        },
        ['specialcarbine_luxuryfinish'] = {
            ['item'] = {
                ['_id'] = 'specialcarbine_luxuryfinish',
                ['_parent'] = '',
                ['_name'] = 'specialcarbine_luxuryfinish',
                ['_data'] = {
                    ['ExamineTime'] = 1.0,
                    ['Rarity'] = 'common',
                    ['Label'] = 'Rifle Finish',
                    ['MaxStack'] = 100,
                    ['Itemimage'] = 'icons/pistol_suppressor.png',
                    ['Type'] = 'Item',
                    ['Backgroundcolor'] = 'black',
                    ['Description'] = 'Special Carbine Luxury Finish',
                    ['Name'] = 'specialcarbine_luxuryfinish',
                    ['Weight'] = 1.0,
                    ['ItemSound'] = 'gear_generic',
                    ['Height'] = 1,
                    ['Width'] = 1
                },
                ['_type'] = 'Item'
            }
        },
        ['advancedrepairkit'] = {
            ['item'] = {
                ['_id'] = 'advancedrepairkit',
                ['_parent'] = '',
                ['_name'] = 'advancedrepairkit',
                ['_data'] = {
                    ['ExamineTime'] = 1.0,
                    ['Rarity'] = 'common',
                    ['Label'] = 'Advanced Repairkit',
                    ['MaxStack'] = 100,
                    ['Itemimage'] = 'icons/advancedkit.png',
                    ['Type'] = 'Item',
                    ['Backgroundcolor'] = 'black',
                    ['Description'] = 'A nice toolbox with stuff to repair your vehicle',
                    ['Name'] = 'advancedrepairkit',
                    ['Weight'] = 4.0,
                    ['ItemSound'] = 'gear_generic',
                    ['Height'] = 1,
                    ['Width'] = 1
                },
                ['_type'] = 'Item'
            }
        },
        ['snp_ammo'] = {
            ['item'] = {
                ['_id'] = 'snp_ammo',
                ['_parent'] = '',
                ['_name'] = 'snp_ammo',
                ['_data'] = {
                    ['ExamineTime'] = 1.0,
                    ['Rarity'] = 'common',
                    ['Label'] = 'Sniper ammo',
                    ['MaxStack'] = 100,
                    ['Itemimage'] = 'icons/rifle_ammo.png',
                    ['Type'] = 'Item',
                    ['Backgroundcolor'] = 'black',
                    ['Description'] = 'Ammo for Sniper Rifles',
                    ['Name'] = 'snp_ammo',
                    ['Weight'] = 1.0,
                    ['ItemSound'] = 'gear_generic',
                    ['Height'] = 1,
                    ['Width'] = 1
                },
                ['_type'] = 'Item'
            }
        },
        ['weapon_hazardcan'] = {
            ['item'] = {
                ['_id'] = 'weapon_hazardcan',
                ['_parent'] = '',
                ['_name'] = 'weapon_hazardcan',
                ['_data'] = {
                    ['ExamineTime'] = 1.0,
                    ['Rarity'] = 'rare',
                    ['Label'] = 'Hazardous Jerry Can',
                    ['MaxStack'] = 1,
                    ['Itemimage'] = 'icons/weapon_hazardcan.png',
                    ['Type'] = 'Weapon',
                    ['Backgroundcolor'] = 'blue',
                    ['Description'] = 'Weapon Hazardcan',
                    ['Name'] = 'weapon_hazardcan',
                    ['Weight'] = 1.0,
                    ['ItemSound'] = 'gear_generic',
                    ['Height'] = 2,
                    ['Width'] = 2
                },
                ['_type'] = 'Item'
            }
        },
        ['weed_white-widow_seed'] = {
            ['item'] = {
                ['_id'] = 'weed_white-widow_seed',
                ['_parent'] = '',
                ['_name'] = 'weed_white-widow_seed',
                ['_data'] = {
                    ['ExamineTime'] = 1.0,
                    ['Rarity'] = 'common',
                    ['Label'] = 'White Widow Seed',
                    ['MaxStack'] = 100,
                    ['Itemimage'] = 'icons/weed_seed.png',
                    ['Type'] = 'Item',
                    ['Backgroundcolor'] = 'black',
                    ['Description'] = 'A weed seed of White Widow',
                    ['Name'] = 'weed_white-widow_seed',
                    ['Weight'] = 0.0,
                    ['ItemSound'] = 'gear_generic',
                    ['Height'] = 1,
                    ['Width'] = 1
                },
                ['_type'] = 'Item'
            }
        },
        ['weapon_crowbar'] = {
            ['item'] = {
                ['_id'] = 'weapon_crowbar',
                ['_parent'] = '',
                ['_name'] = 'weapon_crowbar',
                ['_data'] = {
                    ['ExamineTime'] = 1.0,
                    ['Rarity'] = 'rare',
                    ['Label'] = 'Crowbar',
                    ['MaxStack'] = 1,
                    ['Itemimage'] = 'icons/weapon_crowbar.png',
                    ['Type'] = 'Weapon',
                    ['Backgroundcolor'] = 'blue',
                    ['Description'] = 'An iron bar with a flattened end, used as a lever',
                    ['Name'] = 'weapon_crowbar',
                    ['Weight'] = 1.0,
                    ['ItemSound'] = 'gear_generic',
                    ['Height'] = 2,
                    ['Width'] = 2
                },
                ['_type'] = 'Item'
            }
        },
        ['combatpistol_extendedclip'] = {
            ['item'] = {
                ['_id'] = 'combatpistol_extendedclip',
                ['_parent'] = '',
                ['_name'] = 'combatpistol_extendedclip',
                ['_data'] = {
                    ['ExamineTime'] = 1.0,
                    ['Rarity'] = 'common',
                    ['Label'] = 'Pistol EXT Clip',
                    ['MaxStack'] = 100,
                    ['Itemimage'] = 'icons/pistol_extendedclip.png',
                    ['Type'] = 'Item',
                    ['Backgroundcolor'] = 'black',
                    ['Description'] = 'Combat Pistol Extended Clip',
                    ['Name'] = 'combatpistol_extendedclip',
                    ['Weight'] = 1.0,
                    ['ItemSound'] = 'gear_generic',
                    ['Height'] = 1,
                    ['Width'] = 1
                },
                ['_type'] = 'Item'
            }
        },
        ['glass'] = {
            ['item'] = {
                ['_id'] = 'glass',
                ['_parent'] = '',
                ['_name'] = 'glass',
                ['_data'] = {
                    ['ExamineTime'] = 1.0,
                    ['Rarity'] = 'common',
                    ['Label'] = 'Glass',
                    ['MaxStack'] = 100,
                    ['Itemimage'] = 'icons/glass.png',
                    ['Type'] = 'Item',
                    ['Backgroundcolor'] = 'black',
                    ['Description'] = 'It is very fragile, watch out',
                    ['Name'] = 'glass',
                    ['Weight'] = 0.1,
                    ['ItemSound'] = 'gear_generic',
                    ['Height'] = 1,
                    ['Width'] = 1
                },
                ['_type'] = 'Item'
            }
        },
        ['samsungphone'] = {
            ['item'] = {
                ['_id'] = 'samsungphone',
                ['_parent'] = '',
                ['_name'] = 'samsungphone',
                ['_data'] = {
                    ['ExamineTime'] = 1.0,
                    ['Rarity'] = 'common',
                    ['Label'] = 'Samsung S10',
                    ['MaxStack'] = 100,
                    ['Itemimage'] = 'icons/samsungphone.png',
                    ['Type'] = 'Item',
                    ['Backgroundcolor'] = 'black',
                    ['Description'] = 'Very expensive phone',
                    ['Name'] = 'samsungphone',
                    ['Weight'] = 1.0,
                    ['ItemSound'] = 'gear_generic',
                    ['Height'] = 1,
                    ['Width'] = 1
                },
                ['_type'] = 'Item'
            }
        },
        ['weapon_dagger'] = {
            ['item'] = {
                ['_id'] = 'weapon_dagger',
                ['_parent'] = '',
                ['_name'] = 'weapon_dagger',
                ['_data'] = {
                    ['ExamineTime'] = 1.0,
                    ['Rarity'] = 'rare',
                    ['Label'] = 'Dagger',
                    ['MaxStack'] = 1,
                    ['Itemimage'] = 'icons/weapon_dagger.png',
                    ['Type'] = 'Weapon',
                    ['Backgroundcolor'] = 'blue',
                    ['Description'] = 'A short knife with a pointed and edged blade, used as a weapon',
                    ['Name'] = 'weapon_dagger',
                    ['Weight'] = 1.0,
                    ['ItemSound'] = 'gear_generic',
                    ['Height'] = 2,
                    ['Width'] = 2
                },
                ['_type'] = 'Item'
            }
        },
        ['weapon_battleaxe'] = {
            ['item'] = {
                ['_id'] = 'weapon_battleaxe',
                ['_parent'] = '',
                ['_name'] = 'weapon_battleaxe',
                ['_data'] = {
                    ['ExamineTime'] = 1.0,
                    ['Rarity'] = 'rare',
                    ['Label'] = 'Battle Axe',
                    ['MaxStack'] = 1,
                    ['Itemimage'] = 'icons/weapon_battleaxe.png',
                    ['Type'] = 'Weapon',
                    ['Backgroundcolor'] = 'blue',
                    ['Description'] = 'A large broad-bladed axe used in ancient warfare',
                    ['Name'] = 'weapon_battleaxe',
                    ['Weight'] = 1.0,
                    ['ItemSound'] = 'gear_generic',
                    ['Height'] = 2,
                    ['Width'] = 2
                },
                ['_type'] = 'Item'
            }
        },
        ['coffee'] = {
            ['item'] = {
                ['_id'] = 'coffee',
                ['_parent'] = '',
                ['_name'] = 'coffee',
                ['_data'] = {
                    ['ExamineTime'] = 1.0,
                    ['Rarity'] = 'common',
                    ['Label'] = 'Coffee',
                    ['MaxStack'] = 100,
                    ['Itemimage'] = 'icons/coffee.png',
                    ['Type'] = 'Item',
                    ['Backgroundcolor'] = 'black',
                    ['Description'] = 'Pump 4 Caffeine',
                    ['Name'] = 'coffee',
                    ['Weight'] = 0.2,
                    ['ItemSound'] = 'gear_generic',
                    ['Height'] = 1,
                    ['Width'] = 1
                },
                ['_type'] = 'Item'
            }
        },
        ['heavysniper_defaultclip'] = {
            ['item'] = {
                ['_id'] = 'heavysniper_defaultclip',
                ['_parent'] = '',
                ['_name'] = 'heavysniper_defaultclip',
                ['_data'] = {
                    ['ExamineTime'] = 1.0,
                    ['Rarity'] = 'common',
                    ['Label'] = 'Sniper Clip',
                    ['MaxStack'] = 100,
                    ['Itemimage'] = 'icons/pistol_extendedclip.png',
                    ['Type'] = 'Item',
                    ['Backgroundcolor'] = 'black',
                    ['Description'] = 'Heavy Sniper Default Clip',
                    ['Name'] = 'heavysniper_defaultclip',
                    ['Weight'] = 1.0,
                    ['ItemSound'] = 'gear_generic',
                    ['Height'] = 1,
                    ['Width'] = 1
                },
                ['_type'] = 'Item'
            }
        },
        ['weapon_heavyshotgun'] = {
            ['item'] = {
                ['_id'] = 'weapon_heavyshotgun',
                ['_parent'] = '',
                ['_name'] = 'weapon_heavyshotgun',
                ['_data'] = {
                    ['ExamineTime'] = 1.0,
                    ['Rarity'] = 'rare',
                    ['Label'] = 'Heavy Shotgun',
                    ['MaxStack'] = 1,
                    ['Itemimage'] = 'icons/weapon_heavyshotgun.png',
                    ['Type'] = 'Weapon',
                    ['Backgroundcolor'] = 'blue',
                    ['Description'] = 'A large smoothbore gun for firing small shot at short range',
                    ['Name'] = 'weapon_heavyshotgun',
                    ['Weight'] = 1.0,
                    ['ItemSound'] = 'gear_generic',
                    ['Height'] = 2,
                    ['Width'] = 2
                },
                ['_type'] = 'Item'
            }
        },
        ['diamond'] = {
            ['item'] = {
                ['_id'] = 'diamond',
                ['_parent'] = '',
                ['_name'] = 'diamond',
                ['_data'] = {
                    ['ExamineTime'] = 1.0,
                    ['Rarity'] = 'common',
                    ['Label'] = 'Diamond',
                    ['MaxStack'] = 100,
                    ['Itemimage'] = 'icons/diamond.png',
                    ['Type'] = 'Item',
                    ['Backgroundcolor'] = 'black',
                    ['Description'] = 'A diamond seems like the jackpot to me!',
                    ['Name'] = 'diamond',
                    ['Weight'] = 1.0,
                    ['ItemSound'] = 'gear_generic',
                    ['Height'] = 1,
                    ['Width'] = 1
                },
                ['_type'] = 'Item'
            }
        },
        ['visa'] = {
            ['item'] = {
                ['_id'] = 'visa',
                ['_parent'] = '',
                ['_name'] = 'visa',
                ['_data'] = {
                    ['ExamineTime'] = 1.0,
                    ['Rarity'] = 'common',
                    ['Label'] = 'Visa Card',
                    ['MaxStack'] = 1,
                    ['Itemimage'] = 'icons/visacard.png',
                    ['Type'] = 'Item',
                    ['Backgroundcolor'] = 'black',
                    ['Description'] = 'Visa can be used via ATM',
                    ['Name'] = 'visa',
                    ['Weight'] = 0.0,
                    ['ItemSound'] = 'gear_generic',
                    ['Height'] = 1,
                    ['Width'] = 1
                },
                ['_type'] = 'Item'
            }
        },
        ['weapon_firework'] = {
            ['item'] = {
                ['_id'] = 'weapon_firework',
                ['_parent'] = '',
                ['_name'] = 'weapon_firework',
                ['_data'] = {
                    ['ExamineTime'] = 1.0,
                    ['Rarity'] = 'rare',
                    ['Label'] = 'Firework Launcher',
                    ['MaxStack'] = 1,
                    ['Itemimage'] = 'icons/weapon_firework.png',
                    ['Type'] = 'Weapon',
                    ['Backgroundcolor'] = 'blue',
                    ['Description'] = 'A device containing gunpowder and other combustible chemicals that causes a spectacular explosion when ignited',
                    ['Name'] = 'weapon_firework',
                    ['Weight'] = 1.0,
                    ['ItemSound'] = 'gear_generic',
                    ['Height'] = 2,
                    ['Width'] = 2
                },
                ['_type'] = 'Item'
            }
        },
        ['assaultsmg_extendedclip'] = {
            ['item'] = {
                ['_id'] = 'assaultsmg_extendedclip',
                ['_parent'] = '',
                ['_name'] = 'assaultsmg_extendedclip',
                ['_data'] = {
                    ['ExamineTime'] = 1.0,
                    ['Rarity'] = 'common',
                    ['Label'] = 'SMG EXT Clip',
                    ['MaxStack'] = 100,
                    ['Itemimage'] = 'icons/pistol_extendedclip.png',
                    ['Type'] = 'Item',
                    ['Backgroundcolor'] = 'black',
                    ['Description'] = 'Assault SMG Extended Clip',
                    ['Name'] = 'assaultsmg_extendedclip',
                    ['Weight'] = 1.0,
                    ['ItemSound'] = 'gear_generic',
                    ['Height'] = 1,
                    ['Width'] = 1
                },
                ['_type'] = 'Item'
            }
        },
        ['carbinerifle_extendedclip'] = {
            ['item'] = {
                ['_id'] = 'carbinerifle_extendedclip',
                ['_parent'] = '',
                ['_name'] = 'carbinerifle_extendedclip',
                ['_data'] = {
                    ['ExamineTime'] = 1.0,
                    ['Rarity'] = 'common',
                    ['Label'] = 'Rifle EXT Clip',
                    ['MaxStack'] = 100,
                    ['Itemimage'] = 'icons/pistol_extendedclip.png',
                    ['Type'] = 'Item',
                    ['Backgroundcolor'] = 'black',
                    ['Description'] = 'Carbine Rifle Extended Clip',
                    ['Name'] = 'carbinerifle_extendedclip',
                    ['Weight'] = 1.0,
                    ['ItemSound'] = 'gear_generic',
                    ['Height'] = 1,
                    ['Width'] = 1
                },
                ['_type'] = 'Item'
            }
        },
        ['whiskey'] = {
            ['item'] = {
                ['_id'] = 'whiskey',
                ['_parent'] = '',
                ['_name'] = 'whiskey',
                ['_data'] = {
                    ['ExamineTime'] = 1.0,
                    ['Rarity'] = 'common',
                    ['Label'] = 'Whiskey',
                    ['MaxStack'] = 100,
                    ['Itemimage'] = 'icons/whiskey.png',
                    ['Type'] = 'Item',
                    ['Backgroundcolor'] = 'black',
                    ['Description'] = 'For all the thirsty out there',
                    ['Name'] = 'whiskey',
                    ['Weight'] = 0.5,
                    ['ItemSound'] = 'gear_generic',
                    ['Height'] = 1,
                    ['Width'] = 1
                },
                ['_type'] = 'Item'
            }
        },
        ['weapon_smg_mk2'] = {
            ['item'] = {
                ['_id'] = 'weapon_smg_mk2',
                ['_parent'] = '',
                ['_name'] = 'weapon_smg_mk2',
                ['_data'] = {
                    ['ExamineTime'] = 1.0,
                    ['Rarity'] = 'rare',
                    ['Label'] = 'SMG Mk II',
                    ['MaxStack'] = 1,
                    ['Itemimage'] = 'icons/weapon_smg_mk2.png',
                    ['Type'] = 'Weapon',
                    ['Backgroundcolor'] = 'blue',
                    ['Description'] = 'SMG MK2',
                    ['Name'] = 'weapon_smg_mk2',
                    ['Weight'] = 1.0,
                    ['ItemSound'] = 'gear_generic',
                    ['Height'] = 2,
                    ['Width'] = 2
                },
                ['_type'] = 'Item'
            }
        },
        ['diving_gear'] = {
            ['item'] = {
                ['_id'] = 'diving_gear',
                ['_parent'] = '',
                ['_name'] = 'diving_gear',
                ['_data'] = {
                    ['ExamineTime'] = 1.0,
                    ['Rarity'] = 'common',
                    ['Label'] = 'Diving Gear',
                    ['MaxStack'] = 1,
                    ['Itemimage'] = 'icons/diving_gear.png',
                    ['Type'] = 'Item',
                    ['Backgroundcolor'] = 'black',
                    ['Description'] = 'An oxygen tank and a rebreather',
                    ['Name'] = 'diving_gear',
                    ['Weight'] = 30.0,
                    ['ItemSound'] = 'gear_generic',
                    ['Height'] = 1,
                    ['Width'] = 1
                },
                ['_type'] = 'Item'
            }
        },
        ['vintagepistol_defaultclip'] = {
            ['item'] = {
                ['_id'] = 'vintagepistol_defaultclip',
                ['_parent'] = '',
                ['_name'] = 'vintagepistol_defaultclip',
                ['_data'] = {
                    ['ExamineTime'] = 1.0,
                    ['Rarity'] = 'common',
                    ['Label'] = 'Pistol Clip',
                    ['MaxStack'] = 100,
                    ['Itemimage'] = 'icons/pistol_extendedclip.png',
                    ['Type'] = 'Item',
                    ['Backgroundcolor'] = 'black',
                    ['Description'] = 'Vintage Pistol Default Clip',
                    ['Name'] = 'vintagepistol_defaultclip',
                    ['Weight'] = 1.0,
                    ['ItemSound'] = 'gear_generic',
                    ['Height'] = 1,
                    ['Width'] = 1
                },
                ['_type'] = 'Item'
            }
        },
        ['weapon_heavypistol'] = {
            ['item'] = {
                ['_id'] = 'weapon_heavypistol',
                ['_parent'] = '',
                ['_name'] = 'weapon_heavypistol',
                ['_data'] = {
                    ['ExamineTime'] = 1.0,
                    ['Rarity'] = 'rare',
                    ['Label'] = 'Heavy Pistol',
                    ['MaxStack'] = 1,
                    ['Itemimage'] = 'icons/weapon_heavypistol.png',
                    ['Type'] = 'Weapon',
                    ['Backgroundcolor'] = 'blue',
                    ['Description'] = 'A hefty firearm designed to be held in one hand (or attempted)',
                    ['Name'] = 'weapon_heavypistol',
                    ['Weight'] = 1.0,
                    ['ItemSound'] = 'gear_generic',
                    ['Height'] = 1,
                    ['Width'] = 3
                },
                ['_type'] = 'Item'
            }
        },
        ['tablet'] = {
            ['item'] = {
                ['_id'] = 'tablet',
                ['_parent'] = '',
                ['_name'] = 'tablet',
                ['_data'] = {
                    ['ExamineTime'] = 1.0,
                    ['Rarity'] = 'common',
                    ['Label'] = 'Tablet',
                    ['MaxStack'] = 100,
                    ['Itemimage'] = 'icons/tablet.png',
                    ['Type'] = 'Item',
                    ['Backgroundcolor'] = 'black',
                    ['Description'] = 'Expensive tablet',
                    ['Name'] = 'tablet',
                    ['Weight'] = 2.0,
                    ['ItemSound'] = 'gear_generic',
                    ['Height'] = 1,
                    ['Width'] = 1
                },
                ['_type'] = 'Item'
            }
        },
        ['grapejuice'] = {
            ['item'] = {
                ['_id'] = 'grapejuice',
                ['_parent'] = '',
                ['_name'] = 'grapejuice',
                ['_data'] = {
                    ['ExamineTime'] = 1.0,
                    ['Rarity'] = 'common',
                    ['Label'] = 'Grape Juice',
                    ['MaxStack'] = 100,
                    ['Itemimage'] = 'icons/grapejuice.png',
                    ['Type'] = 'Item',
                    ['Backgroundcolor'] = 'black',
                    ['Description'] = 'Grape juice is said to be healthy',
                    ['Name'] = 'grapejuice',
                    ['Weight'] = 0.2,
                    ['ItemSound'] = 'gear_generic',
                    ['Height'] = 1,
                    ['Width'] = 1
                },
                ['_type'] = 'Item'
            }
        },
        ['carbinerifle_drum'] = {
            ['item'] = {
                ['_id'] = 'carbinerifle_drum',
                ['_parent'] = '',
                ['_name'] = 'carbinerifle_drum',
                ['_data'] = {
                    ['ExamineTime'] = 1.0,
                    ['Rarity'] = 'common',
                    ['Label'] = 'Rifle Drum',
                    ['MaxStack'] = 100,
                    ['Itemimage'] = 'icons/rifle_drummag.png',
                    ['Type'] = 'Item',
                    ['Backgroundcolor'] = 'black',
                    ['Description'] = 'Carbine Rifle Drum',
                    ['Name'] = 'carbinerifle_drum',
                    ['Weight'] = 1.0,
                    ['ItemSound'] = 'gear_generic',
                    ['Height'] = 1,
                    ['Width'] = 1
                },
                ['_type'] = 'Item'
            }
        },
        ['weapon_autoshotgun'] = {
            ['item'] = {
                ['_id'] = 'weapon_autoshotgun',
                ['_parent'] = '',
                ['_name'] = 'weapon_autoshotgun',
                ['_data'] = {
                    ['ExamineTime'] = 1.0,
                    ['Rarity'] = 'rare',
                    ['Label'] = 'Auto Shotgun',
                    ['MaxStack'] = 1,
                    ['Itemimage'] = 'icons/weapon_autoshotgun.png',
                    ['Type'] = 'Weapon',
                    ['Backgroundcolor'] = 'blue',
                    ['Description'] = 'A shotgun capable of rapid continous fire',
                    ['Name'] = 'weapon_autoshotgun',
                    ['Weight'] = 1.0,
                    ['ItemSound'] = 'gear_generic',
                    ['Height'] = 2,
                    ['Width'] = 2
                },
                ['_type'] = 'Item'
            }
        },
        ['weapon_smokegrenade'] = {
            ['item'] = {
                ['_id'] = 'weapon_smokegrenade',
                ['_parent'] = '',
                ['_name'] = 'weapon_smokegrenade',
                ['_data'] = {
                    ['ExamineTime'] = 1.0,
                    ['Rarity'] = 'rare',
                    ['Label'] = 'Smoke Grenade',
                    ['MaxStack'] = 1,
                    ['Itemimage'] = 'icons/weapon_c4.png',
                    ['Type'] = 'Weapon',
                    ['Backgroundcolor'] = 'blue',
                    ['Description'] = 'An explosive charge that can be remotely detonated',
                    ['Name'] = 'weapon_smokegrenade',
                    ['Weight'] = 1.0,
                    ['ItemSound'] = 'gear_generic',
                    ['Height'] = 2,
                    ['Width'] = 2
                },
                ['_type'] = 'Item'
            }
        },
        ['laptop'] = {
            ['item'] = {
                ['_id'] = 'laptop',
                ['_parent'] = '',
                ['_name'] = 'laptop',
                ['_data'] = {
                    ['ExamineTime'] = 1.0,
                    ['Rarity'] = 'common',
                    ['Label'] = 'Laptop',
                    ['MaxStack'] = 100,
                    ['Itemimage'] = 'icons/laptop.png',
                    ['Type'] = 'Item',
                    ['Backgroundcolor'] = 'black',
                    ['Description'] = 'Expensive laptop',
                    ['Name'] = 'laptop',
                    ['Weight'] = 4.0,
                    ['ItemSound'] = 'gear_generic',
                    ['Height'] = 1,
                    ['Width'] = 1
                },
                ['_type'] = 'Item'
            }
        },
        ['weapon_fireextinguisher'] = {
            ['item'] = {
                ['_id'] = 'weapon_fireextinguisher',
                ['_parent'] = '',
                ['_name'] = 'weapon_fireextinguisher',
                ['_data'] = {
                    ['ExamineTime'] = 1.0,
                    ['Rarity'] = 'rare',
                    ['Label'] = 'Fire Extinguisher',
                    ['MaxStack'] = 1,
                    ['Itemimage'] = 'icons/weapon_fireextinguisher.png',
                    ['Type'] = 'Weapon',
                    ['Backgroundcolor'] = 'blue',
                    ['Description'] = 'A portable device that discharges a jet of water, foam, gas, or other material to extinguish a fire',
                    ['Name'] = 'weapon_fireextinguisher',
                    ['Weight'] = 1.0,
                    ['ItemSound'] = 'gear_generic',
                    ['Height'] = 2,
                    ['Width'] = 2
                },
                ['_type'] = 'Item'
            }
        },
        ['oxy'] = {
            ['item'] = {
                ['_id'] = 'oxy',
                ['_parent'] = '',
                ['_name'] = 'oxy',
                ['_data'] = {
                    ['ExamineTime'] = 1.0,
                    ['Rarity'] = 'common',
                    ['Label'] = 'Prescription Oxy',
                    ['MaxStack'] = 100,
                    ['Itemimage'] = 'icons/oxy.png',
                    ['Type'] = 'Item',
                    ['Backgroundcolor'] = 'black',
                    ['Description'] = 'The Label Has Been Ripped Off',
                    ['Name'] = 'oxy',
                    ['Weight'] = 0.0,
                    ['ItemSound'] = 'gear_generic',
                    ['Height'] = 1,
                    ['Width'] = 1
                },
                ['_type'] = 'Item'
            }
        },
        ['weapontint_lspd'] = {
            ['item'] = {
                ['_id'] = 'weapontint_lspd',
                ['_parent'] = '',
                ['_name'] = 'weapontint_lspd',
                ['_data'] = {
                    ['ExamineTime'] = 1.0,
                    ['Rarity'] = 'common',
                    ['Label'] = 'LSPD Tint',
                    ['MaxStack'] = 100,
                    ['Itemimage'] = 'icons/weapontint_lspd.png',
                    ['Type'] = 'Item',
                    ['Backgroundcolor'] = 'black',
                    ['Description'] = 'LSPD Weapon Tint',
                    ['Name'] = 'weapontint_lspd',
                    ['Weight'] = 1.0,
                    ['ItemSound'] = 'gear_generic',
                    ['Height'] = 1,
                    ['Width'] = 1
                },
                ['_type'] = 'Item'
            }
        },
        ['sniper_grip'] = {
            ['item'] = {
                ['_id'] = 'sniper_grip',
                ['_parent'] = '',
                ['_name'] = 'sniper_grip',
                ['_data'] = {
                    ['ExamineTime'] = 1.0,
                    ['Rarity'] = 'common',
                    ['Label'] = 'Sniper Grip',
                    ['MaxStack'] = 100,
                    ['Itemimage'] = 'icons/pistol_suppressor.png',
                    ['Type'] = 'Item',
                    ['Backgroundcolor'] = 'black',
                    ['Description'] = 'Sniper Rifle Grip Attachment',
                    ['Name'] = 'sniper_grip',
                    ['Weight'] = 1.0,
                    ['ItemSound'] = 'gear_generic',
                    ['Height'] = 1,
                    ['Width'] = 1
                },
                ['_type'] = 'Item'
            }
        },
        ['heavypistol_defaultclip'] = {
            ['item'] = {
                ['_id'] = 'heavypistol_defaultclip',
                ['_parent'] = '',
                ['_name'] = 'heavypistol_defaultclip',
                ['_data'] = {
                    ['ExamineTime'] = 1.0,
                    ['Rarity'] = 'common',
                    ['Label'] = 'Pistol Clip',
                    ['MaxStack'] = 100,
                    ['Itemimage'] = 'icons/pistol_extendedclip.png',
                    ['Type'] = 'Item',
                    ['Backgroundcolor'] = 'black',
                    ['Description'] = 'Heavy Pistol Default Clip',
                    ['Name'] = 'heavypistol_defaultclip',
                    ['Weight'] = 1.0,
                    ['ItemSound'] = 'gear_generic',
                    ['Height'] = 1,
                    ['Width'] = 1
                },
                ['_type'] = 'Item'
            }
        },
        ['bullpuprifle_extendedclip'] = {
            ['item'] = {
                ['_id'] = 'bullpuprifle_extendedclip',
                ['_parent'] = '',
                ['_name'] = 'bullpuprifle_extendedclip',
                ['_data'] = {
                    ['ExamineTime'] = 1.0,
                    ['Rarity'] = 'common',
                    ['Label'] = 'Rifle EXT Clip',
                    ['MaxStack'] = 100,
                    ['Itemimage'] = 'icons/pistol_extendedclip.png',
                    ['Type'] = 'Item',
                    ['Backgroundcolor'] = 'black',
                    ['Description'] = 'Bullpup Rifle Extended Clip',
                    ['Name'] = 'bullpuprifle_extendedclip',
                    ['Weight'] = 1.0,
                    ['ItemSound'] = 'gear_generic',
                    ['Height'] = 1,
                    ['Width'] = 1
                },
                ['_type'] = 'Item'
            }
        },
        ['weed_og-kush_seed'] = {
            ['item'] = {
                ['_id'] = 'weed_og-kush_seed',
                ['_parent'] = '',
                ['_name'] = 'weed_og-kush_seed',
                ['_data'] = {
                    ['ExamineTime'] = 1.0,
                    ['Rarity'] = 'common',
                    ['Label'] = 'OGKush Seed',
                    ['MaxStack'] = 100,
                    ['Itemimage'] = 'icons/weed_seed.png',
                    ['Type'] = 'Item',
                    ['Backgroundcolor'] = 'black',
                    ['Description'] = 'A weed seed of OG Kush',
                    ['Name'] = 'weed_og-kush_seed',
                    ['Weight'] = 0.0,
                    ['ItemSound'] = 'gear_generic',
                    ['Height'] = 1,
                    ['Width'] = 1
                },
                ['_type'] = 'Item'
            }
        },
        ['empty_weed_bag'] = {
            ['item'] = {
                ['_id'] = 'empty_weed_bag',
                ['_parent'] = '',
                ['_name'] = 'empty_weed_bag',
                ['_data'] = {
                    ['ExamineTime'] = 1.0,
                    ['Rarity'] = 'common',
                    ['Label'] = 'Empty Weed Bag',
                    ['MaxStack'] = 100,
                    ['Itemimage'] = 'icons/weed_baggy_empty.png',
                    ['Type'] = 'Item',
                    ['Backgroundcolor'] = 'black',
                    ['Description'] = 'A small empty bag',
                    ['Name'] = 'empty_weed_bag',
                    ['Weight'] = 0.0,
                    ['ItemSound'] = 'gear_generic',
                    ['Height'] = 1,
                    ['Width'] = 1
                },
                ['_type'] = 'Item'
            }
        },
        ['lawyerpass'] = {
            ['item'] = {
                ['_id'] = 'lawyerpass',
                ['_parent'] = '',
                ['_name'] = 'lawyerpass',
                ['_data'] = {
                    ['ExamineTime'] = 1.0,
                    ['Rarity'] = 'common',
                    ['Label'] = 'Lawyer Pass',
                    ['MaxStack'] = 1,
                    ['Itemimage'] = 'icons/lawyerpass.png',
                    ['Type'] = 'Item',
                    ['Backgroundcolor'] = 'black',
                    ['Description'] = 'Pass exclusive to lawyers to show they can represent a suspect',
                    ['Name'] = 'lawyerpass',
                    ['Weight'] = 0.0,
                    ['ItemSound'] = 'gear_generic',
                    ['Height'] = 1,
                    ['Width'] = 1
                },
                ['_type'] = 'Item'
            }
        },
        ['weapon_hammer'] = {
            ['item'] = {
                ['_id'] = 'weapon_hammer',
                ['_parent'] = '',
                ['_name'] = 'weapon_hammer',
                ['_data'] = {
                    ['ExamineTime'] = 1.0,
                    ['Rarity'] = 'rare',
                    ['Label'] = 'Hammer',
                    ['MaxStack'] = 1,
                    ['Itemimage'] = 'icons/weapon_hammer.png',
                    ['Type'] = 'Weapon',
                    ['Backgroundcolor'] = 'blue',
                    ['Description'] = 'Used for jobs such as breaking things (legs) and driving in nails',
                    ['Name'] = 'weapon_hammer',
                    ['Weight'] = 1.0,
                    ['ItemSound'] = 'gear_generic',
                    ['Height'] = 2,
                    ['Width'] = 2
                },
                ['_type'] = 'Item'
            }
        },
        ['phone'] = {
            ['item'] = {
                ['_id'] = 'phone',
                ['_parent'] = '',
                ['_name'] = 'phone',
                ['_data'] = {
                    ['ExamineTime'] = 1.0,
                    ['Rarity'] = 'common',
                    ['Label'] = 'Phone',
                    ['MaxStack'] = 1,
                    ['Itemimage'] = 'icons/phone.png',
                    ['Type'] = 'Item',
                    ['Backgroundcolor'] = 'black',
                    ['Description'] = 'Neat phone ya got there',
                    ['Name'] = 'phone',
                    ['Weight'] = 0.7,
                    ['ItemSound'] = 'gear_generic',
                    ['Height'] = 1,
                    ['Width'] = 1
                },
                ['_type'] = 'Item'
            }
        },
        ['weapon_compactlauncher'] = {
            ['item'] = {
                ['_id'] = 'weapon_compactlauncher',
                ['_parent'] = '',
                ['_name'] = 'weapon_compactlauncher',
                ['_data'] = {
                    ['ExamineTime'] = 1.0,
                    ['Rarity'] = 'rare',
                    ['Label'] = 'Compact Launcher',
                    ['MaxStack'] = 1,
                    ['Itemimage'] = 'icons/weapon_compactlauncher.png',
                    ['Type'] = 'Weapon',
                    ['Backgroundcolor'] = 'blue',
                    ['Description'] = 'A compact grenade launcher',
                    ['Name'] = 'weapon_compactlauncher',
                    ['Weight'] = 1.0,
                    ['ItemSound'] = 'gear_generic',
                    ['Height'] = 2,
                    ['Width'] = 2
                },
                ['_type'] = 'Item'
            }
        },
        ['weapon_petrolcan'] = {
            ['item'] = {
                ['_id'] = 'weapon_petrolcan',
                ['_parent'] = '',
                ['_name'] = 'weapon_petrolcan',
                ['_data'] = {
                    ['ExamineTime'] = 1.0,
                    ['Rarity'] = 'rare',
                    ['Label'] = 'Petrol Can',
                    ['MaxStack'] = 1,
                    ['Itemimage'] = 'icons/weapon_petrolcan.png',
                    ['Type'] = 'Weapon',
                    ['Backgroundcolor'] = 'blue',
                    ['Description'] = 'A robust liquid container made from pressed steel',
                    ['Name'] = 'weapon_petrolcan',
                    ['Weight'] = 1.0,
                    ['ItemSound'] = 'gear_generic',
                    ['Height'] = 2,
                    ['Width'] = 2
                },
                ['_type'] = 'Item'
            }
        },
        ['appistol_defaultclip'] = {
            ['item'] = {
                ['_id'] = 'appistol_defaultclip',
                ['_parent'] = '',
                ['_name'] = 'appistol_defaultclip',
                ['_data'] = {
                    ['ExamineTime'] = 1.0,
                    ['Rarity'] = 'common',
                    ['Label'] = 'Pistol Clip',
                    ['MaxStack'] = 100,
                    ['Itemimage'] = 'icons/pistol_extendedclip.png',
                    ['Type'] = 'Item',
                    ['Backgroundcolor'] = 'black',
                    ['Description'] = 'APPistol Default Clip',
                    ['Name'] = 'appistol_defaultclip',
                    ['Weight'] = 1.0,
                    ['ItemSound'] = 'gear_generic',
                    ['Height'] = 1,
                    ['Width'] = 1
                },
                ['_type'] = 'Item'
            }
        },
        ['sheet'] = {
            ['item'] = {
                ['_id'] = 'sheet',
                ['_parent'] = '',
                ['_name'] = 'sheet',
                ['_data'] = {
                    ['ExamineTime'] = 1.0,
                    ['Rarity'] = 'common',
                    ['Label'] = 'Ripped Sheet',
                    ['MaxStack'] = 100,
                    ['Itemimage'] = 'icons/bandage.png',
                    ['Type'] = 'Item',
                    ['Backgroundcolor'] = 'black',
                    ['Description'] = 'Ripped sheet, maybe u can craft a bandage, i dont sure.',
                    ['Name'] = 'sheet',
                    ['Weight'] = 0.0,
                    ['ItemSound'] = 'gear_generic',
                    ['Height'] = 1,
                    ['Width'] = 1
                },
                ['_type'] = 'Item'
            }
        },
        ['metalscrap'] = {
            ['item'] = {
                ['_id'] = 'metalscrap',
                ['_parent'] = '',
                ['_name'] = 'metalscrap',
                ['_data'] = {
                    ['ExamineTime'] = 1.0,
                    ['Rarity'] = 'common',
                    ['Label'] = 'Metal Scrap',
                    ['MaxStack'] = 100,
                    ['Itemimage'] = 'icons/metalscrap.png',
                    ['Type'] = 'Item',
                    ['Backgroundcolor'] = 'black',
                    ['Description'] = 'You can probably make something nice out of this',
                    ['Name'] = 'metalscrap',
                    ['Weight'] = 0.1,
                    ['ItemSound'] = 'gear_generic',
                    ['Height'] = 1,
                    ['Width'] = 1
                },
                ['_type'] = 'Item'
            }
        },
        ['gusenberg_extendedclip'] = {
            ['item'] = {
                ['_id'] = 'gusenberg_extendedclip',
                ['_parent'] = '',
                ['_name'] = 'gusenberg_extendedclip',
                ['_data'] = {
                    ['ExamineTime'] = 1.0,
                    ['Rarity'] = 'common',
                    ['Label'] = 'Rifle EXT Clip',
                    ['MaxStack'] = 100,
                    ['Itemimage'] = 'icons/pistol_extendedclip.png',
                    ['Type'] = 'Item',
                    ['Backgroundcolor'] = 'black',
                    ['Description'] = 'Gusenberg Extended Clip',
                    ['Name'] = 'gusenberg_extendedclip',
                    ['Weight'] = 1.0,
                    ['ItemSound'] = 'gear_generic',
                    ['Height'] = 1,
                    ['Width'] = 1
                },
                ['_type'] = 'Item'
            }
        },
        ['vintagepistol_extendedclip'] = {
            ['item'] = {
                ['_id'] = 'vintagepistol_extendedclip',
                ['_parent'] = '',
                ['_name'] = 'vintagepistol_extendedclip',
                ['_data'] = {
                    ['ExamineTime'] = 1.0,
                    ['Rarity'] = 'common',
                    ['Label'] = 'Pistol EXT Clip',
                    ['MaxStack'] = 100,
                    ['Itemimage'] = 'icons/pistol_extendedclip.png',
                    ['Type'] = 'Item',
                    ['Backgroundcolor'] = 'black',
                    ['Description'] = 'Vintage Pistol Default Clip',
                    ['Name'] = 'vintagepistol_extendedclip',
                    ['Weight'] = 1.0,
                    ['ItemSound'] = 'gear_generic',
                    ['Height'] = 1,
                    ['Width'] = 1
                },
                ['_type'] = 'Item'
            }
        },
        ['weapon_pistol50'] = {
            ['item'] = {
                ['_id'] = 'weapon_pistol50',
                ['_parent'] = '',
                ['_name'] = 'weapon_pistol50',
                ['_data'] = {
                    ['ExamineTime'] = 1.0,
                    ['Rarity'] = 'rare',
                    ['Label'] = 'Pistol .50',
                    ['MaxStack'] = 1,
                    ['Itemimage'] = 'icons/weapon_pistol50.png',
                    ['Type'] = 'Weapon',
                    ['Backgroundcolor'] = 'blue',
                    ['Description'] = 'A .50 caliber firearm designed to be held with both hands',
                    ['Name'] = 'weapon_pistol50',
                    ['Weight'] = 1.0,
                    ['ItemSound'] = 'gear_generic',
                    ['Height'] = 2,
                    ['Width'] = 2
                },
                ['_type'] = 'Item'
            }
        },
        ['weapon_ceramicpistol'] = {
            ['item'] = {
                ['_id'] = 'weapon_ceramicpistol',
                ['_parent'] = '',
                ['_name'] = 'weapon_ceramicpistol',
                ['_data'] = {
                    ['ExamineTime'] = 1.0,
                    ['Rarity'] = 'rare',
                    ['Label'] = 'Ceramic Pistol',
                    ['MaxStack'] = 1,
                    ['Itemimage'] = 'icons/weapon_ceramicpistol.png',
                    ['Type'] = 'Weapon',
                    ['Backgroundcolor'] = 'blue',
                    ['Description'] = 'Weapon Ceramicpistol',
                    ['Name'] = 'weapon_ceramicpistol',
                    ['Weight'] = 1.0,
                    ['ItemSound'] = 'gear_generic',
                    ['Height'] = 2,
                    ['Width'] = 2
                },
                ['_type'] = 'Item'
            }
        },
        ['weapon_assaultrifle_mk2'] = {
            ['item'] = {
                ['_id'] = 'weapon_assaultrifle_mk2',
                ['_parent'] = '',
                ['_name'] = 'weapon_assaultrifle_mk2',
                ['_data'] = {
                    ['ExamineTime'] = 1.0,
                    ['Rarity'] = 'rare',
                    ['Label'] = 'Assault Rifle Mk II',
                    ['MaxStack'] = 1,
                    ['Itemimage'] = 'icons/weapon_assaultrifle_mk2.png',
                    ['Type'] = 'Weapon',
                    ['Backgroundcolor'] = 'blue',
                    ['Description'] = 'Assault Rifle MK2',
                    ['Name'] = 'weapon_assaultrifle_mk2',
                    ['Weight'] = 1.0,
                    ['ItemSound'] = 'gear_generic',
                    ['Height'] = 2,
                    ['Width'] = 2
                },
                ['_type'] = 'Item'
            }
        },
        ['repairkit'] = {
            ['item'] = {
                ['_id'] = 'repairkit',
                ['_parent'] = '',
                ['_name'] = 'repairkit',
                ['_data'] = {
                    ['ExamineTime'] = 1.0,
                    ['Rarity'] = 'common',
                    ['Label'] = 'Repairkit',
                    ['MaxStack'] = 100,
                    ['Itemimage'] = 'icons/repairkit.png',
                    ['Type'] = 'Item',
                    ['Backgroundcolor'] = 'black',
                    ['Description'] = 'A nice toolbox with stuff to repair your vehicle',
                    ['Name'] = 'repairkit',
                    ['Weight'] = 2.5,
                    ['ItemSound'] = 'gear_generic',
                    ['Height'] = 1,
                    ['Width'] = 1
                },
                ['_type'] = 'Item'
            }
        },
        ['weapon_ball'] = {
            ['item'] = {
                ['_id'] = 'weapon_ball',
                ['_parent'] = '',
                ['_name'] = 'weapon_ball',
                ['_data'] = {
                    ['ExamineTime'] = 1.0,
                    ['Rarity'] = 'rare',
                    ['Label'] = 'Ball',
                    ['MaxStack'] = 1,
                    ['Itemimage'] = 'icons/weapon_ball.png',
                    ['Type'] = 'Weapon',
                    ['Backgroundcolor'] = 'blue',
                    ['Description'] = 'A solid or hollow spherical or egg-shaped object that is kicked, thrown, or hit in a game',
                    ['Name'] = 'weapon_ball',
                    ['Weight'] = 1.0,
                    ['ItemSound'] = 'gear_generic',
                    ['Height'] = 2,
                    ['Width'] = 2
                },
                ['_type'] = 'Item'
            }
        },
        ['crack_baggy'] = {
            ['item'] = {
                ['_id'] = 'crack_baggy',
                ['_parent'] = '',
                ['_name'] = 'crack_baggy',
                ['_data'] = {
                    ['ExamineTime'] = 1.0,
                    ['Rarity'] = 'common',
                    ['Label'] = 'Bag of Crack',
                    ['MaxStack'] = 100,
                    ['Itemimage'] = 'icons/crack_baggy.png',
                    ['Type'] = 'Item',
                    ['Backgroundcolor'] = 'black',
                    ['Description'] = 'To get happy faster',
                    ['Name'] = 'crack_baggy',
                    ['Weight'] = 0.0,
                    ['ItemSound'] = 'gear_generic',
                    ['Height'] = 1,
                    ['Width'] = 1
                },
                ['_type'] = 'Item'
            }
        },
        ['security_card_01'] = {
            ['item'] = {
                ['_id'] = 'security_card_01',
                ['_parent'] = '',
                ['_name'] = 'security_card_01',
                ['_data'] = {
                    ['ExamineTime'] = 1.0,
                    ['Rarity'] = 'common',
                    ['Label'] = 'Security Card A',
                    ['MaxStack'] = 100,
                    ['Itemimage'] = 'icons/security_card_01.png',
                    ['Type'] = 'Item',
                    ['Backgroundcolor'] = 'black',
                    ['Description'] = 'A security card... I wonder what it goes to',
                    ['Name'] = 'security_card_01',
                    ['Weight'] = 0.0,
                    ['ItemSound'] = 'gear_generic',
                    ['Height'] = 1,
                    ['Width'] = 1
                },
                ['_type'] = 'Item'
            }
        },
        ['smg_scope'] = {
            ['item'] = {
                ['_id'] = 'smg_scope',
                ['_parent'] = '',
                ['_name'] = 'smg_scope',
                ['_data'] = {
                    ['ExamineTime'] = 1.0,
                    ['Rarity'] = 'common',
                    ['Label'] = 'SMG Scope',
                    ['MaxStack'] = 100,
                    ['Itemimage'] = 'icons/smg_scope.png',
                    ['Type'] = 'Item',
                    ['Backgroundcolor'] = 'black',
                    ['Description'] = 'SMG Scope Attachment',
                    ['Name'] = 'smg_scope',
                    ['Weight'] = 1.0,
                    ['ItemSound'] = 'gear_generic',
                    ['Height'] = 1,
                    ['Width'] = 1
                },
                ['_type'] = 'Item'
            }
        },
        ['aluminumoxide'] = {
            ['item'] = {
                ['_id'] = 'aluminumoxide',
                ['_parent'] = '',
                ['_name'] = 'aluminumoxide',
                ['_data'] = {
                    ['ExamineTime'] = 1.0,
                    ['Rarity'] = 'common',
                    ['Label'] = 'Aluminium Powder',
                    ['MaxStack'] = 100,
                    ['Itemimage'] = 'icons/aluminumoxide.png',
                    ['Type'] = 'Item',
                    ['Backgroundcolor'] = 'black',
                    ['Description'] = 'Some powder to mix with',
                    ['Name'] = 'aluminumoxide',
                    ['Weight'] = 0.1,
                    ['ItemSound'] = 'gear_generic',
                    ['Height'] = 1,
                    ['Width'] = 1
                },
                ['_type'] = 'Item'
            }
        },
        ['jerry_can'] = {
            ['item'] = {
                ['_id'] = 'jerry_can',
                ['_parent'] = '',
                ['_name'] = 'jerry_can',
                ['_data'] = {
                    ['ExamineTime'] = 1.0,
                    ['Rarity'] = 'common',
                    ['Label'] = 'Jerrycan 20L',
                    ['MaxStack'] = 100,
                    ['Itemimage'] = 'icons/jerry_can.png',
                    ['Type'] = 'Item',
                    ['Backgroundcolor'] = 'black',
                    ['Description'] = 'A can full of Fuel',
                    ['Name'] = 'jerry_can',
                    ['Weight'] = 20.0,
                    ['ItemSound'] = 'gear_generic',
                    ['Height'] = 1,
                    ['Width'] = 1
                },
                ['_type'] = 'Item'
            }
        },
        ['wine'] = {
            ['item'] = {
                ['_id'] = 'wine',
                ['_parent'] = '',
                ['_name'] = 'wine',
                ['_data'] = {
                    ['ExamineTime'] = 1.0,
                    ['Rarity'] = 'common',
                    ['Label'] = 'Wine',
                    ['MaxStack'] = 100,
                    ['Itemimage'] = 'icons/wine.png',
                    ['Type'] = 'Item',
                    ['Backgroundcolor'] = 'black',
                    ['Description'] = 'Some good wine to drink on a fine evening',
                    ['Name'] = 'wine',
                    ['Weight'] = 0.3,
                    ['ItemSound'] = 'gear_generic',
                    ['Height'] = 1,
                    ['Width'] = 1
                },
                ['_type'] = 'Item'
            }
        },
        ['pistol50_defaultclip'] = {
            ['item'] = {
                ['_id'] = 'pistol50_defaultclip',
                ['_parent'] = '',
                ['_name'] = 'pistol50_defaultclip',
                ['_data'] = {
                    ['ExamineTime'] = 1.0,
                    ['Rarity'] = 'common',
                    ['Label'] = 'Pistol Clip',
                    ['MaxStack'] = 100,
                    ['Itemimage'] = 'icons/pistol_extendedclip.png',
                    ['Type'] = 'Item',
                    ['Backgroundcolor'] = 'black',
                    ['Description'] = '.50 Pistol Default Clip',
                    ['Name'] = 'pistol50_defaultclip',
                    ['Weight'] = 1.0,
                    ['ItemSound'] = 'gear_generic',
                    ['Height'] = 1,
                    ['Width'] = 1
                },
                ['_type'] = 'Item'
            }
        },
        ['stickynote'] = {
            ['item'] = {
                ['_id'] = 'stickynote',
                ['_parent'] = '',
                ['_name'] = 'stickynote',
                ['_data'] = {
                    ['ExamineTime'] = 1.0,
                    ['Rarity'] = 'common',
                    ['Label'] = 'Sticky note',
                    ['MaxStack'] = 1,
                    ['Itemimage'] = 'icons/stickynote.png',
                    ['Type'] = 'Item',
                    ['Backgroundcolor'] = 'black',
                    ['Description'] = 'Sometimes handy to remember something :)',
                    ['Name'] = 'stickynote',
                    ['Weight'] = 0.0,
                    ['ItemSound'] = 'gear_generic',
                    ['Height'] = 1,
                    ['Width'] = 1
                },
                ['_type'] = 'Item'
            }
        },
        ['pinger'] = {
            ['item'] = {
                ['_id'] = 'pinger',
                ['_parent'] = '',
                ['_name'] = 'pinger',
                ['_data'] = {
                    ['ExamineTime'] = 1.0,
                    ['Rarity'] = 'common',
                    ['Label'] = 'Pinger',
                    ['MaxStack'] = 100,
                    ['Itemimage'] = 'icons/pinger.png',
                    ['Type'] = 'Item',
                    ['Backgroundcolor'] = 'black',
                    ['Description'] = 'With a pinger and your phone you can send out your location',
                    ['Name'] = 'pinger',
                    ['Weight'] = 1.0,
                    ['ItemSound'] = 'gear_generic',
                    ['Height'] = 1,
                    ['Width'] = 1
                },
                ['_type'] = 'Item'
            }
        },
        ['appistol_extendedclip'] = {
            ['item'] = {
                ['_id'] = 'appistol_extendedclip',
                ['_parent'] = '',
                ['_name'] = 'appistol_extendedclip',
                ['_data'] = {
                    ['ExamineTime'] = 1.0,
                    ['Rarity'] = 'common',
                    ['Label'] = 'Pistol EXT Clip',
                    ['MaxStack'] = 100,
                    ['Itemimage'] = 'icons/pistol_extendedclip.png',
                    ['Type'] = 'Item',
                    ['Backgroundcolor'] = 'black',
                    ['Description'] = 'APPistol Extended Clip',
                    ['Name'] = 'appistol_extendedclip',
                    ['Weight'] = 1.0,
                    ['ItemSound'] = 'gear_generic',
                    ['Height'] = 1,
                    ['Width'] = 1
                },
                ['_type'] = 'Item'
            }
        },
        ['weapon_grenadelauncher_smoke'] = {
            ['item'] = {
                ['_id'] = 'weapon_grenadelauncher_smoke',
                ['_parent'] = '',
                ['_name'] = 'weapon_grenadelauncher_smoke',
                ['_data'] = {
                    ['ExamineTime'] = 1.0,
                    ['Rarity'] = 'rare',
                    ['Label'] = 'Smoke Grenade Launcher',
                    ['MaxStack'] = 1,
                    ['Itemimage'] = 'icons/weapon_smokegrenade.png',
                    ['Type'] = 'Weapon',
                    ['Backgroundcolor'] = 'blue',
                    ['Description'] = 'A bomb that produces a lot of smoke when it explodes',
                    ['Name'] = 'weapon_grenadelauncher_smoke',
                    ['Weight'] = 1.0,
                    ['ItemSound'] = 'gear_generic',
                    ['Height'] = 2,
                    ['Width'] = 2
                },
                ['_type'] = 'Item'
            }
        },
        ['weapon_specialcarbine_mk2'] = {
            ['item'] = {
                ['_id'] = 'weapon_specialcarbine_mk2',
                ['_parent'] = '',
                ['_name'] = 'weapon_specialcarbine_mk2',
                ['_data'] = {
                    ['ExamineTime'] = 1.0,
                    ['Rarity'] = 'rare',
                    ['Label'] = 'Special Carbine Mk II',
                    ['MaxStack'] = 1,
                    ['Itemimage'] = 'icons/weapon_specialcarbine_mk2.png',
                    ['Type'] = 'Weapon',
                    ['Backgroundcolor'] = 'blue',
                    ['Description'] = 'Weapon Wpecialcarbine MK2',
                    ['Name'] = 'weapon_specialcarbine_mk2',
                    ['Weight'] = 1.0,
                    ['ItemSound'] = 'gear_generic',
                    ['Height'] = 2,
                    ['Width'] = 2
                },
                ['_type'] = 'Item'
            }
        },
        ['weapon_gusenberg'] = {
            ['item'] = {
                ['_id'] = 'weapon_gusenberg',
                ['_parent'] = '',
                ['_name'] = 'weapon_gusenberg',
                ['_data'] = {
                    ['ExamineTime'] = 1.0,
                    ['Rarity'] = 'rare',
                    ['Label'] = 'Thompson SMG',
                    ['MaxStack'] = 1,
                    ['Itemimage'] = 'icons/weapon_gusenberg.png',
                    ['Type'] = 'Weapon',
                    ['Backgroundcolor'] = 'blue',
                    ['Description'] = 'An automatic rifle commonly referred to as a tommy gun',
                    ['Name'] = 'weapon_gusenberg',
                    ['Weight'] = 1.0,
                    ['ItemSound'] = 'gear_generic',
                    ['Height'] = 2,
                    ['Width'] = 2
                },
                ['_type'] = 'Item'
            }
        },
        ['weapon_assaultsmg'] = {
            ['item'] = {
                ['_id'] = 'weapon_assaultsmg',
                ['_parent'] = '',
                ['_name'] = 'weapon_assaultsmg',
                ['_data'] = {
                    ['ExamineTime'] = 1.0,
                    ['Rarity'] = 'rare',
                    ['Label'] = 'Assault SMG',
                    ['MaxStack'] = 1,
                    ['Itemimage'] = 'icons/weapon_assaultsmg.png',
                    ['Type'] = 'Weapon',
                    ['Backgroundcolor'] = 'blue',
                    ['Description'] = 'An assault version of a handheld lightweight machine gun',
                    ['Name'] = 'weapon_assaultsmg',
                    ['Weight'] = 1.0,
                    ['ItemSound'] = 'gear_generic',
                    ['Height'] = 2,
                    ['Width'] = 2
                },
                ['_type'] = 'Item'
            }
        },
        ['diamond_ring'] = {
            ['item'] = {
                ['_id'] = 'diamond_ring',
                ['_parent'] = '',
                ['_name'] = 'diamond_ring',
                ['_data'] = {
                    ['ExamineTime'] = 1.0,
                    ['Rarity'] = 'common',
                    ['Label'] = 'Diamond Ring',
                    ['MaxStack'] = 100,
                    ['Itemimage'] = 'icons/diamond_ring.png',
                    ['Type'] = 'Item',
                    ['Backgroundcolor'] = 'black',
                    ['Description'] = 'A diamond ring seems like the jackpot to me!',
                    ['Name'] = 'diamond_ring',
                    ['Weight'] = 1.5,
                    ['ItemSound'] = 'gear_generic',
                    ['Height'] = 1,
                    ['Width'] = 1
                },
                ['_type'] = 'Item'
            }
        },
        ['assaultsmg_defaultclip'] = {
            ['item'] = {
                ['_id'] = 'assaultsmg_defaultclip',
                ['_parent'] = '',
                ['_name'] = 'assaultsmg_defaultclip',
                ['_data'] = {
                    ['ExamineTime'] = 1.0,
                    ['Rarity'] = 'common',
                    ['Label'] = 'SMG Clip',
                    ['MaxStack'] = 100,
                    ['Itemimage'] = 'icons/pistol_extendedclip.png',
                    ['Type'] = 'Item',
                    ['Backgroundcolor'] = 'black',
                    ['Description'] = 'Assault SMG Default Clip',
                    ['Name'] = 'assaultsmg_defaultclip',
                    ['Weight'] = 1.0,
                    ['ItemSound'] = 'gear_generic',
                    ['Height'] = 1,
                    ['Width'] = 1
                },
                ['_type'] = 'Item'
            }
        },
        ['combatpdw_drum'] = {
            ['item'] = {
                ['_id'] = 'combatpdw_drum',
                ['_parent'] = '',
                ['_name'] = 'combatpdw_drum',
                ['_data'] = {
                    ['ExamineTime'] = 1.0,
                    ['Rarity'] = 'common',
                    ['Label'] = 'SMG Drum',
                    ['MaxStack'] = 100,
                    ['Itemimage'] = 'icons/rifle_drummag.png',
                    ['Type'] = 'Item',
                    ['Backgroundcolor'] = 'black',
                    ['Description'] = 'Combat PDW Drum',
                    ['Name'] = 'combatpdw_drum',
                    ['Weight'] = 1.0,
                    ['ItemSound'] = 'gear_generic',
                    ['Height'] = 1,
                    ['Width'] = 1
                },
                ['_type'] = 'Item'
            }
        },
        ['snspistol_grip'] = {
            ['item'] = {
                ['_id'] = 'snspistol_grip',
                ['_parent'] = '',
                ['_name'] = 'snspistol_grip',
                ['_data'] = {
                    ['ExamineTime'] = 1.0,
                    ['Rarity'] = 'common',
                    ['Label'] = 'Pistol Grip',
                    ['MaxStack'] = 100,
                    ['Itemimage'] = 'icons/pistol_suppressor.png',
                    ['Type'] = 'Item',
                    ['Backgroundcolor'] = 'black',
                    ['Description'] = 'SNS Pistol Grip Attachment',
                    ['Name'] = 'snspistol_grip',
                    ['Weight'] = 1.0,
                    ['ItemSound'] = 'gear_generic',
                    ['Height'] = 1,
                    ['Width'] = 1
                },
                ['_type'] = 'Item'
            }
        },
        ['weed_nutrition'] = {
            ['item'] = {
                ['_id'] = 'weed_nutrition',
                ['_parent'] = '',
                ['_name'] = 'weed_nutrition',
                ['_data'] = {
                    ['ExamineTime'] = 1.0,
                    ['Rarity'] = 'common',
                    ['Label'] = 'Plant Fertilizer',
                    ['MaxStack'] = 100,
                    ['Itemimage'] = 'icons/weed_nutrition.png',
                    ['Type'] = 'Item',
                    ['Backgroundcolor'] = 'black',
                    ['Description'] = 'Plant nutrition',
                    ['Name'] = 'weed_nutrition',
                    ['Weight'] = 2.0,
                    ['ItemSound'] = 'gear_generic',
                    ['Height'] = 1,
                    ['Width'] = 1
                },
                ['_type'] = 'Item'
            }
        },
        ['shotgun_suppressor'] = {
            ['item'] = {
                ['_id'] = 'shotgun_suppressor',
                ['_parent'] = '',
                ['_name'] = 'shotgun_suppressor',
                ['_data'] = {
                    ['ExamineTime'] = 1.0,
                    ['Rarity'] = 'common',
                    ['Label'] = 'Shotgun Suppressor',
                    ['MaxStack'] = 100,
                    ['Itemimage'] = 'icons/pistol_suppressor.png',
                    ['Type'] = 'Item',
                    ['Backgroundcolor'] = 'black',
                    ['Description'] = 'Shotgun Suppressor Attachment',
                    ['Name'] = 'shotgun_suppressor',
                    ['Weight'] = 1.0,
                    ['ItemSound'] = 'gear_generic',
                    ['Height'] = 1,
                    ['Width'] = 1
                },
                ['_type'] = 'Item'
            }
        },
        ['combatpdw_grip'] = {
            ['item'] = {
                ['_id'] = 'combatpdw_grip',
                ['_parent'] = '',
                ['_name'] = 'combatpdw_grip',
                ['_data'] = {
                    ['ExamineTime'] = 1.0,
                    ['Rarity'] = 'common',
                    ['Label'] = 'SMG Grip',
                    ['MaxStack'] = 100,
                    ['Itemimage'] = 'icons/pistol_suppressor.png',
                    ['Type'] = 'Item',
                    ['Backgroundcolor'] = 'black',
                    ['Description'] = 'Combat PDW Grip Attachment',
                    ['Name'] = 'combatpdw_grip',
                    ['Weight'] = 1.0,
                    ['ItemSound'] = 'gear_generic',
                    ['Height'] = 1,
                    ['Width'] = 1
                },
                ['_type'] = 'Item'
            }
        },
        ['certificate'] = {
            ['item'] = {
                ['_id'] = 'certificate',
                ['_parent'] = '',
                ['_name'] = 'certificate',
                ['_data'] = {
                    ['ExamineTime'] = 1.0,
                    ['Rarity'] = 'common',
                    ['Label'] = 'Certificate',
                    ['MaxStack'] = 100,
                    ['Itemimage'] = 'icons/certificate.png',
                    ['Type'] = 'Item',
                    ['Backgroundcolor'] = 'black',
                    ['Description'] = 'Certificate that proves you own certain stuff',
                    ['Name'] = 'certificate',
                    ['Weight'] = 0.0,
                    ['ItemSound'] = 'gear_generic',
                    ['Height'] = 1,
                    ['Width'] = 1
                },
                ['_type'] = 'Item'
            }
        },
        ['weapon_railgun'] = {
            ['item'] = {
                ['_id'] = 'weapon_railgun',
                ['_parent'] = '',
                ['_name'] = 'weapon_railgun',
                ['_data'] = {
                    ['ExamineTime'] = 1.0,
                    ['Rarity'] = 'rare',
                    ['Label'] = 'Railgun',
                    ['MaxStack'] = 1,
                    ['Itemimage'] = 'icons/weapon_railgun.png',
                    ['Type'] = 'Weapon',
                    ['Backgroundcolor'] = 'blue',
                    ['Description'] = 'A weapon that uses electromagnetic force to launch high velocity projectiles',
                    ['Name'] = 'weapon_railgun',
                    ['Weight'] = 1.0,
                    ['ItemSound'] = 'gear_generic',
                    ['Height'] = 2,
                    ['Width'] = 2
                },
                ['_type'] = 'Item'
            }
        },
        ['weapon_snspistol'] = {
            ['item'] = {
                ['_id'] = 'weapon_snspistol',
                ['_parent'] = '',
                ['_name'] = 'weapon_snspistol',
                ['_data'] = {
                    ['ExamineTime'] = 1.0,
                    ['Rarity'] = 'rare',
                    ['Label'] = 'SNS Pistol',
                    ['MaxStack'] = 1,
                    ['Itemimage'] = 'icons/weapon_snspistol.png',
                    ['Type'] = 'Weapon',
                    ['Backgroundcolor'] = 'blue',
                    ['Description'] = 'A very small firearm designed to be easily concealed',
                    ['Name'] = 'weapon_snspistol',
                    ['Weight'] = 1.0,
                    ['ItemSound'] = 'gear_generic',
                    ['Height'] = 1,
                    ['Width'] = 2
                },
                ['_type'] = 'Item'
            }
        },
        ['weapon_flashlight'] = {
            ['item'] = {
                ['_id'] = 'weapon_flashlight',
                ['_parent'] = '',
                ['_name'] = 'weapon_flashlight',
                ['_data'] = {
                    ['ExamineTime'] = 1.0,
                    ['Rarity'] = 'rare',
                    ['Label'] = 'Flashlight',
                    ['MaxStack'] = 1,
                    ['Itemimage'] = 'icons/weapon_flashlight.png',
                    ['Type'] = 'Weapon',
                    ['Backgroundcolor'] = 'blue',
                    ['Description'] = 'A battery-operated portable light',
                    ['Name'] = 'weapon_flashlight',
                    ['Weight'] = 1.0,
                    ['ItemSound'] = 'gear_generic',
                    ['Height'] = 2,
                    ['Width'] = 2
                },
                ['_type'] = 'Item'
            }
        },
        ['weapon_hominglauncher'] = {
            ['item'] = {
                ['_id'] = 'weapon_hominglauncher',
                ['_parent'] = '',
                ['_name'] = 'weapon_hominglauncher',
                ['_data'] = {
                    ['ExamineTime'] = 1.0,
                    ['Rarity'] = 'rare',
                    ['Label'] = 'Homing Launcher',
                    ['MaxStack'] = 1,
                    ['Itemimage'] = 'icons/weapon_hominglauncher.png',
                    ['Type'] = 'Weapon',
                    ['Backgroundcolor'] = 'blue',
                    ['Description'] = 'A weapon fitted with an electronic device that enables it to find and hit a target',
                    ['Name'] = 'weapon_hominglauncher',
                    ['Weight'] = 1.0,
                    ['ItemSound'] = 'gear_generic',
                    ['Height'] = 2,
                    ['Width'] = 2
                },
                ['_type'] = 'Item'
            }
        },
        ['grape'] = {
            ['item'] = {
                ['_id'] = 'grape',
                ['_parent'] = '',
                ['_name'] = 'grape',
                ['_data'] = {
                    ['ExamineTime'] = 1.0,
                    ['Rarity'] = 'common',
                    ['Label'] = 'Grape',
                    ['MaxStack'] = 100,
                    ['Itemimage'] = 'icons/grape.png',
                    ['Type'] = 'Item',
                    ['Backgroundcolor'] = 'black',
                    ['Description'] = 'Mmmmh yummie, grapes',
                    ['Name'] = 'grape',
                    ['Weight'] = 0.1,
                    ['ItemSound'] = 'gear_generic',
                    ['Height'] = 1,
                    ['Width'] = 1
                },
                ['_type'] = 'Item'
            }
        },
        ['sandwich'] = {
            ['item'] = {
                ['_id'] = 'sandwich',
                ['_parent'] = '',
                ['_name'] = 'sandwich',
                ['_data'] = {
                    ['ExamineTime'] = 1.0,
                    ['Rarity'] = 'common',
                    ['Label'] = 'Sandwich',
                    ['MaxStack'] = 100,
                    ['Itemimage'] = 'icons/sandwich.png',
                    ['Type'] = 'Item',
                    ['Backgroundcolor'] = 'black',
                    ['Description'] = 'Nice bread for your stomach',
                    ['Name'] = 'sandwich',
                    ['Weight'] = 0.2,
                    ['ItemSound'] = 'gear_generic',
                    ['Height'] = 1,
                    ['Width'] = 1
                },
                ['_type'] = 'Item'
            }
        },
        ['weapon_minigun'] = {
            ['item'] = {
                ['_id'] = 'weapon_minigun',
                ['_parent'] = '',
                ['_name'] = 'weapon_minigun',
                ['_data'] = {
                    ['ExamineTime'] = 1.0,
                    ['Rarity'] = 'rare',
                    ['Label'] = 'Minigun',
                    ['MaxStack'] = 1,
                    ['Itemimage'] = 'icons/weapon_minigun.png',
                    ['Type'] = 'Weapon',
                    ['Backgroundcolor'] = 'blue',
                    ['Description'] = 'A portable machine gun consisting of a rotating cluster of six barrels and capable of variable rates of fire of up to 6,000 rounds per minute',
                    ['Name'] = 'weapon_minigun',
                    ['Weight'] = 1.0,
                    ['ItemSound'] = 'gear_generic',
                    ['Height'] = 2,
                    ['Width'] = 2
                },
                ['_type'] = 'Item'
            }
        },
        ['coke_brick'] = {
            ['item'] = {
                ['_id'] = 'coke_brick',
                ['_parent'] = '',
                ['_name'] = 'coke_brick',
                ['_data'] = {
                    ['ExamineTime'] = 1.0,
                    ['Rarity'] = 'common',
                    ['Label'] = 'Coke Brick',
                    ['MaxStack'] = 1,
                    ['Itemimage'] = 'icons/coke_brick.png',
                    ['Type'] = 'Item',
                    ['Backgroundcolor'] = 'black',
                    ['Description'] = 'Heavy package of cocaine, mostly used for deals and takes a lot of space',
                    ['Name'] = 'coke_brick',
                    ['Weight'] = 1.0,
                    ['ItemSound'] = 'gear_generic',
                    ['Height'] = 1,
                    ['Width'] = 1
                },
                ['_type'] = 'Item'
            }
        },
        ['advancedrifle_defaultclip'] = {
            ['item'] = {
                ['_id'] = 'advancedrifle_defaultclip',
                ['_parent'] = '',
                ['_name'] = 'advancedrifle_defaultclip',
                ['_data'] = {
                    ['ExamineTime'] = 1.0,
                    ['Rarity'] = 'common',
                    ['Label'] = 'Rifle Clip',
                    ['MaxStack'] = 100,
                    ['Itemimage'] = 'icons/pistol_extendedclip.png',
                    ['Type'] = 'Item',
                    ['Backgroundcolor'] = 'black',
                    ['Description'] = 'Advanced Rifle Default Clip',
                    ['Name'] = 'advancedrifle_defaultclip',
                    ['Weight'] = 1.0,
                    ['ItemSound'] = 'gear_generic',
                    ['Height'] = 1,
                    ['Width'] = 1
                },
                ['_type'] = 'Item'
            }
        },
        ['tosti'] = {
            ['item'] = {
                ['_id'] = 'tosti',
                ['_parent'] = '',
                ['_name'] = 'tosti',
                ['_data'] = {
                    ['ExamineTime'] = 1.0,
                    ['Rarity'] = 'common',
                    ['Label'] = 'Grilled Cheese Sandwich',
                    ['MaxStack'] = 100,
                    ['Itemimage'] = 'icons/tosti.png',
                    ['Type'] = 'Item',
                    ['Backgroundcolor'] = 'black',
                    ['Description'] = 'Nice to eat',
                    ['Name'] = 'tosti',
                    ['Weight'] = 0.2,
                    ['ItemSound'] = 'gear_generic',
                    ['Height'] = 1,
                    ['Width'] = 1
                },
                ['_type'] = 'Item'
            }
        },
        ['weaponlicense'] = {
            ['item'] = {
                ['_id'] = 'weaponlicense',
                ['_parent'] = '',
                ['_name'] = 'weaponlicense',
                ['_data'] = {
                    ['ExamineTime'] = 1.0,
                    ['Rarity'] = 'common',
                    ['Label'] = 'Weapon License',
                    ['MaxStack'] = 1,
                    ['Itemimage'] = 'icons/weapon_license.png',
                    ['Type'] = 'Item',
                    ['Backgroundcolor'] = 'black',
                    ['Description'] = 'Weapon License',
                    ['Name'] = 'weaponlicense',
                    ['Weight'] = 0.0,
                    ['ItemSound'] = 'gear_generic',
                    ['Height'] = 1,
                    ['Width'] = 1
                },
                ['_type'] = 'Item'
            }
        },
        ['weapon_musket'] = {
            ['item'] = {
                ['_id'] = 'weapon_musket',
                ['_parent'] = '',
                ['_name'] = 'weapon_musket',
                ['_data'] = {
                    ['ExamineTime'] = 1.0,
                    ['Rarity'] = 'rare',
                    ['Label'] = 'Musket',
                    ['MaxStack'] = 1,
                    ['Itemimage'] = 'icons/weapon_musket.png',
                    ['Type'] = 'Weapon',
                    ['Backgroundcolor'] = 'blue',
                    ['Description'] = 'An infantryman\'s light gun with a long barrel, typically smooth-bored, muzzleloading, and fired from the shoulder',
                    ['Name'] = 'weapon_musket',
                    ['Weight'] = 1.0,
                    ['ItemSound'] = 'gear_generic',
                    ['Height'] = 2,
                    ['Width'] = 2
                },
                ['_type'] = 'Item'
            }
        },
        ['weapon_remotesniper'] = {
            ['item'] = {
                ['_id'] = 'weapon_remotesniper',
                ['_parent'] = '',
                ['_name'] = 'weapon_remotesniper',
                ['_data'] = {
                    ['ExamineTime'] = 1.0,
                    ['Rarity'] = 'rare',
                    ['Label'] = 'Remote Sniper',
                    ['MaxStack'] = 1,
                    ['Itemimage'] = 'icons/weapon_remotesniper.png',
                    ['Type'] = 'Weapon',
                    ['Backgroundcolor'] = 'blue',
                    ['Description'] = 'A portable high-precision, long-range rifle',
                    ['Name'] = 'weapon_remotesniper',
                    ['Weight'] = 1.0,
                    ['ItemSound'] = 'gear_generic',
                    ['Height'] = 2,
                    ['Width'] = 2
                },
                ['_type'] = 'Item'
            }
        },
        ['handcuffs'] = {
            ['item'] = {
                ['_id'] = 'handcuffs',
                ['_parent'] = '',
                ['_name'] = 'handcuffs',
                ['_data'] = {
                    ['ExamineTime'] = 1.0,
                    ['Rarity'] = 'common',
                    ['Label'] = 'Handcuffs',
                    ['MaxStack'] = 100,
                    ['Itemimage'] = 'icons/handcuffs.png',
                    ['Type'] = 'Item',
                    ['Backgroundcolor'] = 'black',
                    ['Description'] = 'Comes in handy when people misbehave. Maybe it can be used for something else?',
                    ['Name'] = 'handcuffs',
                    ['Weight'] = 0.1,
                    ['ItemSound'] = 'gear_generic',
                    ['Height'] = 1,
                    ['Width'] = 1
                },
                ['_type'] = 'Item'
            }
        },
        ['empty_evidence_bag'] = {
            ['item'] = {
                ['_id'] = 'empty_evidence_bag',
                ['_parent'] = '',
                ['_name'] = 'empty_evidence_bag',
                ['_data'] = {
                    ['ExamineTime'] = 1.0,
                    ['Rarity'] = 'common',
                    ['Label'] = 'Empty Evidence Bag',
                    ['MaxStack'] = 100,
                    ['Itemimage'] = 'icons/evidence.png',
                    ['Type'] = 'Item',
                    ['Backgroundcolor'] = 'black',
                    ['Description'] = 'Used a lot to keep DNA from blood, bullet shells and more',
                    ['Name'] = 'empty_evidence_bag',
                    ['Weight'] = 0.0,
                    ['ItemSound'] = 'gear_generic',
                    ['Height'] = 1,
                    ['Width'] = 1
                },
                ['_type'] = 'Item'
            }
        },
        ['labkey'] = {
            ['item'] = {
                ['_id'] = 'labkey',
                ['_parent'] = '',
                ['_name'] = 'labkey',
                ['_data'] = {
                    ['ExamineTime'] = 1.0,
                    ['Rarity'] = 'common',
                    ['Label'] = 'Key',
                    ['MaxStack'] = 1,
                    ['Itemimage'] = 'icons/labkey.png',
                    ['Type'] = 'Item',
                    ['Backgroundcolor'] = 'black',
                    ['Description'] = 'Key for a lock...?',
                    ['Name'] = 'labkey',
                    ['Weight'] = 0.5,
                    ['ItemSound'] = 'gear_generic',
                    ['Height'] = 1,
                    ['Width'] = 1
                },
                ['_type'] = 'Item'
            }
        },
        ['assaultrifle_extendedclip'] = {
            ['item'] = {
                ['_id'] = 'assaultrifle_extendedclip',
                ['_parent'] = '',
                ['_name'] = 'assaultrifle_extendedclip',
                ['_data'] = {
                    ['ExamineTime'] = 1.0,
                    ['Rarity'] = 'common',
                    ['Label'] = 'Rifle EXT Clip',
                    ['MaxStack'] = 100,
                    ['Itemimage'] = 'icons/pistol_extendedclip.png',
                    ['Type'] = 'Item',
                    ['Backgroundcolor'] = 'black',
                    ['Description'] = 'Assault Rifle Extended Clip',
                    ['Name'] = 'assaultrifle_extendedclip',
                    ['Weight'] = 1.0,
                    ['ItemSound'] = 'gear_generic',
                    ['Height'] = 1,
                    ['Width'] = 1
                },
                ['_type'] = 'Item'
            }
        },
        ['compactrifle_drum'] = {
            ['item'] = {
                ['_id'] = 'compactrifle_drum',
                ['_parent'] = '',
                ['_name'] = 'compactrifle_drum',
                ['_data'] = {
                    ['ExamineTime'] = 1.0,
                    ['Rarity'] = 'common',
                    ['Label'] = 'Rifle Drum',
                    ['MaxStack'] = 100,
                    ['Itemimage'] = 'icons/rifle_drummag.png',
                    ['Type'] = 'Item',
                    ['Backgroundcolor'] = 'black',
                    ['Description'] = 'Compact Rifle Drum',
                    ['Name'] = 'compactrifle_drum',
                    ['Weight'] = 1.0,
                    ['ItemSound'] = 'gear_generic',
                    ['Height'] = 1,
                    ['Width'] = 1
                },
                ['_type'] = 'Item'
            }
        },
        ['weapon_assaultrifle'] = {
            ['item'] = {
                ['_id'] = 'weapon_assaultrifle',
                ['_parent'] = '',
                ['_name'] = 'weapon_assaultrifle',
                ['_data'] = {
                    ['ExamineTime'] = 1.0,
                    ['Rarity'] = 'rare',
                    ['Label'] = 'Assault Rifle',
                    ['MaxStack'] = 1,
                    ['Itemimage'] = 'icons/weapon_assaultrifle.png',
                    ['Type'] = 'Weapon',
                    ['Backgroundcolor'] = 'blue',
                    ['Description'] = 'A rapid-fire, magazine-fed automatic rifle designed for infantry use',
                    ['Name'] = 'weapon_assaultrifle',
                    ['Weight'] = 1.0,
                    ['ItemSound'] = 'gear_generic',
                    ['Height'] = 2,
                    ['Width'] = 3,
                },
                ['_type'] = 'Item'
            }
        },
        ['combatpdw_extendedclip'] = {
            ['item'] = {
                ['_id'] = 'combatpdw_extendedclip',
                ['_parent'] = '',
                ['_name'] = 'combatpdw_extendedclip',
                ['_data'] = {
                    ['ExamineTime'] = 1.0,
                    ['Rarity'] = 'common',
                    ['Label'] = 'SMG EXT Clip',
                    ['MaxStack'] = 100,
                    ['Itemimage'] = 'icons/pistol_extendedclip.png',
                    ['Type'] = 'Item',
                    ['Backgroundcolor'] = 'black',
                    ['Description'] = 'Combat PDW Extended Clip',
                    ['Name'] = 'combatpdw_extendedclip',
                    ['Weight'] = 1.0,
                    ['ItemSound'] = 'gear_generic',
                    ['Height'] = 1,
                    ['Width'] = 1
                },
                ['_type'] = 'Item'
            }
        },
        ['weapon_unarmed'] = {
            ['item'] = {
                ['_id'] = 'weapon_unarmed',
                ['_parent'] = '',
                ['_name'] = 'weapon_unarmed',
                ['_data'] = {
                    ['ExamineTime'] = 1.0,
                    ['Rarity'] = 'rare',
                    ['Label'] = 'Fists',
                    ['MaxStack'] = 1,
                    ['Itemimage'] = 'icons/placeholder.png',
                    ['Type'] = 'Weapon',
                    ['Backgroundcolor'] = 'blue',
                    ['Description'] = 'Fisticuffs',
                    ['Name'] = 'weapon_unarmed',
                    ['Weight'] = 1.0,
                    ['ItemSound'] = 'gear_generic',
                    ['Height'] = 2,
                    ['Width'] = 2
                },
                ['_type'] = 'Item'
            }
        },
        ['radio'] = {
            ['item'] = {
                ['_id'] = 'radio',
                ['_parent'] = '',
                ['_name'] = 'radio',
                ['_data'] = {
                    ['ExamineTime'] = 1.0,
                    ['Rarity'] = 'common',
                    ['Label'] = 'Radio',
                    ['MaxStack'] = 1,
                    ['Itemimage'] = 'icons/radio.png',
                    ['Type'] = 'Item',
                    ['Backgroundcolor'] = 'black',
                    ['Description'] = 'You can communicate with this through a signal',
                    ['Name'] = 'radio',
                    ['Weight'] = 2.0,
                    ['ItemSound'] = 'gear_generic',
                    ['Height'] = 1,
                    ['Width'] = 1
                },
                ['_type'] = 'Item'
            }
        },
        ['weapon_wrench'] = {
            ['item'] = {
                ['_id'] = 'weapon_wrench',
                ['_parent'] = '',
                ['_name'] = 'weapon_wrench',
                ['_data'] = {
                    ['ExamineTime'] = 1.0,
                    ['Rarity'] = 'rare',
                    ['Label'] = 'Wrench',
                    ['MaxStack'] = 1,
                    ['Itemimage'] = 'icons/weapon_wrench.png',
                    ['Type'] = 'Weapon',
                    ['Backgroundcolor'] = 'blue',
                    ['Description'] = 'A tool used for gripping and turning nuts, bolts, pipes, etc',
                    ['Name'] = 'weapon_wrench',
                    ['Weight'] = 1.0,
                    ['ItemSound'] = 'gear_generic',
                    ['Height'] = 2,
                    ['Width'] = 2
                },
                ['_type'] = 'Item'
            }
        },
        ['weapon_bullpupshotgun'] = {
            ['item'] = {
                ['_id'] = 'weapon_bullpupshotgun',
                ['_parent'] = '',
                ['_name'] = 'weapon_bullpupshotgun',
                ['_data'] = {
                    ['ExamineTime'] = 1.0,
                    ['Rarity'] = 'rare',
                    ['Label'] = 'Bullpup Shotgun',
                    ['MaxStack'] = 1,
                    ['Itemimage'] = 'icons/weapon_bullpupshotgun.png',
                    ['Type'] = 'Weapon',
                    ['Backgroundcolor'] = 'blue',
                    ['Description'] = 'A compact smoothbore gun for firing small shot at short range',
                    ['Name'] = 'weapon_bullpupshotgun',
                    ['Weight'] = 1.0,
                    ['ItemSound'] = 'gear_generic',
                    ['Height'] = 2,
                    ['Width'] = 2
                },
                ['_type'] = 'Item'
            }
        },
        ['firstaid'] = {
            ['item'] = {
                ['_id'] = 'firstaid',
                ['_parent'] = '',
                ['_name'] = 'firstaid',
                ['_data'] = {
                    ['ExamineTime'] = 1.0,
                    ['Rarity'] = 'common',
                    ['Label'] = 'First Aid',
                    ['MaxStack'] = 100,
                    ['Itemimage'] = 'icons/firstaid.png',
                    ['Type'] = 'Item',
                    ['Backgroundcolor'] = 'black',
                    ['Description'] = 'You can use this First Aid kit to get people back on their feet',
                    ['Name'] = 'firstaid',
                    ['Weight'] = 2.5,
                    ['ItemSound'] = 'gear_generic',
                    ['Height'] = 1,
                    ['Width'] = 1
                },
                ['_type'] = 'Item'
            }
        },
        ['snspistol_extendedclip'] = {
            ['item'] = {
                ['_id'] = 'snspistol_extendedclip',
                ['_parent'] = '',
                ['_name'] = 'snspistol_extendedclip',
                ['_data'] = {
                    ['ExamineTime'] = 1.0,
                    ['Rarity'] = 'common',
                    ['Label'] = 'Pistol EXT Clip',
                    ['MaxStack'] = 100,
                    ['Itemimage'] = 'icons/pistol_extendedclip.png',
                    ['Type'] = 'Item',
                    ['Backgroundcolor'] = 'black',
                    ['Description'] = 'SNS Pistol Extended Clip',
                    ['Name'] = 'snspistol_extendedclip',
                    ['Weight'] = 1.0,
                    ['ItemSound'] = 'gear_generic',
                    ['Height'] = 1,
                    ['Width'] = 1
                },
                ['_type'] = 'Item'
            }
        },
        ['weapon_combatmg'] = {
            ['item'] = {
                ['_id'] = 'weapon_combatmg',
                ['_parent'] = '',
                ['_name'] = 'weapon_combatmg',
                ['_data'] = {
                    ['ExamineTime'] = 1.0,
                    ['Rarity'] = 'rare',
                    ['Label'] = 'Combat MG',
                    ['MaxStack'] = 1,
                    ['Itemimage'] = 'icons/weapon_combatmg.png',
                    ['Type'] = 'Weapon',
                    ['Backgroundcolor'] = 'blue',
                    ['Description'] = 'A combat version of an automatic gun that fires bullets in rapid succession for as long as the trigger is pressed',
                    ['Name'] = 'weapon_combatmg',
                    ['Weight'] = 1.0,
                    ['ItemSound'] = 'gear_generic',
                    ['Height'] = 2,
                    ['Width'] = 2
                },
                ['_type'] = 'Item'
            }
        },
        ['advancedrifle_extendedclip'] = {
            ['item'] = {
                ['_id'] = 'advancedrifle_extendedclip',
                ['_parent'] = '',
                ['_name'] = 'advancedrifle_extendedclip',
                ['_data'] = {
                    ['ExamineTime'] = 1.0,
                    ['Rarity'] = 'common',
                    ['Label'] = 'Rifle EXT Clip',
                    ['MaxStack'] = 100,
                    ['Itemimage'] = 'icons/pistol_extendedclip.png',
                    ['Type'] = 'Item',
                    ['Backgroundcolor'] = 'black',
                    ['Description'] = 'Advanced Rifle Extended Clip',
                    ['Name'] = 'advancedrifle_extendedclip',
                    ['Weight'] = 1.0,
                    ['ItemSound'] = 'gear_generic',
                    ['Height'] = 1,
                    ['Width'] = 1
                },
                ['_type'] = 'Item'
            }
        },
        ['assaultrifle_defaultclip'] = {
            ['item'] = {
                ['_id'] = 'assaultrifle_defaultclip',
                ['_parent'] = '',
                ['_name'] = 'assaultrifle_defaultclip',
                ['_data'] = {
                    ['ExamineTime'] = 1.0,
                    ['Rarity'] = 'common',
                    ['Label'] = 'Rifle Clip',
                    ['MaxStack'] = 100,
                    ['Itemimage'] = 'icons/pistol_extendedclip.png',
                    ['Type'] = 'Item',
                    ['Backgroundcolor'] = 'black',
                    ['Description'] = 'Assault Rifle Default Clip',
                    ['Name'] = 'assaultrifle_defaultclip',
                    ['Weight'] = 1.0,
                    ['ItemSound'] = 'gear_generic',
                    ['Height'] = 1,
                    ['Width'] = 1
                },
                ['_type'] = 'Item'
            }
        },
        ['firework4'] = {
            ['item'] = {
                ['_id'] = 'firework4',
                ['_parent'] = '',
                ['_name'] = 'firework4',
                ['_data'] = {
                    ['ExamineTime'] = 1.0,
                    ['Rarity'] = 'common',
                    ['Label'] = 'Weeping Willow',
                    ['MaxStack'] = 100,
                    ['Itemimage'] = 'icons/firework4.png',
                    ['Type'] = 'Item',
                    ['Backgroundcolor'] = 'black',
                    ['Description'] = 'Fireworks',
                    ['Name'] = 'firework4',
                    ['Weight'] = 1.0,
                    ['ItemSound'] = 'gear_generic',
                    ['Height'] = 1,
                    ['Width'] = 1
                },
                ['_type'] = 'Item'
            }
        },
        ['appistol_luxuryfinish'] = {
            ['item'] = {
                ['_id'] = 'appistol_luxuryfinish',
                ['_parent'] = '',
                ['_name'] = 'appistol_luxuryfinish',
                ['_data'] = {
                    ['ExamineTime'] = 1.0,
                    ['Rarity'] = 'common',
                    ['Label'] = 'Pistol Finish',
                    ['MaxStack'] = 100,
                    ['Itemimage'] = 'icons/pistol_suppressor.png',
                    ['Type'] = 'Item',
                    ['Backgroundcolor'] = 'black',
                    ['Description'] = 'APPistol Luxury Finish',
                    ['Name'] = 'appistol_luxuryfinish',
                    ['Weight'] = 1.0,
                    ['ItemSound'] = 'gear_generic',
                    ['Height'] = 1,
                    ['Width'] = 1
                },
                ['_type'] = 'Item'
            }
        },
        ['weapon_compactrifle'] = {
            ['item'] = {
                ['_id'] = 'weapon_compactrifle',
                ['_parent'] = '',
                ['_name'] = 'weapon_compactrifle',
                ['_data'] = {
                    ['ExamineTime'] = 1.0,
                    ['Rarity'] = 'rare',
                    ['Label'] = 'Compact Rifle',
                    ['MaxStack'] = 1,
                    ['Itemimage'] = 'icons/weapon_compactrifle.png',
                    ['Type'] = 'Weapon',
                    ['Backgroundcolor'] = 'blue',
                    ['Description'] = 'A compact version of an assault rifle',
                    ['Name'] = 'weapon_compactrifle',
                    ['Weight'] = 1.0,
                    ['ItemSound'] = 'gear_generic',
                    ['Height'] = 2,
                    ['Width'] = 2
                },
                ['_type'] = 'Item'
            }
        },
        ['security_card_02'] = {
            ['item'] = {
                ['_id'] = 'security_card_02',
                ['_parent'] = '',
                ['_name'] = 'security_card_02',
                ['_data'] = {
                    ['ExamineTime'] = 1.0,
                    ['Rarity'] = 'common',
                    ['Label'] = 'Security Card B',
                    ['MaxStack'] = 100,
                    ['Itemimage'] = 'icons/security_card_02.png',
                    ['Type'] = 'Item',
                    ['Backgroundcolor'] = 'black',
                    ['Description'] = 'A security card... I wonder what it goes to',
                    ['Name'] = 'security_card_02',
                    ['Weight'] = 0.0,
                    ['ItemSound'] = 'gear_generic',
                    ['Height'] = 1,
                    ['Width'] = 1
                },
                ['_type'] = 'Item'
            }
        },
        ['pistol_extendedclip'] = {
            ['item'] = {
                ['_id'] = 'pistol_extendedclip',
                ['_parent'] = '',
                ['_name'] = 'pistol_extendedclip',
                ['_data'] = {
                    ['ExamineTime'] = 1.0,
                    ['Rarity'] = 'common',
                    ['Label'] = 'Pistol EXT Clip',
                    ['MaxStack'] = 100,
                    ['Itemimage'] = 'icons/pistol_extendedclip.png',
                    ['Type'] = 'Item',
                    ['Backgroundcolor'] = 'black',
                    ['Description'] = 'Pistol Extended Clip',
                    ['Name'] = 'pistol_extendedclip',
                    ['Weight'] = 1.0,
                    ['ItemSound'] = 'gear_generic',
                    ['Height'] = 1,
                    ['Width'] = 1
                },
                ['_type'] = 'Item'
            }
        },
        ['10kgoldchain'] = {
            ['item'] = {
                ['_id'] = '10kgoldchain',
                ['_parent'] = '',
                ['_name'] = '10kgoldchain',
                ['_data'] = {
                    ['ExamineTime'] = 1.0,
                    ['Rarity'] = 'common',
                    ['Label'] = '10k Gold Chain',
                    ['MaxStack'] = 100,
                    ['Itemimage'] = 'icons/10kgoldchain.png',
                    ['Type'] = 'Item',
                    ['Backgroundcolor'] = 'black',
                    ['Description'] = '10 carat golden chain',
                    ['Name'] = '10kgoldchain',
                    ['Weight'] = 2.0,
                    ['ItemSound'] = 'gear_generic',
                    ['Height'] = 1,
                    ['Width'] = 1
                },
                ['_type'] = 'Item'
            }
        },
        ['rifle_flashlight'] = {
            ['item'] = {
                ['_id'] = 'rifle_flashlight',
                ['_parent'] = '',
                ['_name'] = 'rifle_flashlight',
                ['_data'] = {
                    ['ExamineTime'] = 1.0,
                    ['Rarity'] = 'common',
                    ['Label'] = 'Rifle Flashlight',
                    ['MaxStack'] = 100,
                    ['Itemimage'] = 'icons/smg_flashlight.png',
                    ['Type'] = 'Item',
                    ['Backgroundcolor'] = 'black',
                    ['Description'] = 'Rifle Flashlight Attachment',
                    ['Name'] = 'rifle_flashlight',
                    ['Weight'] = 1.0,
                    ['ItemSound'] = 'gear_generic',
                    ['Height'] = 1,
                    ['Width'] = 1
                },
                ['_type'] = 'Item'
            }
        },
        ['weapon_bullpuprifle'] = {
            ['item'] = {
                ['_id'] = 'weapon_bullpuprifle',
                ['_parent'] = '',
                ['_name'] = 'weapon_bullpuprifle',
                ['_data'] = {
                    ['ExamineTime'] = 1.0,
                    ['Rarity'] = 'rare',
                    ['Label'] = 'Bullpup Rifle',
                    ['MaxStack'] = 1,
                    ['Itemimage'] = 'icons/weapon_bullpuprifle.png',
                    ['Type'] = 'Weapon',
                    ['Backgroundcolor'] = 'blue',
                    ['Description'] = 'A compact automatic assault rifle',
                    ['Name'] = 'weapon_bullpuprifle',
                    ['Weight'] = 1.0,
                    ['ItemSound'] = 'gear_generic',
                    ['Height'] = 2,
                    ['Width'] = 2
                },
                ['_type'] = 'Item'
            }
        }

}