GM:newThread(function()
    ...
    GM.Command:register({
        name = "load",
        label = "Load son joueur",
        description = "Permet de load son joueur",
    }, function(playerSrc)
        if (playerSrc == 0) then return end
        ...
        TriggerEvent("esx:playerLoaded", playerSrc)
    end)

    GM.Command:register({
        name = "heading",
        label = "Afficher son heading",
        description = "Permet d'afficher son heading",
    }, function(playerSrc)
        if (playerSrc == 0) then return end

        local heading = GetEntityHeading(GetPlayerPed(playerSrc))

        TriggerClientEvent("esx:showNotification", playerSrc, "~b~Heading: ~w~"..heading)
    end)

    GM.Command:register({
        name = "metier",
        label = "Metier",
        description = "Ouvrir le menu métier",
        permissions = false,
        keys = {"keyboard", "F6"},
    }, function(playerSrc)
        if (playerSrc == 0) then return end
        ...
        if (playerSelected.job.name == "unemployed") then return end

        local jobName = playerSelected.job.name:gsub("^%l", string.upper)

        local response = GM.Request:sendMessage(playerSrc, "Souhaitez-vous ouvrir le menu métier ?")

        print(response)

        if (response == "accept") then
            TriggerClientEvent(jobName..":openMenu", playerSrc)
            TriggerClientEvent("esx:showNotification", playerSrc, "~g~Vous avez ouvert le menu métier.")
            return
        elseif (response == "decline") then
            TriggerClientEvent("esx:showNotification", playerSrc, "~r~Vous avez refusé l'ouverture du menu métier.")
            return
        elseif (response == "delay") then
            TriggerClientEvent("esx:showNotification", playerSrc, "~r~Vous avez mis trop de temps à répondre.")
            return
        end
    end)
end)