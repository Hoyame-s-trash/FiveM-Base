...
RegisterServerEvent("pac:deleteentityfromall", function(entity)
    if (DoesEntityExist(entity)) then
        DeleteEntity(entity)
        Wait(1000)
        if (DoesEntityExist(entity)) then TriggerClientEvent("pac:deleteentityfromall", -1, entity) end
    end
end)

RegisterServerEvent("pac:eulen", function(subject, description)
    local src = source
    if (Config.Ban) then
