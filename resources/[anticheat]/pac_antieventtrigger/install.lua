...
-- Server side
if (IsDuplicityVersion()) then
    local Config = exports["pac_antieventtrigger"]:getConfig()
    ...
    RegisterServerEvent = function(name, cb)
        ...
        for k, v in pairs(Config.WhitelistedEvents) do
            if not (v == name) then
                cb = function(...)
                    ...
                    if eventToleranceRate[source] >= Config.ToleranceRate then
                        TriggerEvent("pac:detection", enc(tostring(source)), enc("Anti Event Trigger"), enc("Player tried to execute a event with executor. Event: " .. name .. " Tolerance: " .. eventToleranceRate[source] .. " Args: " ..json.encode(args)..")"))
                    end
                end
